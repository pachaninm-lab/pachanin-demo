...
export function runPlatformV7ExecutionEvidenceScenario() {
  ...
  const firstAction = seller ? runPlatformAction(state, { type: 'createLot', actor: seller, payload: { lotId: 'LOT-P7-E2E-2096', volumeTonnes: 240, pricePerTonneRub: 16140, basis: 'EXW', qualityClass: '3' }, runtimeLabel: 'manual', now: '2026-07-01T08:00:00.000Z' }) : null;
  const stateAfterFirstAction = firstAction?.ok ? firstAction.state : state;
  const secondAction = seller && firstAction?.ok ? runPlatformAction(stateAfterFirstAction, { type: 'publishLot', actor: seller, payload: { lotId: 'LOT-P7-E2E-2096' }, runtimeLabel: 'manual', now: '2026-07-01T08:01:00.000Z' }) : null;
  const stateAfterSecondAction = secondAction?.ok ? secondAction.state : stateAfterFirstAction;
  const thirdAction = operator && secondAction?.ok ? runPlatformAction(stateAfterSecondAction, { type: 'createDeal', actor: operator, payload: { lotId: 'LOT-P7-E2E-2096', dealId: 'DL-P7-E2E-2096', buyerId: 'CP-B-001' }, runtimeLabel: 'manual', now: '2026-07-01T08:02:00.000Z' }) : null;
  const scenarioState = thirdAction?.ok ? thirdAction.state : stateAfterSecondAction;
  const preCloseAction = buyer && thirdAction?.ok ? runPlatformAction(scenarioState, { type: 'requestReserve', actor: buyer, payload: { dealId: 'DL-P7-E2E-2096' }, idempotencyKey: 'P7-E2E-2096-PRE-CLOSE', runtimeLabel: 'manual', now: '2026-07-01T08:03:00.000Z' }) : null;
  const preCloseBlockCode = preCloseAction?.ok ? 'NONE' : 'INVALID_TRANSITION';
  const preCloseBlockScope = preCloseAction?.ok ? 'none' : 'transition-before-bank-basis-guards';
  ...
  const passedSteps = [
    ...
    { id: 'pre-close-check', status: preCloseAction?.ok ? 'UNEXPECTED_BANK_BASIS' : 'BLOCKED_AS_EXPECTED' },
  ];
  ...
  return {
    ...
    closeBasis,
    source: 'manual-action-check-runner',
  };
}
