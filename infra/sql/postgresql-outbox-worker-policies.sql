...
-- app_outbox therefore never evaluates tenant visibility functions and still
-- has no tenant-table privileges. The caller owns the transaction. Never add
-- BEGIN/COMMIT here.
...
BEGIN
  ...
  EXECUTE format(
    'CREATE POLICY outbox_entries_select ON public."outbox_entries" FOR SELECT TO %s USING (public.app_rls_context_ready() AND "dealId" IS NOT NULL AND public.app_rls_deal_visible("dealId"))',
    tenant_targets
  );

  EXECUTE format(
    'CREATE POLICY outbox_entries_insert ON public."outbox_entries" FOR INSERT TO %s WITH CHECK (public.app_rls_context_ready() AND "dealId" IS NOT NULL AND public.app_rls_deal_visible("dealId"))',
    tenant_targets
  );
END
$outbox_policy_scope$;
