...
vault write database/roles/grainflow-app-role \
  ...
  creation_statements="CREATE ROLE \"{{name}}\" WITH LOGIN PASSWORD '{{password}}' VALID UNTIL '{{expiration}}' IN ROLE app_readonly; GRANT INSERT,UPDATE,SELECT ON ALL TABLES IN SCHEMA public TO \"{{name}}\";" \
