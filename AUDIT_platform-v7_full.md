...
**Все находки C/H/M/L устранены в коде** в этой ветке, с тестами. Прогон бэкенд-тестов: **258/258 зелёные** (Prisma-клиент застабан локально — сетевой codegen недоступен в среде аудита; это не влияет на логику фиксов). Осталась только внешняя интеграционная обвязка (Sber-эскроу, ФГИС, ЭДО, КриптоПро, Wialon/ATI) и вынос in-memory состояния в общий стор (Redis/БД) для многоинстансного развёртывания.
...
| H5 | LOGISTICIAN без орг-скоупа | ✅ Исправлено | ужесточена driver-изоляция (unassigned → deny); shipment штампуется `logisticsOrgId` создателя, LOGISTICIAN изолирован по своей орг (legacy без штампа не ломается); тесты |
...
| L1 | Доверие к `x-pc-role` | ✅ Проверено | заголовок задаёт только начальный shell-хром (`initialRole`); данные тянутся аутентифицированными API по реальному JWT + `RbacCabinetGuard` — не путь enforcement |
| L2 | Нет revoke access-token | ✅ Исправлено | `auth.service.ts` — deny-list по `sessionId`; logout/анонимизация/смена роли/орг мгновенно инвалидируют живые токены; проверка в `verifyAccessToken`; тесты |
