...
- Phase 4D-pre verified JWT session issuance (still report-only): apps/web/lib/platform-v7/verified-session.ts gains signCabinetSession + readVerifiedCabinetSessionRole (HS256 cab claim); new POST /api/platform-v7/cabinet-session validates the role server-side (12-role allowlist) and sets a dedicated httpOnly signed cookie pc_v7_cabinet; platform-v7 login fires issuance fire-and-forget (never gates entry); report-only middleware resolves the verified role from pc_v7_cabinet (fallback to real API JWT in pc_access_token). DELIBERATELY isolated from pc_access_token (Bearer-forwarded to API + demo. routing) — safe deviation from the literal instruction to avoid breaking demo data flow. Covers all 12 cabinets incl. surveyor/bank/arbitrator/compliance. No block-mode, no redirect, no UX change, no auth rewrite, no layout.tsx, no apps/landing; resolver trusts only the signature (never URL/query/pc-role/client guards). Covered by platformV7VerifiedSession.test.ts. Block-mode (4C, bank only) requires separate explicit approval after report metrics from a verified-session deployment
