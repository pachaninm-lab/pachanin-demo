...
- открывать, закрывать или решать спор;
- создавать, готовить к исполнению, исполнять или сохранять любую команду платформы — в том числе после явного подтверждения пользователя;
...
## 12. Будущий command-контур — `DEFERRED_BY_OWNER_DECISION`

**Отложено решением владельца от 26.07.2026.** Текущий промышленный релиз —
`INFORMATIONAL_ONLY` / `READ_ONLY` для всех ролей:
...
6. «Следующее действие» означает только информационную рекомендацию.
7. Confirmed actions возвращаются только отдельным будущим решением владельца и новой промышленной приёмкой.

Ниже — описание будущего этапа, а не действующего контура. Ни один из перечисленных
механизмов сейчас не исполняется: write-инструментов в реестре ноль, `authorize_tool`
отказывает по режиму и не читает подтверждение пользователя, `confirm_action` и подготовка
действий отказывают fail-closed. Раздел сохранён как неисполняемая документация, чтобы
возврат командного контура был осознанным решением, а не восстановлением по памяти.
...
- write-инструментов в реестре = 0;
- AI-initiated write = 0, AI-confirmed write = 0, AI-persisted draft = 0;
