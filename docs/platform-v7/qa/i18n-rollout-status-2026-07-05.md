...
- Мигрированы: главная `/platform-v7` (namespace `landing`) и легаси-логин
  `LoginLegacyOverlay` (namespace `loginLegacy`). Сервер отдаёт HTML сразу на
  выбранном языке: работает SEO, нет «русской вспышки», `html[lang]`
  выставляется на сервере.
...
План миграции дальше (по мере надобности): register/contact/demo/docs
(сейчас самолокализованы клиентски), затем кабинеты ролей.
