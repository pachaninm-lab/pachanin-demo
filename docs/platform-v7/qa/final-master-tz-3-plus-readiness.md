...
### Runtime Binding QA (§38) — M3-6

Сводный тест `m3RuntimeBindingScenarios.test.ts` покрывает 7 сквозных сценариев на
runtime-view-model'ях: S1/S4 (нет release без банка и при незакрытых документах),
S2/S3 (спор по весу/качеству → hold), S5 (расхождения видны оператору), S6 (водитель
не видит денег), S7 (off-platform payment → bypass заблокирован), BI из runtime.

**Итог:** критические DoD-пункты закрыты; §21/§23/§32/§35/§38 доведены в M3-4…M3-6.
Открытые (16, 17, 30/31, 35-mobile, UX по остальным ролям) — точечная доводка, не блокеры
внутреннего контура. Завышения зрелости нет. Остаток по существу — **только live-интеграции (§43)**.
