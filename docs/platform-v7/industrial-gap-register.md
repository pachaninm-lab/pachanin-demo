...
- **Статус**: OPEN (частично закрывается блоком 1: BIGINT + append-only + тесты
  инварианта; полный reconciliation — блок «Деньги и ledger»).
...
- **Статус**: CLOSED (Money, блок 1); Weight/PricePerTon/QualityAdjustment — блок «Деньги и ledger».
