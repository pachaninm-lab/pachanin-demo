...
- **Статус**: IN_PROGRESS: time-range индексы на audit_events/deal_events/integration_events/ledger_entries/outbox_entries добавлены (блок 3); партиционирование/архивация — при подтверждённых production-объёмах.
...
- **Статус**: IN_PROGRESS (блок 8): CI-scale load-proof на реальном PostgreSQL — 24 сделки параллельно через полный цикл 19 команд на двух instance: p50 186 ms, p95 283 ms, p99 359 ms, 78 команд/с, ноль потерянных/двойных эффектов, hash-цепочки целы; load-proof вскрыл и устранил SSI-трэш: канонический pipeline переведён с глобального Serializable на per-deal advisory lock (pg_advisory_xact_lock) + ReadCommitted — сделки полностью параллельны, порядок в рамках сделки гарантирован локом, lost update отсекает version-CAS. Suite test/industrial — обязательный CI-gate (job industrial-core-e2e). Полная production-нагрузка (1000 users/50k deals) — требует внешнего окружения.
...
- **Статус**: CLOSED (ядро) в блоках 4–5: workspace обслуживает любую сделку (/platform-v7/deals/[id]/execution), 409 → автообновление с человеческим объяснением, RU/EN/ZH словарь покрывает все строки workspace, офлайн-очередь команд для полевых ролей (стабильные commandId/idempotencyKey, видимый статус синхронизации, конфликт решает сервер). Миграция остальных экранов с fixtures — продолжается.
