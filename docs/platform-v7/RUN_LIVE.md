...
| `PLATFORM_V7_DEAL_REPOSITORY` | — (runtime/in-memory) | `prisma` (БД-репозиторий) |
...
1. **Идентичность в БД.** Сейчас пользователи в auth — это in-memory массив (нет модели `User`/`Organization`
   в Prisma). Перенести в БД: модели `User`/`Organization`, репозиторий, сид, refresh-токены в БД.
