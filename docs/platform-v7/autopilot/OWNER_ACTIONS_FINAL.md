...
## 4. Разморозить три predecessor-workflow для миграции PostgreSQL — открыто
...
**Следствие.** Эти три workflow продолжают тянуть `postgres:16` из Docker Hub и
остаются под тем же сбоем, из-за которого вся работа и затевалась. Миграция
закрыла **17 потребителей из 20**, а не все.

**Что сделать.** Принять решение о разморозке: либо снять эти workflow из-под
лока, либо санкционировать перегенерацию лока с новым baseline. После этого
миграция трёх оставшихся — правка одной строки в каждом.

**Блокирует ли это merge.** Нет. Guard называет эти три файла поимённо вместе с
образом, на котором каждый заморожен, и отвергает там любое **другое** значение.
