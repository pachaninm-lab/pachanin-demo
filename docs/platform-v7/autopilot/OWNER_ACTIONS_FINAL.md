...
## 7. Место на хосте модели — открыто, блокирует конвертацию

Доступ CI к хосту доказан: preflight и capacity probe отработали 28 июля на
exact main `9854fcad36cf382395f5f5bcb654ce663d2be5ee`. Хост подходит по всем
параметрам, кроме одного.

| Требование | Нужно | На хосте | |
|---|---|---|---|
| Ядер | 4 | 16 | ок |
| ОЗУ всего | 16.0 ГБ | 16.76 ГБ | ок |
| ОЗУ свободно | 8.0 ГБ | 15.96 ГБ | ок |
| Место в `/srv/tai-models` | **180 ГБ** | **74.68 ГБ** | **не хватает** |
| Эндпоинты Hugging Face | 200 | все 200 | ок |

Код отказа: `WORKSPACE_STORAGE_BELOW_MINIMUM`.

Это не формальность контроля. Конвертация обоих бандлов занимает около
80 ГБ: Qwen — исходники 16.4, промежуточный BF16 GGUF 16.4, `Q4_K_M` 4.9,
`Q8_0` 8.7; Mistral — исходники 14.5, F16 14.5, `Q4_K_M` 4.4. При 74.68 ГБ
места не хватит, и не хватит **в конце**, после нескольких часов счёта.

Освобождать корень бесполезно: рабочий каталог — отдельный раздел. Проба
показала 92.5 ГБ на `/` и 74.68 ГБ в `/srv/tai-models`; это разные числа,
значит разные файловые системы. Удаление скачанного в `/srv/tai/model-sources`
освободит корень, а не рабочий каталог.

### 7.1. Сначала — три read-only команды

Ничего не меняют, нужны чтобы выбрать верный способ расширения:

```
df -hT /srv/tai-models
findmnt -T /srv/tai-models
lsblk -o NAME,SIZE,FSTYPE,MOUNTPOINTS,UUID
```

Из вывода нужны три факта: тип файловой системы (`ext4` или `xfs`),
устройство раздела, и есть ли между диском и разделом слой LVM
(в `lsblk` он выглядит как `lvm` и имена вида `vg-lv`).

### 7.2. Сколько добавить

Минимум по требованию — 180 ГБ свободных, то есть добавить не менее
**106 ГБ**. Рекомендую **+150 ГБ**: получится около 225 ГБ свободных, и
останется запас на повтор, отладку и вторую квантизацию без расчистки
между попытками.

### 7.3. Расширение — три случая, выполнять только свой

Общее правило: **ничего не удалять**. Ни модели, ни артефакты, ни каталоги
прогонов. Все три сценария сохраняют данные.

**Случай A. Диск увеличен у провайдера, LVM нет.**
Раздел надо растянуть, затем файловую систему:

```
lsblk                          # увидеть новый размер диска
growpart /dev/vdX N            # X и N — из lsblk, например /dev/vda 2
```

затем ровно одну строку по типу файловой системы:

```
resize2fs /dev/vdXN            # только если ext4
xfs_growfs /srv/tai-models     # только если xfs
```

`resize2fs` на XFS и `xfs_growfs` на ext4 не сработают; выполнять
неподходящий вариант не нужно.

**Случай B. Диск увеличен у провайдера, есть LVM.**
...
затем одна строка по типу файловой системы, как в случае A.

**Случай C. Подключён отдельный новый диск.**
Здесь важен порядок: если смонтировать новый том поверх непустого каталога,
существующие файлы станут невидимы. Поэтому сначала перенос, потом монтирование:

```
mkfs.ext4 /dev/vdY                       # новый диск целиком
mkdir -p /mnt/tai-new
mount /dev/vdY /mnt/tai-new
cp -a /srv/tai-models/. /mnt/tai-new/    # -a сохраняет права и время
diff -r /srv/tai-models /mnt/tai-new     # пусто — значит перенос полный
umount /mnt/tai-new
mount /dev/vdY /srv/tai-models
```

В `/etc/fstab` добавить строку с UUID (`blkid /dev/vdY`), чтобы монтирование
пережило перезагрузку. Старые данные остаются на прежнем разделе нетронутыми,
пока вы сами не решите их удалить.

### 7.4. Проверка после расширения

```
df -hT /srv/tai-models
```

Свободного должно стать не меньше 180 ГБ. Затем повторить полный preflight —
комментарием в issue 2835, ровно этой строкой и без перевода строки в конце:

```
/tai probe model-host exact-main
```

Конвертацию (`/tai convert model-bundles exact-main`) запускать **только**
после того, как preflight покажет `READY_FOR_CONTROLLED_ACQUISITION`.

### 7.5. Ninja устанавливать не нужно

Проба отмечает `missing required tools: NINJA`, но это наблюдение, а не
требование:

- `required_tools` в `model-host-preflight-requirements.v1.json` — шесть
  инструментов: `curl`, `docker`, `git`, `gzip`, `sha256sum`, `tar`;
- проба перечисляет тринадцать, включая `cmake`, `ninja`, `cc`, `c++`, —
  она снимает картину, а не выносит вердикт;
- в драйвере конвертации и в `model-conversion-authority.v1.json` слово
  `ninja` не встречается ни разу;
- конвертация не собирает llama.cpp: она распаковывает принятый архив
  инструментария и сверяет четыре бинарника по точным SHA-256, после чего
  использует их.

Менять систему без доказанной необходимости не нужно.
