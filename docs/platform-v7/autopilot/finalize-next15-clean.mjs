...
for (const file of roots.flatMap(sourceFiles)) {
  ...
  for (const helper of ['runtimeAuthHeaders', 'serverAuthHeaders']) {
    ...
    const declarations = source.match(new RegExp(`function\\s+${helper}\\s*\\(`, 'g'))?.length ?? 0;
    const awaited = source.match(new RegExp(`\\bawait\\s+${helper}\\s*\\(`, 'g'))?.length ?? 0;
    if (calls - declarations !== awaited) violations.push(`${file}: unawaited ${helper}`);
  }
}

if (violations.length > 0) {
  throw new Error(`Next.js 15 cleanup incomplete:\n${violations.join('\n')}`);
}
