...
Прогресс: **6.7%** по матрице §H. PR #4216 открыт, не смержен.
...
2. **Политика нумерации.** БД уже требует номер при выпуске (`accounting_documents_issued_number_check`),
   так что нумератор нельзя обойти — но его самого ещё нет.
