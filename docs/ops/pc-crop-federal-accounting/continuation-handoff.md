...
Прогресс: **7.1%** по матрице §H. PR #4216 открыт, не смержен.

> Число снизилось с 6.9 при том, что работа прибавилась. Это не ошибка: доля
> компонента workstream 4 была завышена (28% за signing authority плюс политику
> устаревания, под которой не было хранилища). Пересчёт применён в той же
...
4. Остаток workstream 4: format registry, версии договоров, налоговый профиль, версии
   регуляторных правил.
5. Wave 3 (task engine) и Wave 9 (ГЕКТА read models) требуют `apps/web` — **не одобрен**.
