...
Прогресс: **11.8%** по матрице §H. PR #4216 **смержен** в main как `066dc5e3e`.
...
| Wave 3.5 | Репозиторий задач, деривер и три маршрута: идемпотентный подъём, переход под ожидаемой версией, условие проверяет сервер | `work-task.repository.ts`, `work-task.deriver.ts` |
