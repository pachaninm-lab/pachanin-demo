...
5. The runner publishes only that two verified applications are waiting, with no application identifier or applicant data. In the production browser, the existing `PLATFORM_OWNER` performs the normal password + fresh-TOTP login, activates bounded `CONTROL_PLANE`, opens the registration queue and approves the two marked P0 applications. For a marked acceptance application, that same browser repeats the decision once with the same idempotency key. The BFF accepts PASS only when the first response reports delivered notification and the replay reports `replayed=true` with no `notificationDelivered`; it emits only the bounded `P0_HUMAN_REVIEWER_CEREMONY` marker. The runner polls read-only application/event state and those exact Web-runtime markers for at most 30 minutes.
