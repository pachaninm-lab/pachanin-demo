...
fail(){ FAIL_EMITTED=1; emit ROLE_ELIGIBILITY_API_RELEASE FAIL; emit ERROR_CODE "$1"; exit "${2:-1}"; }
...
[[ "$baseline_image_ref" =~ ^ghcr\.io/pachaninm-lab/grainflow-api:sha-[0-9a-f]{7}$ ]] || fail BASELINE_API_REFERENCE_NOT_IMMUTABLE 15
...
[[ -n "$prod_dir" && -d "$prod_dir" ]] || fail PRODUCTION_DIRECTORY_INVALID 16
[[ -n "$prod_compose" ]] || fail PRODUCTION_COMPOSE_AUTHORITY_MISSING 17
[[ "$prod_project" =~ ^[a-z0-9][a-z0-9_-]*$ ]] || fail PRODUCTION_PROJECT_INVALID 18

override="$prod_dir/$OVERRIDE_NAME"
[[ "$override" == "$prod_dir"/* ]] || fail OVERRIDE_PATH_INVALID 19
...
for raw in "${raw_files[@]}"; do
  ...
  [[ -f "$file" ]] || fail PRODUCTION_COMPOSE_FILE_MISSING 20
  compose_files+=("$file")
done
(( ${#compose_files[@]} >= 1 )) || fail PRODUCTION_COMPOSE_AUTHORITY_EMPTY 21

dc=(docker compose --project-directory "$prod_dir" --project-name "$prod_project")
for file in "${compose_files[@]}"; do dc+=(-f "$file"); done
"${dc[@]}" config --quiet
"${dc[@]}" config --services | grep -Fxq api || fail API_SERVICE_MISSING 22
...
api_ready "$baseline_api_id" || fail BASELINE_API_NOT_READY 23
...
cleanup_on_exit(){
  ...
  if [[ "$rc" -ne 0 && "$MUTATION_STARTED" == 1 && "$ROLLBACK_ACTIVE" == 0 ]]; then
    ...
    if write_override "$baseline_image_ref"; then
      if "${dc_target[@]}" config --quiet && "${dc_target[@]}" up -d --no-deps --force-recreate --pull never api >/dev/null 2>&1; then
    ...
    emit ROLE_ELIGIBILITY_API_ROLLBACK_COMPLETED "$rollback_ok"
    if [[ "$rollback_ok" != 1 ]]; then exit 90; fi
  fi
  if [[ "$rc" -ne 0 && "$FAIL_EMITTED" == 0 ]]; then
    emit ROLE_ELIGIBILITY_API_RELEASE FAIL
    emit ERROR_CODE UNHANDLED_RELEASE_FAILURE
  fi
  exit "$rc"
}
