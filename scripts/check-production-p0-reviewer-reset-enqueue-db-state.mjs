...
const branch = 'diag/p0-reviewer-reset-enqueue-db-state-3785';
...
for (const token of [
  ...
  'has_function_privilege(current_user,to_regprocedure($1)',
...
for (const key of ['databaseMutation','identityMutation','passwordMutation','mfaMutation','sessionMutation','resetReplay','mailSend','deploymentMutation','containerLifecycleMutation','piiOutput','credentialOutput','rawDbErrorOutput','customerRowRead']) {
  if (b[key] !== false) failures.push(`scope boundary ${key}`);
}
if (b.databaseRead !== true || b.catalogReadOnly !== true || b.migrationMetadataReadOnly !== true || b.productionMutation !== 'NONE' || b.ownerOnly !== true || b.exactMainGuard !== true || b.activeApiPrincipalProbe !== true || b.migrationAuthorityProbe !== true || b.newRecurringCostRub !== 0) {
...
console.log('PASS: enqueue DB state classifier is owner-bound, exact-main guarded, catalog/migration-metadata read-only, active-API-principal aware, migration-authority aware, customer-row-free, reset-free, mail-free and mutation-free.');
