...
if [ "${GITHUB_HEAD_REF:-}" = "agent/harden-platform-v7-public-entry" ] || [ "${GITHUB_HEAD_REF:-}" = "fix/public-entry-human-copy" ] || [ "${GITHUB_HEAD_REF:-}" = "fix/landing-hero-support" ] || [ "${GITHUB_HEAD_REF:-}" = "fix/login-human-grade-ui" ] || [ "${GITHUB_HEAD_REF:-}" = "fix/exact-approved-header-logo" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$PUBLIC_ENTRY_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "fix/public-registration-final-copy-4916" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$PUBLIC_REGISTRATION_FINAL_COPY_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "agent/public-home-typography" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$PUBLIC_HOME_TYPOGRAPHY_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "feat/platform-v7-staff-control-center" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$STAFF_CONTROL_CENTER_TEMPLATE_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "design/owner-access-center-task-ux" ] || printf '%s\n' "$DIFF_FILES" | grep -qx 'apps/web/components/platform-v7/staff/OwnerAccessCenter.tsx'; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$OWNER_ACCESS_CENTER_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "fix/controlled-test-access" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$CONTROLLED_TEST_ACCESS_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "fix/test-organizations-all-cabinets" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$TEST_ORGANIZATIONS_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "fix/platform-v7-russian-default-locale" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$RUSSIAN_DEFAULT_LOCALE_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "fix/public-auth-server-authority" ] || [ "${GITHUB_HEAD_REF:-}" = "fix/login-human-grade-ui" ] || printf '%s\n' "$DIFF_FILES" | grep -qx 'apps/web/lib/server/mfa-login-ticket.ts'; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$PUBLIC_AUTH_FIX_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "fix/public-entry-lcp-css-boundary" ] || [ "${GITHUB_HEAD_REF:-}" = "fix/login-human-grade-ui" ] || printf '%s\n' "$DIFF_FILES" | grep -qx 'apps/web/components/platform-v7/PlatformV7FullStyleRuntime.tsx'; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$PUBLIC_LCP_FIX_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "agent/industrial-readiness-v1-security-gates-v2" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$INDUSTRIAL_SECURITY_GATE_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "agent/ir-sec-transitive-runtime-remediation" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$TRANSITIVE_RUNTIME_REMEDIATION_SCOPE"); fi
BRACE_EXPANSION_5_0_9_SCOPE='package.json
pnpm-lock.yaml
docs/platform-v7/autopilot/security-exceptions.json'
if [ "${GITHUB_HEAD_REF:-}" = "fix/security-brace-expansion-5-0-8" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$BRACE_EXPANSION_5_0_9_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "agent/ir-sec-opentelemetry-220" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$OPENTELEMETRY_REMEDIATION_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "agent/ir-sec-next-15-5-16-final" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$NEXT15_REMEDIATION_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "ir/immutable-release-authority-2652" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$IMMUTABLE_RELEASE_AUTHORITY_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "ir/runtime-images-2664" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$IR_RUNTIME_IMAGE_SCOPE"); fi
if [ "${GITHUB_HEAD_REF:-}" = "fix/exact-main-live-evidence-2659" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$EXACT_MAIN_LIVE_EVIDENCE_SCOPE"); fi
...
if (!baseRef || !stateFile || !branch) throw new Error('P7_IMMUTABLE_SCOPE: immutable scope inputs are required');
let scopes;
if (branch === publicHomeGovernanceBranch) {
  ...
  let manifest;
  try { manifest = JSON.parse(execFileSync('git', ['show', `${baseRef}:${publicHomeImplementationManifest}`], { encoding: 'utf8' })); }
  catch (error) { const message = error instanceof Error ? error.message : String(error); throw new Error(`P7_IMMUTABLE_SCOPE: cannot load accepted public-home manifest: ${message}`); }
  if (manifest.schemaVersion !== 'platform-v7.concurrent-scope.v1' || manifest.status !== 'active' || manifest.branch !== publicHomeImplementationBranch) throw new Error('P7_IMMUTABLE_SCOPE: accepted public-home manifest identity is invalid');
  ...
  let state;
  try { state = JSON.parse(execFileSync('git', ['show', `${baseRef}:${stateFile}`], { encoding: 'utf8' })); }
  catch (error) { const message = error instanceof Error ? error.message : String(error); throw new Error(`P7_IMMUTABLE_SCOPE: cannot load ${baseRef}:${stateFile}: ${message}`); }
  scopes = state.approvedConcurrentScopes?.[branch];
}
if (!Array.isArray(scopes) || scopes.length === 0) throw new Error(`P7_IMMUTABLE_SCOPE: no immutable approved scope for ${branch}`);
const normalized = scopes.map((entry, index) => {
  if (typeof entry !== 'string') throw new Error(`P7_IMMUTABLE_SCOPE: scope[${index}] is not a string`);
  const value = entry.trim().replace(/\/+$/u, '');
  if (!value || value.includes('\\') || value === '..' || value.startsWith('../') || value.includes('/../')) throw new Error(`P7_IMMUTABLE_SCOPE: unsafe path ${JSON.stringify(entry)}`);
  return value;
});
if (new Set(normalized).size !== normalized.length) throw new Error('P7_IMMUTABLE_SCOPE: duplicate immutable approved paths');
...
if is_immutable_scope_branch "$CURRENT_BRANCH"; then ALLOWED_CURRENT="$APPROVED_BRANCH_SCOPE"; elif [ -n "$APPROVED_BRANCH_SCOPE" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$APPROVED_BRANCH_SCOPE"); fi
if is_immutable_scope_branch "$CURRENT_BRANCH"; then SOURCE_CONTROLLED_SCOPE=''; else SOURCE_CONTROLLED_SCOPE=$(GITHUB_HEAD_REF="${GITHUB_HEAD_REF:-}" node scripts/p7-source-controlled-scope.mjs); fi
if [ -n "$SOURCE_CONTROLLED_SCOPE" ]; then ALLOWED_CURRENT=$(printf '%s\n%s\n' "$ALLOWED_CURRENT" "$SOURCE_CONTROLLED_SCOPE"); fi
...
if is_immutable_scope_branch "$CURRENT_BRANCH" && [ "$CURRENT_BRANCH" != "$SCOPE_GOVERNANCE_BRANCH" ] && [ "$CURRENT_BRANCH" != "$INVENTORY_SCOPE_GOVERNANCE_BRANCH" ] && [ "$CURRENT_BRANCH" != "$PUBLIC_HOME_SCOPE_GOVERNANCE_BRANCH" ] && [ "$CURRENT_BRANCH" != "$POISON_ISOLATION_SCOPE_GOVERNANCE_BRANCH" ]; then
  ...
  if [ -n "$MUTABLE_SCOPE_AUTHORITIES" ]; then echo "Mutable scope authority changed on a PC-CROP immutable-scope implementation branch:"; printf '%s\n' "$MUTABLE_SCOPE_AUTHORITIES"; exit 1; fi
fi

if [ "${GITHUB_HEAD_REF:-}" = "agent/ir-sec-transitive-runtime-remediation" ] || [ "${GITHUB_HEAD_REF:-}" = "agent/ir-sec-opentelemetry-220" ] || [ "${GITHUB_HEAD_REF:-}" = "agent/ir-sec-next-15-5-16-final" ] || [ "${GITHUB_HEAD_REF:-}" = "claude/tai-production-attestation-gizgzh" ] || [ "${GITHUB_HEAD_REF:-}" = "fix/security-brace-expansion-5-0-8" ] || [ "${GITHUB_HEAD_REF:-}" = "identity-rls-3670" ]; then FORBIDDEN_ALWAYS='^(apps/landing/|package-lock\.json$|\.env|.*\.pem$|.*\.key$)'; else FORBIDDEN_ALWAYS='^(apps/landing/|package-lock\.json$|pnpm-lock\.yaml$|\.env|.*\.pem$|.*\.key$)'; fi
...
if [ "${GITHUB_HEAD_REF:-}" = "agent/tai-ap-14d7-live-remediation" ]; then
  ...
  FORBIDDEN_FILES=$(printf '%s\n' "$FORBIDDEN_FILES" | grep -Ev "$TAI_PUBLIC_CA_PATHS" || true)
  while IFS=' ' read -r expected_fingerprint certificate_path; do
    actual_fingerprint=$(openssl x509 -in "$certificate_path" -outform DER | sha256sum | cut -d' ' -f1)
    if [ "$actual_fingerprint" != "$expected_fingerprint" ]; then echo "Audited public CA fingerprint mismatch: $certificate_path"; exit 1; fi
...
if [ -n "$FORBIDDEN_FILES" ]; then printf '%s\n' "$FORBIDDEN_FILES"; echo "Forbidden path changed for platform-v7 autopilot."; exit 1; fi
if is_immutable_scope_branch "$CURRENT_BRANCH"; then P7_EXACT_APPROVED_SCOPE=1; else P7_EXACT_APPROVED_SCOPE=0; fi
...
function globToRegExp(glob) { const normalized = normalizePath(glob); let pattern=''; for (let index=0; index<normalized.length; index+=1) { const character=normalized[index]; const next=normalized[index+1]; if (character==='*'&&next==='*') { pattern+='.*'; index+=1; } else if (character==='*') pattern+='[^/]*'; else pattern+=escapeRegExp(character); } return new RegExp(`^${pattern}$`); }
function scopeMatches(allowedEntry, candidate) { const allowed=normalizePath(allowedEntry); const file=normalizePath(candidate); if (!allowed||!file) return false; if (allowed===file) return true; if (allowed.includes('*')) return globToRegExp(allowed).test(file); return file.startsWith(`${allowed}/`); }
function exactScopeMatches(allowedEntry, candidate) { const allowed=normalizePath(allowedEntry); const file=normalizePath(candidate); if (!allowed||!file) return false; if (allowed.includes('*')) return globToRegExp(allowed).test(file); return allowed===file; }
const disallowed = files.filter((file) => { const approved = allowedCurrent.some((scope) => exactApprovedScope ? exactScopeMatches(scope,file) : scopeMatches(scope,file)); return exactApprovedScope ? !approved : !allowedInfra.test(file) && !approved; });
if (disallowed.length > 0) { process.stdout.write(disallowed.join('\n')); process.exitCode=1; }
JS
) || true
if [ -n "$SCOPE_RESULT" ]; then echo "Files outside current autopilot scope:"; printf '%s\n' "$SCOPE_RESULT"; echo "Allowed current scope from $STATE_FILE plus source-controlled branch scopes:"; printf '%s\n' "$ALLOWED_CURRENT"; exit 1; fi
