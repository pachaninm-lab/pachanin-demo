...
if is_immutable_scope_branch "$CURRENT_BRANCH" && [ "$CURRENT_BRANCH" != "$SCOPE_GOVERNANCE_BRANCH" ] && [ "$CURRENT_BRANCH" != "$INVENTORY_SCOPE_GOVERNANCE_BRANCH" ] && [ "$CURRENT_BRANCH" != "$PUBLIC_HOME_SCOPE_GOVERNANCE_BRANCH" ] && [ "$CURRENT_BRANCH" != "$POISON_ISOLATION_SCOPE_GOVERNANCE_BRANCH" ]; then
  ...
  if [ "$CURRENT_BRANCH" = "$PROVIDER_NEUTRAL_REVIEW_BRANCH" ]; then
    # Only the reviewed verifier pair is mutable; scope authority stays on the base.
    MUTABLE_SCOPE_AUTHORITIES=$(printf '%s\n' "$MUTABLE_SCOPE_AUTHORITIES" | grep -Fxv \
      -e 'docs/platform-v7/autopilot/verify-pr-review-gate.mjs' \
      -e 'docs/platform-v7/autopilot/verify-pr-review-gate.test.mjs' || true)
  fi
