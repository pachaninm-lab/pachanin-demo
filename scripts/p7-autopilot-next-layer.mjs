...
function parseCurrentCriteria(queueText) {
  return parseSection(queueText, 'CURRENT CRITERIA')
...
function issueMentionsLayer(issueBody, layerName) {
  ...
  if (normalizedLayer.includes('selector')) return /\bPR\s+B\b/i.test(issueBody) && /selector/i.test(issueBody);
  if (normalizedLayer.includes('analyzer')) return /\bPR\s+C\b/i.test(issueBody) && /analy[sz]er/i.test(issueBody);
  if (normalizedLayer.includes('merge gate')) return /\bPR\s+D\b/i.test(issueBody) && /merge gate/i.test(issueBody);
  if (normalizedLayer.includes('dry-run')) return /\bPR\s+E\b/i.test(issueBody) && /dry-run/i.test(issueBody);
  return false;
}

function collectRiskNotes(issueBody) {
  const notes = ['maturity must remain controlled-pilot / pre-integration'];
  if (/forbidden scope/i.test(issueBody)) notes.push('stop if the selected layer requires forbidden scope');
  if (/unmergeable/i.test(issueBody)) notes.push('do not merge unmergeable pull requests');
  if (/fake-live/i.test(issueBody) || /fake live/i.test(issueBody)) notes.push('do not introduce fake-live claims');
  return notes;
}
...
function unsafeAllowedFiles(allowedFiles, forbiddenZones) {
  ...
  for (const file of allowedFiles) {
    for (const forbidden of forbiddenZones) {
      if (scopeMatches(forbidden, file) || scopeMatches(file, forbidden)) unsafe.push(file);
    }
  }
  return unsafe;
}
...
function deterministicBranchName(layerName, options) {
  const slug = slugify(layerName) || 'layer';
  return `${options.branchPrefix}-${slug}-001`;
}
...
export async function selectNextLayer(options = {}) {
  ...
  if (!validation.valid) {
    const forbiddenScopeErrors = validation.errors
      ...
      .filter(Boolean);
    if (forbiddenScopeErrors.length > 0) return stop(`forbidden_scope_required:${forbiddenScopeErrors.join(',')}`, state, queue);
    return stop(`state_queue_conflict:${validation.errors.join(';')}`, state, queue);
  }
  if (state.status !== 'active') return stop(`state_not_active:${state.status}`, state, queue);
  if (state.currentStatus !== 'ready') return stop(`current_not_ready:${state.currentStatus}`, state, queue);
  if (state.openPr) return stop(`open_pr_exists:${state.openPr}`, state, queue);
  if ((state.lastClosed ?? []).includes(state.current)) return stop(`current_already_closed:${state.current}`, state, queue);
  if (!issueMentionsLayer(issueBody, state.current)) return stop(`issue_body_does_not_authorize_current_layer:${state.current}`, state, queue);

  const allowedFiles = (state.allowedCurrentScope ?? []).map(normalizePath).filter(Boolean);
  if (allowedFiles.length === 0) return stop('no_allowed_current_scope', state, queue);

  const forbiddenFiles = (state.forbiddenZones ?? CRITICAL_FORBIDDEN_ZONES).map(normalizePath).filter(Boolean);
  const unsafe = unsafeAllowedFiles(allowedFiles, FORBIDDEN_ALLOWED_SCOPE);
  if (unsafe.length > 0) {
    return {
