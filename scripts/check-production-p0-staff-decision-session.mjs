...
if (selfTest) {
  ...
  if (sha256(controller) !== scope.baselineSha256?.[paths.controller]) failures.push(`${paths.controller}: baseline SHA-256 mismatch`);
  if (sha256(acceptedController) !== scope.acceptedSha256?.[paths.controller]) failures.push(`${paths.controller}: synthesized accepted SHA-256 mismatch`);
  if (sha256(acceptedTest) !== scope.acceptedSha256?.[paths.test]) failures.push(`${paths.test}: synthesized accepted SHA-256 mismatch`);
} else {
  ...
  if (sha256(controller) !== scope.acceptedSha256?.[paths.controller]) failures.push(`${paths.controller}: accepted SHA-256 mismatch`);
