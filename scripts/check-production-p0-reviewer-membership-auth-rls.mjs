...
for (const forbiddenLiteral of [
  ...
  assert.ok(
    !sql.includes(forbiddenLiteral),
    `forbidden authority widening: ${forbiddenLiteral}`,
  );
}

for (const forbiddenPattern of [
  /(?:CREATE|ALTER)\s+ROLE[\s\S]*?\bBYPASSRLS\b/i,
  /(?:CREATE|ALTER)\s+ROLE[\s\S]*?\bSUPERUSER\b/i,
  /GRANT\s+(?:ALL|ALL PRIVILEGES)\b/i,
]) {
  assert.doesNotMatch(sql, forbiddenPattern, 'forbidden role or privilege widening');
}
