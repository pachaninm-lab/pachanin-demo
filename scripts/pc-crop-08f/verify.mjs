...
for (const path of [schemaPath, migrationPath, contractPath, repositoryPath, modulePath]) {
  requireFile(path, 'authority');
}
...
const invariants = {
  ...
  boundedSortedBatch: contract.includes('FGIS_GRAIN_SDIZ_MAX_BATCH_SIZE')
    && contract.includes('.sort('),
  batchFingerprintBound: contract.includes('BATCH_FINGERPRINT_MISMATCH'),
  completePrismaFgisAuthority: [
  ...
  ].every((value) => schema.includes(value)),
  twoFieldSignatureAuthority: schema.includes('signatureAlgorithm        String?   @db.VarChar(64)')
    && schema.includes('signatureAlgorithmUri     String?')
    && migration.includes('ADD COLUMN "signatureAlgorithmUri" text')
    && repository.includes('FGIS_GRAIN_1_0_23_SIGNING_POLICY.algorithms.signatureAlgorithmUri'),
  materializedSignatureUriAuthority: migration.includes('UPDATE public."regulatory_integration_inbox_entries"')
    && migration.includes('materialize_fgis_grain_signature_algorithm_uri')
    && migration.includes('regulatory_integration_inbox_materialize_fgis_signature_uri')
    && migration.includes("NEW.\"signatureStatus\" = 'VERIFIED'")
    && migration.includes("NEW.\"signatureAlgorithm\" = 'GOST3410_2012_256'")
    && migration.includes(exactSignatureUri),
  ...
  canonicalInboxAuthority: repository.includes("inbox.state !== 'PROCESSING'")
    ...
    && repository.includes('signatureAlgorithmUri')
...
for (const [key, value] of Object.entries(invariants)) {
  if (!value) failures.push(`invariant:${key}`);
}
...
writeFileSync(
  join(root, 'pc-crop-08f-acceptance.json'),
