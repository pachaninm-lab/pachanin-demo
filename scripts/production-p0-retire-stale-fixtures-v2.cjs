...
async function main() {
  ...
  try {
    ...
    const candidates = await prisma.$queryRaw(Prisma.sql`
      ...
      LIMIT 1025
    `);
    if (candidates.length > 1024) fail('CANDIDATE_BOUND_EXCEEDED');
    ...
    console.log(`P0_RETIRE_RETIRED_ALL_ROLE=${allRoleRetired}`);
    console.log(`P0_RETIRE_RETIRED_FIRST_CUSTOMER=${firstCustomerRetired}`);
