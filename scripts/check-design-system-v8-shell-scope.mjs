...
const exact = new Set([
  ...
  'apps/web/tests/unit/designSystemV8ShellFieldRoles.test.ts',
  'design-governance-v8.json',
  'scripts/check-design-system-v8-shell-scope.mjs',
]);
...
try {
  ...
  console.error(`[design-system-v8-shell-scope] Cannot resolve merge base for ${base}.`);
  process.exit(1);
}
...
if (files.length === 0) {
  console.error('[design-system-v8-shell-scope] No changed files found.');
  process.exit(1);
}
...
for (const file of files) {
  ...
  if (!allowed(file)) violations.push(`${file}: outside approved shell and field-role scope`);
}

if (violations.length > 0) {
  console.error('[design-system-v8-shell-scope] FAIL');
  for (const violation of violations) console.error(`- ${violation}`);
  process.exit(1);
}

console.log(`[design-system-v8-shell-scope] PASS: ${files.length} changed files are inside the approved scope.`);
