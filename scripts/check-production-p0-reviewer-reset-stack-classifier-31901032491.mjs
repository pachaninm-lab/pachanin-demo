...
for (const token of [`COMMAND='${command}'`,'Password reset challenge/outbox transaction failed','PrismaClientKnownRequestError','AUTH_MAIL_ENQUEUE','ROW_LEVEL_SECURITY','SQLSTATE_42501','PRODUCTION_MUTATION=NONE','raw logs / PII / credentials / reset material']) need('script', script, token);
for (const re of [/password-reset\/request/,/password-reset\/confirm/,/\bcurl\s/,/docker\s+(restart|stop|rm|compose\s+up)/]) deny('script', script, re);
...
if(failures.length){for(const f of failures) console.error(`FAIL: ${f}`); process.exit(1);} console.log('PASS: sanitized reset stack classifier is exact-window, owner-only, read-only and secret-safe.');
