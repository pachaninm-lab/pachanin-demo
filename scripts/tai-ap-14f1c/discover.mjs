...
const EXECUTABLE_HTML_TAGS = new Set(['script', 'style', 'noscript']);
...
function scanHtmlToken(source, start) {
  ...
  while (isHtmlTagNameCharacter(source[cursor])) cursor += 1;
  ...
  while (cursor < source.length) {
    ...
    if (quote !== null) {
      ...
      return { kind: 'tag', name, closing, end: cursor + 1 };
    }
    cursor += 1;
  }
  return { kind: 'tag', name, closing, end: source.length };
}
...
function htmlVisibleText(body) {
  ...
  while (cursor < source.length) {
    if (source[cursor] !== '<') {
      if (blockedTag === null) visible += source[cursor];
      cursor += 1;
      continue;
    }

    const token = scanHtmlToken(source, cursor);
    if (token === null) {
      if (blockedTag === null) visible += source[cursor];
      cursor += 1;
      continue;
    ...
    if (token.kind === 'comment') {
      if (blockedTag === null) visible += ' ';
      cursor = token.end;
      continue;
    }

    if (blockedTag !== null) {
      if (token.closing && token.name === blockedTag) blockedTag = null;
      cursor = token.end;
      continue;
    }

    if (!token.closing && EXECUTABLE_HTML_TAGS.has(token.name)) {
      blockedTag = token.name;
    } else {
      visible += ' ';
    }
    cursor = token.end;
  ...
  return normalizeText(decodeHtmlEntities(visible));
}
...
<Structure xmlns="${SDMX_MESSAGE_NS}"
  ...
  const executableSemanticBodies = [
  '<html><body>Без заключения договора'
    + '<script>Не искажать открытые данные</script ></body></html>',
  '<html><body>Без заключения договора'
    + '<script>Не искажать открытые данные</script\t\n bar></body></html>',
  '<html><body>Без заключения договора'
    + '<style data-value=">">Не искажать открытые данные</style data-x></body></html>',
  '<html><body>Без заключения договора'
    + '<noscript>Не искажать открытые данные</body></html>',
  '<html><body>Без заключения договора'
    + '<!-- Не искажать открытые данные --></body></html>',
];
for (const executableSemanticBody of executableSemanticBodies) {
  let executableMarkerRejected = false;
  try {
    assertBody(semanticResource, Buffer.from(executableSemanticBody));
  } catch {
    executableMarkerRejected = true;
  }
  if (!executableMarkerRejected) {
    fail('ROSSTAT_SELF_TEST_EXECUTABLE_MARKER_ACCEPTED');
  }
}
