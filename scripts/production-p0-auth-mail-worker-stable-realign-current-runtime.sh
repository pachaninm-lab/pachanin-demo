...
root=sys.argv[1]
items=[]
for rel in ('current-key-version','database-url','transport.env'):
    items.append(rel)
keyring=os.path.join(root,'keyring')
for name in sorted(os.listdir(keyring)):
    p=os.path.join(keyring,name)
    st=os.lstat(p)
...
check_image_boundary() {
  ...
  [[ "$entry" == '["/nodejs/bin/node"]' ]]
}

check_current_worker_boundary() {
  local id="$1" image="$2" current_env base_env mounts_json cmd_json entry_json user restart health_test tmpdir
  tmpdir="$(mktemp -d)"
  current_env="$tmpdir/current-env.json"
  base_env="$tmpdir/base-env.json"
  mounts_json="$tmpdir/mounts.json"
  trap 'rm -rf -- "$tmpdir"' RETURN
  docker inspect --format '{{json .Config.Env}}' "$id" > "$current_env"
  docker image inspect --format '{{json .Config.Env}}' "$image" > "$base_env"
  ...
  case "$user" in ''|0|0:0|root|root:root) return 1;; esac
  [[ "$cmd_json" == '["dist/apps/api/src/auth-mail-worker.js"]' ]]
  [[ "$entry_json" == '["/nodejs/bin/node"]' ]]
  [[ "$restart" == unless-stopped ]]
  [[ "$health_test" == *'127.0.0.1:3003/ready'* ]]
  python3 - "$current_env" "$base_env" "$mounts_json" <<'PY'
import json,sys
...
allowed=set(b)|set(expected)
if set(c)-allowed: raise SystemExit(1)
expected_mounts={
...
create_or_validate_overlay() {
  ...
  if [[ -e "$path" ]]; then
    ...
    python3 - "$path" "$image" "$runtime" "$network" <<'PY'
import json,os,sys
...
PY
    chmod 0600 "$path"; chown 0:0 "$path"
    created=1
  fi
  cfg="$(mktemp)"
  set +e
  docker compose --project-directory "$working_dir" --project-name "$project" -f "$path" config --format json > "$cfg" 2>/dev/null
  local render_rc=$?
  set -e
  if (( render_rc != 0 )); then rm -f -- "$cfg"; (( created == 0 )) || rm -f -- "$path"; return 1; fi
...
if str(w.get('pull_policy') or '')!='never': raise SystemExit(1)
cmd=w.get('command') or []
if list(map(str,cmd))!=['dist/apps/api/src/auth-mail-worker.js']: raise SystemExit(1)
...
health=w.get('healthcheck') or {}
if '127.0.0.1:3003/ready' not in ' '.join(map(str,health.get('test') or [])): raise SystemExit(1)
PY
  then
...
rollback() {
  ...
  set +e
  local rb_ok=0
  if [[ -n "$old_worker_image" && -n "$rollback_overlay" ]] && create_or_validate_overlay "$rollback_overlay" "$old_worker_image"; then
    docker compose --project-directory "$working_dir" --project-name "$project" -f "$rollback_overlay" \
      up -d --no-deps --pull never --force-recreate auth-mail-worker >/dev/null 2>&1
    if [[ $? -eq 0 ]] && wait_worker "$worker_revision" 20 >/dev/null 2>&1; then
      rb_ok=1
    fi
  fi
  if (( rb_ok == 1 )); then
...
fail() {
  ...
  emit PRE_WORKER_BOUNDARY "$pre_worker_boundary"
  emit BASELINE_SERVICE_MISSING "$baseline_service_missing"
  emit WORKER_RECREATED "$([[ "$mutation_started" == 1 ]] && echo ATTEMPTED || echo NO)"
  emit WORKER_REVISION UNKNOWN
  emit WORKER_READY UNKNOWN
  emit API_WEB_UNCHANGED UNKNOWN
  emit RUNTIME_UNCHANGED "$runtime_unchanged"
  ...
  elif [[ "$rollback_complete" == PASS ]]; then
    emit PRODUCTION_MUTATION AUTH_MAIL_WORKER_STABLE_REALIGN_ATTEMPTED_ROLLED_BACK
  else
    emit PRODUCTION_MUTATION AUTH_MAIL_WORKER_STABLE_REALIGN_ATTEMPTED
  fi
  exit "$rc"
}
...
target_image="$(docker inspect --format '{{.Config.Image}}' "$api_id" 2>/dev/null || true)"
old_worker_image="$(docker inspect --format '{{.Config.Image}}' "$worker_id" 2>/dev/null || true)"
...
baseline="$(mktemp)"
trap 'rm -f -- "$baseline" 2>/dev/null || true' EXIT
...
PY
then
  baseline_service_missing=PASS
else
  baseline_service_missing=FAIL
  fail BASELINE_SERVICE_PRESENT_USE_STANDARD_REALIGN 52
fi
...
mutation_started=1
docker compose --project-directory "$working_dir" --project-name "$project" -f "$target_overlay" \
  up -d --no-deps --pull never --force-recreate auth-mail-worker >/dev/null 2>&1 || fail WORKER_RECREATE_FAILED 54
...
emit PRE_WORKER_BOUNDARY PASS
emit BASELINE_SERVICE_MISSING PASS
