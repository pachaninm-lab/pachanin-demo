...
COMMAND='/production p0-reviewer-reset-attempt-classify 31819549601 current-main'
SOURCE='scripts/production-p0-reviewer-reset-attempt-classifier.sh'
SOURCE_BLOB='7dcfb19d247aab2f0dc8c8075416673499c9dc84'
TMP="$RUNNER_TEMP/pc-p0-reviewer-reset-attempt-31819549601.sh"
...
replacements = [
    (
        ...
        "ATTEMPT_COMMAND='/production p0-reviewer-reset-attempt-classify 31819549601 current-main'",
    ),
    ("SOURCE_RUN_ID='31706325376'", "SOURCE_RUN_ID='31819549601'"),
    ("ATTEMPT_SINCE='2026-08-13T13:43:10Z'", "ATTEMPT_SINCE='2026-08-14T16:29:44Z'"),
    ("ATTEMPT_UNTIL='2026-08-13T13:43:26Z'", "ATTEMPT_UNTIL='2026-08-14T16:30:00Z'"),
    (
        "SOURCE_REVISION='7c768ad7c54523837b06999a8f69bdffe2a840db'",
        "SOURCE_REVISION='7b66f65f8fc7fc4bbedb56c94088ad1473462c92'",
    ),
]
...
remote_replacements = [
    ...
    (
        ...
        "[[ \"$attempt_since\" == '2026-08-14T16:29:44Z' ]]",
    ),
    (
        "[[ \"$attempt_until\" == '2026-08-13T13:43:26Z' ]]",
        "[[ \"$attempt_until\" == '2026-08-14T16:30:00Z' ]]",
    ),
]
...
if [[ "$VALIDATE_ONLY" == '1' ]]; then
  printf '%s\n' 'PASS: transformed reset 31819549601 classifier preflight'
  exit 0
fi
