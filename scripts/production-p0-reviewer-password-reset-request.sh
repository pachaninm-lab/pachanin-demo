...
# an artifact, an issue comment or a process argument.
...
(async () => {
  ...
  const counts = [
    ...
    Number(row.usable_reviewer_count || 0),
  ...
  if (counts.join('|') !== '1|1|1|1|1|0|0|0') {
    fail('P0_REVIEWER_RESET_READINESS_NOT_EXACT');
  }
...
# Strict email syntax above makes this JSON construction non-injectable. The
# body stays in a root-only temp file and curl reads it from disk so the email is
...
# The web route logs only correlation/account hash/provider metadata. Require a
# synchronous positive provider result for this exact correlation, but never
...
# Only allowlisted non-secret evidence crosses the SSH boundary.
printf 'REVIEWER_PASSWORD_RESET_REQUEST|%s|%s|%s\n' "$post_status" "$correlation_id" "$delivery_ok"
...
IFS='|' read -r tag status correlation_id delivered <<< "$marker"
...
- transactional mail provider reported delivery: \`PASS\`
