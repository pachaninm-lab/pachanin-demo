...
for (const contributor of register.identities.filter((entry) => entry.class === 'HUMAN_CONTRIBUTOR')) {
  ...
  const md = [
    ...
    `| № | файл | строк | blob SHA |`,
    `|---:|---|---:|---|`,
    ...crown.map((f, i) => `| ${i + 1} | \`${markdownCell(f.path, 200)}\` | ${f.lines} | \`${markdownCell(f.blobSha.slice(0, 12))}\` |`),
    ``,
    ...
    ...files.filter((f) => f.criticality !== 'CROWN_JEWEL')
      .map((f, i) => `| ${i + 1} | \`${markdownCell(f.path, 200)}\` | ${f.lines} | ${markdownCell(f.criticality)} | \`${markdownCell(f.blobSha.slice(0, 12))}\` |`),
    ``,
    `## Раздел В. Коммиты автора`,
    ``,
    `| № | SHA | дата | описание |`,
    `|---:|---|---|---|`,
    ...commits.map((c, i) => `| ${i + 1} | \`${markdownCell(c.sha.slice(0, 12))}\` | ${markdownCell(c.date.slice(0, 10))} | ${markdownCell(c.subject)} |`),
    ``,
  ].join('\n');
