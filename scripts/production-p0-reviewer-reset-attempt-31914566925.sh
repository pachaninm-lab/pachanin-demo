...
COMMAND='/production p0-reviewer-reset-attempt-classify 31914566925 current-main'
SOURCE='scripts/production-p0-reviewer-reset-attempt-classifier.sh'
SOURCE_BLOB='7dcfb19d247aab2f0dc8c8075416673499c9dc84'
TMP="$RUNNER_TEMP/pc-p0-reviewer-reset-attempt-31914566925.sh"
...
replacements = [
    (
        ...
        "ATTEMPT_COMMAND='/production p0-reviewer-reset-attempt-classify 31914566925 current-main'",
    ),
    ("SOURCE_RUN_ID='31706325376'", "SOURCE_RUN_ID='31914566925'"),
    ("ATTEMPT_SINCE='2026-08-13T13:43:10Z'", "ATTEMPT_SINCE='2026-08-15T23:23:06Z'"),
    ("ATTEMPT_UNTIL='2026-08-13T13:43:26Z'", "ATTEMPT_UNTIL='2026-08-15T23:23:25Z'"),
    (
        "SOURCE_REVISION='7c768ad7c54523837b06999a8f69bdffe2a840db'",
        "SOURCE_REVISION='0bc2827fea4d5425d6fb7b72c74d25f84a43baf8'",
    ),
]
...
remote_replacements = [
    ...
    (
        ...
        "[[ \"$attempt_since\" == '2026-08-15T23:23:06Z' ]]",
    ),
    (
        "[[ \"$attempt_until\" == '2026-08-13T13:43:26Z' ]]",
        "[[ \"$attempt_until\" == '2026-08-15T23:23:25Z' ]]",
    ),
]
...
if [[ "$VALIDATE_ONLY" == '1' ]]; then
  printf '%s\n' 'PASS: transformed reset 31914566925 classifier preflight'
  exit 0
fi
