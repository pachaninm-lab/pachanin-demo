...
function isAllowed(file, state) {
  return (state.allowedCurrentScope ?? []).some((scope) => scopeMatches(scope, file)) || isAllowedInfra(file);
}
...
function fakeLiveClaims(diffText) {
  ...
  return FORBIDDEN_CLAIMS.filter((claim) => {
    const index = lowered.indexOf(claim.toLowerCase());
    if (index === -1) return false;
    const prefix = lowered.slice(Math.max(0, index - 80), index);
    return !/(no|not|do not|forbidden|запрещ|без)\s[^.\n]*$/.test(prefix);
  });
}
...
function buildAuditRecord({ pr, changedFiles, statusFailuresList, workflowFailuresList, decision, stopReason }) {
  return {
    ...
    autoFixAttempts: 0,
    mergeDecision: decision === 'allowed' ? 'merged' : 'blocked',
    stopReason,
  };
}
