...
COMMAND='/production p0-reviewer-reset-attempt-classify 31821893070 current-main'
SOURCE='scripts/production-p0-reviewer-reset-attempt-classifier.sh'
SOURCE_BLOB='7dcfb19d247aab2f0dc8c8075416673499c9dc84'
TMP="$RUNNER_TEMP/pc-p0-reviewer-reset-attempt-31821893070.sh"
...
replacements = [
    (
        ...
        "ATTEMPT_COMMAND='/production p0-reviewer-reset-attempt-classify 31821893070 current-main'",
    ),
    ("SOURCE_RUN_ID='31706325376'", "SOURCE_RUN_ID='31821893070'"),
    ("ATTEMPT_SINCE='2026-08-13T13:43:10Z'", "ATTEMPT_SINCE='2026-08-14T17:00:02Z'"),
    ("ATTEMPT_UNTIL='2026-08-13T13:43:26Z'", "ATTEMPT_UNTIL='2026-08-14T17:00:16Z'"),
    (
...
remote_replacements = [
    ...
    (
        ...
        "[[ \"$attempt_since\" == '2026-08-14T17:00:02Z' ]]",
    ),
    (
        "[[ \"$attempt_until\" == '2026-08-13T13:43:26Z' ]]",
        "[[ \"$attempt_until\" == '2026-08-14T17:00:16Z' ]]",
    ),
]
...
if [[ "$VALIDATE_ONLY" == '1' ]]; then
  printf '%s\n' 'PASS: transformed reset 31821893070 classifier preflight'
  exit 0
fi
