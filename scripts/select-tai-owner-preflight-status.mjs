...
const STATUS_CONTEXT = 'TAI REG.RU Preflight';

function parseRunId(targetUrl, repositoryFullName) {
  if (typeof targetUrl !== 'string' || targetUrl.length > 2048) {
    throw new Error('Preflight status target URL is missing or invalid.');
  }
  let url;
  try {
    url = new URL(targetUrl);
  } catch {
    throw new Error('Preflight status target URL is malformed.');
  }
  if (url.protocol !== 'https:' || url.hostname !== 'github.com' || url.search || url.hash) {
    throw new Error('Preflight status target URL is outside canonical GitHub Actions authority.');
  }
  ...
  if (!match) {
    throw new Error('Preflight status target URL does not identify a run in this repository.');
  }
  const runId = Number(match[1]);
  if (!Number.isSafeInteger(runId) || runId < 1) {
    throw new Error('Preflight status run ID is invalid.');
  }
  return runId;
}

export function selectTaiOwnerPreflightStatus(report, targetSha, repositoryFullName) {
  ...
  if (matching.length < 1) {
    throw new Error(`Exact commit status ${STATUS_CONTEXT} is missing.`);
  }
  ...
  return Object.freeze({
    runId: parseRunId(latest.target_url, repositoryFullName),
...
if (import.meta.url === new URL(`file://${process.argv[1]}`).href) {
  ...
  if (!path || !targetSha || !repositoryFullName) {
    console.error('Usage: select-tai-owner-preflight-status.mjs <status.json> <exact-sha> <owner/repo>');
    process.exit(64);
  }
  try {
    const report = JSON.parse(readFileSync(path, 'utf8'));
    const selected = selectTaiOwnerPreflightStatus(report, targetSha, repositoryFullName);
