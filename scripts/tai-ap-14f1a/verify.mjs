...
assert(manifest.schema_version === 'tai.migration.manifest.v1', 'migration manifest schema changed');
...
assert(JSON.stringify(versions) === JSON.stringify(Array.from({ length: 20 }, (_, index) => index + 1)), 'migration versions must be exactly 1..20');
const latest = manifest.migrations.at(-1);
assert(latest?.path === '0019_public_official_corpus.sql' && latest.version === 20, 'AP-14F1A migration must be version 20');
...
assert(!/DROP\s+(TABLE|SCHEMA)|TRUNCATE|CASCADE\s*;/iu.test(sql), 'destructive SQL is forbidden');
...
for (const token of [
  ...
  'data_plane, source_class, rights_decision_id',
  'tai_activate_public_corpus_snapshot',
  'tai_withdraw_public_corpus_source',
  'tai_active_public_corpus_chunks_v1',
...
for (const path of [
  ...
  assert(!/\b(requests|httpx|socket|subprocess|urllib\.request)\b/u.test(source), `${path} may not perform network or process execution`);
  assert(!/(password|private[_-]?key|access[_-]?key|secret[_-]?key)\s*=/iu.test(source), `${path} may not contain credential material`);
}
...
if (scopeIndex !== -1) {
  ...
  assert(JSON.stringify(changedPaths) === JSON.stringify(allowedPaths), `scope mismatch: ${changedPaths.join(', ')}`);
}
