...
const scopePath='docs/platform-v7/autopilot/scopes/tai-reg-ru-local-runner-authority-20260731.json';
...
for(const fragment of [
  ...
  'Verify canonical exact-SHA rootless TAI image outside production','Execute strict local read-only pre-deployment inventory',
  'Recover active private model token from local API without disclosure','Deploy exact TAI image locally on existing REG.RU authority',
  'Collect redacted local deployment evidence','Execute strict local post-deployment preflight',
  'Roll back failed local deployment acceptance',"context='TAI REG.RU Deployment'",
  'TAI_SERVICE_NOT_MATERIALIZED','TAI_DEDICATED_ENV_NOT_MATERIALIZED','TAI_DEDICATED_DB_PRINCIPAL_NOT_ATTESTED',
...
forbid(deploy,/ALTER\s+ROLE\s+\$?\{?ROLE_NAME\}?/iu,`${deployPath}: existing role mutation is forbidden`);
...
forbid(deploy,/compose[.]base[.]json/iu,`${deployPath}: expanded Compose configuration must not persist`);

if(scope.schemaVersion!=='platform-v7.concurrent-scope.v1')violations.push(`${scopePath}: invalid schemaVersion`);
if(scope.branch!=='agent/tai-reg-ru-local-runner-authority-20260731')violations.push(`${scopePath}: branch mismatch`);
...
console.log('TAI REG.RU deployment contract PASS: exact-main, production-local, immutable-digest, rootless-runtime, rollback-bound and zero-cost.');
