...
test.describe('platform-v7 mobile compact shell isolation', () => {
  for (const route of COMPACT_ROLE_ROUTES) {
    test(`${route} keeps compact mobile shell without navigation panel`, async ({ page }) => {
      ...
      await expect(page.locator('[data-role-header-switcher="true"]')).toHaveCount(1);
