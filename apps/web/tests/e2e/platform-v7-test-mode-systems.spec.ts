...
test.describe('platform-v7 test mode systems panel', () => {
  test('renders on connectors page and keeps external systems in check mode', async ({ page }) => {
    ...
    await expect(page.getByText('Внешние подключения · контур проверки')).toBeVisible();
    await expect(page.getByText('Статусы внешних контуров')).toBeVisible();
