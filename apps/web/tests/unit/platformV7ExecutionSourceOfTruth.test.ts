...
describe('platform-v7 deal execution source of truth', () => {
  ...
  it('executionBlockers returns only non-empty blockers from non-ready gates', () => {
    ...
    // ready gates must not contribute blockers
  ...
  it('canRequestMoneyRelease is false when holdRub > 0', () => {
    ...
    // The source has holdRub = 0, so we verify logic via the function directly
    // by checking the conditions manually
    ...
    // if holdRub would be > 0, release must be false regardless of gates
    if (money.holdRub > 0) {
      expect(canRequestMoneyRelease()).toBe(false);
    } else {
      // With current demo data, gates are not all ready, so result is still false
  ...
  it('reservedRub is consistent with deal amount', () => {
    ...
    // reservedRub must not exceed the deal amount without explanation
    expect(money.reservedRub).toBeLessThanOrEqual(expected);
    // in this demo it equals exactly volume * price
