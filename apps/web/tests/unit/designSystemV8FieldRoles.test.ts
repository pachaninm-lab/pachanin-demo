...
describe('Design System v8 field-role migration', () => {
  ...
  it('keeps field density and touch targets token-governed', () => {
    const template = read('apps/web/components/transaction-ux/FieldTaskTemplate.tsx');
    const sharedStyles = read('apps/web/components/transaction-ux/FieldRoleWorkspace.module.css');
    const tokens = read('packages/design-tokens/tokens.css');
    expect(template).toContain("data-density='field'");
    expect(sharedStyles).toContain('var(--ds-control-height)');
  ...
  it('preserves role-specific evidence/runtime components', () => {
    expect(read(fieldRolePages[0])).toContain('DriverOfflineQueue');
    expect(read(fieldRolePages[1])).toContain('WeighStationPanel');
    expect(read(fieldRolePages[2])).toContain('GostQualityForm');
    expect(read(fieldRolePages[3])).toContain('RoleExecutionSummary');
  });
});
