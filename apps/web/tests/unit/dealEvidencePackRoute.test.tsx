...
describe('DealEvidencePackPage', () => {
  it('renders deal evidence pack preview route', async () => {
    render(await DealEvidencePackPage({ params: { id: 'DL-9113' } }));

    expect(screen.getByText('Доказательный пакет · DL-9113')).toBeInTheDocument();
  ...
  it('links back to the deal route', async () => {
    render(await DealEvidencePackPage({ params: { id: 'DL-9113' } }));
