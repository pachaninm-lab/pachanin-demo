...
const fixtureStateMarker = ['FARMER', 'FGIS', 'ACCESS', 'STATE'].join('_');
const fixtureLotMarker = ['FGIS', 'LOT', '2607', '014'].join('-');
const fixtureCertificateMarker = ['SDIZ', '2607', '5512'].join('-');
const forbiddenFixtureMarkers = [
  fixtureStateMarker,
  fixtureLotMarker,
  fixtureCertificateMarker,
  'ООО «Колос-Агро»',
  '7701234567',
];
...
describe('platform-v7 FGIS access PostgreSQL authority flow', () => {
  ...
  it('cross-checks tenant actor and lot identity and remains read-only', () => {
    ...
    expect(workspace).not.toContain(fixtureLotMarker);
  });
