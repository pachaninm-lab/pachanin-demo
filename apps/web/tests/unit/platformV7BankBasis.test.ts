...
function doc(documentId: string, title: string, ownerRole: PlatformV7CanonicalRole, status: PlatformV7DocumentStatus, source: PlatformV7DocumentSource): PlatformV7DocumentRequirement {
  return { documentId, dealId: 'deal-1', type: documentId, title, ownerRole, responsibleRole: ownerRole, status, blockStages: ['release'], affectsMoney: true, source, deadline: null, signatureStatus: 'not_required', nextAction: 'ok', createdAt: '', updatedAt: '' };
}
...
const bankBasisRequestedTree: PlatformV7MoneyTree = {
  dealId: 'deal-1',
  currency: 'RUB',
...
function readyBasis() {
  return p7BuildBankBasis({ dealId: 'deal-1', releaseGate: bankBasisGate, documents: docs, disputeResolved: true, amount: 300, currency: 'RUB', correlationId: 'corr-1', auditId: 'audit-1' });
}

function readySentBasis() {
  return p7MarkBankBasisSent({ decision: readyBasis(), moneyTree: bankBasisRequestedTree, actorId: 'operator-1', actorRole: 'operator', organizationId: 'org-operator', createdAt: '2026-05-23T07:50:00.000Z' }).decision;
}
...
function confirmation<Path extends P7BankConfirmationPath>(path: Path, overrides: Partial<P7BankConfirmationEvent<Path>> = {}): P7BankConfirmationEvent<Path> {
  ...
  return {
    bankEventId,
    idempotencyKey: overrides.idempotencyKey ?? buildPlatformV7IdempotencyKey({ boundaryId: 'confirm_bank_basis', actorId: overrides.actorId ?? 'bank-1', entityId: 'money-1', dealId: 'deal-1', amountMinor: amount, currency: 'RUB', attemptId: bankEventId }),
    path,
...
describe('platform-v7 bank basis foundation', () => {
  ...
  it('blocks bank basis when gate, documents or dispute are not ready', () => {
    const decision = p7BuildBankBasis({ dealId: 'deal-1', releaseGate: { allowed: false, reason: 'blocked', nextStatus: 'blocked' }, documents: [{ ...docs[0], status: 'missing' }, docs[1], docs[2]], disputeResolved: false, amount: 1000, currency: 'RUB', correlationId: 'corr-1', auditId: 'audit-1' });
  ...
  it('confirms bank basis only from sent basis and records bank_basis_confirmed operation', () => {
    const result = p7ConfirmBankRelease({ decision: readySentBasis(), moneyTree: bankBasisRequestedTree, confirmation: confirmation('release'), ...noHistory });
  ...
  it('uses first-class functions for refund, hold, reject and manual-review paths', () => {
    ...
    const reject = p7RejectBankRelease({ decision: readySentBasis(), moneyTree: bankBasisRequestedTree, confirmation: confirmation('reject', { amount: 0 }), ...noHistory });
  ...
  it('blocks non-bank actors and duplicate bank events without mutating MoneyTree', () => {
    const denied = p7ConfirmBankRelease({ decision: readySentBasis(), moneyTree: bankBasisRequestedTree, confirmation: confirmation('release', { actorRole: 'arbitrator', actorId: 'arb-1' }), ...noHistory });
    ...
    const duplicate = p7ConfirmBankRelease({ decision: readySentBasis(), moneyTree: bankBasisRequestedTree, confirmation: first, existingBankEventIds: [first.bankEventId], usedIdempotencyKeys: [], existingOperationIds: [] });
