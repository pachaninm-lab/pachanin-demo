...
describe('Design System v8 foundation', () => {
  ...
  it('uses a legacy-free runtime for accepted v8 routes', () => {
    ...
    expect(runtime).not.toContain('@/app/v9');
    expect(runtime).not.toContain('@/styles/platform-v7-');
    expect(layout).toContain('isDesignSystemV8Route(pathname)');
    expect(layout).toContain('<PlatformV7DesignSystemV8Runtime>{protectedContent}</PlatformV7DesignSystemV8Runtime>');
  });

  it('keeps legacy styles inside an explicit compatibility runtime', () => {
    ...
    expect(runtime).toContain('Historical and partially migrated routes');
  });
