...
const source = fs.readFileSync(path.join(process.cwd(), 'apps/web/components/v7r/BankRuntime.tsx'), 'utf8');
...
describe('platform-v7 bank runtime copy guard', () => {
  it('keeps BankRuntime framed around bank basis review', () => {
    ...
    expect(renderSource).toContain('финальное банковское подтверждение нельзя считать безопасным');
    expect(renderSource).toContain('зелёный сценарий банковской проверки');
  });

  it('normalizes imported callback and transport labels before display', () => {
  ...
  it('does not render autonomous payment-release wording outside the normalizer', () => {
    ...
    expect(renderSource).not.toContain('с выпуском денег');
