...
function webPath(path: string): string { return join(process.cwd(), path); }
function readWebFile(path: string): string { return readFileSync(webPath(path), 'utf8'); }
...
const forbiddenRuntimeImports = ['apps/landing', 'components/platform-v7', 'components/v7r', 'lib/platform-v7/adapters', 'lib/platform-v7/ai', 'app/api'] as const;
const forbiddenLiveCalls = ['fetch(', 'XMLHttpRequest', 'WebSocket', 'EventSource', 'axios.', 'bank connected', 'fgis connected', 'edo connected'] as const;

describe('platform-v7 Stage 5 final runtime QA', () => {
  it('keeps the complete Stage 5 runtime file set present after merge', () => {
    stage5RuntimeFiles.forEach((path) => expect(existsSync(webPath(path)), path).toBe(true));
  });
  ...
  it('keeps runtime server actions as wrappers over application services', () => {
    ...
    expect(source).not.toContain('createP7ReleaseWorkflowService');
    expect(source).not.toContain('executeP7RuntimeReleaseWorkflowAction');
  });

  it('keeps runtime server actions isolated from UI, adapters and live network calls', () => {
    const source = readWebFile('app/platform-v7/actions/runtime-actions.ts');
    [...forbiddenRuntimeImports, ...forbiddenLiveCalls].forEach((forbidden) => expect(source).not.toContain(forbidden));
  });
  ...
  it('keeps application services behind persistence ports and deterministic result contracts', () => {
    ...
    expect(service).toContain('createP7BankBasisWorkflowService');
    expect(service).not.toContain('createP7ReleaseWorkflowService');
    ...
    expect(types).toContain('P7BankBasisWorkflowService');
    expect(types).not.toContain('P7ReleaseWorkflowService');
  });
  ...
  it('keeps DTO validation, mock persistence and runtime integration coverage aligned', () => {
    ...
    expect(dtoSchemas).toContain('validateP7BankBasisRequestDto');
    ...
    expect(dtoSchemas).not.toContain('validateP7ReleaseRequestDto');
