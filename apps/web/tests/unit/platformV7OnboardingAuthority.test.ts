...
describe('platform-v7 onboarding authority', () => {
  ...
  it('keeps external systems outside the onboarding authority boundary', () => {
    ...
    expect(page).toContain('does not connect a bank, grain registry, EDI or transport system');
    expect(page).toContain('不会连接银行、粮食登记、电子单证或运输系统');
    expect(page).toContain("href: '/platform-v7/connectors'");
  });

  it('runs under the universal protected v8 runtime and remains governed', () => {
