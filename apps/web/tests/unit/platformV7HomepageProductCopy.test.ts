...
const publicHeader = read('components/platform-v7/PublicSiteHeader.tsx');
...
describe('platform-v7 linked public-page trust', () => {
  it('keeps About crop-oriented, bounded in the canonical header and free of internal maturity shorthand', () => {
    ...
    expect(about).toContain('showMobileMenu');
    expect(about).toContain('localeControl={<PublicLocaleLink />}');
    expect(about).toContain('href={`/platform-v7/register${lang}`}');
    expect(publicHeader).toContain('min-width: 44px !important');
    expect(publicHeader).toContain('height: 44px !important');
  ...
  it('keeps Contact a real inquiry channel and preserves one server-owned locale across content and chrome', () => {
    ...
    expect(contactHeader).toContain('href={`/platform-v7/login${suffix}`}');
