...
const confirmedPaths = [
  ...
  '/api/auth/me',
  '/api/deals',
  '/api/deals/{dealId}/workspace',
  '/api/deals/{dealId}/execution-workspace',
  '/api/logistics/shipments',
  '/api/logistics/shipments/{shipmentId}/workspace',
  '/api/disputes',
  '/api/disputes/{disputeId}',
  '/api/settlement-engine/deal/{dealId}/bank-workspace',
  '/api/settlement-engine/outbox',
...
describe('platform-v7 verified API contract', () => {
  ...
  it('uses a protected Design System v8 route without synthetic API UI', () => {
  ...
  it('keeps mutating and external-provider contracts unpublished', () => {
    expect(canonical).not.toContain('/bank-callback');
    expect(canonical).not.toContain('/fgis/webhook');
    expect(canonical).not.toContain('/edo/webhook');
    expect(canonical).not.toContain('/commands/{actionId}');
    expect(canonical).not.toContain('/reserve');
    expect(canonical).not.toContain('/release');
    expect(canonical).not.toContain('/api/api/partner');
