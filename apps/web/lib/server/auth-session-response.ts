...
export type SurfaceRole = 'operator' | 'buyer' | 'seller' | 'logistics' | 'driver' | 'elevator' | 'lab' | 'surveyor' | 'bank' | 'arbitrator' | 'compliance' | 'executive';
...
export function platformHome(role: SurfaceRole) {
  const routes: Record<SurfaceRole, string> = {
    operator: '/platform-v7/control-tower', buyer: '/platform-v7/buyer', seller: '/platform-v7/seller',
    logistics: '/platform-v7/logistics', driver: '/platform-v7/driver', elevator: '/platform-v7/elevator',
    lab: '/platform-v7/lab', surveyor: '/platform-v7/surveyor', bank: '/platform-v7/bank',
    arbitrator: '/platform-v7/arbitrator', compliance: '/platform-v7/compliance', executive: '/platform-v7/executive',
  };
  return routes[role];
}

export async function applyAuthenticatedSession(response: NextResponse, payload: AuthenticatedSessionPayload): Promise<{ role: SurfaceRole; redirectTo: string } | null> {
  ...
  const cabinetToken = await signCabinetSession(role, secret, { nowSeconds: Math.floor(Date.now() / 1000), ttlSeconds: expiresIn });
  ...
  response.cookies.set(SESSION_COOKIE, encodeURIComponent(JSON.stringify({ role, exp, email: payload.user.email })), sessionMarkerCookie());
  ...
  response.cookies.set(CABINET_SESSION_COOKIE, cabinetToken, {
    path: '/', maxAge: expiresIn, httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production',
  });
