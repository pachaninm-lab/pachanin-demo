...
export const ACTION_FEEDBACK_PREVIEWS: Record<ActionFeedbackContext, ActionFeedbackPreview> = {
  seller: {
    ...
    whatHappensNext:
      'пилотный контур фиксирует намерение — документы передаются на ручную проверку оператором перед тем, как выплата попадёт на банковскую проверку',
    auditDraft:
      'черновик записи: «продавец инициировал передачу СДИЗ/ЭТрН — ожидается подтверждение ФГИС «Зерно» и оператора»',
  ...
  bank: {
    actionLabel: 'выпустить резерв на проверку',
    whatHappensNext:
      'пилотный контур фиксирует решение о выпуске — все условия (СДИЗ, ЭТрН, приёмка, спор) должны быть закрыты; банк проводит финальную ручную проверку перед фактическим движением денег',
    auditDraft:
      'черновик записи: «банк инициировал проверку условий выпуска резерва по DL-9106 — ожидается закрытие всех блокеров»',
    ...
    externalConfirmationBoundary:
      'фактический выпуск денег — внешнее банковское событие; пилотный контур только фиксирует намерение и черновик записи',
    pilotNote:
