...
export const ACTION_FEEDBACK_PREVIEWS: Record<ActionFeedbackContext, ActionFeedbackPreview> = {
  seller: {
    ...
    whatHappensNext:
      'контур исполнения фиксирует намерение — документы передаются на ручную проверку оператором перед передачей выплаты на банковскую проверку',
    auditDraft:
    ...
    externalConfirmationBoundary:
      'ФГИС «Зерно» должен подтвердить СДИЗ; банк подтверждает резерв отдельно — контур не создаёт внешние события без подтверждения',
    pilotNote:
      'ручная проверка · деньги не движутся до банковского события',
  },
  buyer: {
    actionLabel: 'запросить банковское подтверждение резерва',
    whatHappensNext:
      'контур исполнения фиксирует запрос резерва — банк проверяет готовность средств вручную; сделка не переходит к логистике до получения подтверждения',
    auditDraft:
    ...
    externalConfirmationBoundary:
      'банк подтверждает или отклоняет резерв — контур не генерирует банковское событие автоматически',
    pilotNote:
      'ручная проверка · банковское событие не эмитируется автоматически',
  },
  bank: {
    actionLabel: 'передать резерв на банковскую проверку',
    whatHappensNext:
      'контур исполнения фиксирует решение о проверке — все условия сделки должны быть закрыты; банк проводит финальную ручную проверку перед фактическим движением денег',
    auditDraft:
    ...
    externalConfirmationBoundary:
      'фактическое движение денег — внешнее банковское событие; контур только фиксирует намерение и черновик записи',
    pilotNote:
      'ручная проверка · фактическое движение денег происходит только после банковского подтверждения',
  },
  disputes: {
    actionLabel: 'зафиксировать решение по спору',
    whatHappensNext:
      'контур исполнения фиксирует решение оператора — удержание корректируется или снимается после ручной проверки документов; банк получает уведомление о решении',
    auditDraft:
    ...
    externalConfirmationBoundary:
      'банк применяет корректировку удержания — контур фиксирует черновик; фактическое изменение — банковское событие',
    pilotNote:
      'ручная проверка · изменение удержания не происходит автоматически',
  },
};
