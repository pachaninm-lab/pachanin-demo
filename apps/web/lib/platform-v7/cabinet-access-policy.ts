...
// операциям) гейтингом на уровне маршрутов-кабинетов. По умолчанию НЕ применяется
// (пилот: «вход открыт во все ЛК»). Включается флагом окружения или в боевом
// контуре — тогда участник видит только свой кабинет, а перейти в другой можно
// через экран выбора кабинета. Надзорные роли (оператор, руководитель) видят всё.
...
export function canRoleAccessCabinet(role: PlatformRole, pathname: string): boolean {
  ...
  // Маршрут «принадлежит» роли по тому же выводу, что и оболочка.
  const owningRole = inferPlatformRoleFromPath(pathname, role);
