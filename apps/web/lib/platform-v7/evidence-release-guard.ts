...
export function decideReleaseWithEvidencePack(
  ...
  const evidenceReady = readiness.status === 'ready_for_review';
  const release = decideMoneyRelease({
    ...releaseInput,
    docsComplete: releaseInput.docsComplete && evidenceReady,
  });

  return {
    state: evidenceReady ? 'evidence_ready' : 'evidence_incomplete',
    readiness,
    release,
  };
}
