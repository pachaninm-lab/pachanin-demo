...
export const PROVIDER_CONNECTION_STATUS_LABEL: Record<ProviderConnectionStatus, string> = {
  ...
  test_connected: 'контур проверки',
...
export const PLATFORM_V7_PROVIDER_REGISTRY: Record<ProviderCode, ProviderBranding> = {
  sber_safe_deals: {
    ...
    logoMode: 'text',
    publicLabel: 'Сбер · контур проверки',
  ...
  sber_business_id: {
    ...
    connectionStatus: 'test_connected',
    logoMode: 'text',
    publicLabel: 'СберБизнес ID · контур проверки',
    legalClaim: 'test_access',
  ...
  sber_credit: {
    ...
    visibility: 'public_fact_source',
    nextLiveStep: 'заключить договор, получить виджет и проверить подключение на стенде',
    hasContract: false,
    hasCredentials: false,
    hasCallbacks: false,
    confirmedOperationsCount: 0,
