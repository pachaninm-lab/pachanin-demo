...
export function platformV7DemoDashboardModel(): PlatformV7DemoDashboardModel {
  return {
    title: 'Сквозной сценарий исполнения сделки',
    subtitle: 'Показ за 2–3 минуты: лот → RFQ → логистика → приёмка → качество → деньги.',
    disclosure: 'Сквозной сценарий показывает механику контура исполнения и не является подтверждением боевого исполнения без реальных интеграций.',
...
export function platformV7DemoDashboardIsReady(model = platformV7DemoDashboardModel()): boolean {
  return (
    ...
    model.disclosure.includes('контура исполнения')
  );
}
