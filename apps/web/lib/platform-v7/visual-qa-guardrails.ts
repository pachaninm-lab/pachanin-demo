...
export const PLATFORM_V7_QA_VIEWPORTS: Readonly<Record<PlatformV7ViewportKey, { width: number; label: string }>> = {
  android360: { width: 360, label: 'Android 360 / Chrome' },
...
export const PLATFORM_V7_WORLD_CLASS_VISUAL_ACCEPTANCE = [
  'единая визуальная система на всех экранах',
  'без горизонтальной прокрутки на мобильных экранах',
  'одинаковая плотность, отступы, кнопки и состояния',
  'полноценный светлый и тёмный режим без светлых пятен',
  'понятность за 5 секунд для каждой роли',
  'сильные пустые, ошибочные и загрузочные состояния',
  'мягкая реакция интерфейса без визуального шума',
  'русский деловой язык без технических следов',
...
export const PLATFORM_V7_VISUAL_QA_ROUTES: readonly PlatformV7VisualQaRoute[] = [
  {
    ...
    group: 'core',
    priority: 'P0',
    expectedSurface: 'премиальный центр управления сделками, не витрина лотов',
    mustAnswer: ['что происходит сейчас', 'цепочка сделки', 'где деньги', 'где груз', 'где документы', 'что заблокировано', 'следующее действие'],
  },
  {
    path: '/platform-v7/seller',
    group: 'core',
    priority: 'P0',
    expectedSurface: 'кабинет продавца: лоты, сделка, документы, деньги',
    mustAnswer: ['мой лот', 'принятое предложение', 'почему деньги не выпущены', 'документы', 'следующий ответственный', 'без лишнего банкового шума'],
  },
  {
    path: '/platform-v7/buyer',
    group: 'core',
    priority: 'P0',
    expectedSurface: 'кабинет покупателя: закупка, резерв, качество, следующий шаг',
    mustAnswer: ['моя ставка', 'мой резерв', 'сценарий оплаты', 'чужие закрытые ставки скрыты', 'следующее действие'],
  },
  {
    ...
    mustAnswer: ['заявка после выбора победителя', 'машина', 'водитель', 'время прибытия', 'транспортные документы', 'без цены зерна и банкового резерва'],
  },
  {
  ...
  },
  {
    ...
    group: 'core',
    priority: 'P0',
    expectedSurface: 'мобильный полевой кабинет водителя',
