...
export type ExecutionStageKey = 'deal_created' | 'carrier_assigned' | 'driver_assigned' | 'pickup_arrived' | 'loaded' | 'in_transit' | 'elevator_arrived' | 'weighed' | 'lab_passed' | 'documents_signed' | 'money_ready';
...
const stages: ExecutionStage[] = [
  { key: 'deal_created', title: 'Сделка создана', owner: 'operator', description: 'Лот и выигравшая ставка зафиксированы в единой сделке.', requiredForMoney: true },
  { key: 'carrier_assigned', title: 'Перевозчик назначен', owner: 'logistics', description: 'Логист выбирает перевозчика, машину и окно подачи.', requiredForMoney: true },
  { key: 'driver_assigned', title: 'Водитель назначен', owner: 'driver', description: 'Водитель получает рейс, маршрут, пломбу и контрольные точки.', requiredForMoney: true },
...
function roleClosures(dealId: string): ExecutionRoleClosure[] {
  return [
    { role: 'seller', label: 'Продавец', route: `/platform-v7/deals/${dealId}/execution`, sees: 'рейс, приёмку, документы и причину задержки денег', action: 'контролирует отгрузку и документы', next: `/platform-v7/deals/${dealId}/documents`, closed: true },
    { role: 'buyer', label: 'Покупатель', route: `/platform-v7/deals/${dealId}/execution`, sees: 'груз, приёмку, качество и основание выпуска денег', action: 'подтверждает готовность к оплате после приёмки', next: `/platform-v7/bank/release-safety`, closed: true },
    ...
    { role: 'logistics', label: 'Логистика', route: '/platform-v7/logistics', sees: 'перевозчика, водителя, машину, ETA и маршрут', action: 'назначает рейс и контролирует срыв подачи', next: `/platform-v7/deals/${dealId}/execution`, closed: true },
    { role: 'driver', label: 'Водитель', route: '/platform-v7/driver', sees: 'только свой рейс, маршрут и кнопки статуса', action: 'фиксирует прибытие, погрузку, движение и прибытие', next: `/platform-v7/deals/${dealId}/execution`, closed: true },
    { role: 'elevator', label: 'Элеватор', route: '/platform-v7/elevator', sees: 'машину, очередь, вес, приёмку и отклонения', action: 'подтверждает вес и факт приёмки', next: `/platform-v7/deals/${dealId}/execution`, closed: true },
    { role: 'lab', label: 'Лаборатория', route: '/platform-v7/lab', sees: 'пробу, показатели качества и протокол', action: 'закрывает качество или открывает отклонение', next: `/platform-v7/deals/${dealId}/execution`, closed: true },
...
export function selectExecutionSimulationByDealId(dealId: string): ExecutionSimulation | null {
  ...
  return {
    ...
    releaseBlockers: ['рейс не завершён', 'приёмка не подтверждена', 'лаборатория не закрыта', 'ЭДО-пакет не подписан'],
