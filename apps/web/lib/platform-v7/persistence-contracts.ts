...
export const PLATFORM_V7_PERSISTENCE_CONTRACTS: readonly PlatformV7PersistenceContract[] = [
  ...
  { entity: 'market_intent', tableName: 'platform_v7_market_intents', storageMode: 'required', requiresOwnerId: true, requiresDealId: false, requiresAuditLink: true, requiresIdempotencyKey: true, canBeDeletedByUser: false, summary: 'Pre-deal buy or sell intent with source price, route price, gate status and conversion target.' },
  ...
  { entity: 'support_case', tableName: 'platform_v7_support_cases', storageMode: 'required', requiresOwnerId: true, requiresDealId: false, requiresAuditLink: true, requiresIdempotencyKey: true, canBeDeletedByUser: false, summary: 'Support case tied to documents, logistics, acceptance, quality, dispute or access blockers.' },
  { entity: 'dispute', tableName: 'platform_v7_disputes', storageMode: 'required', requiresOwnerId: false, requiresDealId: true, requiresAuditLink: true, requiresIdempotencyKey: true, canBeDeletedByUser: false, summary: 'Dispute record with reason, evidence pack, decision state and value impact.' },
  ...
  { entity: 'idempotency_record', tableName: 'platform_v7_idempotency_records', storageMode: 'required', requiresOwnerId: false, requiresDealId: false, requiresAuditLink: true, requiresIdempotencyKey: true, canBeDeletedByUser: false, summary: 'Idempotency store that prevents duplicate sensitive actions.' },
  { entity: 'reconciliation_record', tableName: 'platform_v7_reconciliation_records', storageMode: 'append_only', requiresOwnerId: false, requiresDealId: true, requiresAuditLink: true, requiresIdempotencyKey: true, canBeDeletedByUser: false, summary: 'Reconciliation record for mismatches, manual review, source amount and resolved state.' },
];
