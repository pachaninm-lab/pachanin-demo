...
function validationFailure<T>(validation: P7ValidationResult<T>): P7ApplicationServiceResult<never> {
  return {
    ...
    reason: 'Runtime DTO validation failed.',
...
function runtimeFailure<T>(result: P7RuntimeResult<T>): P7ApplicationServiceResult<never> {
  const error = 'error' in result
    ? result.error
    : { code: 'transaction_error' as const, message: 'Unexpected runtime result.' };

  return {
    ok: false,
    status: error.code === 'duplicate'
      ? 'duplicate'
      : error.code === 'conflict'
        ? 'conflict'
...
function repositoryFailure<T>(result: P7RepositoryResult<T>): P7ApplicationServiceResult<never> {
  const error = 'error' in result
    ? result.error
    : { code: 'persistence_error' as const, message: 'Unexpected repository result.' };

  return {
    ok: false,
    status: result.status === 'not_found' ? 'not_found' : result.status === 'conflict' ? 'conflict' : 'domain_blocked',
    code: error.code,
    reason: error.message,
    auditPayloads: [],
  };
...
function boundaryFailure<T>(
  boundary: PlatformV7ActionBoundaryResult,
  auditPayloads: readonly P7AuditPayload[],
): P7ApplicationServiceResult<T> {
  return {
    ok: false,
    status: boundary.status === 'denied' ? 'denied' : boundary.status === 'duplicate' ? 'duplicate' : 'domain_blocked',
    code: boundary.code,
...
function persisted<T>(
  value: T,
  boundary: PlatformV7ActionBoundaryResult | undefined,
  auditPayloads: readonly P7AuditPayload[],
): P7ApplicationServiceResult<T> {
  return {
    ok: true,
...
function saveOptions(dto: P7RuntimeRequestBaseDto, record: P7PersistedRecord<unknown>, ports: P7RuntimeTransactionalPorts): P7SaveOptions {
  return {
    expectedVersion: record.version.version,
...
function moneyOperation(dto: P7ReleaseRequestDto, type: PlatformV7MoneyOperation['type'], createdAt: string): PlatformV7MoneyOperation {
  return {
    operationId: operationId(dto),
    dealId: dto.resource.dealId,
    type,
    amount: dto.amount,
    currency: dto.currency,
    basisDocumentIds: [dto.resource.resourceId],
    actorId: dto.actor.actorId,
...
function auditPayloadsFromBoundary(boundary: PlatformV7ActionBoundaryResult): readonly P7AuditPayload[] {
  ...
  if (typeof domainResult === 'object' && domainResult !== null && 'auditPayloads' in domainResult) {
    ...
    if (Array.isArray(candidate)) {
      candidate.forEach((payload) => payloads.push(payload as P7AuditPayload));
    }
  }
...
async function runTransaction<T>(
  ...
  fn: (ports: P7RuntimeTransactionalPorts) => Promise<P7ApplicationServiceResult<T>>,
): Promise<P7ApplicationServiceResult<T>> {
  const tx = await deps.unitOfWork.runInTransaction(async (ports) => ({
    ok: true,
    value: await fn(ports),
  }));
...
async function reserveIdempotency(
  ...
  scope: P7IdempotencyScope,
): Promise<P7ApplicationServiceResult<never> | null> {
  const reserved = await ports.idempotency.reserveKey({
    key: dto.idempotency.idempotencyKey,
    scope,
    operationId: dto.idempotency.operationId,
    bankEventId: dto.idempotency.bankEventId,
    auditId: dto.audit.auditId,
    correlationId: dto.audit.correlationId,
  });
...
async function recordBoundaryResult(
  ports: P7RuntimeTransactionalPorts,
  dto: P7RuntimeRequestBaseDto,
  scope: P7IdempotencyScope,
  boundary: PlatformV7ActionBoundaryResult,
): Promise<P7ApplicationServiceResult<never> | null> {
  const recorded = await ports.idempotency.recordResult({
    key: dto.idempotency.idempotencyKey,
    scope,
    result: boundary,
    operationId: dto.idempotency.operationId,
...
async function appendAuditPayloads(
  ports: P7RuntimeTransactionalPorts,
  payloads: readonly P7AuditPayload[],
): Promise<P7ApplicationServiceResult<never> | null> {
...
function bankConfirmationEvent<Path extends 'release' | 'refund' | 'hold' | 'reject' | 'manual_review'>(
  ...
  path: Path,
  operationType: P7BankConfirmationEvent<Path>['operationType'],
  createdAt: string,
  ...
  return {
    ...
    amount: dto.amount,
    actorId: dto.actor.actorId,
    actorRole: toP7CanonicalActorRole(dto.actor) ?? 'bank_officer',
    organizationId: dto.actor.organizationId,
    bankOrganizationId: dto.bankOrganizationId,
    bankReference: dto.bankEventId,
...
function bankActionInput(
  dto: P7BankConfirmationRequestDto,
  decision: P7BankBasisDecision,
  moneyTree: PlatformV7MoneyTree,
  context: P7ActionIdempotencyContext,
  createdAt: string,
): PlatformV7BankBasisActionInput {
  if (dto.path === 'refund') {
    return bankBoundaryBase(dto, context, createdAt, 'bank_refund_confirmed', {
      decision,
      moneyTree,
      confirmation: bankConfirmationEvent(dto, 'refund', 'refund_confirmed', createdAt),
    });
  }

  if (dto.path === 'hold') {
    return bankBoundaryBase(dto, context, createdAt, 'bank_hold_confirmed', {
      decision,
      moneyTree,
      confirmation: bankConfirmationEvent(dto, 'hold', 'hold_created', createdAt),
    });
  }

  if (dto.path === 'reject') {
    return bankBoundaryBase(dto, context, createdAt, 'bank_release_rejected', {
      decision,
      moneyTree,
      confirmation: bankConfirmationEvent(dto, 'reject', 'release_failed', createdAt),
    });
  }

  if (dto.path === 'manual_review') {
    return bankBoundaryBase(dto, context, createdAt, 'bank_manual_review_started', {
      decision,
      moneyTree,
      confirmation: bankConfirmationEvent(dto, 'manual_review', 'manual_review_started', createdAt),
    });
  }

  return bankBoundaryBase(dto, context, createdAt, 'bank_release_confirmed', {
    decision,
    moneyTree,
    confirmation: bankConfirmationEvent(dto, 'release', 'release_confirmed', createdAt),
  });
}

function bankBoundaryBase<Action extends PlatformV7BankBasisActionInput['action']>(
  dto: P7BankConfirmationRequestDto,
  context: P7ActionIdempotencyContext,
  createdAt: string,
  action: Action,
  payload: Extract<PlatformV7BankBasisActionInput, { readonly action: Action }>['payload'],
  ...
  return {
    ...
    reason: dto.audit.reason,
    createdAt,
  } as Extract<PlatformV7BankBasisActionInput, { readonly action: Action }>;
}
...
function confirmationFromAction(input: PlatformV7BankBasisActionInput): P7BankConfirmationEvent {
  ...
  return {
    bankEventId: input.idempotencyKey,
    idempotencyKey: input.idempotencyKey,
    path: 'release',
    operationType: 'release_confirmed',
    amount: 0,
    actorId: input.actor.userId,
    actorRole: toP7CanonicalActorRole({
      actorId: input.actor.userId,
      actorRole: input.actor.activeRole,
      organizationId: input.actor.organizationId,
    }) ?? 'bank_officer',
    organizationId: input.actor.organizationId,
    bankOrganizationId: input.actor.organizationId,
    bankReference: input.idempotencyKey,
...
async function completeBoundary<T>(
  ports: P7RuntimeTransactionalPorts,
  dto: P7RuntimeRequestBaseDto,
  boundary: PlatformV7ActionBoundaryResult,
  persist: () => Promise<P7ApplicationServiceResult<T>>,
): Promise<P7ApplicationServiceResult<T>> {
    ...
    async requestRelease(dto) {
      ...
      return runTransaction(deps, async (ports) => {
        ...
        const boundary = executePlatformV7MoneyAction({
          ...
          payload: {
            beforeMoneyTree: money.value.value,
            operation: moneyOperation(dto, 'release_requested', createdAt),
        ...
        return completeBoundary(ports, dto, boundary, async () => {
          const saved = await ports.moneyTree.saveMoneyTree(
            recordWithValue(money.value, boundary.afterState, createdAt),
            saveOptions(dto, money.value, ports),
          );
...
export function createP7DocumentExecutionService(deps: P7ApplicationServiceDependencies): P7DocumentExecutionService {
    ...
    return runTransaction(deps, async (ports) => {
      ...
      if (!document) {
        return {
          ...
          reason: 'Document requirement was not found in the matrix.',
          auditPayloads: [],
        };
      }
      ...
      const createdAt = nowFrom(deps);
      const boundary = executePlatformV7DocumentAction({
        actor: actorFromDto(nextDto),
        resource: resourceFromDto(nextDto),
        action,
        payload: { document },
        idempotencyContext: context,
        idempotencyKey: nextDto.idempotency.idempotencyKey,
        correlationId: nextDto.audit.correlationId,
        auditId: nextDto.audit.auditId,
        reason: nextDto.audit.reason,
        createdAt,
      });

      return completeBoundary(ports, nextDto, boundary, async () => {
        const afterMatrix: PlatformV7DocumentMatrix = {
          ...matrix.value.value,
          documents: matrix.value.value.documents.map((item) =>
            item.documentId === nextDto.documentId ? boundary.afterState as PlatformV7DocumentRequirement : item
          ),
        };
        const saved = await ports.documentMatrix.saveDocumentMatrix(
          recordWithValue(matrix.value, afterMatrix, createdAt),
          saveOptions(nextDto, matrix.value, ports),
        );
        return saved.ok ? persisted(saved.value.value, boundary, []) : repositoryFailure(saved);
      });
    });
  ...
  return {
    uploadDocument: (dto) => executeDocument(dto, 'document_uploaded'),
    confirmDocument: (dto) => executeDocument(dto, 'document_confirmed'),
    rejectDocument: (dto) => executeDocument(dto, 'document_rejected'),
    sendDocument: (dto) => executeDocument(dto, 'document_sent'),
    markManualReview: (dto) => executeDocument(dto, 'document_manual_review_started'),
  };
}
...
export function createP7BankBasisExecutionService(deps: P7ApplicationServiceDependencies): P7BankBasisExecutionService {
  async function sendBankBasis(dto: P7BankBasisSendRequestDto): Promise<P7ApplicationServiceResult<P7BankBasisDecision>> {
    ...
    return runTransaction(deps, async (ports) => {
      ...
      return completeBoundary(ports, dto, boundary, async () => {
        ...
        const saved = await ports.bankBasis.saveBankBasisDecision(
          recordWithValue(decision.value, domain.decision, createdAt),
          saveOptions(dto, decision.value, ports),
        );
        return saved.ok ? persisted(saved.value.value, boundary, []) : repositoryFailure(saved);
      });
    });
  ...
  async function confirmBankMovement(dto: P7BankConfirmationRequestDto): Promise<P7ApplicationServiceResult<PlatformV7MoneyTree>> {
    ...
    return runTransaction(deps, async (ports) => {
      ...
      return completeBoundary(ports, dto, boundary, async () => {
        ...
        const savedMoney = await ports.moneyTree.saveMoneyTree(
          recordWithValue(money.value, domain.moneyTree, createdAt),
          saveOptions(dto, money.value, ports),
        );
        if (!savedMoney.ok) return repositoryFailure(savedMoney);

        const savedDecision = await ports.bankBasis.saveBankBasisDecision(
          recordWithValue(decision.value, domain.decision, createdAt),
          saveOptions(dto, decision.value, ports),
        );
        if (!savedDecision.ok) return repositoryFailure(savedDecision);

        const confirmationRecord: P7PersistedRecord<P7BankConfirmationRecord> = {
          recordId: `${dto.resource.dealId}:${dto.bankEventId}`,
          dealId: dto.resource.dealId,
          value: {
            decision: domain.decision,
            confirmation: confirmationFromAction(actionInput),
            result: boundary,
          },
          version: {
            resourceType: 'bank_confirmation',
            resourceId: dto.bankEventId,
            version: decision.value.version.version,
            updatedAt: createdAt,
          },
          createdAt,
          updatedAt: createdAt,
        };
        const savedConfirmation = await ports.bankBasis.saveBankConfirmation(
          confirmationRecord,
          saveOptions(dto, decision.value, ports),
        );
  ...
  return {
    sendBankBasis,
    confirmBankRelease: (dto) => confirmBankMovement({ ...dto, path: 'release', action: 'bank_release_confirmed' }),
    rejectBankRelease: (dto) => confirmBankMovement({ ...dto, path: 'reject', action: 'bank_release_rejected' }),
...
export function createP7ReleaseWorkflowService(deps: P7ApplicationServiceDependencies): P7ReleaseWorkflowService {
  ...
  async function getReleaseStatus(dto: P7RuntimeRequestBaseDto): Promise<P7ApplicationServiceResult<P7ReleaseWorkflowStatus>> {
    return runTransaction(deps, async (ports) => {
      ...
      const status: P7ReleaseWorkflowStatus = {
        dealId: dto.resource.dealId,
        moneyTree: loadedMoney.value.value,
        bankBasis: loadedBasis.ok ? loadedBasis.value.value : null,
      };
      return persisted(status, undefined, []);
    });
  }

  return {
    prepareRelease: getReleaseStatus,
    requestRelease: money.requestRelease,
    sendBasisToBank: bank.sendBankBasis,
    handleBankEvent: (dto) => {
      if (dto.path === 'refund') return bank.confirmBankRefund(dto);
      if (dto.path === 'hold') return bank.confirmBankHold(dto);
      if (dto.path === 'reject') return bank.rejectBankRelease(dto);
      if (dto.path === 'manual_review') return bank.startBankManualReview(dto);
      return bank.confirmBankRelease(dto);
    },
    getReleaseStatus,
  };
}
...
export function createP7DisputeSettlementService(deps: P7ApplicationServiceDependencies): P7DisputeSettlementService {
  ...
  return {
    async openDispute(dto) {
      return impact(dto, 'Dispute opened without direct money movement.');
    },
    async attachEvidence(dto) {
      return impact(dto, 'Evidence attached without direct money movement.');
    },
    async prepareArbitrationBasis(dto) {
      const validation = validateP7ArbitrationBasisRequestDto(dto);
      if (!validation.ok) return validationFailure(validation);

      return runTransaction(deps, async (ports) => {
        const createdAt = nowFrom(deps);
        const record: P7PersistedRecord<P7ArbitrationDecisionRecord> = {
          ...
          version: {
            ...
            version: operationId(dto),
            updatedAt: createdAt,
          },
          createdAt,
          updatedAt: createdAt,
        };
        const saved = await ports.disputeSettlement.saveArbitrationDecision(record, {
          expectedVersion: operationId(dto),
          transaction: ports.transaction,
          correlationId: dto.audit.correlationId,
          auditId: dto.audit.auditId,
          actorId: dto.actor.actorId,
    ...
    applyArbitrationOutcomeToBankBasis: (dto) => {
      if (dto.path === 'refund') return bank.confirmBankRefund(dto);
      if (dto.path === 'hold') return bank.confirmBankHold(dto);
      if (dto.path === 'reject') return bank.rejectBankRelease(dto);
      if (dto.path === 'manual_review') return bank.startBankManualReview(dto);
    ...
    async getDisputeMoneyImpact(dto) {
      return impact(dto, 'Dispute impact is read-only in PR 5.1 service layer.');
    },
  };
}
