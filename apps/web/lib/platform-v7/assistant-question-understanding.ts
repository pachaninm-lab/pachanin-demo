...
const DOMAIN_DICTIONARY = [
  ...
  'банк', 'номинальный', 'счет', 'счёт', 'кредит', 'спор', 'претензия', 'доказательства',
  'безопасность', 'роль', 'доступ', 'кабинет', 'покупатель', 'продавец', 'оператор',
  'комплаенс', 'арбитр', 'сюрвейер', 'руководитель', 'регистрация', 'вход', 'пароль',
  'интеграция', 'статус', 'этап', 'срок', 'риск', 'помощник', 'поддержка',
...
function chooseKeyboardVariant(value: string): string {
  ...
  const variants = [value, ruToEn, enToRu];
  return variants.sort((a, b) => score(b) - score(a))[0];
}

export function understandAssistantQuestion(
  input: string,
  fallbackLocale: AssistantUnderstandingLocale = 'ru',
): AssistantQuestionUnderstanding {
  ...
  const correctedTokens = sourceTokens.flatMap((token) => {
    ...
    if (explicit) {
      corrections.push({ from: token, to: explicit, confidence: 0.99 });
    ...
    if (fuzzy) {
      corrections.push({ from: token, to: fuzzy.value, confidence: fuzzy.confidence });
      return [fuzzy.value];
    }
    return [token];
  });
  ...
  return {
    ...
    detectedLocale: detectLocale(corrected, fallbackLocale),
    corrections,
