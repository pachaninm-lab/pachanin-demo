...
const copies = {
  ru: { access: 'Доступ', submit: 'Создать запрос', activate: 'Активировать защищённую сессию', operationalTitle: 'Рабочие контуры платформы', tabs: [['Поддержка','Отсутствует ЭПД'],['Исполнение','TP-2026-001'],['Деньги','12500000'],['Диагностика','bank-sandbox'],['Сотрудники','Platform Owner'],['Двухконтрольные действия','deal:operation:retry'],['Аварийный доступ','Нет элементов, требующих внимания.']], viewBanner: 'Режим просмотра чужого кабинета', cabinet: 'Просмотр кабинета' },
  en: { access: 'Access', submit: 'Create request', activate: 'Activate protected session', operationalTitle: 'Platform staff workspaces', tabs: [['Support','Отсутствует ЭПД'],['Execution','TP-2026-001'],['Money','12500000'],['Diagnostics','bank-sandbox'],['Staff','Platform Owner'],['Dual-control actions','deal:operation:retry'],['Emergency access','Nothing currently requires attention.']], viewBanner: 'Viewing another cabinet', cabinet: 'Cabinet view' },
  zh: { access: '访问', submit: '创建申请', activate: '激活受保护会话', operationalTitle: '平台员工工作台', tabs: [['支持','Отсутствует ЭПД'],['履约','TP-2026-001'],['资金','12500000'],['诊断','bank-sandbox'],['员工','Platform Owner'],['双人控制操作','deal:operation:retry'],['紧急访问','当前没有需要处理的项目。']], viewBanner: '正在查看其他账户', cabinet: '账户视图' },
};
...
for (const project of projects) {
  ...
  for (const locale of Object.keys(copies)) {
    ...
    const context = await browser.newContext({ viewport: project.viewport, isMobile: project.isMobile, hasTouch: project.hasTouch, locale: localeSettings[locale] });
    ...
    const layout = await page.evaluate(() => ({ width: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth, text: document.body.innerText.length }));
    assert(layout.text > 1400, `${project.name}/${locale}: probable blank staff screen`);
    ...
    if (project.name === 'chromium-desktop' && locale === 'ru') {
      const result = await page.evaluate(async () => {
        ...
        if (!request.ok) return { ok: false, stage: 'request', status: request.status, requested };
        ...
        return { ok: activated.ok, stage: 'activate', status: activated.status, activation };
      });
