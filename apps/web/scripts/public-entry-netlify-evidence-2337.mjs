...
try {
  for (const route of routes) {
    ...
    const consoleErrors = [];
    const runtimeErrors = [];
    page.on('console', (message) => { const text = message.text(); if (message.type() === 'error' && !isNoise(text)) consoleErrors.push(text); });
...
const summary = { baseUrl: BASE_URL, generatedAt: new Date().toISOString(), passed: results.filter((item) => item.outcome === 'passed').length, appFailures: results.filter((item) => item.outcome === 'failed').length, hostingInterference: results.filter((item) => item.outcome === 'hosting-interference').length, results };
