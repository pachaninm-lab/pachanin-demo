...
export default function Page() {
  return (
    <main style={{ minHeight: '100vh', background: '#060b16', color: '#f8fafc', fontFamily: '-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif', paddingBottom: 40 }}>
      <div style={{ position: 'sticky', top: 0, zIndex: 20, padding: '14px 16px', background: 'rgba(6,11,22,.96)', borderBottom: '1px solid rgba(255,255,255,.08)', display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ maxWidth: 620, margin: '0 auto', padding: '18px 16px 0' }}>
        <section style={{ background: '#0b1220', border: '1px solid rgba(255,255,255,.08)', borderRadius: 28, padding: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
            <div>
              <div style={{ fontWeight: 800, fontSize: 30, lineHeight: 1.05 }}>НТБ Мост</div>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {prices.map(([name, local, ntb, delta]) => (
              <a key={name} href="#" style={{ textDecoration: 'none', color: 'inherit', display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr auto', gap: 10, alignItems: 'center', padding: '10px 0', borderTop: '1px solid rgba(255,255,255,.05)' }}>
          ...
          <div style={{ marginTop: 14, background: 'rgba(251,191,36,.08)', border: '1px solid rgba(251,191,36,.22)', borderRadius: 20, padding: 16 }}>
            <div style={{ fontSize: 22, fontWeight: 800, marginBottom: 8 }}>Готовы к биржевому контуру?</div>
            <div style={{ color: '#aebdce', lineHeight: 1.5, fontSize: 15 }}>Цифровая сертификация, стандартизованное качество и подтверждённая репутация — всё, что нужно для выхода на НТБ.</div>
          </div>
        </section>
        <div style={{ fontSize: 24, fontWeight: 800, margin: '20px 0 12px' }}>Новости рынка</div>
        {['Пшеница 3 кл. — рост на 2.3% за неделю','Экспортная пошлина на подсолнечник снижена','Спрос переработчиков по кукурузе усилился'].map((title) => (
