...
const METRICS = [
  { label: 'Оборот в проверочном срезе', value: '118 млн ₽', sub: 'Сценарные данные, не подтверждённый внешний оборот' },
  { label: 'Модель комиссии', value: '1,8%', sub: 'Гипотеза unit economics для проверки' },
  { label: 'Сделок в срезе', value: '31', sub: 'Ряд данных для проверки UX' },
...
const TRUTH_BLOCKS = [
  ...
  {
    title: 'Что требует внешних подключений',
...
const RISKS = [
  ...
  { label: 'Ручная нагрузка', value: 'часть действий пока закрывается оператором и ручными сценариями' },
  { label: 'Данные', value: 'часть метрик основана на сценарном ряду' },
  { label: 'Банк', value: 'основание и статус денег должен подтверждать банк, а не платформа' },
];

const PLAN = [
  'Проверить сквозной сценарий сделки с реальными участниками',
...
const SHIPPED = [
  {
    ...
    state: 'Рабочий контур',
    note: 'Карта для банка и инвестора: что собрано, что требует проверки на реальной сделке.',
  ...
  },
  {
    ...
    state: 'Рабочий контур',
    note: 'Готовность сервисов и модулей показана честно: ручная проверка отдельно от внешних подключений.',
  ...
  return (
    <div style={{ display: 'grid', gap: 24 }}>
      ...
      <div data-testid="platform-v7-investor-truth-banner" style={{ padding: '14px 16px', borderRadius: 16, background: 'rgba(217,119,6,0.08)', border: '1px solid rgba(217,119,6,0.18)', color: '#0F1419', fontSize: 13, lineHeight: 1.6 }}>
        Инвесторский экран показывает проверочную картину исполнения. Оборот, регионы и часть действий являются сценарным срезом. Внешние подключения, банковские операции и реальное исполнение требуют договоров, доступов и проверки на фактической сделке.
      </div>
      ...
      <section data-testid="platform-v7-investor-truth-grid" style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 18, padding: 20, display: 'grid', gap: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: '#6B778C', lineHeight: 1.7, marginTop: 8 }}>
            Экран разделяет готовые UX-слои, рабочий контур, сценарные данные и внешние подключения. Это снижает риск завышенных ожиданий у инвестора.
          </div>
        </div>
      ...
      <div style={{ padding: '16px 20px', borderRadius: 16, background: 'rgba(10,122,95,0.06)', border: '1px solid rgba(10,122,95,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 800, color: '#0F1419' }}>Запросить материалы по контуру</div>
