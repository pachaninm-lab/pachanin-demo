...
const METRICS = [
  ...
  { label: 'Модель комиссии', value: '1,8%', sub: 'Гипотеза unit economics сценария' },
  { label: 'Сделок в сценарии', value: '31', sub: 'Проверочный ряд для проверки UX' },
...
const RISKS = [
  ...
  { label: 'Ручная нагрузка', value: 'часть действий пока закрывается оператором и проверочными сценариями' },
...
const PLAN = [
  'Проверить сквозной сценарий сделки с реальными участниками сценария',
  ...
  'Расширять регионы только после повторяемого проверочного результата',
];
...
const SHIPPED = [
  {
    ...
    note: 'Карта для банка и инвестора: что собрано, что проверочное и что требует проверки на реальной сделке.',
  ...
  },
  {
    ...
    state: 'Предынтеграционный режим',
  ...
  },
  {
    ...
    state: 'Контур исполнения',
    note: 'Готовность сервисов и модулей показана честно: предынтеграционный режим отдельно от live-подключений.',
