...
export const metadata: Metadata = {
  ...
  description: 'Служебная поверхность выбора способа входа и связки в контролируемого пилота / преинтеграции контуре Прозрачной Цена.',
  alternates: {
...
const flows = [
  ...
  {
    title: 'Email / пароль',
    description: 'Внутренний сценарий для операторов и предынтеграционной среды.',
    ...
    <div style={{ display: 'grid', gap: 18, maxWidth: 1080 }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
        <div style={{ display: 'grid', gap: 10 }}>
          ...
          <div style={{ fontSize: 30, lineHeight: 1.08, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Способы входа и связки</div>
          <div style={{ fontSize: 13, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.7, maxWidth: 820 }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
        {flows.map((flow) => (
          <div key={flow.title} style={{ background: '#fff', border: `1px solid ${flow.border}`, borderRadius: 16, padding: 16, display: 'grid', gap: 10 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--pc-text-secondary, #475569)', lineHeight: 1.6 }}>{flow.description}</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {flow.actions.map((action) => (
                <span key={action} style={{ display: 'inline-flex', alignItems: 'center', padding: '6px 10px', borderRadius: 999, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)', fontSize: 12, fontWeight: 700 }}>{action}</span>
              ))}
            </div>
          </div>
        ))}
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Быстрый переход</div>
          <div style={{ fontSize: 13, color: 'var(--pc-text-muted, #6B778C)', marginTop: 6 }}>Открывай нужный контур без лишнего поиска по платформе.</div>
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <Link href="/platform-v7/register" style={{ textDecoration: 'none', borderRadius: 12, padding: '10px 14px', background: '#fff', color: 'var(--pc-text-primary, #0F1419)', border: '1px solid var(--pc-border, #E4E6EA)', fontSize: 13, fontWeight: 800 }}>Регистрация</Link>
        </div>
      </section>
    </div>
  );
