...
function statusTone(status: FGISPartyStatus) {
  if (status === 'verified') return { bg: BRAND_BG, border: BRAND_BORDER, color: BRAND, label: 'Подтверждено' };
  if (status === 'pending_sync') return { bg: WARN_BG, border: WARN_BORDER, color: WARN, label: 'Ожидает синхронизации' };
  if (status === 'sync_error') return { bg: ERR_BG, border: ERR_BORDER, color: ERR, label: 'Ошибка синхронизации' };
  if (status === 'manual_mode') return { bg: WARN_BG, border: WARN_BORDER, color: WARN, label: 'Ручной режим' };
...
export default function SellerFgisPartiesPage() {
  const [expanded, setExpanded] = React.useState<string | null>(SANDBOX_FGIS_PARTIES[0]?.id ?? null);
...
function Chip({ label, tone = 'neutral' }: { label: string; tone?: 'neutral' | 'warn' }) {
  return (
    <span style={{ display: 'inline-block', padding: '4px 10px', borderRadius: 99, fontSize: 11, fontWeight: 700, border: `1px solid ${tone === 'warn' ? WARN_BORDER : B}`, background: tone === 'warn' ? WARN_BG : SS, color: tone === 'warn' ? WARN : M }}>
      {label}
    </span>
  );
}

function PartyCard({ party, isExpanded, onToggle }: { party: FGISParty; isExpanded: boolean; onToggle: () => void }) {
  const tone = statusTone(party.status);
  const canCreate = canCreateLotPassport(party);
  const totalTons = party.batches.reduce((sum, batch) => sum + batch.volumeTons, 0);
  ...
  return (
    <section style={{ background: S, border: `1px solid ${B}`, borderRadius: 18, overflow: 'hidden' }}>
      <button
        type='button'
        onClick={onToggle}
        style={{ width: '100%', padding: 18, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'center', background: 'transparent', border: 0, textAlign: 'left' }}
      >
        <div>
          <div style={{ fontSize: 16, fontWeight: 800, color: T }}>{party.orgName}</div>
          <div style={{ fontSize: 12, color: M, marginTop: 4 }}>{party.region} · {party.batches.length} партий · {totalTons} т</div>
        </div>
      ...
      {isExpanded ? (
        <div style={{ borderTop: `1px solid ${B}`, padding: 18 }}>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {party.batches.map((batch) => <BatchRow key={batch.batchId} batch={batch} />)}
          </div>
          ...
          {canCreate ? (
            ...
            <div style={{ marginTop: 14, fontSize: 13, color: WARN }}>Нельзя создать лот: партия не подтверждена или заблокирована.</div>
          )}
        </div>
      ) : null}
    </section>
