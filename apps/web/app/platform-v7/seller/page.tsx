...
import { buildDemoPaymentHeatmapData } from '@/components/platform-v7/payment-heatmap-demo-data';
              ...
              <div style={pathGrid} className='seller-path-grid'>
                {sellerPaths.map((path) => (
                  <Link key={path.href} href={path.href} style={pathCard}>
                    ...
                    <span style={{ color: 'var(--pc-accent)', fontSize: 12, fontWeight: 900 }}>Открыть</span>
                  </Link>
                ))}
              </div>
            </section>
...
function Cell({
  ...
  return (
    <div style={cell} className='seller-cell'>
      ...
      <div style={{ marginTop: 4, color: warning ? '#B45309' : strong ? 'var(--pc-accent)' : 'var(--pc-text-primary, #0F1419)', fontSize: 13, lineHeight: 1.35, fontWeight: 900 }}>{value}</div>
...
const blockerCard = { display: 'grid', gap: 6, minWidth: 220, maxWidth: 280, padding: 14, borderRadius: 18, background: 'var(--pc-warning-bg)', border: '1px solid var(--pc-warning)', boxShadow: '0 12px 28px rgba(180,83,9,0.08)' } as const;
...
const primaryAction = { textDecoration: 'none', display: 'inline-flex', minHeight: 44, alignItems: 'center', justifyContent: 'center', borderRadius: 999, padding: '0 16px', background: 'var(--pc-accent)', color: 'var(--pc-accent-contrast)', fontSize: 13, fontWeight: 950, boxShadow: '0 12px 24px rgba(10,122,95,0.18)' } as const;
...
const idText = { color: 'var(--pc-accent)', fontSize: 13, fontWeight: 950 } as const;
