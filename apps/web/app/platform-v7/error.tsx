  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24, background: '#f7faf7', color: '#071611', fontFamily: 'Inter, system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      <section style={{ width: 'min(100%, 560px)', border: '1px solid rgba(7,22,17,.10)', borderRadius: 28, background: '#fff', boxShadow: '0 18px 44px rgba(7,22,17,.08)', padding: 24 }}>
        <strong style={{ display: 'block', fontSize: 26, lineHeight: 1.1, letterSpacing: '-.04em', marginBottom: 10 }}>Не удалось показать страницу</strong>
        <p style={{ margin: '0 0 18px', color: '#5c6862', fontSize: 15, lineHeight: 1.45, fontWeight: 650 }}>Произошла ошибка при загрузке данных. Нажмите «Повторить» — обычно это помогает. Если ошибка повторяется, войдите заново.</p>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
          ...
          <Link href='/platform-v7/login' style={{ minHeight: 48, padding: '0 18px', borderRadius: 16, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#087a3b', color: '#fff', textDecoration: 'none', fontWeight: 950 }}>Войти заново</Link>
        </div>
      </section>
    </main>
  );
