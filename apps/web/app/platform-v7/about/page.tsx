...
const COPY: Record<Locale, Copy> = {
  ru: {
    ...
    home: 'На главную', brandHome: 'Прозрачная Цена — на главную', trustCenter: 'Центр доверия', register: 'Зарегистрироваться', headerRegister: 'Регистрация', login: 'Войти',
  ...
  en: {
    ...
    home: 'Home', brandHome: 'Transparent Price — home', trustCenter: 'Trust Center', register: 'Register', headerRegister: 'Register', login: 'Sign in',
  ...
  zh: {
    ...
    home: '返回首页', brandHome: '透明价格 — 返回首页', trustCenter: '信任中心', register: '注册', headerRegister: '注册', login: '登录',
...
export default async function AboutPage() {
  ...
  const nav = (
    <>
      ...
      <Link href={`/platform-v7/login${lang}`} className='p7-about-login' aria-label={copy.login}>{copy.login}</Link>
    </>
  );
  ...
  return (
    <div className='p7-about-page'>
      <PublicSiteHeader
        ...
        actions={(
          <>
            <Link href={`/platform-v7/register${lang}`} className='pc-v6-header-cta p7-about-register' aria-label={copy.headerRegister}>{copy.headerRegister}</Link>
          </>
        )}
      />
