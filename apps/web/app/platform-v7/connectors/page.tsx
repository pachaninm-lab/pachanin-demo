...
const integrations = [
  { name: 'Сбер · Безопасные сделки', role: 'резерв и выплата денег', status: 'симуляция условий выплаты', screen: '/platform-v7/bank', state: 'wait' },
  { name: 'Сбер · Оплата в кредит', role: 'кредитный лимит покупателя', status: 'сценарий покупателя', screen: '/platform-v7/buyer', state: 'ok' },
  { name: 'ФГИС «Зерно»', role: 'СДИЗ и партия зерна', status: 'СДИЗ блокирует выплату', screen: '/platform-v7/documents', state: 'stop' },
  { name: 'Контур.Диадок', role: 'договор, УПД, акт', status: 'УПД ждёт подписи', screen: '/platform-v7/documents', state: 'stop' },
  { name: 'СБИС / Saby ЭТрН', role: 'электронная транспортная накладная', status: 'ждёт подписи грузополучателя', screen: '/platform-v7/logistics', state: 'stop' },
  { name: 'ГИС ЭПД', role: 'передача перевозочного документа', status: 'ожидает закрытия ЭТрН', screen: '/platform-v7/logistics', state: 'wait' },
  { name: 'КриптоПро DSS', role: 'КЭП и полномочия подписи', status: 'сертификат в тестовом контуре', screen: '/platform-v7/documents', state: 'ok' },
  { name: 'ATI.SU', role: 'расчёт перевозчика', status: 'перевозчик выбран в симуляции', screen: '/platform-v7/logistics', state: 'ok' },
  { name: 'Wialon', role: 'телематика и GPS-события', status: 'точка водителя 62% пути', screen: '/platform-v7/driver', state: 'ok' },
  { name: 'Яндекс.Карты', role: 'визуальная карта маршрута', status: 'демо-карта рейса', screen: '/platform-v7/driver', state: 'ok' },
  { name: 'ФГБУ ЦОК АПК', role: 'протокол качества', status: 'протокол ожидается', screen: '/platform-v7/elevator', state: 'wait' },
] as const;
  ...
  return (
    <main style={{ display: 'grid', gap: 14, padding: '4px 0 24px' }}>
      <section style={hero}>
        ...
        <p style={lead}>Интеграции показаны как симуляция по логике реального подключения. Пользователь видит влияние на сделку: деньги, документы, рейс и качество.</p>
        <div style={actions}>
