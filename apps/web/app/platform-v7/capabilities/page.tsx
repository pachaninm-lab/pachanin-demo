...
const COPY={
ru:{e:'Возможности',t:'Весь путь Сделки — без разрывов между этапами',p:'Рынок, торги, доставка, качество, документы и расчёт работают как один связный процесс. Контекст остаётся связанным между этапами, поэтому проще видеть текущее состояние и следующий шаг.',items:{
...
en:{e:'Capabilities',t:'The whole Deal without hand-offs between disconnected tools',p:'Market, trading, delivery, quality, documents and settlement work as one connected flow. The context stays connected between stages, making the current state and next step easier to see.',items:{
...
zh:{e:'功能',t:'整笔交易，不再被分散工具切断',p:'市场、交易、交付、质量、文件和结算连接成一个流程。上下文在各阶段之间保持关联，更容易看清当前状态和下一步。',items:{
