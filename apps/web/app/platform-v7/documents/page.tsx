...
function badgeTone(state: string, blocks = false) {
  if (blocks || state.includes('не') || state.includes('ждёт') || state.includes('ожида')) return { bg: 'rgba(220,38,38,0.08)', border: 'rgba(220,38,38,0.18)', color: '#B91C1C' };
    ...
    <main style={{ display: 'grid', gap: 14, maxWidth: 1120, margin: '0 auto' }}>
      ...
      <section style={card}>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {scenario.documents.map((doc) => {
            const tone = badgeTone(doc.status, doc.blocksMoney);
            return (
              <article key={`${doc.title}-${doc.source}`} style={{ background: doc.blocksMoney ? 'rgba(220,38,38,0.05)' : '#F8FAFB', border: `1px solid ${doc.blocksMoney ? 'rgba(220,38,38,0.16)' : '#E4E6EA'}`, borderRadius: 16, padding: 13, display: 'grid', gap: 10 }}>
                ...
                <div style={rowGrid}>
                  ...
                  <Cell label='Статус' value={doc.status} danger={doc.blocksMoney} />
                  <Cell label='Влияние на деньги' value={doc.blocksMoney ? 'останавливает выплату' : 'не останавливает'} danger={doc.blocksMoney} strong={!doc.blocksMoney} />
                </div>
              </article>
            );
          })}
      ...
      <section style={card}>
        ...
        <div style={cardsGrid}>
          ...
          <ProviderCard title='СБИС / Saby ЭТрН' text='Электронная транспортная накладная и передача в ГИС ЭПД.' />
...
function ProviderCard({ title, text }: { title: string; text: string }) {
  return <div style={controlCard}><strong style={{ color: '#0F1419', fontSize: 15 }}>{title}</strong><span style={{ color: '#64748B', fontSize: 12, lineHeight: 1.35 }}>{text}</span><span style={{ ...pill, background: 'rgba(217,119,6,0.08)', borderColor: 'rgba(217,119,6,0.18)', color: '#B45309' }}>симуляция</span></div>;
}
