...
const surveyorSteps = [
  { label: 'Осмотр', value: 'фото и состояние' },
  { label: 'Расхождение', value: 'причина и место' },
  { label: 'Замечание', value: 'текст и вложения' },
  { label: 'Заключение', value: 'независимая фиксация' },
] as const;
      ...
      <section style={{ background: 'linear-gradient(135deg, #FFFFFF 0%, #F8FAFB 62%, #EEF6F3 100%)', border: '1px solid #E4E6EA', borderRadius: 26, padding: 22, display: 'grid', gap: 16 }}>
        <div style={{ display: 'grid', gap: 9, maxWidth: 840 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 'clamp(30px, 4.8vw, 52px)', lineHeight: 1.04, letterSpacing: '-0.045em', color: '#0F1419', fontWeight: 950 }}>
            Сюрвейер фиксирует факты без доступа к чужим рабочим контурам
          </h1>
          <p style={{ margin: 0, color: '#475569', fontSize: 15, lineHeight: 1.7 }}>
            Роль сюрвейера должна показывать только независимую проверку: фото, состояние груза, расхождения, замечания по приёмке и заключение для доказательного контура.
          </p>
        </div>
