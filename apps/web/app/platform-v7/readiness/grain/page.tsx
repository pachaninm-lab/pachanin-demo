  ...
  return (
    <main style={{ display: 'grid', gap: 14, padding: '4px 0 24px' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 24, padding: 18, display: 'grid', gap: 12 }}>
        ...
        <h1 style={{ margin: 0, color: 'var(--pc-text-primary, #0F1419)', fontSize: 'clamp(30px,8vw,48px)', lineHeight: 1.03, letterSpacing: '-0.045em', fontWeight: 950 }}>Роли, деньги, документы, интеграции, риски и демо-показ</h1>
        <p style={{ margin: 0, color: 'var(--pc-text-secondary, #475569)', fontSize: 15, lineHeight: 1.55 }}>Финальная сводка показывает, какие блоки готовы к демонстрации и куда перейти для проверки каждого контура.</p>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 10 }}>
        {checks.map(([title, value, href], index) => <Link key={title} href={href} style={card}><b>{index + 1}. {title}</b><span style={{ color: '#0A7A5F', fontWeight: 900 }}>{value}</span><span style={{ color: 'var(--pc-text-muted, #64748B)' }}>Открыть</span></Link>)}
      </section>
    </main>
  );
}
...
const ghost = { ...btn, background: '#fff', border: '1px solid #CBD5E1', color: 'var(--pc-text-primary, #0F1419)' } as const;
const card = { textDecoration: 'none', minHeight: 150, display: 'grid', gap: 8, padding: 14, borderRadius: 18, background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-primary, #0F1419)' } as const;
