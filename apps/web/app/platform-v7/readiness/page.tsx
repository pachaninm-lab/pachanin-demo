...
export default function PlatformV7ReadinessPage() {
  ...
  return (
    <div data-testid="platform-v7-readiness-page" style={{ display: 'grid', gap: 18, padding: '8px 0', width: '100%', maxWidth: '100%', minWidth: 0, overflowX: 'clip' }}>
      <style dangerouslySetInnerHTML={{ __html: `
        [data-testid='platform-v7-readiness-page'] *{min-width:0;overflow-wrap:anywhere}
        @media(max-width:767px){
          [data-testid='platform-v7-readiness-page']{gap:10px!important;padding:0!important;max-width:100%!important;overflow-x:hidden!important}
          [data-testid='platform-v7-readiness-hero']{padding:16px!important;border-radius:24px!important;overflow:hidden!important}
          ...
          [data-testid='platform-v7-readiness-hero'] a{width:100%!important;min-height:54px!important;display:flex!important;align-items:center!important;justify-content:center!important;border-radius:16px!important;white-space:normal!important}
          ...
          [data-testid='platform-v7-readiness-metrics'] > div{padding:12px!important;border-radius:16px!important;overflow:hidden!important}
          [data-testid='platform-v7-readiness-metrics'] > div:nth-child(3){display:none!important}
          [data-testid='platform-v7-readiness-demo']{padding:14px!important;border-radius:20px!important;gap:10px!important;overflow:hidden!important}
          ...
          [data-testid='platform-v7-readiness-list']{padding:14px!important;border-radius:20px!important;gap:9px!important;overflow:hidden!important}
          [data-testid='platform-v7-readiness-list'] > div:nth-of-type(n+5){display:none!important}
          [data-testid='platform-v7-readiness-list'] > div:not(:first-child){padding:12px!important;border-radius:16px!important;gap:8px!important;overflow:hidden!important}
        ...
        @media(max-width:380px){
          [data-testid='platform-v7-readiness-metrics'],[data-testid='platform-v7-readiness-demo'] > div:nth-child(2),[data-testid='platform-v7-readiness-list'] > div:not(:first-child) > div:nth-child(2){grid-template-columns:1fr!important}
        }
      ` }} />
      <section data-testid="platform-v7-readiness-hero" style={{ background: S, border: `1px solid ${B}`, borderRadius: 18, padding: 18, minWidth: 0, overflow: 'hidden' }}>
      ...
      <div data-testid="platform-v7-readiness-metrics" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 12, minWidth: 0, maxWidth: '100%' }}>
      ...
      <section data-testid="platform-v7-readiness-list" style={{ background: S, border: `1px solid ${B}`, borderRadius: 18, padding: 18, display: 'grid', gap: 12, minWidth: 0, overflow: 'hidden' }}>
        <div style={{ fontSize: 18, fontWeight: 900, color: T }}>Сделки по готовности</div>
        {rows.map(({ deal, gates, score, blocked: isBlocked }) => (
          <div key={deal.id} style={{ background: SS, border: `1px solid ${B}`, borderRadius: 14, padding: 14, display: 'grid', gap: 12, minWidth: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', alignItems: 'center', minWidth: 0 }}>
              <div style={{ minWidth: 0 }}>
                <Link href={`${PLATFORM_V7_DEALS_ROUTE}/${deal.id}`} style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 900, color: BRAND, textDecoration: 'none', overflowWrap: 'anywhere' }}>{deal.id}</Link>
                <span style={{ marginLeft: 8, fontSize: 13, color: M, overflowWrap: 'anywhere' }}>{deal.grain} · {statusLabel(deal.status)}</span>
              </div>
              <span style={{ padding: '5px 10px', borderRadius: 999, background: isBlocked ? 'rgba(220,38,38,0.08)' : 'rgba(10,122,95,0.08)', border: `1px solid ${isBlocked ? 'rgba(220,38,38,0.18)' : 'rgba(10,122,95,0.18)'}`, color: isBlocked ? ERR : BRAND, fontSize: 12, fontWeight: 900, maxWidth: '100%' }}>
                {score}% готово
              </span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 8, minWidth: 0, maxWidth: '100%' }}>
              {gates.map((gate) => (
                <div key={gate.label} style={{ border: `1px solid ${gate.ready ? 'rgba(10,122,95,0.18)' : 'rgba(217,119,6,0.18)'}`, background: gate.ready ? 'rgba(10,122,95,0.06)' : 'rgba(217,119,6,0.08)', borderRadius: 12, padding: 10, minWidth: 0, overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: 12, color: M, overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
...
function DL9106ReadinessCard() {
  ...
  return (
    <section data-testid="platform-v7-readiness-demo" style={{ background: S, border: `1px solid ${BRAND}`, borderRadius: 18, padding: 18, display: 'grid', gap: 14, minWidth: 0, overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start', minWidth: 0 }}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ marginTop: 4, fontSize: 20, fontWeight: 900, color: T, overflowWrap: 'anywhere' }}>{deal.id} · {deal.lotId} · {deal.crop}</div>
          <div style={{ marginTop: 4, fontSize: 13, color: M, overflowWrap: 'anywhere' }}>{deal.fgisPartyId} · {formatTons(deal.volumeTons)} · {deal.basis}</div>
        </div>
        <span style={{ padding: '6px 12px', borderRadius: 999, background: score === 100 ? 'rgba(10,122,95,0.08)' : 'rgba(217,119,6,0.08)', border: `1px solid ${score === 100 ? 'rgba(10,122,95,0.18)' : 'rgba(217,119,6,0.18)'}`, color: score === 100 ? BRAND : WARN, fontSize: 14, fontWeight: 900, maxWidth: '100%' }}>{score}% готово</span>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 8, minWidth: 0, maxWidth: '100%' }}>
        {gates.map(([key, gate]) => (
          <div key={key} style={{ border: `1px solid ${gate.status === 'готово' ? 'rgba(10,122,95,0.18)' : gate.status === 'стоп' ? 'rgba(220,38,38,0.18)' : 'rgba(217,119,6,0.18)'}`, background: gate.status === 'готово' ? 'rgba(10,122,95,0.06)' : gate.status === 'стоп' ? 'rgba(220,38,38,0.06)' : 'rgba(217,119,6,0.06)', borderRadius: 12, padding: 10, minWidth: 0, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', fontSize: 12, color: M, minWidth: 0, overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
...
function DL9102ScenarioCard() {
  return (
    <section style={{ background: '#FFFFFF', border: '1px solid rgba(217, 119, 6, 0.28)', borderRadius: 18, padding: 18, display: 'grid', gap: 12, minWidth: 0, overflow: 'hidden' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start', minWidth: 0 }}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ marginTop: 5, fontSize: 20, fontWeight: 900, color: '#0F1419', overflowWrap: 'anywhere' }}>{'DL-9102'} · текущий state: <span style={{ fontFamily: 'monospace' }}>dealDraft</span></div>
        ...
        <span style={{ borderRadius: 999, padding: '6px 10px', background: 'rgba(217, 119, 6, 0.08)', border: '1px solid rgba(217, 119, 6, 0.18)', color: '#B45309', fontSize: 12, fontWeight: 900, maxWidth: '100%' }}>final gate заблокирован</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 10, minWidth: 0, maxWidth: '100%' }}>
        <div style={{ border: '1px solid #D7DEE3', borderRadius: 14, padding: 12, background: '#E7EFE8', minWidth: 0, overflow: 'hidden' }}>
          <div style={{ fontSize: 11, color: '#58606E', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Блокеры:</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8, minWidth: 0 }}>
        ...
        <div style={{ border: '1px solid #D7DEE3', borderRadius: 14, padding: 12, background: '#E7EFE8', minWidth: 0, overflow: 'hidden' }}>
          ...
          <div style={{ marginTop: 4, fontSize: 11, color: '#58606E', overflowWrap: 'anywhere' }}>{DL9102_CARD_SCENARIO_LABEL} · не прошёл все gate</div>
        </div>
      </div>
    </section>
  );
...
function Metric({ label, value, tone }: { label: string; value: string; tone: 'good' | 'bad' }) {
  return (
    <div style={{ background: tone === 'good' ? 'rgba(10,122,95,0.08)' : 'rgba(220,38,38,0.08)', border: `1px solid ${tone === 'good' ? 'rgba(10,122,95,0.18)' : 'rgba(220,38,38,0.18)'}`, borderRadius: 16, padding: 16, minWidth: 0, overflow: 'hidden' }}>
      ...
      <div style={{ marginTop: 8, fontSize: 26, fontWeight: 900, color: tone === 'good' ? BRAND : ERR, lineHeight: 1.1, overflowWrap: 'anywhere' }}>{value}</div>
    </div>
  );
}

function btn() {
  return { textDecoration: 'none', borderRadius: 12, padding: '10px 14px', background: SS, border: `1px solid ${B}`, color: T, fontSize: 13, fontWeight: 800, maxWidth: '100%', whiteSpace: 'normal' as const };
}
