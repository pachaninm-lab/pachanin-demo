        ...
        <table className="v9-table">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr key={d.id} onClick={() => { window.location.href = `/platform-v7/disputes/${d.id}`; }} style={{ cursor: 'pointer' }}>
                <td>
                  <Link href={`/platform-v7/disputes/${d.id}`} style={{ fontFamily: '"JetBrains Mono",monospace', fontSize: 12, fontWeight: 700, color: '#DC2626', textDecoration: 'none' }} onClick={e => e.stopPropagation()}>
                    {d.id}
                  </Link>
                </td>
