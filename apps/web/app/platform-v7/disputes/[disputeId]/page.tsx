...
export default function DisputeDetailPage({ params }: { params: { disputeId: string } }) {
  ...
  function handleReminder() {
    ...
    trackEvent('dispute_reminder_sent', { disputeId: params.disputeId });
    toast('Напоминание отправлено контрагенту', 'success');
  }

  function handlePackageSubmit() {
    trackEvent('dispute_package_submitted', { disputeId: params.disputeId, docs: checked.size });
    ...
    return (
      <div style={{ display: 'grid', gap: 16 }}>
        <section style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 18, padding: 18 }}>
          <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--pc-text-primary)' }}>Спор {params.disputeId}</div>
    ...
    <div style={{ display: 'grid', gap: 16, maxWidth: 800, margin: '0 auto' }}>
      ...
      <section style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
        ...
        {!showPackage ? (
          <button
            ...
            style={{ alignSelf: 'start', padding: '10px 16px', borderRadius: 12, border: 'none', background: 'var(--pc-accent)', color: '#fff', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}
          >
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <button
          ...
          style={{ padding: '10px 16px', borderRadius: 12, border: '1px solid rgba(37,99,235,0.2)', background: 'rgba(37,99,235,0.08)', color: reminderSent ? '#9CA3AF' : '#2563EB', fontSize: 13, fontWeight: 800, cursor: reminderSent ? 'default' : 'pointer' }}
        >
