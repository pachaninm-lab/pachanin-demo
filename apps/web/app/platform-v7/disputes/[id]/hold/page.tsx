...
function CalcCard({ label, value, note, accent = false }: { label: string; value: string; note: string; accent?: boolean }) {
  return (
    <section style={{ background: accent ? 'rgba(10,122,95,0.06)' : '#fff', border: `1px solid ${accent ? 'rgba(10,122,95,0.18)' : 'var(--pc-border, #E4E6EA)'}`, borderRadius: 14, padding: 14 }}>
      ...
      <div style={{ marginTop: 8, fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{value}</div>
      <div style={{ marginTop: 6, fontSize: 12, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.5 }}>{note}</div>
    </section>
  );
}
    ...
    return (
      <div style={{ display: 'grid', gap: 16, maxWidth: 900, margin: '0 auto' }}>
        <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
          <div style={{ fontSize: 24, fontWeight: 900, color: 'var(--pc-text-primary, #0F1419)' }}>Спор не найден</div>
          <div style={{ marginTop: 8, fontSize: 13, color: 'var(--pc-text-muted, #6B778C)' }}>Для расчёта удержания нужен существующий dispute id.</div>
          <Link href='/platform-v7/disputes' style={{ display: 'inline-flex', marginTop: 14, textDecoration: 'none', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: 'var(--pc-text-primary, #0F1419)', fontSize: 13, fontWeight: 700 }}>Все споры</Link>
        </section>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 980, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 8 }}>
        ...
        <div style={{ fontSize: 28, fontWeight: 900, color: 'var(--pc-text-primary, #0F1419)' }}>Калькулятор удержания</div>
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
        <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Рекомендация</div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <Link href='/platform-v7/bank/escrow' style={{ textDecoration: 'none', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: 'var(--pc-text-primary, #0F1419)', fontSize: 13, fontWeight: 700 }}>
          Эскроу
        </Link>
