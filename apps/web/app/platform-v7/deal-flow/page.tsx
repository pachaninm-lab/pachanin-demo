...
export default async function PlatformV7DealFlowPage(){
 const locale=canonicalPublicLocale(await getLocale());
 const c=locale==='ru'
 ?{e:'Сделка в работе',t:'Как выглядит рабочая Сделка',p:'На публичной странице показана структура рабочего экрана, но не выдуманные данные. После входа фактическое состояние загружается с сервера для участника с подтверждённым доступом.',h:'Что показывает рабочий экран',lead:'Платформа показывает основание для расчёта, но не создаёт финансовое событие на клиенте.',state:['Текущий статус конкретной Сделки','Участник с подтверждёнными полномочиями','Связанный факт, условие или документ','Только подтверждённое влияние на расчёт','Действие, доступное текущей роли'],login:'Войти',how:'Как проходит Сделка'}
 :locale==='en'
 ?{e:'Deal workspace',t:'What a working Deal looks like',p:'This public page shows the workspace structure without inventing live data. After sign-in, the actual Deal state is loaded from the server for a participant with confirmed access.',h:'What the workspace shows',lead:'The platform shows settlement basis but does not create a financial event on the client.',state:['Current state of the specific Deal','Participant with confirmed authority','Linked fact, condition or document','Only confirmed settlement impact','Action available to the current role'],login:'Sign in',how:'How the Deal works'}
 :{e:'交易工作区',t:'实际交易工作区是什么样',p:'公开页面只展示工作区结构，不虚构实时数据。登录后，服务器会为已确认权限的参与方加载真实交易状态。',h:'工作区展示什么',lead:'平台展示结算依据，但不会在客户端创建金融事件。',state:['具体交易的真实状态','具有已确认权限的参与方','关联事实、条件或文件','仅展示服务器确认的结算影响','当前角色允许的操作'],login:'登录',how:'交易如何进行'};
