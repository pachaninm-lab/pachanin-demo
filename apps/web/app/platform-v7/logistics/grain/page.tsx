...
const checks = [
  { label: 'Деньги и ставки', value: 'скрыты для логистики', tone: 'neutral' },
  { label: 'Рейс', value: 'активен', tone: 'good' },
  { label: 'ЭТрН', value: 'ждёт подписи', tone: 'warn' },
  { label: 'СДИЗ', value: 'не подтверждён', tone: 'bad' },
] as const;

export default function LogisticsGrainPage() {
  return (
    <main style={{ display: 'grid', gap: 14, padding: '4px 0 24px' }}>
      <section style={card}>
        <div style={badge}>Зерновой маршрут логистики</div>
      ...
      <section style={card}>
        <div style={micro}>Цепочка исполнения</div>
        <div style={stepGrid}>{steps.map((step, index) => <Step key={step.title} step={step} index={index} />)}</div>
      </section>

      <section style={darkCard}>
        <div style={{ ...micro, color: '#A7F3D0' }}>Правило доступа</div>
        <h2 style={{ margin: 0, color: '#fff', fontSize: 26, lineHeight: 1.08, letterSpacing: '-0.035em', fontWeight: 950 }}>Логистика не видит лишнего</h2>
        <p style={{ margin: 0, color: '#CBD5E1', fontSize: 14, lineHeight: 1.55 }}>Роль логистики должна закрывать физическое исполнение рейса и транспортные документы. Деньги, ставки покупателей, кредитный лимит и банковский выпуск остаются в своих ролевых контурах.</p>
      </section>
    </main>
  );
}
...
function color(tone: string) { return tone === 'good' ? '#0A7A5F' : tone === 'warn' ? '#B45309' : tone === 'bad' ? '#B91C1C' : 'var(--pc-text-primary, #0F1419)'; }
