...
const COPY: Record<Locale, Copy> = {
  ru: {
    ...
    description: 'Каталог содержит маршруты, существование и границы которых подтверждены фактическими NestJS-контроллерами. Доступ к данным по-прежнему ограничивается серверной ролью, организацией, tenant и участием в Сделке.',
    ...
    boundary: 'Спецификация подтверждает наличие внутреннего HTTP-маршрута, но не подтверждает договор, боевые credentials, доступность внешнего провайдера, callback, reconciliation или промышленную эксплуатацию. Диагностика адаптера не равна live-интеграции.',
    catalogueTitle: 'Опубликованные области',
    catalogueDescription: '14 подтверждённых GET-маршрутов. Все изменяющие операции исключены из текущей публикации.',
  ...
  en: {
    metadataTitle: 'API contract · Transparent Price',
    metadataDescription: 'Verified read-only OpenAPI contract without simulated external integrations or unconfirmed commands.',
    ...
    boundary: 'The specification confirms an internal HTTP route. It does not confirm a contract, production credentials, external-provider availability, callback, reconciliation or production operation. Adapter diagnostics are not live-integration proof.',
    catalogueTitle: 'Published areas',
    catalogueDescription: '14 confirmed GET routes. All mutating operations are excluded from this publication.',
  ...
  zh: {
    metadataTitle: 'API 合同 · 透明价格',
    metadataDescription: '经过确认的只读 OpenAPI 合同，不包含模拟外部集成或未确认命令。',
    eyebrow: '集成合同',
    title: '仅记录已确认的服务器接口',
    ...
    connectors: '检查连接',
    boundaryTitle: '证据边界',
    boundary: '规范只确认内部 HTTP 路由存在，不证明合同、生产凭据、外部供应商可用性、回调、对账或生产运行。适配器诊断不等于实时集成证明。',
    catalogueTitle: '已发布领域',
    catalogueDescription: '14 个已确认 GET 路由。当前发布不包含任何修改操作。',
