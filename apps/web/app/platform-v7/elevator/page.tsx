...
export default async function Page() {
  ...
  const primary = (
    <div className={workspace.stack}>
      <NextActionCard
        label='Текущий объект приёмки'
        action='Зафиксировать вес и оформить акт расхождения'
        reason='Рейс прибыл. Фактический вес ниже заявленного на 1,2 т. После фиксации веса передай пробу в лабораторию.'
        blocked
        impact='акт расхождения и протокол качества не закрыты'
        owner='элеватор'
        deadline='до передачи документного основания'
        actions={(
          <>
            <a className={workspace.primaryLink} href='#weight'>Открыть фиксацию веса</a>
            <a className={workspace.secondaryLink} href='#quality'>Передать пробу</a>
          </>
        )}
      />
        ...
        <CollapsibleSection title='Качество и условия допуска' summary='примесь · акт · протокол' defaultOpen={false}>
          <div className={workspace.sectionStack}>
            <div className={workspace.twoColumn}>
              {gates.map((gate) => (
                <Surface key={`${gate.title}-${gate.value}`}>
                  <div className={workspace.sectionStack}>
                    <StatusChip tone={gate.tone}>{gate.title}</StatusChip>
            ...
            <div className={workspace.twoColumn}>
              {quality.map((item) => (
                <Surface key={item.label}>
                  <div className={workspace.sectionStack}>
                    <StatusChip tone={item.state === 'stop' ? 'critical' : 'warning'}>{item.label}</StatusChip>
  ...
  const context = (
    <div className={workspace.stack}>
      <StatusChip tone={pendingSamples.length ? 'warning' : 'success'}>
