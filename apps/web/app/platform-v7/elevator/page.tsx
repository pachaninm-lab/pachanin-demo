...
export default async function Page() {
  ...
  return (
    <main data-testid='platform-v7-elevator-page' style={{ display: 'grid', gap: 14, padding: '4px 0 24px' }}>
      <LiveApiStatusBar apiOnline={apiOnline} blockers={liveBlockers} activeShipments={shipmentCount} role="ELEVATOR · Приёмка" summary={apiOnline ? `${shipmentCount} рейсов на приёмке · ${pendingSamples.length} проб ожидают протокола` : 'Демонстрационные данные — сервер недоступен'} />
...
const idText = { color: 'var(--pc-warning)', fontSize: 13, fontWeight: 950 } as const;
const rowHead = { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'start', flexWrap: 'wrap' } as const;
const statusPill = { borderRadius: 999, background: 'var(--pc-success-bg)', border: '1px solid var(--pc-success)', color: 'var(--pc-success)', padding: '8px 12px', fontSize: 12, fontWeight: 950 } as const;
const primaryBtn = { minHeight: 48, display: 'grid', placeItems: 'center', borderRadius: 16, background: 'var(--pc-accent)', color: 'var(--pc-accent-contrast)', fontWeight: 950, textDecoration: 'none', padding: '0 16px' } as const;
...
const notice = { border: '1px solid var(--pc-danger)', background: 'var(--pc-danger-bg)', color: 'var(--pc-danger)', borderRadius: 18, padding: 14, fontSize: 13, lineHeight: 1.45, fontWeight: 900 } as const;

function FirstScreenCell({ item }: { item: (typeof firstScreen)[number] }) { return <div style={{ border: '1px solid var(--pc-border)', borderRadius: 18, padding: 14, display: 'grid', gap: 6 }}><span style={micro}>{item.label}</span><strong style={{ color: item.label === 'Деньги под риском' || item.label === 'Что заблокировано' ? 'var(--pc-danger)' : 'var(--pc-text-primary)', fontSize: 17, lineHeight: 1.2 }}>{item.value}</strong><span style={{ color: 'var(--pc-text-muted)', fontSize: 12, lineHeight: 1.4 }}>{item.note}</span></div>; }
function Cell({ label, value, strong, danger }: { label: string; value: string; strong?: boolean; danger?: boolean }) { return <div style={{ border: '1px solid var(--pc-border)', borderRadius: 18, padding: 14, display: 'grid', gap: 6 }}><span style={micro}>{label}</span><strong style={{ color: danger ? 'var(--pc-danger)' : strong ? 'var(--pc-success)' : 'var(--pc-text-primary)', fontSize: 18, lineHeight: 1.2 }}>{value}</strong></div>; }
function Gate({ gate }: { gate: (typeof gates)[number] }) { return <div style={{ border: `1px solid ${gate.state === 'stop' ? 'var(--pc-danger)' : 'var(--pc-warning)'}`, background: gate.state === 'stop' ? 'var(--pc-danger-bg)' : 'var(--pc-warning-bg)', borderRadius: 18, padding: 14, display: 'grid', gap: 6 }}><span style={{ ...micro, color: gate.state === 'stop' ? 'var(--pc-danger)' : 'var(--pc-warning)' }}>{gate.title}</span><strong style={{ color: 'var(--pc-text-primary)', fontSize: 16 }}>{gate.value}</strong><span style={{ color: 'var(--pc-text-muted)', fontSize: 12, lineHeight: 1.4 }}>{gate.impact}</span></div>; }
function QualityCell({ item }: { item: (typeof quality)[number] }) { return <div style={{ border: `1px solid ${item.state === 'stop' ? 'var(--pc-danger)' : 'var(--pc-warning)'}`, background: item.state === 'stop' ? 'var(--pc-danger-bg)' : 'var(--pc-warning-bg)', borderRadius: 18, padding: 14, display: 'grid', gap: 6 }}><span style={{ ...micro, color: item.state === 'stop' ? 'var(--pc-danger)' : 'var(--pc-warning)' }}>{item.label}</span><strong style={{ color: 'var(--pc-text-primary)', fontSize: 18 }}>{item.value}</strong><span style={{ color: item.state === 'stop' ? 'var(--pc-danger)' : 'var(--pc-warning)', fontSize: 12 }}>{item.limit}</span></div>; }
