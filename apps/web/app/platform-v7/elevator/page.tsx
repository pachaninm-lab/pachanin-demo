...
const notice = { border: '1px solid var(--pc-danger, #FECACA)', background: 'var(--pc-danger-bg, #FEF2F2)', color: '#B91C1C', borderRadius: 18, padding: 14, fontSize: 13, lineHeight: 1.45, fontWeight: 900 } as const;
...
function Gate({ gate }: { gate: (typeof gates)[number] }) { return <div style={{ border: `1px solid ${gate.state === 'stop' ? 'var(--pc-danger, #FECACA)' : '#FED7AA'}`, background: gate.state === 'stop' ? 'var(--pc-danger-bg, #FEF2F2)' : '#FFF7ED', borderRadius: 18, padding: 14, display: 'grid', gap: 6 }}><span style={{ ...micro, color: gate.state === 'stop' ? '#B91C1C' : '#C2410C' }}>{gate.title}</span><strong style={{ color: 'var(--pc-text-primary)', fontSize: 16 }}>{gate.value}</strong><span style={{ color: 'var(--pc-text-muted)', fontSize: 12, lineHeight: 1.4 }}>{gate.impact}</span></div>; }
function QualityCell({ item }: { item: (typeof quality)[number] }) { return <div style={{ border: `1px solid ${item.state === 'stop' ? 'var(--pc-danger, #FECACA)' : '#FED7AA'}`, background: item.state === 'stop' ? 'var(--pc-danger-bg, #FEF2F2)' : '#FFFBEB', borderRadius: 18, padding: 14, display: 'grid', gap: 6 }}><span style={{ ...micro, color: item.state === 'stop' ? '#B91C1C' : '#B45309' }}>{item.label}</span><strong style={{ color: 'var(--pc-text-primary)', fontSize: 18 }}>{item.value}</strong><span style={{ color: item.state === 'stop' ? '#B91C1C' : '#B45309', fontSize: 12 }}>{item.limit}</span></div>; }
