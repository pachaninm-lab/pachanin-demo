...
const demoRows = [
  { lot: 'Лот ВРЖ-1811', crop: 'Кукуруза', volume: '800 т', basis: 'Воронеж · склад продавца', target: '13 450 ₽/т', best: '13 300 ₽/т', bids: 4, status: 'Ожидает сверки', risk: 'Средний', blocker: 'нет СДИЗ' },
...
export default function PlatformV7TradingPage() {
  ...
  return (
    <div data-testid="platform-v7-trading-page" style={{ display: 'grid', gap: 18, padding: '8px 0', width: '100%', maxWidth: '100%', minWidth: 0, overflowX: 'clip' }}>
      <style dangerouslySetInnerHTML={{ __html: `
        [data-testid='platform-v7-trading-page']{max-width:100%!important;overflow-x:hidden!important}
        [data-testid='platform-v7-trading-page'] *{min-width:0;overflow-wrap:anywhere;box-sizing:border-box}
        [data-testid='platform-v7-trading-page'] a{max-width:100%!important;white-space:normal!important}
        @media(max-width:767px){
          [data-testid='platform-v7-trading-page']{gap:10px!important;padding:0!important}
          [data-testid='platform-v7-trading-page'] > section:nth-of-type(1){padding:16px!important;border-radius:24px!important;overflow:hidden!important}
          ...
          [data-testid='platform-v7-trading-page'] > div:nth-of-type(1) > div{padding:12px!important;border-radius:16px!important;overflow:hidden!important}
          [data-testid='platform-v7-trading-page'] > div:nth-of-type(1) > div:nth-child(n+4){display:none!important}
          [data-testid='platform-v7-trading-page'] > section:nth-of-type(2){padding:12px!important;border-radius:16px!important;overflow:hidden!important}
          ...
          [data-testid='platform-v7-trading-page'] > section:nth-of-type(3){padding:14px!important;border-radius:20px!important;gap:9px!important;overflow:hidden!important}
          [data-testid='platform-v7-trading-page'] > section:nth-of-type(3) > div:not(:first-child){padding:12px!important;border-radius:16px!important;gap:8px!important;overflow:hidden!important}
        ...
        @media(max-width:380px){
          [data-testid='platform-v7-trading-page'] > div:nth-of-type(1),
          [data-testid='platform-v7-trading-page'] > section:nth-of-type(3) > div:not(:first-child) > div:nth-child(2){grid-template-columns:1fr!important}
        }
      ` }} />
      <section style={{ background: S, border: `1px solid ${B}`, borderRadius: 18, padding: 18, minWidth: 0, overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start', minWidth: 0 }}>
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ marginTop: 6, fontSize: 28, lineHeight: 1.1, fontWeight: 900, color: T, overflowWrap: 'anywhere' }}>Лоты, заявки, ставки и допуск к сделке</div>
            <div style={{ marginTop: 8, fontSize: 14, color: M, maxWidth: 920, overflowWrap: 'anywhere' }}>
      ...
      <section style={{ background: S, border: `1px solid ${B}`, borderRadius: 18, padding: 18, display: 'grid', gap: 12, minWidth: 0, overflow: 'hidden' }}>
        ...
        {rows.map((row) => {
          ...
          return (
            <div key={row.lot} style={{ background: SS, border: `1px solid ${B}`, borderRadius: 14, padding: 14, display: 'grid', gap: 12, minWidth: 0, overflow: 'hidden' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', alignItems: 'center', minWidth: 0 }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 900, color: T, overflowWrap: 'anywhere' }}>{row.lot}</div>
                  <div style={{ marginTop: 4, fontSize: 13, color: M, overflowWrap: 'anywhere' }}>{row.crop} · {row.volume} · {row.basis}</div>
                </div>
                <span style={{ padding: '5px 10px', borderRadius: 999, background: t.bg, border: `1px solid ${t.border}`, color: t.color, fontSize: 12, fontWeight: 900, maxWidth: '100%', overflowWrap: 'anywhere' }}>{row.status}</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(145px,1fr))', gap: 8, minWidth: 0, maxWidth: '100%' }}>
...
function Cell({ label, value, danger = false }: { label: string; value: string; danger?: boolean }) {
  return (
    <div style={{ border: `1px solid ${B}`, borderRadius: 12, padding: 10, background: S, minWidth: 0, overflow: 'hidden' }}>
      <div style={{ fontSize: 10, color: M, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.06em', overflowWrap: 'anywhere' }}>{label}</div>
      <div style={{ marginTop: 5, fontSize: 13, fontWeight: 900, color: danger ? ERR : T, overflowWrap: 'anywhere', wordBreak: 'break-word' }}>{value}</div>
    </div>
  );
}
