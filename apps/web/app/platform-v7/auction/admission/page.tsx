...
export async function generateMetadata(): Promise<Metadata> {
  const copy = getAuctionAuthorityRouteCopy('admission', normalizeAuctionLocale(await getLocale()));
  return { title: copy.metadataTitle, description: copy.metadataDescription };
}

export default async function AuctionAdmissionPage() {
  const copy = getAuctionAuthorityRouteCopy('admission', normalizeAuctionLocale(await getLocale()));
  return <AuctionAuthorityRoute testId='platform-v7-auction-admission-authority-v8' {...copy} />;
}
