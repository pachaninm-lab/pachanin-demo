  ...
  return (
    <main style={{ display: 'grid', gap: 14, padding: '4px 0 24px' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 24, padding: 18, display: 'grid', gap: 12 }}>
        ...
        <h1 style={{ margin: 0, color: 'var(--pc-text-primary, #0F1419)', fontSize: 'clamp(30px,8vw,48px)', lineHeight: 1.03, letterSpacing: '-0.045em', fontWeight: 950 }}>Каналы партии, перевозки, расчёта, качества и логистики</h1>
        <p style={{ margin: 0, color: 'var(--pc-text-secondary, #475569)', fontSize: 15, lineHeight: 1.55 }}>Экран показывает статус внешних каналов, ответственного и следующий шаг по сделке.</p>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 10 }}>
        {items.map(([title, value, href], index) => <Link key={title} href={href} style={card}><b>{index + 1}. {title}</b><span style={{ color: '#B45309', fontWeight: 900 }}>{value}</span><span style={{ color: 'var(--pc-text-muted, #64748B)' }}>Открыть</span></Link>)}
      </section>
    </main>
  );
}
