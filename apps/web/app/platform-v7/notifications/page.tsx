...
function tone(type: InboxItem['type']) {
  ...
  return { bg: '#F8FAFB', border: 'var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)', label: 'Сделка' };
}
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 1040, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
        <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Уведомления</div>
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
        <div>
          <div style={{ fontSize: 20, lineHeight: 1.2, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Inbox-центр</div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            ...
            style={{ flex: '1 1 280px', minWidth: 220, padding: '10px 12px', borderRadius: 10, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', fontSize: 13 }}
          />
          {search ? (
            <button onClick={() => setSearch('')} style={{ padding: '8px 12px', borderRadius: 10, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', fontSize: 12, fontWeight: 700, color: 'var(--pc-text-muted, #6B778C)', cursor: 'pointer' }}>
              Сбросить
            </button>
          ) : null}
        </div>
          ...
          {FILTERS.map((filter) => (
            <button
              ...
              style={{
                ...
                border: activeFilter === filter ? '1px solid rgba(10,122,95,0.18)' : '1px solid var(--pc-border, #E4E6EA)',
                color: activeFilter === filter ? '#0A7A5F' : 'var(--pc-text-secondary, #475569)',
                fontSize: 12,
                fontWeight: 800,
                cursor: 'pointer',
              }}
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {visibleItems.length ? visibleItems.map((item) => {
          ...
          return (
            <article key={item.id} style={{ background: '#fff', border: isPinned ? '1px solid rgba(10,122,95,0.24)' : '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 16, display: 'grid', gap: 10, boxShadow: isPinned ? '0 0 0 2px rgba(10,122,95,0.06) inset' : 'none' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                <div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ display: 'inline-flex', alignItems: 'center', padding: '4px 8px', borderRadius: 999, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)', fontSize: 11, fontWeight: 800 }}>{item.bucket}</span>
                  ...
                  <div style={{ marginTop: 6, fontSize: 16, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{item.title}</div>
                </div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <button onClick={() => togglePin(item.id)} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: isPinned ? '#0A7A5F' : 'var(--pc-text-secondary, #475569)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                  {isPinned ? 'Снять pin' : 'Pin'}
                </button>
                <button onClick={() => toggleSnooze(item.id)} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: isSnoozed ? '#B45309' : 'var(--pc-text-secondary, #475569)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                  {isSnoozed ? 'Снять snooze' : 'Snooze'}
                </button>
                <button onClick={() => archiveItem(item.id)} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: 'var(--pc-text-secondary, #475569)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                  В архив
                </button>
              </div>
            </article>
          ...
          <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 24, display: 'grid', gap: 10 }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Ничего не найдено</div>
            <div style={{ fontSize: 13, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.7 }}>Сбрось фильтры, архив или поиск, чтобы снова увидеть активные сигналы.</div>
          </section>
        )}
      </div>
...
function ActionCard({ title, note, onClick }: { title: string; note: string; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ textAlign: 'left', display: 'grid', gap: 8, padding: 14, borderRadius: 14, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', cursor: 'pointer' }}>
      <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{title}</div>
      <div style={{ fontSize: 12, lineHeight: 1.6, color: 'var(--pc-text-secondary, #475569)' }}>{note}</div>
    </button>
  );
}
