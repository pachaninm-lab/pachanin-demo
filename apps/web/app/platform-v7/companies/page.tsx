...
const COMPANIES = [
  {
    ...
    deals: '12',
    status: 'Низкий риск',
    note: 'Якорный контрагент контура исполнения.',
  },
  {
    inn: '3664098765',
    name: 'МаслоПресс ООО',
  ...
  },
  {
    ...
    note: 'Крупный объём, требует дисциплины срок реакции.',
  },
];
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 1040, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
        <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Контрагенты</div>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {COMPANIES.map((company) => (
          <article key={company.inn} style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 16, display: 'grid', gap: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <div>
                <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{company.name}</div>
                <div style={{ marginTop: 4, fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>ИНН {company.inn} · {company.region}</div>
              </div>
...
function Cell({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 12, padding: 12, background: '#F8FAFB' }}>
      ...
      <div style={{ marginTop: 6, fontSize: 13, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', lineHeight: 1.5 }}>{value}</div>
    </div>
  );
}
