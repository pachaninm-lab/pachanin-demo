      ...
      <section className='login-shell'>
        ...
        <section className='login-main' aria-label='Форма входа'>
          ...
          <form className='login-form' onSubmit={onSubmit}>
            {!entryRole ? (
              <section className='login-workspace-picker' aria-label='Рабочее место'><span>Рабочее место</span><div>{workspaces.map((item) => <button key={item.role} type='button' className={directRole === item.role ? 'active' : ''} onClick={() => { setDirectRole(item.role); setError(''); }}>{item.title}</button>)}</div></section>
            ) : selected ? (
