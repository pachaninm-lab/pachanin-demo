...
function requestedRoleFromUrl(): PlatformRole | null {
...
export default function LoginPage() {
  ...
  const [role, setRole] = React.useState<PlatformRole>(initialRole ?? 'operator');
  const [lockedRole, setLockedRole] = React.useState<PlatformRole | null>(initialRole);
      ...
      <section className='login-shell'>
        ...
        <section className='login-main' aria-label='Форма входа'>
          <div className='login-copy'>
            ...
            <p>{lockedRole ? 'Роль уже выбрана на главном экране. Введи данные доступа — форма откроет нужный рабочий кабинет.' : 'Сначала логин, пароль и организация. Рабочее место выбирается внутри формы только при прямом входе.'}</p>
          </div>
          ...
          <form className='login-form' onSubmit={onSubmit}>
            ...
            {lockedRole ? (
              <article className='login-role-locked' aria-label='Выбранное рабочее место'>
                <span>Рабочее место выбрано</span>
                <strong>{selected.title}</strong>
                <small>{selected.scope}</small>
              </article>
            ) : (
              <label><span>Рабочее место</span><select value={role} onChange={(event) => setRole(event.target.value as PlatformRole)}>{workspaces.map((item) => <option key={item.role} value={item.role}>{item.title}</option>)}</select></label>
            )}
            {!lockedRole ? <article className='login-selected'><strong>{selected.title}</strong><small>{selected.scope}</small></article> : null}
        ...
        <aside className='login-side' aria-label='Контур доступа'>
          ...
          <ul>
            <li>единая форма доступа</li>
            <li>роль фиксируется на сессию</li>
            <li>чужие кабинеты закрываются guard-контуром</li>
            <li>рабочее место открывается после формы входа</li>
          </ul>
        </aside>
      </section>
    </main>
