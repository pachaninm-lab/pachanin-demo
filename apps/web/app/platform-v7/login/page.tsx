...
const copy = {
  ru: { brand: 'Прозрачная Цена', kicker: 'Единый вход', title: 'Вход в рабочую платформу', lead: 'Введите корпоративные данные для доступа к рабочему контуру.', chooseWorkspace: 'Выберите один рабочий кабинет', login: 'Логин', loginPh: 'Введите логин', password: 'Пароль / код доступа', passwordPh: 'Введите пароль или код', forgot: 'Забыли пароль?', company: 'Организация', optional: 'необязательно', companyPh: 'Компания / ИНН', submit: 'Войти как', register: 'Зарегистрироваться', choose: 'Выберите рабочий кабинет.', fill: 'Заполните логин и пароль / код доступа.', back: 'Вернуться на главную' },
  en: { brand: 'Transparent Price', kicker: 'Single entry', title: 'Sign in to the workspace', lead: 'Enter corporate access data to open the working circuit.', chooseWorkspace: 'Choose one workspace', login: 'Login', loginPh: 'Enter login', password: 'Password / access code', passwordPh: 'Enter password or code', forgot: 'Forgot password?', company: 'Organisation', optional: 'optional', companyPh: 'Company / TIN', submit: 'Sign in as', register: 'Register', choose: 'Select a workspace.', fill: 'Enter login and password / access code.', back: 'Back to home' },
  zh: { brand: '透明价格', kicker: '统一入口', title: '进入工作平台', lead: '请输入企业访问数据以进入工作闭环。', chooseWorkspace: '选择一个工作区', login: '登录名', loginPh: '输入登录名', password: '密码 / 访问码', passwordPh: '输入密码或访问码', forgot: '忘记密码？', company: '组织', optional: '可选', companyPh: '公司 / 税号', submit: '以此身份登录：', register: '注册', choose: '请选择工作区。', fill: '请输入登录名和密码 / 访问码。', back: '返回首页' },
} as const;
...
export default function LoginPage() {
  ...
  return (
    <main className="pc-login-page-v2" data-login-lang={lang} data-p7-no-translate="true">
      <style>{css}</style>
      <PlatformPublicHeader mode="back" brandLabel={t.brand} backLabel={t.back} />
