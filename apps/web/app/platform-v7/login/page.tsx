...
function readRoleFromPublicEntry(): PlatformRole | null {
  ...
  return workspaces.some((item) => item.role === requestedRole) ? (requestedRole as PlatformRole) : null;
}
      ...
      <section className='login-shell'>
        ...
        <section className='login-main' aria-label='Форма входа'>
          <div className='login-copy'>
            ...
            <p>Сначала логин, пароль и организация. Рабочее место выбирается внутри формы, а не отдельной каруселью кабинетов.</p>
          </div>
          ...
          <form className='login-form' onSubmit={onSubmit}>
            ...
            <label><span>Рабочее место</span><select value={role} onChange={(event) => { setRole(event.target.value as PlatformRole); setRoleFromEntry(false); }}>{workspaces.map((item) => <option key={item.role} value={item.role}>{item.title}</option>)}</select></label>
            <article className='login-selected'>
              {roleFromEntry ? <em>выбрано на публичной странице</em> : null}
        ...
        <aside className='login-side' aria-label='Контур доступа'>
          ...
          <ul>
            ...
            <li>роль фиксируется на сессию только после входа</li>
            <li>чужие кабинеты закрываются guard-контуром</li>
            <li>публичный выбор роли не открывает кабинет</li>
          </ul>
        </aside>
      </section>
    </main>
