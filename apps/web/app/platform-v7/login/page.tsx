...
type ManualField = 'login' | 'accessCode' | 'company';
...
export default function LoginPage() {
  ...
  const accessCodeRef = React.useRef<HTMLInputElement | null>(null);
  const companyRef = React.useRef<HTMLInputElement | null>(null);
  const manualInputRef = React.useRef<Record<ManualField, boolean>>({ login: false, accessCode: false, company: false });

  const clearFieldDom = React.useCallback((field: ManualField) => {
    const ref = field === 'login' ? loginRef : field === 'accessCode' ? accessCodeRef : companyRef;
    if (ref.current) ref.current.value = '';
  }, []);

  const clearAllAutofill = React.useCallback(() => {
    if (!manualInputRef.current.login) {
      setLogin('');
      clearFieldDom('login');
    }
    if (!manualInputRef.current.accessCode) {
      setAccessCode('');
      clearFieldDom('accessCode');
    }
    if (!manualInputRef.current.company) {
      setCompany('');
      clearFieldDom('company');
    }
  }, [clearFieldDom]);
  ...
  React.useEffect(() => {
    ...
    manualInputRef.current = { login: false, accessCode: false, company: false };
    clearAllAutofill();

    const clear = () => clearAllAutofill();
    const timers = [50, 120, 300, 700, 1200, 2000, 3200, 5000].map((ms) => window.setTimeout(clear, ms));
    const interval = window.setInterval(clear, 250);
    const stopInterval = window.setTimeout(() => window.clearInterval(interval), 6500);
  ...
  function unlockField(field: ManualField) {
    setInputsUnlocked(true);
    setError('');
    if (!manualInputRef.current[field]) {
      if (field === 'login') setLogin('');
      if (field === 'accessCode') setAccessCode('');
      if (field === 'company') setCompany('');
      clearFieldDom(field);
    }
  }

  function markManualInput(field: ManualField) {
    manualInputRef.current[field] = true;
    setInputsUnlocked(true);
    setError('');
  }

  function handleChange(field: ManualField, setter: React.Dispatch<React.SetStateAction<string>>) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError('');
    try {
      const response = await fetch(`/api/platform-v7/cabinet-lock-login?ts=${Date.now()}&v=manual-only`, {
        method: 'POST',
        cache: 'no-store',
        headers: {
      ...
      if (!response.ok) {
        ...
        setError(reason === 'password_mismatch' ? 'Пароль не совпал с кодом доступа.' : reason === 'login_mismatch' ? 'Логин не совпал с разрешённым доступом.' : 'Неверный логин или пароль.');
        return;
      }
        ...
        <section className='login-main' aria-label='Форма входа'>
          ...
          <form className='login-form' onSubmit={onSubmit} autoComplete='off' data-form-nonce={formNonce}>
            <div className='login-autofill-trap' aria-hidden='true'>
              <input type='search' name={`trap_a_${formNonce}`} autoComplete='off' tabIndex={-1} />
              <input type='text' name={`trap_b_${formNonce}`} autoComplete='off' tabIndex={-1} />
              <input type='search' name={`trap_c_${formNonce}`} autoComplete='off' tabIndex={-1} />
            </div>
            ...
            <label>
              ...
              <input
                ref={loginRef}
                key={`login-${formNonce}`}
                name={`field_${formNonce}_a`}
                value={login}
                onPointerDown={() => unlockField('login')}
                onFocus={() => unlockField('login')}
                onBeforeInput={() => markManualInput('login')}
                onPaste={() => markManualInput('login')}
                onChange={handleChange('login', setLogin)}
                type='search'
                autoComplete='off'
                autoCapitalize='none'
                autoCorrect='off'
            ...
            <label>
              ...
              <input
                ...
                name={`field_${formNonce}_b`}
                value={accessCode}
                onPointerDown={() => unlockField('accessCode')}
                onFocus={() => unlockField('accessCode')}
                onBeforeInput={() => markManualInput('accessCode')}
                onPaste={() => markManualInput('accessCode')}
                onChange={handleChange('accessCode', setAccessCode)}
            ...
            <label>
              ...
              <input
                ...
                name={`field_${formNonce}_c`}
                value={company}
                onPointerDown={() => unlockField('company')}
                onFocus={() => unlockField('company')}
                onBeforeInput={() => markManualInput('company')}
                onPaste={() => markManualInput('company')}
