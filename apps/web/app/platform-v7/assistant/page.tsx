...
const CONFIG: Record<PlatformRole, Config> = {
  operator: {
    ...
    questions: ['Почему банковская проверка выплаты остановлен?', 'Кто отвечает за следующий шаг?', 'Каких документов не хватает?', 'Куда перейти дальше?'],
    actions: [
  ...
  buyer: {
    ...
    actions: [
      ...
      { label: 'Банк', href: '/platform-v7/bank', note: 'Резерв, удержание и основания банковской проверки выплаты' },
    ],
  },
  seller: {
    ...
    icon: Wheat,
    questions: ['Почему лот на проверке?', 'Каких документов не хватает?', 'Что нужно для банковской проверки выплаты?', 'Куда перейти дальше?'],
    actions: [
  ...
  bank: {
    ...
    subtitle: 'Резерв, удержание, ответы банка и основания банковской проверки выплаты.',
    icon: Banknote,
    questions: ['Почему удержание не снято?', 'Что нужно для банковской проверки выплаты?', 'Есть ли конфликт статусов?', 'Куда перейти дальше?'],
    actions: [
      { label: 'Банковый контур', href: '/platform-v7/bank', note: 'Резерв, удержание и банковская проверка выплаты' },
...
function buildAnswer(role: PlatformRole, question: string, from: string) {
  ...
  if (lower.includes('документ') || lower.includes('пакет')) {
    return {
      ...
      summary: `Для роли «${roleLabel}» на экране «${screen}» сначала проверяется обязательный пакет, затем ответственный и влияние на банковскую проверку выплаты или спор.`,
  ...
  if (lower.includes('деньг') || lower.includes('выпуск') || lower.includes('удерж')) {
    return {
      ...
      summary: `Для роли «${roleLabel}» на экране «${screen}» проверяется резерв, удержание, документы, качество и спор. Банковская проверка выплаты нельзя показывать как завершённый без подтверждённых оснований.`,
