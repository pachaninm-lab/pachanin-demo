...
const CONFIG: Record<PlatformRole, Config> = {
  operator: {
    ...
    questions: ['Почему банковское подтверждение остановлено?', 'Кто отвечает за следующий шаг?', 'Каких документов не хватает?', 'Куда перейти дальше?'],
    actions: [
  ...
  buyer: {
    ...
    actions: [
      ...
      { label: 'Банк', href: '/platform-v7/bank', note: 'Резерв, удержание и основания банковского подтверждения' },
    ],
  },
  seller: {
    ...
    icon: Wheat,
    questions: ['Почему лот на проверке?', 'Каких документов не хватает?', 'Что нужно для банковского подтверждения?', 'Куда перейти дальше?'],
    actions: [
  ...
  bank: {
    ...
    subtitle: 'Резерв, удержание, ответы банка и основания банковского подтверждения.',
    icon: Banknote,
    questions: ['Почему удержание не снято?', 'Что нужно для банковского подтверждения?', 'Есть ли конфликт статусов?', 'Куда перейти дальше?'],
    actions: [
      { label: 'Банковый контур', href: '/platform-v7/bank', note: 'Резерв, удержание и банковское подтверждение' },
  ...
  executive: {
    ...
    icon: LayoutDashboard,
    questions: ['Где главный риск проверки?', 'Какие деньги под риском?', 'Что мешает следующему шагу?', 'Куда перейти дальше?'],
    actions: [
...
function buildAnswer(role: PlatformRole, question: string, from: string) {
  ...
  if (lower.includes('документ') || lower.includes('пакет')) {
    return {
      ...
      summary: `Для роли «${roleLabel}» на экране «${screen}» сначала проверяется обязательный пакет, затем ответственный и влияние на банковское подтверждение или спор.`,
  ...
  if (lower.includes('деньг') || lower.includes('выпуск') || lower.includes('удерж')) {
    return {
      ...
      summary: `Для роли «${roleLabel}» на экране «${screen}» проверяется резерв, удержание, документы, качество и спор. Банковское подтверждение нельзя показывать как завершённое без подтверждённых оснований.`,
