...
const MISSING_TEXT: Readonly<Record<MissingPrerequisite, string>> = {
  ADAPTER_NOT_IMPLEMENTED: 'На платформе ещё нет готового адаптера для этого подключения.',
  ENDPOINT_NOT_CONFIGURED: 'Не настроено окружение, с которым должна работать интеграция.',
  VENDOR_CREDENTIALS_NOT_ISSUED: 'Нет подтверждённых учётных данных внешней системы для организации.',
  CONTRACT_NOT_ATTESTED: 'Интеграция ещё не прошла обязательные независимые проверки.',
...
export const CONNECTION_CENTER_REQUIRED_BUT_NOT_MODELED = Object.freeze([
  {
    title: 'ФГИС «Зерно»',
    detail: 'Статус ФГИС пока не подключён к серверной модели этого экрана.',
  },
  {
    title: 'Перевозочные документы',
    detail: 'Статус электронных перевозочных документов пока не подключён к серверной модели этого экрана.',
  },
]);
...
export function presentConnection(
  ...
  return {
    ...
    actionDisabledReason:
      kind === 'ONE_C'
        ? 'Подключение станет доступно после серверного pairing и выпуска scoped machine credential.'
        : kind === 'EDO'
          ? 'Подключение станет доступно после серверного контура авторизации конкретной организации у оператора.'
          : null,
  };
}
...
> {
  if (maturity === 'CONFIRMED_LIVE' && mayCarryRealTraffic) {
    return {
      ...
      statusTone: 'positive',
      detail: 'Внешняя система вернула доказанное подтверждение. Только этот уровень считается готовым к реальному трафику.',
  ...
  if (mayCarryRealTraffic && maturity !== 'CONFIRMED_LIVE') {
    return {
      ...
      statusTone: 'danger',
      detail: 'Сервер вернул несогласованные признаки. Реальный обмен на этом экране не считается подтверждённым.',
  ...
  if (maturity === 'ADAPTER_READY') {
    return {
      status: 'Адаптер готов',
      statusTone: 'warning',
      detail: 'Контракт адаптера проверен, но внешнее подключение и реальный обмен ещё не подтверждены.',
      realTrafficConfirmed: false,
    };
  }
  ...
  return {
    ...
    statusTone: 'danger',
    detail: 'Получен неизвестный серверный статус. Реальный обмен считается неподтверждённым до разбирательства.',
    realTrafficConfirmed: false,
  };
}
...
function attestationsFor(
  ...
  if (subjects.some((subject) => subject.state.attested)) {
    return 'Есть предмет подключения, прошедший все обязательные независимые проверки.';
  }
