...
import { PlatformV7AiCommand } from '@/components/v7r/PlatformV7AiCommand';
...
const processSteps: ProcessStep[] = [
  { label: 'Цена', text: 'Фиксируются цена, объём, базис и допуски.', Icon: Wheat },
  { label: 'Рейс', text: 'Назначаются транспорт, водитель и контрольные точки.', Icon: Truck },
  { label: 'Приёмка', text: 'Вес, факт поставки и расхождения попадают в контур.', Icon: Building2 },
  { label: 'Качество', text: 'Показатели лаборатории связаны с партией.', Icon: FlaskConical },
  { label: 'Документы', text: 'СДИЗ, ЭДО, ТТН и акты сверяются с событиями.', Icon: FileCheck2 },
  { label: 'Деньги', text: 'Банк видит основание расчёта и блокеры.', Icon: Banknote },
  { label: 'Спор', text: 'Факты собираются в доказательный пакет.', Icon: Scale },
];

const controlCards: ControlCard[] = [
  { title: 'ИИ-сигналы', text: 'Подсветка риска, следующего действия и слабого места сделки.', Icon: ShieldCheck },
  { title: 'Доказательства', text: 'Факты, документы и роли соединяются в проверяемую цепочку.', Icon: FileCheck2 },
  { title: 'Основание оплаты', text: 'Оплата привязана к подтверждённым событиям исполнения.', Icon: Banknote },
  { title: 'Контроль ролей', text: 'Каждый участник видит своё рабочее место и свои действия.', Icon: UserRound },
];
    ...
    <main data-testid='platform-v7-root-execution-cockpit' className='pc-v7-entry-page pc-v7-public-entry'>
      ...
      <header className='entry-header' aria-label='Публичная навигация'>
        <Link href='/platform-v7' className='entry-brand' aria-label='Прозрачная Цена'>
          ...
          <span><strong>Прозрачная Цена</strong><small>AI-контур исполнения сделки</small></span>
        </Link>
        <nav className='entry-desktop-nav' aria-label='Разделы главной страницы'>
          <a href='#ai-control'>ИИ-контур</a>
      ...
      <section className='entry-hero' aria-labelledby='entry-hero-title'>
        <div className='entry-hero-copy'>
          <span className='entry-kicker'>AI execution layer</span>
          <h1 id='entry-hero-title'>
            <span>ИИ видит сделку</span>
            <span>после цены:</span>
            <span>риск, факт, деньги</span>
          </h1>
          ...
          <div className='entry-hero-actions'>
            <a href='#ai-control' className='entry-primary-cta'>Показать ИИ-контур<ArrowRight size={20} /></a>
        ...
        <div className='entry-hero-visual' aria-hidden='true'>
          ...
          <div className='entry-floating-card docs'><FileCheck2 size={22} /><span>Документы</span><b>связаны с фактом</b></div>
          <div className='entry-floating-card quality'><FlaskConical size={22} /><span>Качество</span><b>сверяется с допуском</b></div>
          <div className='entry-floating-card money'><Banknote size={22} /><span>Оплата</span><b>ждёт основание</b></div>
        </div>
      </section>
      ...
      <section id='control' className='entry-section' aria-labelledby='control-title'>
        <div className='entry-section-head'>
          <h2 id='control-title'>Что видит система</h2>
          <p>На главной не нужно перечислять абстрактные преимущества. Нужно показать интеллект контура: где риск, что подтверждено и что делать дальше.</p>
        </div>
      ...
      <section id='process' className='entry-section entry-process-section' aria-labelledby='process-title'>
        <div className='entry-section-head compact'>
          ...
          <p>Цена — только старт. После неё платформа ведёт исполнение, документы, деньги и спор.</p>
        </div>
      ...
      <section id='roles' className='entry-section' aria-labelledby='roles-title'>
        <div className='entry-section-head'>
          <h2 id='roles-title'>Выберите роль в сделке</h2>
          <p>Каждый участник входит в своё рабочее место: без смешения ролей, лишних экранов и ручной путаницы.</p>
        </div>
...
.entry-control-tile, .entry-role-tile, .entry-process-tile { border: 1px solid rgba(7,22,17,.075); background: rgba(255,255,255,.80); box-shadow: 0 14px 34px rgba(7,22,17,.06); }
...
@media (max-width: 980px) {
  ...
  .entry-header { grid-template-columns: 1fr auto; padding: 9px 16px; gap: 10px; }
  ...
  .entry-kicker { margin-bottom: 12px; padding: 7px 11px; font-size: 10px; }
  .entry-hero h1 { font-size: clamp(32px, 8.9vw, 40px); line-height: 1.015; letter-spacing: -.044em; }
  ...
  .entry-section h2 { font-size: clamp(27px, 8vw, 36px); }
...
@media (max-width: 420px) {
  ...
  .entry-hero h1 { font-size: clamp(30px, 8.55vw, 34px); }
  .entry-hero p { font-size: 15px; }
