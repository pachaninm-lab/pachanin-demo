...
const mobileRoleCardStyle = {
  ...
  boxShadow: '0 8px 20px rgba(15,23,42,.052)',
  color: '#061A16',
  textDecoration: 'none',
  display: 'grid',
...
const mobileIconStyle = { width: 28, height: 28, color: '#008B2E' } as const;
const mobileRoleTitleStyle = { maxWidth: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#061A16', fontSize: 12.4, lineHeight: 1.08, fontWeight: 950 } as const;
const mobileRoleTextStyle = { color: '#66717C', fontSize: 10.2, lineHeight: 1.2, fontWeight: 640 } as const;
  ...
  return (
    <main data-testid='platform-v7-root-execution-cockpit' className='pc-v7-entry-page'>
      ...
      <section className='entry-roles' aria-label='Выберите свою роль'>
        ...
        <div className='entry-mobile-role-grid' style={mobileRoleGridStyle}>
...
function RoleTile({ role, desktop = false, small = false }: { readonly role: RoleCard; readonly desktop?: boolean; readonly small?: boolean }) {
  if (!desktop && !small) {
    return (
      <Link href={role.href} className='entry-role-card mobile' style={mobileRoleCardStyle}>
        <span style={mobileIconStyle}><Icon name={role.icon} /></span>
        <strong style={mobileRoleTitleStyle}>{role.title}</strong>
        <span style={mobileRoleTextStyle}>{role.text}</span>
      </Link>
    );
  }

  return (
    <Link href={role.href} className={desktop ? 'entry-role-card desktop' : 'entry-role-card small'}>
      <Icon name={role.icon} />
