...
export default function PlatformV7RootPage() {
  return (
    <main data-testid='platform-v7-root-execution-cockpit' className='pc-v7-entry-page pc-v7-public-entry'>
      ...
      <section className='entry-trust-strip' aria-label='Доверие и контроль'>{trustItems.map((item) => <TrustTile key={item.title} item={item} />)}<Link href='/platform-v7/register' className='entry-trust-cta'>Подключить организацию</Link></section>
    </main>
  );
}
...
.entry-login { gap: 8px; padding: 0 15px; } .entry-header-register { padding: 0 16px; color: #087a3b; background: rgba(0,122,47,.07); border-color: rgba(0,122,47,.18); }
...
.entry-hero-copy { position: relative; z-index: 2; align-self: center; max-width: 700px; padding: clamp(24px, 3vw, 40px); border: 1px solid rgba(7,22,17,.075); border-radius: 34px; background: rgba(255,255,255,.74); box-shadow: 0 20px 52px rgba(7,22,17,.07); backdrop-filter: blur(14px); }
