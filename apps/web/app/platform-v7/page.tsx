  ...
  return (
    <main data-testid='platform-v7-root-execution-cockpit' className='pc-v7-entry-page pc-v7-public-entry'>
      ...
      <section id='control' className='entry-section' aria-labelledby='control-title'>
        <div className='entry-section-head'><h2 id='control-title'>Что контролирует платформа</h2><p>После согласования цены под контролем остаётся главное: рейс, приёмка, документы, качество и основание для оплаты.</p></div>
      ...
      <section id='process' className='entry-section entry-process-section' aria-labelledby='process-title'>
        <div className='entry-section-head compact'><h2 id='process-title'>Как проходит сделка</h2><p>На каждом этапе видно, что уже подтверждено, что мешает движению сделки и кто должен действовать.</p></div>
      ...
      <section id='roles' className='entry-section' aria-labelledby='roles-title'>
        <div className='entry-section-head'><h2 id='roles-title'>Выберите свою роль в сделке</h2><p>Сначала выберите роль участника сделки. После этого вход выполняется по логину, паролю и организации.</p></div>
...
function ProcessTile({ step, index }: { step: ProcessStep; index: number }) {
  ...
  return <article className='entry-process-tile'><span className='entry-process-index'>{index + 1}</span><span className='entry-process-icon'><Icon size={21} strokeWidth={2.2} /></span><strong>{step.label}</strong><small>{step.text}</small></article>;
}
function RoleTile({ role }: { role: RoleCard }) {
  const Icon = role.Icon;
  return <Link href={role.href} className='entry-role-tile'><Icon size={27} strokeWidth={2.25} /><strong>{role.title}</strong><span>{role.text}</span><em>{role.cta}</em></Link>;
}
...
.entry-desktop-nav a:hover { color: #087a3b; }
.entry-header-actions { display: flex; align-items: center; justify-content: flex-end; gap: 9px; }
.entry-login, .entry-header-register { min-height: 42px; border-radius: 15px; border: 1px solid rgba(7,22,17,.10); background: rgba(255,255,255,.86); color: #071611; font-size: 14px; font-weight: 900; }
...
.entry-hero { position: relative; display: grid; grid-template-columns: minmax(0, .96fr) minmax(390px, 1.04fr); align-items: stretch; gap: 20px; padding: clamp(26px, 4.8vw, 68px) clamp(18px, 4vw, 56px) 28px; overflow: hidden; }
...
.entry-primary-cta { gap: 9px; color: #fff !important; background: #087a3b; box-shadow: 0 18px 38px rgba(0,122,47,.22); }
.entry-secondary-cta { color: #087a3b !important; background: rgba(255,255,255,.82); border: 1px solid rgba(0,122,47,.24); }
.entry-register-cta { color: #09251b !important; background: rgba(0,122,47,.08); border: 1px solid rgba(0,122,47,.16); }
