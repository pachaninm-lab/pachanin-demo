...
export default function DealsComparePage({ searchParams }: { searchParams?: { ids?: string } }) {
  ...
  const rows = [
    { label: 'Статус', get: (item: DomainDeal) => statusLabel(item.status) },
    { label: 'Лот', get: (item: DomainDeal) => item.lotId ?? '—' },
    { label: 'Маршрут', get: (item: DomainDeal) => item.routeId ?? '—' },
    { label: 'Сумма', get: (item: DomainDeal) => formatMoney(item.reservedAmount) },
    { label: 'Удержано', get: (item: DomainDeal) => formatMoney(item.holdAmount) },
