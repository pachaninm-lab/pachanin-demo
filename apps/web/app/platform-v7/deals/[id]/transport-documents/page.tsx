    ...
    <div style={{ display: 'grid', gap: 16, maxWidth: 1120, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div style={{ display: 'grid', gap: 8 }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Перевозочные документы · {deal.id}</div>
            <div style={{ fontSize: 13, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.7 }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 16 }}>
        <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Что важно по пакету</div>
          <div style={{ fontSize: 13, color: 'var(--pc-text-secondary, #475569)', lineHeight: 1.7 }}>{pack.summary}</div>
          <div style={{ display: 'grid', gap: 8 }}>
            {pack.blockers.length ? pack.blockers.map((blocker) => (
              <div key={blocker} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 13, color: 'var(--pc-text-secondary, #475569)', lineHeight: 1.6 }}>
        ...
        <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Текущий сценарий по деньгам</div>
          {scenario && currentStep ? (
            <>
              <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{scenario.headline}</div>
              <div style={{ fontSize: 13, color: 'var(--pc-text-secondary, #475569)', lineHeight: 1.7 }}>{scenario.objective}</div>
              <div style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 14, padding: 12, background: '#F8FAFB', display: 'grid', gap: 6 }}>
                <div style={{ fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>Текущий шаг</div>
                <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{currentStep.label}</div>
                <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #475569)', lineHeight: 1.6 }}>{currentStep.detail}</div>
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Документы пакета</div>
          {pack.driverActionUrl ? <span style={{ fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>Для водителя доступен отдельный мобильный сценарий провайдера</span> : null}
        </div>
        <div style={{ display: 'grid', gap: 12 }}>
          {pack.documents.map((document) => {
            const documentTone = tone(document.status);
            return (
              <article key={document.id} style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 16, padding: 16, background: '#F8FAFB', display: 'grid', gap: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  <div>
                    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, fontWeight: 800, color: '#0A7A5F' }}>{document.providerDocumentId}</div>
                    <div style={{ marginTop: 6, fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{document.title}</div>
                  </div>
                  <div style={{ display: 'grid', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>ГИС ЭПД: {document.gisStatus === 'registered' ? 'зарегистрирован' : document.gisStatus === 'pending' ? 'ожидается' : document.gisStatus === 'error' ? 'ошибка' : 'не требуется'}</span>
                  </div>
                </div>
                <div style={{ display: 'grid', gap: 8 }}>
                  {document.signatures.map((signature) => (
                    <div key={signature.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 120px 180px', gap: 12, border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 12, padding: 12, background: '#fff' }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--pc-text-primary, #0F1419)' }}>{signature.actor}</div>
                        <div style={{ marginTop: 4, fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>{signature.role}</div>
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #475569)', fontWeight: 700 }}>{signatureLabel(signature.signatureType)}</div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <Link href='/platform-v7/control-tower/hotlist' style={{ textDecoration: 'none', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: 'var(--pc-text-primary, #0F1419)', fontSize: 13, fontWeight: 700 }}>
...
function Metric({ title, value, note }: { title: string; value: string; note: string }) {
  return (
    <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
      ...
      <div style={{ marginTop: 8, fontSize: 24, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{value}</div>
