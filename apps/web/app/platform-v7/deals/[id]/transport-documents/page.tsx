...
function tone(status: string) {
  if (['completed', 'registered_in_gis_epd', 'release_allowed'].includes(status)) {
    return { bg: 'rgba(10,122,95,0.08)', border: 'rgba(10,122,95,0.18)', color: '#0A7A5F' };
  }
  if (['provider_error', 'manual_review', 'blocks_release', 'declined', 'cancelled'].includes(status)) {
    ...
    return (
      <div style={{ display: 'grid', gap: 16, maxWidth: 980, margin: '0 auto' }}>
        <section style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 18, padding: 18 }}>
          <div style={{ fontSize: 28, fontWeight: 800, color: '#0F1419' }}>Перевозочные документы</div>
          <div style={{ marginTop: 8, fontSize: 13, color: '#6B778C', lineHeight: 1.7 }}>
            Для сделки {deal.id} пакет перевозочных документов пока не создан. Это значит, что юридически значимый контур логистики ещё не заведён во внешний провайдер.
          </div>
        </section>
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 1080, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 18, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ marginTop: 8, fontSize: 13, color: '#6B778C', lineHeight: 1.7 }}>
              Внешний юридически значимый контур рейса через СберКорус. Здесь платформа показывает не внутренние события, а состояние пакета перевозочных документов, подписей и влияние на деньги.
            </div>
          </div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
        ...
        <Metric title='Провайдер' value='СберКорус' note='Внешний юридически значимый контур перевозки' />
      ...
      <section style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {pack.driverActionUrl ? <span style={{ fontSize: 12, color: '#6B778C' }}>Для водителя доступен отдельный mobile-flow</span> : null}
        </div>
              ...
              <article key={document.id} style={{ border: '1px solid #E4E6EA', borderRadius: 16, padding: 16, background: '#F8FAFB', display: 'grid', gap: 12 }}>
                ...
                <div style={{ display: 'grid', gap: 8 }}>
                  {document.signatures.map((signature) => (
                    <div key={signature.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 120px 120px', gap: 12, border: '1px solid #E4E6EA', borderRadius: 12, padding: 12, background: '#fff' }}>
                      <div>
