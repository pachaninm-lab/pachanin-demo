              ...
              return (
                <article key={step.id} style={{ border: `1px solid ${palette.border}`, background: palette.bg, borderRadius: 16, padding: 16, display: 'grid', gap: 8, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <span style={{ display: 'inline-flex', alignItems: 'center', padding: '6px 10px', borderRadius: 999, background: active || done ? '#fff' : '#F1F5F9', border: '1px solid rgba(15,20,25,0.08)', color: done ? '#0A7A5F' : active ? palette.color : 'var(--pc-text-muted, #64748B)', fontSize: 11, fontWeight: 800 }}>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--pc-text-secondary, #334155)', lineHeight: 1.65, wordBreak: 'break-word' }}><strong style={{ color: 'var(--pc-text-primary, #0F1419)' }}>{step.event}.</strong> {step.detail}</div>
                  <div style={{ fontSize: 12, color: active ? palette.color : 'var(--pc-text-muted, #6B778C)', fontWeight: 700, lineHeight: 1.6, wordBreak: 'break-word' }}>Почему это важно для денег: {step.releaseReason}</div>
                </article>
              );
            })}
          </div>
          ...
          <section className='transport-sim-card' style={{ display: 'grid', gap: 12 }}>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {visibleWebhooks.map((hook) => {
                ...
                return (
                  <div key={hook.id} style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 14, padding: 12, background: '#F8FAFB', display: 'grid', gap: 6, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', wordBreak: 'break-word' }}>{hook.direction === 'outbound' ? 'Исходящий' : 'Входящий'} · {hook.topic}</div>
                    <div style={{ fontSize: 12, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.6, wordBreak: 'break-word' }}>{hook.endpoint}</div>
                    <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #334155)', lineHeight: 1.6, wordBreak: 'break-word' }}>{hook.summary}</div>
                  </div>
                );
              })}
            </div>
          ...
          <section className='transport-sim-card' style={{ display: 'grid', gap: 12 }}>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {visibleActions.map((action) => (
                <div key={action.id} style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 14, padding: 12, background: '#F8FAFB', display: 'grid', gap: 4, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', wordBreak: 'break-word' }}>{action.title}</div>
                  <div style={{ fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>Роль: {action.role}</div>
                  <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #334155)', lineHeight: 1.6, wordBreak: 'break-word' }}>{action.note}</div>
                </div>
              ))}
            </div>
          </section>
      ...
      <section className='transport-sim-card' style={{ display: 'grid', gap: 12 }}>
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {visibleAudit.map((item) => {
            ...
            return (
              <div key={item.id} style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 14, padding: 12, background: '#F8FAFB', display: 'grid', gap: 4, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', wordBreak: 'break-word' }}>{item.action}</div>
                ...
                <div style={{ fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>{item.actor}</div>
                <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #334155)', lineHeight: 1.6, wordBreak: 'break-word' }}>{item.detail}</div>
              </div>
            );
          })}
        </div>
...
function Metric({ title, value, note }: { title: string; value: string; note: string }) {
  return (
    <section style={{ background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 16, padding: 16, minWidth: 0 }}>
      ...
      <div style={{ marginTop: 8, fontSize: 22, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', wordBreak: 'break-word' }}>{value}</div>
      <div style={{ marginTop: 8, fontSize: 12, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.6, wordBreak: 'break-word' }}>{note}</div>
    </section>
  );
}
