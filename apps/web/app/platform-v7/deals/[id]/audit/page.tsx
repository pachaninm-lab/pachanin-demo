...
export default function DealAuditPage({ params }: { params: { id: string } }) {
  return (
    <>
      <GrainWorkflowPage eyebrow='Сделка · журнал' title={`Журнал сделки ${params.id}`} lead='Журнал показывает действия, роли, объекты, причины, изменения статуса и материалы, на которые опирается сделка.' primaryHref='/platform-v7/data-room/grain' primaryLabel='Data-room' items={[
