...
export default function DealDocumentsPage({ params }: { params: { id: string } }) {
  ...
  const auditTrail = React.useMemo(() => {
    const checklistEvents = REQUIRED_DOCS.filter((doc) => checklist[doc.id]).map((doc, index) => ({
      ...
      note: doc.blocker ? 'Снимает критичный блокер банковского основания.' : 'Не блокирует основание, но усиливает досье сделки.',
    }));
  ...
  function requestRelease() {
    setToast(releaseReady ? `Запрос на банковскую проверку по ${params.id} подготовлен.` : `Нельзя запросить банковскую проверку: ${blockerDone}/${blockerTotal} критичных документов готовы.`);
  }
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 1040, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 18, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: '#6B778C', marginTop: 6, lineHeight: 1.6 }}>Отдельный документный контур: файлы, чек-лист, журнал действий и готовность банковского основания.</div>
          </div>
          <span style={{ display: 'inline-flex', alignItems: 'center', padding: '6px 10px', borderRadius: 999, background: releaseReady ? 'rgba(10,122,95,0.08)' : 'rgba(217,119,6,0.08)', border: `1px solid ${releaseReady ? 'rgba(10,122,95,0.18)' : 'rgba(217,119,6,0.18)'}`, color: releaseReady ? '#0A7A5F' : '#B45309', fontSize: 11, fontWeight: 800 }}>
            {releaseReady ? 'Документы готовы к проверке' : `До проверки: ${blockerDone}/${blockerTotal}`}
          </span>
        </div>
      </section>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
        ...
        <Metric title='Чек-лист' value={`${requiredDone}/${REQUIRED_DOCS.length}`} note='Подтверждённые пункты документного набора.' />
        <Metric title='Критичные документы' value={`${blockerDone}/${blockerTotal}`} note='Документы, влияющие на банковское основание.' />
      </section>
      ...
      <section style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ marginTop: 6, fontSize: 12, color: '#6B778C', lineHeight: 1.6 }}>Что уже подтверждено и что ещё держит банковское основание.</div>
          </div>
            ...
            return (
              <div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 14, border: '1px solid #E4E6EA', background: checklist[doc.id] ? 'rgba(10,122,95,0.06)' : '#F8FAFB' }}>
                ...
                <div style={{ display: 'grid', gap: 2, flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#6B778C' }}>{doc.blocker ? 'Блокирует банковское основание' : 'Не блокирует основание, но нужен для полного досье'}</div>
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.1fr) minmax(320px,.9fr)', gap: 16 }}>
        ...
        <section style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: '#0F1419' }}>Журнал действий</div>
