    ...
    <div style={{ display: 'grid', gap: 18, maxWidth: 980, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: 30, lineHeight: 1.08, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', marginTop: 10 }}>{params.id}</div>
            <div style={{ fontSize: 13, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.7, marginTop: 8, maxWidth: 760 }}>
          ...
          <div style={{ display: 'grid', gap: 6 }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', padding: '8px 12px', borderRadius: 999, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)', fontSize: 12, fontWeight: 800 }}>
              Для закрытых сделок
            </span>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
        {scoreBlocks.map((item) => (
          <div key={item.title} style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 16, padding: 16 }}>
            <div style={{ fontSize: 11, color: 'var(--pc-text-muted, #6B778C)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 800 }}>{item.title}</div>
            <div style={{ marginTop: 10, fontSize: 30, lineHeight: 1.05, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{item.value}</div>
            <div style={{ marginTop: 8, fontSize: 12, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.5 }}>{item.note}</div>
          </div>
        ))}
      </section>

      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Что оценивается</div>
        <div style={{ display: 'grid', gap: 10 }}>
          {questions.map((item, index) => (
            <div key={item} style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 14, padding: 14, background: '#F8FAFB', display: 'grid', gap: 8 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', padding: '4px 8px', borderRadius: 999, background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)', fontSize: 11, fontWeight: 800, width: 'fit-content' }}>
                Вопрос {index + 1}
              </div>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--pc-text-primary, #0F1419)', lineHeight: 1.5 }}>{item}</div>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: `1px solid ${active ? 'rgba(10,122,95,0.18)' : 'var(--pc-border, #E4E6EA)'}`,
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Комментарий</div>
        <textarea
          value={comment}
          onChange={(event) => setComment(event.target.value)}
          rows={5}
          style={{ width: '100%', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 14, padding: 14, background: '#F8FAFB', fontSize: 13, color: 'var(--pc-text-secondary, #475569)', lineHeight: 1.7, resize: 'vertical' }}
        />
      </section>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <Link href={`/platform-v7/deals/${params.id}`} style={{ textDecoration: 'none', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: 'var(--pc-text-primary, #0F1419)', fontSize: 13, fontWeight: 700 }}>
          Вернуться к сделке
        </Link>
        <Link href='/platform-v7/companies/6829123456' style={{ textDecoration: 'none', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: 'var(--pc-text-primary, #0F1419)', fontSize: 13, fontWeight: 700 }}>
