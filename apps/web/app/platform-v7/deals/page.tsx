...
export default function PlatformV7DealsPage() {
  ...
  return (
    <div data-testid='platform-v7-deals-page' style={{ display: 'grid', gap: 18 }}>
      ...
      <section className='pc-deals-shell' style={{ background: 'linear-gradient(135deg, #FFFFFF 0%, #F8FAFB 60%, #EEF6F3 100%)', border: '1px solid #E4E6EA', borderRadius: 26, padding: 22, display: 'grid', gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div style={{ display: 'grid', gap: 8, maxWidth: 860 }}>
            ...
            <h1 className='pc-deals-title' style={{ margin: 0, fontSize: 'clamp(26px, 4.2vw, 44px)', lineHeight: 1.06, letterSpacing: '-0.04em', color: '#0F1419', fontWeight: 950 }}>
        ...
        <div className='pc-deals-list' style={{ display: 'grid', gap: 10 }}>
          {dealSnapshots.map((deal) => (
            <Link key={deal.id} href={deal.href} className='pc-deal-row' style={{ textDecoration: 'none', background: '#fff', border: '1px solid #E4E6EA', borderRadius: 16, padding: 14, display: 'grid', gap: 10 }}>
              <div className='pc-deal-row-top' style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                <div>
                  <span className='pc-deal-row-id' style={{ color: '#64748B', fontSize: 11, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.07em' }}>{deal.id} · {deal.lot}</span>
                  <p className='pc-deal-row-stage' style={{ margin: '4px 0 0', color: '#0F1419', fontSize: 14, fontWeight: 900 }}>{deal.stage}</p>
                </div>
