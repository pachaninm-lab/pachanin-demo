        ...
        {scenarios.map(([name, signal, money, next, risk]) => (
          <article key={name} style={cardStyle}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
              ...
              <span style={{ padding: '5px 9px', borderRadius: 999, background: '#F8FAFB', border: '1px solid #CBD5E1', fontSize: 11, color: 'var(--pc-text-secondary, #334155)', fontWeight: 900 }}>{risk}</span>
            </div>
...
const heroStyle = { border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 20, padding: 22, background: '#fff', display: 'grid', gap: 14 } as const;
const badgeStyle = { display: 'inline-flex', width: 'fit-content', padding: '6px 10px', borderRadius: 999, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)', fontSize: 12, fontWeight: 900 } as const;
const cardStyle = { background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 16, display: 'grid', gap: 10 } as const;
...
const miniLink = { display: 'inline-flex', minHeight: 38, alignItems: 'center', justifyContent: 'center', padding: '8px 10px', borderRadius: 10, background: '#fff', color: 'var(--pc-text-primary, #0F1419)', border: '1px solid #CBD5E1', textDecoration: 'none', fontSize: 12, fontWeight: 850 } as const;
