...
import { ArrowRight, Building2, ClipboardCheck, FileCheck2, FlaskConical, Landmark, LockKeyhole, MessageCircleQuestion, Scale, ShieldCheck, Truck, Wheat, type LucideIcon } from 'lucide-react';
...
const deal = {
  ...
  status: 'Ожидается подтверждение качества и комплекта документов',
};

type Stage = { title: string; status: 'done' | 'active' | 'pending'; owner: string; text: string; Icon: LucideIcon };
type RoleView = { role: string; access: string; responsibility: string; Icon: LucideIcon };

const stages: Stage[] = [
  { title: 'Коммерческие условия', status: 'done', owner: 'Продавец · покупатель', text: 'Зафиксированы цена, объём, базис поставки и допустимые показатели качества.', Icon: Wheat },
  { title: 'Рейс', status: 'done', owner: 'Логистика', text: 'Назначены маршрут, транспорт, водитель и контрольные точки исполнения.', Icon: Truck },
  { title: 'Приёмка', status: 'done', owner: 'Элеватор', text: 'Фиксируются факт поставки, вес, расхождения и связь партии с документами.', Icon: Building2 },
  { title: 'Качество', status: 'active', owner: 'Лаборатория', text: 'Показатели качества проверяются до формирования окончательного основания для расчёта.', Icon: FlaskConical },
  { title: 'Документы', status: 'pending', owner: 'Стороны сделки', text: 'СДИЗ, ЭДО, транспортные документы и акты должны соответствовать событиям сделки.', Icon: FileCheck2 },
  { title: 'Расчёт', status: 'pending', owner: 'Банк', text: 'Основание для оплаты формируется после подтверждения приёмки, качества и документов.', Icon: Landmark },
];
...
export default function PlatformV7DemoPage() {
  return (
    <main className='p7-demo-page' data-testid='platform-v7-demo-public-workspace'>
      <style>{css}</style>
      <header className='p7-demo-header' aria-label='Навигация демонстрационного режима'>
      ...
      <section className='p7-demo-hero' aria-labelledby='p7-demo-title'>
        <div className='p7-demo-hero-copy'>
          <span className='p7-demo-kicker'>Демонстрационный режим · без регистрации</span>
          <h1 id='p7-demo-title'>Контроль исполнения сделки после согласования цены</h1>
          <p>На примере условной сделки показано, как фиксируются ключевые события исполнения: рейс, приёмка, качество, документы, основание для расчёта и возможный спор.</p>
          <div className='p7-demo-actions'><a href='#stages' className='p7-demo-primary'>Перейти к этапам<ArrowRight size={18} /></a><Link href='/platform-v7/contact' className='p7-demo-secondary'><MessageCircleQuestion size={18} />Задать вопрос</Link></div>
        </div>
        <aside className='p7-demo-case' aria-label='Параметры демонстрационной сделки'>
          <span>Условные данные</span><strong>{deal.id}</strong>
      ...
      <section id='stages' className='p7-demo-card' aria-labelledby='stages-title'>
        <SectionHead n='01' title='Этапы исполнения' text='Каждый этап показывает статус, ответственного участника и условие перехода к следующему действию.' id='stages-title' />
        <div className='p7-demo-timeline'>{stages.map((stage, index) => <StageCard key={stage.title} stage={stage} index={index} />)}</div>
      </section>

      <section className='p7-demo-money' aria-labelledby='money-title'>
        <div><span className='p7-demo-kicker'>Текущий статус</span><h2 id='money-title'>Расчёт возможен после подтверждения оснований</h2><p>{deal.status}. До закрытия этих условий платформа показывает участникам, какие действия остаются незавершёнными.</p></div>
        <div className='p7-demo-money-box'><small>Сумма сделки</small><strong>{deal.amount}</strong><p>В демонстрационном режиме финансовые действия недоступны. В рабочем контуре расчёт выполняется только при наличии подтверждённых оснований.</p><button disabled>Финансовые действия недоступны в демо</button></div>
      </section>

      <section className='p7-demo-card' aria-labelledby='roles-title'>
        <SectionHead n='02' title='Информация по ролям' text='Каждый участник получает доступ только к своему слою данных и к действиям, относящимся к его зоне ответственности.' id='roles-title' />
      ...
      <section className='p7-demo-proof' aria-labelledby='proof-title'>
        <div><span className='p7-demo-kicker'>Доказательная база</span><h2 id='proof-title'>Разбор расхождений проводится на основании зафиксированных данных</h2><p>Платформа связывает события сделки, документы и действия участников в единый проверяемый контур.</p></div>
        <ul>{evidence.map((item) => <li key={item}><ShieldCheck size={18} />{item}</li>)}</ul>
      </section>

      <section className='p7-demo-final' aria-label='Следующий шаг'>
        <div><LockKeyhole size={24} /><h2>Рабочие кабинеты доступны только после подключения организации</h2><p>Демонстрационный режим не создаёт сессию, не меняет роль пользователя и не открывает рабочие маршруты платформы.</p></div>
...
function StageCard({ stage, index }: { stage: Stage; index: number }) { const Icon = stage.Icon; const statusText = stage.status === 'done' ? 'выполнено' : stage.status === 'active' ? 'на проверке' : 'ожидает'; return <article className={`p7-demo-gate ${stage.status}`}><div><span>{String(index + 1).padStart(2, '0')}</span><Icon size={22} /></div><strong>{stage.title}</strong><p>{stage.text}</p><small>{stage.owner} · {statusText}</small></article>; }
function RoleCard({ item }: { item: RoleView }) { const Icon = item.Icon; return <article className='p7-demo-role-card'><Icon size={22} /><div><strong>{item.role}</strong><p>Доступ: {item.access}. Ответственность: {item.responsibility}.</p></div></article>; }
