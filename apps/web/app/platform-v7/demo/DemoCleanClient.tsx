...
const cards = [
  ...
  { key: 'acceptance', state: 'done', Icon: Building2 },
  { key: 'quality', state: 'review', Icon: FlaskConical },
  { key: 'documents', state: 'pending', Icon: FileCheck2 },
...
export function DemoCleanClient(){
  ...
  const t = COPY[lang];
  return <main className='p7-demo-clean' data-lang={lang} data-p7-no-translate='true'><style>{css}</style><header><Link href='/platform-v7' className='brand'><BrandMark size={42}/><span><b>{t.brand}</b><small>{t.sub}</small></span></Link><nav><Link href='/platform-v7' aria-label={t.back}><ArrowLeft size={22}/></Link><Link href='/platform-v7/contact' aria-label={t.q}><CircleHelp size={22}/></Link></nav></header><section className='hero'><div><h1>{t.title}</h1><p>{t.lead}</p><Link href='/platform-v7/contact'>{t.q}</Link></div><aside><span>{t.status}</span><strong>{t.current}</strong></aside></section><section className='case'><article><span>{t.deal}</span><b>DL-EXEC-001</b><p>{t.volume}</p></article><article><span>{t.amount}</span><b>2 964 000 ₽</b><p>{t.disabled}</p></article><article><span>{t.route}</span><b>Farm → Elevator → Buyer</b></article></section><h2>{t.map}</h2><section className='grid'>{cards.map(({key,state,Icon})=><article key={key}><Icon size={24}/><b>{t.cards[key]}</b><span>{t.states[state]}</span></article>)}</section><section className='money'><Banknote size={28}/><h2>{t.money}</h2><p>{t.current}</p><button disabled>{t.disabled}</button></section><section className='proof'><ShieldCheck size={28}/><h2>{t.proof}</h2><p>{t.lead}</p></section><footer><Link href='/platform-v7'>{t.home}</Link><Link href='/platform-v7/contact'>{t.q}</Link></footer></main>
}
