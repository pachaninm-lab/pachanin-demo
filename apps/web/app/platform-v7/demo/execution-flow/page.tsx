  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <section style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 20, padding: 22, background: '#fff', display: 'grid', gap: 14 }}>
        <div style={{ display: 'inline-flex', width: 'fit-content', padding: '6px 10px', borderRadius: 999, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)', fontSize: 12, fontWeight: 900 }}>Сквозной путь · контур исполнения</div>
        <h1 style={{ margin: 0, fontSize: 30, lineHeight: 1.12, color: 'var(--pc-text-primary, #0F1419)' }}>Сквозной путь сделки</h1>
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
        {steps.map((step, index) => (
          <article key={step} style={{ display: 'grid', gridTemplateColumns: '44px 1fr', gap: 12, alignItems: 'center', padding: 12, borderRadius: 14, background: '#F8FAFB', border: '1px solid #EEF1F4' }}>
            ...
            <strong style={{ fontSize: 15, color: 'var(--pc-text-primary, #0F1419)' }}>{step}</strong>
          </article>
        ))}
      </section>
    </div>
