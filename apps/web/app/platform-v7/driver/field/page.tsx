...
export default async function DriverFieldPage() {
  ...
  const primary = (
    <div className={workspace.stack}>
      <NextActionPanel
        eyebrow='Один рейс · одно действие'
        title='Подтвердить прибытие'
        description='Сначала проверь точку назначения. После прибытия зафиксируй фото, пломбу и документ рейса.'
        blocker={blockedShipments.length ? `${blockedShipments.length} рейса с блокерами` : undefined}
        deadline='текущая точка маршрута'
        action={<a className={workspace.primaryLink} href='#driver-next-action'>Открыть действие</a>}
        secondaryAction={<a className={workspace.secondaryLink} href='#driver-photo-seal'>Фото и пломба</a>}
      />
  ...
  const context = (
    <div className={workspace.stack}>
      <StatusBadge tone={apiOnline ? 'success' : 'warning'}>
