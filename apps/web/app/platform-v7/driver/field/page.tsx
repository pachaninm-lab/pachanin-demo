...
export default async function DriverFieldPage() {
  ...
  const primary = (
    <div className={workspace.stack}>
      <NextActionCard
        label='Один рейс · одно действие'
        action='Подтвердить прибытие'
        reason='Сначала проверь точку назначения. После прибытия зафиксируй фото, пломбу и документ рейса.'
        blocked={blockedShipments.length > 0}
        impact={blockedShipments.length ? `${blockedShipments.length} рейса с блокерами` : 'рейс продолжает исполнение'}
        owner='назначенный водитель'
        deadline='текущая точка маршрута'
        actions={(
          <>
            <a className={workspace.primaryLink} href='#driver-next-action'>Открыть действие</a>
            <a className={workspace.secondaryLink} href='#driver-photo-seal'>Фото и пломба</a>
          </>
        )}
      />
      ...
      <section id='driver-next-action' className={workspace.sectionAnchor}>
        <CollapsibleSection title='Текущий рейс' summary='маршрут · следующая точка · ETA' defaultOpen>
      ...
      <section id='driver-photo-seal' className={workspace.sectionAnchor}>
        <CollapsibleSection title='Фото, пломба и документ рейса' summary='камера · доказательства · весовой талон' defaultOpen>
          <DriverCameraCapture tripId={mission.tripId} />
        </CollapsibleSection>
      </section>

      <section id='driver-runtime' className={workspace.sectionAnchor}>
        <CollapsibleSection title='Полевой runtime' summary='события · конфликт · повтор' defaultOpen={false}>
  ...
  const context = (
    <div className={workspace.stack}>
      <StatusChip tone={apiOnline ? 'success' : 'warning'}>
        {apiOnline ? 'Связь с сервером есть' : 'Сервер недоступен'}
      </StatusChip>
      <ol className={workspace.contextList}>
        <li>
          <span>Следующая точка</span>
          <strong>{to}</strong>
          <small>подтверди прибытие только на площадке</small>
        </li>
        <li>
          <span>Фото</span>
          <strong>{mission.photoChecklist.length} пункта проверки</strong>
          <small>фото привязываются к рейсу и этапу</small>
        </li>
        <li>
          <span>Проблема</span>
          <strong>зафиксировать факт, не решать спор</strong>
          <small>оператор получит событие и доказательства</small>
        </li>
      </ol>
    </div>
  );
  ...
  const evidence = (
    <section className={workspace.evidenceStack} aria-label='Полевые доказательства и связь'>
      <Surface>
        <div className={workspace.sectionStack}>
          <strong>Работа при нестабильной связи</strong>
          <p className={workspace.muted}>
            Команды не считаются подтверждёнными, пока сервер не принял их в привязанной сессии. При конфликте пользователь получает явную остановку и повторную проверку.
          </p>
        </div>
      </Surface>
      <CollapsibleSection title='Очередь полевых событий' summary='защищённая синхронизация · повтор · конфликт' defaultOpen={false}>
        <DriverOfflineQueue tripId={mission.tripId} />
      </CollapsibleSection>
      <CollapsibleSection title='PWA и локальный кэш' summary='доступность · восстановление · ограничения' defaultOpen={false}>
        <PwaOfflinePanel />
      </CollapsibleSection>
    </section>
  );
  ...
  return (
    ...
    >
      <FieldTaskTemplate
        testId='platform-v7-driver-field-shell'
        eyebrow='Водитель · полевая роль'
        title='Рейс, следующая точка и доказательства'
        description='На первом уровне только назначенный рейс, одна CTA, связь, фото, пломба, документ и фиксация проблемы.'
        statusLabel={blockedShipments.length ? 'Есть блокер' : 'Рейс в работе'}
        statusTone={blockedShipments.length ? 'warning' : 'success'}
