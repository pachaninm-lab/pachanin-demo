...
import { createP7MockRuntimeStore, type P7MockRuntimeStore } from '@/lib/platform-v7/runtime/mock-persistence-adapter';

export type P7RuntimeActionErrorCode = 'VALIDATION_ERROR' | 'PERMISSION_DENIED' | 'DUPLICATE' | 'CONFLICT' | 'NOT_FOUND' | 'PERSISTENCE_ERROR' | 'DOMAIN_BLOCKED' | 'UNKNOWN_ERROR';
...
export type P7RuntimeMoneyAction = 'request_bank_basis' | 'confirm_bank_basis' | 'confirm_refund' | 'confirm_hold' | 'start_manual_review';
export type P7RuntimeMoneyActionInput =
  | { readonly action: 'request_bank_basis'; readonly dto: P7BankBasisRequestDto }
  | { readonly action: Exclude<P7RuntimeMoneyAction, 'request_bank_basis'>; readonly dto: P7BankConfirmationRequestDto };

export type P7RuntimeDocumentAction = 'upload_document' | 'confirm_document' | 'reject_document' | 'send_document' | 'mark_manual_review';
export interface P7RuntimeDocumentActionInput { readonly action: P7RuntimeDocumentAction; readonly dto: P7DocumentActionRequestDto }

export type P7RuntimeBankBasisAction = 'send_bank_basis' | 'confirm_bank_basis' | 'reject_bank_basis' | 'confirm_bank_refund' | 'confirm_bank_hold' | 'start_bank_manual_review';
...
export type P7RuntimeReleaseWorkflowActionInput =
  ...
  | { readonly action: 'get_bank_basis_status'; readonly dto: P7RuntimeRequestBaseDto };

export type P7RuntimeDisputeSettlementAction = 'open_dispute' | 'attach_evidence' | 'prepare_arbitration_basis' | 'apply_arbitration_outcome_to_bank_basis' | 'get_dispute_money_impact';
...
export interface P7RuntimeServerActionDependencies { readonly store?: P7MockRuntimeStore; readonly createStore?: () => P7MockRuntimeStore; readonly now?: () => string }
...
function toSerializable(value: unknown): P7Serializable { return JSON.parse(JSON.stringify(value)) as P7Serializable; }
function validationResult(errors: readonly P7ValidationError[]): P7RuntimeActionResult { return { ok: false, status: 'validation_error', error: { code: 'VALIDATION_ERROR', message: 'Runtime DTO validation failed.', details: toSerializable({ errors }) }, validationErrors: errors, auditPayloads: [], duplicate: false, meta: {} }; }
function validationFailed<T>(validation: P7ValidationResult<T>): validation is Extract<P7ValidationResult<T>, { readonly ok: false }> { return validation.ok === false; }
function validateOrFail<T>(validation: P7ValidationResult<T>): P7RuntimeActionResult | null { return validationFailed(validation) ? validationResult(validation.errors) : null; }
function isRecord(value: unknown): value is Record<string, unknown> { return typeof value === 'object' && value !== null; }
function nonEmptyString(value: unknown): value is string { return typeof value === 'string' && value.trim().length > 0; }
function addRequiredRuntimeStringError(errors: P7ValidationError[], field: string, value: unknown): void { if (!nonEmptyString(value)) errors.push({ code: 'REQUIRED', field, message: `${field} is required.` }); }
...
function validateP7RuntimeRequestBaseDto(input: P7RuntimeRequestBaseDto): P7ValidationResult<P7RuntimeRequestBaseDto> {
  ...
  if (!isRecord(candidate)) return { ok: false, errors: [{ code: 'INVALID_OBJECT', field: 'dto', message: 'dto must be an object.' }] };
  const errors: P7ValidationError[] = [];
  const actor = candidate.actor;
  const resource = candidate.resource;
  const audit = candidate.audit;
  const idempotency = candidate.idempotency;
  if (!isRecord(actor)) errors.push({ code: 'REQUIRED', field: 'actor', message: 'actor is required.' });
  else { addRequiredRuntimeStringError(errors, 'actor.actorId', actor.actorId); addRequiredRuntimeStringError(errors, 'actor.actorRole', actor.actorRole); addRequiredRuntimeStringError(errors, 'actor.organizationId', actor.organizationId); }
  if (!isRecord(resource)) errors.push({ code: 'REQUIRED', field: 'resource', message: 'resource is required.' });
  else { addRequiredRuntimeStringError(errors, 'resource.resourceType', resource.resourceType); addRequiredRuntimeStringError(errors, 'resource.resourceId', resource.resourceId); addRequiredRuntimeStringError(errors, 'resource.dealId', resource.dealId); }
  if (!isRecord(audit)) errors.push({ code: 'REQUIRED', field: 'audit', message: 'audit is required.' });
  else { addRequiredRuntimeStringError(errors, 'audit.auditId', audit.auditId); addRequiredRuntimeStringError(errors, 'audit.correlationId', audit.correlationId); addRequiredRuntimeStringError(errors, 'audit.reason', audit.reason); }
  if (!isRecord(idempotency)) errors.push({ code: 'REQUIRED', field: 'idempotency', message: 'idempotency is required.' });
  else addRequiredRuntimeStringError(errors, 'idempotency.idempotencyKey', idempotency.idempotencyKey);
  return errors.length === 0 ? { ok: true, value: input } : { ok: false, errors };
}

function duplicateBeforeCall(store: P7MockRuntimeStore, dto: P7RuntimeRequestBaseDto): boolean { return store.snapshot().idempotencyKeys.includes(dto.idempotency.idempotencyKey); }
function errorCode(status: P7ApplicationServiceResult<unknown>['status'], code: string): P7RuntimeActionErrorCode { if (status === 'validation_error') return 'VALIDATION_ERROR'; if (status === 'denied') return 'PERMISSION_DENIED'; if (status === 'duplicate') return 'DUPLICATE'; if (status === 'conflict') return 'CONFLICT'; if (status === 'not_found') return 'NOT_FOUND'; if (code === 'persistence_error') return 'PERSISTENCE_ERROR'; return 'DOMAIN_BLOCKED'; }
function statusFromService(status: P7ApplicationServiceResult<unknown>['status']): Exclude<P7RuntimeActionStatus, 'success'> { if (status === 'persisted') return 'domain_blocked'; if (status === 'validation_error') return 'validation_error'; if (status === 'denied') return 'denied'; if (status === 'duplicate') return 'duplicate'; if (status === 'conflict') return 'conflict'; if (status === 'not_found') return 'not_found'; if (status === 'domain_blocked') return 'domain_blocked'; return 'persistence_error'; }

function fromServiceResult<T>(result: P7ApplicationServiceResult<T>, duplicateReplay: boolean): P7RuntimeActionResult {
  const auditPayloads = result.auditPayloads.map(toSerializable);
  const meta = { boundaryStatus: result.boundaryResult?.status, boundaryCode: result.boundaryResult?.code, boundaryReason: result.boundaryResult?.reason };
  if (result.ok === true) return { ok: true, status: duplicateReplay ? 'duplicate' : 'success', data: toSerializable(result.value), auditPayloads, duplicate: duplicateReplay, meta };
  const failed = result as Extract<P7ApplicationServiceResult<T>, { readonly ok: false }>;
  return { ok: false, status: statusFromService(failed.status), error: { code: errorCode(failed.status, failed.code), message: failed.reason, details: toSerializable({ serviceCode: failed.code }) }, validationErrors: failed.validationErrors, auditPayloads, duplicate: failed.status === 'duplicate', meta };
}

function unknownFailure(error: unknown): P7RuntimeActionResult { return { ok: false, status: 'unknown_error', error: { code: 'UNKNOWN_ERROR', message: error instanceof Error ? error.message : 'Unknown runtime action error.' }, auditPayloads: [], duplicate: false, meta: {} }; }
function resolveStore(deps: P7RuntimeServerActionDependencies): P7MockRuntimeStore { if (deps.store) return deps.store; if (deps.createStore) return deps.createStore(); return createP7MockRuntimeStore(); }

async function executeWithService<T>(deps: P7RuntimeServerActionDependencies, dto: P7RuntimeRequestBaseDto, validation: P7ValidationResult<unknown>, run: (store: P7MockRuntimeStore) => Promise<P7ApplicationServiceResult<T>>): Promise<P7RuntimeActionResult> {
  const invalid = validateOrFail(validation);
  if (invalid) return invalid;
  const store = resolveStore(deps);
  const duplicateReplay = duplicateBeforeCall(store, dto);
  try { return fromServiceResult(await run(store), duplicateReplay); } catch (error) { return unknownFailure(error); }
}
  ...
  return {
    ...
    async bankBasis(input) {
      if (input.action === 'send_bank_basis') {
        return executeWithService(deps, input.dto, validateP7BankBasisSendRequestDto(input.dto), async (store) => createP7BankBasisExecutionService({ unitOfWork: store, now: deps.now }).sendBankBasis(input.dto));
      }
      return executeWithService(deps, input.dto, validateP7BankConfirmationRequestDto(input.dto), async (store) => {
    ...
    async disputeSettlement(input) {
      ...
      if (input.action === 'apply_arbitration_outcome_to_bank_basis') return executeWithService(deps, input.dto, validateP7BankConfirmationRequestDto(input.dto), async (store) => createP7DisputeSettlementService({ unitOfWork: store, now: deps.now }).applyArbitrationOutcomeToBankBasis(input.dto));
