...
export default async function PlatformV7MarketPage({searchParams}:{searchParams?:Promise<Params>}){
  ...
  const c=locale==='ru'
    ?{e:'Рынок',t:'Публичный рынок лотов',p:'Показываем только обезличенные данные, разрешённые к публикации. Личность продавца, внутренние идентификаторы и закрытые условия не раскрываются.',search:'Культура, класс или регион',filters:'Фильтры',path:'Путь лота в Сделке'}
    :locale==='en'
      ?{e:'Market',t:'Public lots',p:'We show only anonymised data approved for public listing. Seller identity, internal identifiers and private terms remain protected.',search:'Crop, grade or region',filters:'Filters',path:'Lot path through the Deal'}
      :{e:'市场',t:'公开批次',p:'只展示获准公开的匿名数据；卖方身份、内部标识和非公开条件仍受保护。',search:'作物、等级或地区',filters:'筛选',path:'批次在交易中的路径'};
  if(lotIndex!==null){
