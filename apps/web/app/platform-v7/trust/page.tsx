 ...
 return <main className='pc-canonical-public'>
  ...
  <section className='pc-cp-hero' style={{minHeight:500}}>
   <div className='pc-cp-container pc-cp-hero-grid'>
    ...
    <aside className='pc-cp-card pc-cp-state-shell'><div className='pc-cp-section-head' style={{marginBottom:14}}><span className='pc-cp-eyebrow'>{c.fact}</span><p>{c.factLead}</p></div>{TRUST_MODEL[locale].map(([title])=><div key={title} className='pc-cp-chip pc-cp-chip--ok' style={{margin:'4px'}}><CheckCircle2 size={13}/>{title}</div>)}</aside>
   </div>
  </section>
