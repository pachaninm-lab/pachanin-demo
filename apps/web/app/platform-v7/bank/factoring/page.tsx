...
const COPY: Record<Locale, Copy> = {
  ...
  en: {
    ...
    authority: { title: 'Accept an industrial factoring circuit before enabling operations', description: 'The platform needs a tenant-scoped PostgreSQL application registry, server commands, assignment-term versions, bank scoring, callbacks, reconciliation, audit/outbox and an atomic link to the canonical Deal.', blocker: 'no confirmed factoring read/write API or bank adapter', impact: 'a limit, approval or advance cannot be treated as a fact', result: 'a factoring process without duplicate applications, with access checks, audit and bank confirmation' },
  ...
  zh: {
    ...
    authority: { title: '启用操作前验收工业级保理闭环', description: '需要 tenant-scoped PostgreSQL 申请登记册、服务器命令、债权转让条件版本、银行评分、回调、对账、audit/outbox，以及与规范交易的原子关联。', blocker: '没有已确认的保理读写 API 和银行适配器', impact: '额度、审批和预付款不能视为事实', result: '防止重复申请、检查权限、记录审计并由银行确认的保理流程' },
