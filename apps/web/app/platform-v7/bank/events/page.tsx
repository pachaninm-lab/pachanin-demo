...
const bankEvents = [
  {
    ...
    status: 'успешно',
    amount: '3 873 600 ₽',
    effect: 'деньги видны в резерве; банковское подтверждение невозможно до документов и закрытия рейса',
    next: 'проверить документы и рейс',
  },
  {
    code: 'BE-RELEASE-02',
    title: 'Запрос подтверждения остановлен',
    deal: 'DL-9103',
    status: 'остановлено',
    amount: '1 240 000 ₽',
  ...
  },
  {
    ...
    amount: '780 000 ₽',
    effect: 'подтверждение запрещено до сверки события банка и статуса сделки',
  ...
  },
  {
    ...
    status: 'ожидает',
    amount: '410 000 ₽',
    effect: 'повтор не создаёт двойное событие; используется ключ операции',
...
const eventTypes = [
  ...
  ['Удержание', 'Временная остановка подтверждения из-за документа, спора или расхождения.'],
  ['Подтверждение', 'Банк проверяет основание только после выполнения условий сделки.'],
      ...
      <section style={{ border: '1px solid #E4E6EA', borderRadius: 20, padding: 22, background: '#fff', display: 'grid', gap: 14 }}>
        ...
        <div style={{ maxWidth: 920 }}>
          ...
          <p style={{ margin: '10px 0 0', fontSize: 14, lineHeight: 1.7, color: '#5B6576' }}>
            Экран показывает, как денежные события влияют на сделку: резерв, удержание, подтверждение, расхождение и ручная проверка. Это контур проверки оснований, а не подтверждение внешнего банковского подключения.
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <Link href={PLATFORM_V7_RELEASE_SAFETY_ROUTE} style={primaryLink}>Проверка основания</Link>
