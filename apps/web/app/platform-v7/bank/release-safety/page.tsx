...
type SelectedCopy = {
  ...
  values: { confirmed: string; missing: string; ready: string; blocked: string; none: string; recorded: string; waiting: string; released: string; notExposed: string };
...
const SELECTED_COPY: Record<Locale, SelectedCopy> = {
  ru: {
    unavailableTitle: 'Серверная проверка выплаты недоступна', unavailableDescription: 'Экран закрыт: Сделка не указана, недоступна участнику или канонический Deal workspace вернул неполное либо противоречивое денежное основание.', openDeals: 'Открыть реестр Сделок', openDeal: 'Открыть рабочее место Сделки', openDocuments: 'Проверить документы', boundary: 'Источник истины — tenant-scoped PostgreSQL Deal workspace. Этот экран только читает сумму, резерв, документы, спор, банковскую операцию и outbox. Он не вызывает legacy SettlementEngine, не создаёт release request и не меняет денежный статус.', sourceNoticeTitle: 'Разделение полномочий сохранено', sourceNoticeBody: 'Уполномоченная роль инициирует каноническую команду в Сделке с актуальной MFA. Выплату подтверждает только проверенный callback банка. Reconciliation не подменяется визуальным статусом.', facts: { amount: 'Сумма Сделки', reserve: 'Резерв', documents: 'Документное основание', disputes: 'Открытые споры / удержание', request: 'Release request', callback: 'Callback банка' }, values: { confirmed: 'подтверждён', missing: 'не зафиксирован', ready: 'готово', blocked: 'заблокировано', none: 'нет', recorded: 'зафиксирован', waiting: 'внешний исход неизвестен', released: 'выплата подтверждена', notExposed: 'результат reconciliation не опубликован' }, states: {
      ...
      awaiting_bank: { status: 'внешний исход не подтверждён', title: 'Release request зафиксирован; внешний исход не подтверждён', description: 'PENDING request/outbox подтверждает только внутреннюю фиксацию запроса. Он не доказывает отправку, получение банком, списание или отсутствие списания и не подтверждает финальность. Не создавай новый запрос только из-за задержки: сначала нужна status/statement reconciliation по той же operation identity.', owner: 'банк / integration operations', result: 'зафиксированный request → status/statement reconciliation → проверенное внешнее evidence → ledger' },
  ...
  en: {
    unavailableTitle: 'Server payout review is unavailable', unavailableDescription: 'The screen is closed because the Deal is missing, inaccessible to the participant, or its canonical workspace returned an incomplete or contradictory money basis.', openDeals: 'Open Deal registry', openDeal: 'Open Deal workspace', openDocuments: 'Review documents', boundary: 'The authority is the tenant-scoped PostgreSQL Deal workspace. This screen only reads amount, reserve, documents, disputes, bank operations and outbox. It never calls the legacy SettlementEngine, creates a release request or changes money state.', sourceNoticeTitle: 'Separation of authority is preserved', sourceNoticeBody: 'An authorized role initiates the canonical Deal command with current MFA. Only a verified bank callback confirms payout. Reconciliation is never replaced by a visual status.', facts: { amount: 'Deal amount', reserve: 'Reserve', documents: 'Document basis', disputes: 'Open disputes / hold', request: 'Release request', callback: 'Bank callback' }, values: { confirmed: 'confirmed', missing: 'not recorded', ready: 'ready', blocked: 'blocked', none: 'none', recorded: 'recorded', waiting: 'external outcome unknown', released: 'payout confirmed', notExposed: 'reconciliation result is not exposed' }, states: {
      ...
      awaiting_bank: { status: 'external outcome unconfirmed', title: 'The release request is recorded; the external outcome is unknown', description: 'A PENDING request/outbox proves only that the request was recorded internally. It does not prove dispatch, provider receipt, debit or no debit, or finality. Do not create a new request merely because of delay; first reconcile status/statements for the same operation identity.', owner: 'bank / integration operations', result: 'recorded request → status/statement reconciliation → verified external evidence → ledger' },
  ...
  zh: {
    unavailableTitle: '服务器付款检查不可用', unavailableDescription: '页面已关闭：未指定交易、参与方无权访问，或规范交易工作区返回不完整或矛盾的资金依据。', openDeals: '打开交易列表', openDeal: '打开交易工作区', openDocuments: '检查文件', boundary: '权威来源是租户范围内的 PostgreSQL 交易工作区。本页面只读取金额、预留、文件、争议、银行操作和 outbox；不会调用旧 SettlementEngine、创建付款申请或改变资金状态。', sourceNoticeTitle: '权限分离保持不变', sourceNoticeBody: '授权角色在交易工作区中使用当前 MFA 发起规范命令。只有经过验证的银行回调才能确认付款。界面状态不能替代对账。', facts: { amount: '交易金额', reserve: '资金预留', documents: '文件依据', disputes: '未结争议 / 冻结', request: '付款申请', callback: '银行回调' }, values: { confirmed: '已确认', missing: '未记录', ready: '已就绪', blocked: '已阻塞', none: '无', recorded: '已记录', waiting: '外部结果未知', released: '付款已确认', notExposed: '未发布对账结果' }, states: {
      ...
      awaiting_bank: { status: '外部结果尚未确认', title: '付款申请已记录；外部结果未知', description: 'PENDING 申请/outbox 只能证明申请已在内部记录，不能证明已发送、银行已接收、已扣款或未扣款，也不能证明最终结果。不要仅因延迟创建新申请；应先对同一 operation identity 执行状态/账单对账。', owner: '银行 / 集成运营', result: '已记录申请 → 状态/账单对账 → 已验证外部证据 → 账本' },
  ...
  return (
    <MoneyObligationCockpit testId='platform-v7-bank-release-safety-v8' eyebrow={common.eyebrow} title={`${common.metadataTitle} · ${projection.dealId}`} description={stateCopy.description} statusLabel={stateCopy.status} statusTone={statusTone} labels={common.labels}
      ...
      facts={[
        ...
        { label: copy.facts.request, value: projection.releaseRequested ? copy.values.recorded : copy.values.missing, hint: projection.releaseOutbox?.id ?? copy.values.missing },
    ...
    >
      ...
      <MoneyCockpitSection id='release-authority'>
        ...
        <MoneyQueue>
          ...
          <MoneyQueueLink href={dealHref} title={copy.queue.request} detail={projection.releaseOutbox?.id ?? copy.values.missing} status={<StatusChip tone={projection.releaseRequested ? 'information' : 'neutral'}>{projection.releaseRequested ? copy.values.recorded : copy.values.missing}</StatusChip>} />
          ...
          <MoneyQueueLink href={dealHref} title={copy.queue.reconciliation} detail={copy.values.notExposed} status={<StatusChip tone='warning'>{copy.values.notExposed}</StatusChip>} />
        </MoneyQueue>
