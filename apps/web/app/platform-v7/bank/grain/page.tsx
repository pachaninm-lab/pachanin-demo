...
const gates = [
  { title: 'Резерв', value: 'ожидает подтверждения', note: 'Деньги не считаются доступными к банковскому шагу, пока резерв не подтверждён банком.', href: '/platform-v7/bank/release-safety', tone: 'warn' },
  { title: 'СДИЗ', value: 'не закрыт', note: 'Документальное основание партии должно быть подтверждено до банковского шага.', href: '/platform-v7/deals/grain-sdiz', tone: 'stop' },
  ...
  return (
    <main style={{ display: 'grid', gap: 14, padding: '4px 0 24px' }}>
      <section style={card}>
        ...
        <h1 style={h1}>Резерв, удержание, банковское основание и журнал</h1>
        <p style={lead}>Банк видит денежный статус сделки и основания остановки. Банковское подтверждение возможно только после закрытия документов, приёмки, качества и спора.</p>
        <div style={actions}>
          <Link href='/platform-v7/bank' style={primaryBtn}>Кабинет банка</Link>
          <Link href='/platform-v7/bank/release-safety' style={ghostBtn}>Проверка основания</Link>
      ...
      <section style={card}>
        <div style={micro}>Условия перед банковским шагом</div>
      ...
      <section style={darkCard}>
        ...
        <h2 style={{ margin: 0, color: '#fff', fontSize: 26, lineHeight: 1.08, letterSpacing: '-0.035em', fontWeight: 950 }}>Банковское подтверждение не является ручной кнопкой</h2>
        <p style={{ margin: 0, color: '#CBD5E1', fontSize: 14, lineHeight: 1.55 }}>Любое денежное действие должно иметь закрытые условия, ответственного, документальное основание и след в журнале. Если хотя бы одно условие открыто, банковский шаг остаётся закрытым.</p>
      </section>
    </main>
  );
}
...
function color(tone: string) { return tone === 'bad' || tone === 'stop' ? '#B91C1C' : tone === 'warn' ? '#B45309' : tone === 'good' ? '#0A7A5F' : 'var(--pc-text-primary, #0F1419)'; }
...
const metricCard = { background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 16 } as const;
const stepCard = { textDecoration: 'none', minHeight: 178, display: 'grid', gap: 8, padding: 14, borderRadius: 18, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-primary, #0F1419)' } as const;
...
const title = { color: 'var(--pc-text-primary, #0F1419)', fontSize: 17, lineHeight: 1.2, fontWeight: 900 } as const;
const value = { fontSize: 13, lineHeight: 1.3, fontWeight: 900 } as const;
const note = { color: 'var(--pc-text-muted, #64748B)', fontSize: 13, lineHeight: 1.5 } as const;
