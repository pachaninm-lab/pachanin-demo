...
const initialRows: EscrowRow[] = [
  {
    ...
    state: 'Удержание',
    owner: 'Оператор / банк',
    note: 'Деньги остаются на удержании до закрытия качества и полного пакета документов. Боевой эскроу-счёт не открыт.'
  },
  {
    id: 'ESC-302',
    deal: 'DL-9108',
    amount: 12.8,
    released: 0,
    trigger: 'Приёмка + банковское подтверждение',
    state: 'Готово к раскрытию',
    owner: 'Банк',
    note: 'Все обязательные события подтверждены в пилотной логике; можно отметить готовность без боевого платежа.'
  },
  {
    id: 'ESC-303',
    deal: 'DL-9111',
    ...
    note: 'Денежный режим зависит от решения по спорной качественной дельте; движение денег заблокировано до доказательств.'
  }
];
...
export default function BankEscrowPage() {
  ...
  const releasePartial = (id: string) => {
    setRows((current) => current.map((row) => {
      ...
      if (row.state === 'Готово к раскрытию') {
        ...
        setMessage(`Эскроу ${row.id}: отмечено частичное раскрытие ${formatMillions(partial)} по сделке ${row.deal}; боевой платёж не выполнялся.`);
        return {
          ...row,
          released: partial,
          state: 'Частично раскрыто',
          owner: 'Банк / оператор',
          note: 'Первая часть отмечена в тестовом контуре. Остаток ждёт финального подтверждения закрытия кейса.'
        };
      }
      if (row.state === 'Частично раскрыто') {
        setMessage(`Эскроу ${row.id}: полностью отмечено к раскрытию в тестовом контуре и закрыто по сделке ${row.deal}.`);
        return {
          ...row,
          released: row.amount,
          state: 'Раскрыто',
          owner: 'Банк',
          note: 'Все условия выполнены в пилотной логике; денежный кейс закрыт без боевого платежа.'
        };
      }
  ...
  const moveToReady = (id: string) => {
    setRows((current) => current.map((row) => {
      ...
      if (row.state === 'Удержание' || row.state === 'Ожидание решения') {
        setMessage(`Эскроу ${row.id}: причина удержания закрыта, кейс готов к тестовому раскрытию.`);
        return {
          ...row,
          state: 'Готово к раскрытию',
          owner: 'Банк',
          trigger: 'Финальная сверка + банковское подтверждение',
          note: 'Критическая причина удержания закрыта, можно переходить к имитации денежного выпуска.'
        };
      }
      setMessage(`Эскроу ${row.id}: дополнительного перевода в готовность не требуется.`);
      return row;
    }));
  };
  ...
  const returnToHold = (id: string) => {
    setRows((current) => current.map((row) => {
      if (row.id !== id) return row;
      if (row.state === 'Частично раскрыто') {
        setMessage(`Эскроу ${row.id}: остаток возвращён на удержание до закрытия документа или спора.`);
        return {
          ...row,
          state: 'Удержание',
          owner: 'Оператор / банк',
          note: 'Часть денег уже отмечена в тестовом контуре, остаток снова удержан до дополнительной проверки.'
        };
      }
      setMessage(`Эскроу ${row.id}: возврат на удержание возможен после частичного раскрытия.`);
      return row;
    }));
  };
    ...
    <div style={{ display: 'grid', gap: 18 }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 28, lineHeight: 1.1, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Эскроу</div>
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Активные эскроу-кейсы</div>
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          {filteredRows.map((item) => (
            <div key={item.id} style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 14, padding: 14, display: 'grid', gap: 10 }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--pc-text-primary, #0F1419)' }}>{item.deal}</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
                <div style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 12, padding: 12 }}>
                  <div style={{ fontSize: 11, color: 'var(--pc-text-muted, #6B778C)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 800 }}>Сумма под управлением</div>
                  <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', marginTop: 8 }}>{formatMillions(item.amount)}</div>
                </div>
                <div style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 12, padding: 12 }}>
                  <div style={{ fontSize: 11, color: 'var(--pc-text-muted, #6B778C)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 800 }}>Отмечено к раскрытию</div>
                  <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', marginTop: 8 }}>{formatMillions(item.released)}</div>
                </div>
              </div>
              <div style={{ fontSize: 13, color: 'var(--pc-text-secondary, #475569)', lineHeight: 1.6 }}>
                Условие раскрытия: <span style={{ color: 'var(--pc-text-primary, #0F1419)', fontWeight: 700 }}>{item.trigger}</span><br />
                Владелец шага: <span style={{ color: 'var(--pc-text-primary, #0F1419)', fontWeight: 700 }}>{item.owner}</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                ...
                <button onClick={() => releasePartial(item.id)} style={{ appearance: 'none', border: '1px solid rgba(10,122,95,0.18)', background: 'rgba(10,122,95,0.08)', color: '#0A7A5F', minHeight: 44, padding: '10px 14px', borderRadius: 12, fontWeight: 800, cursor: 'pointer' }}>
                  {item.state === 'Готово к раскрытию' ? 'Отметить частичное раскрытие' : item.state === 'Частично раскрыто' ? 'Отметить финальное раскрытие' : 'Проверить раскрытие'}
                </button>
