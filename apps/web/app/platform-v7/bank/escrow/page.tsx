...
const initialRows: EscrowRow[] = [
  ...
  },
  {
    ...
    owner: 'Банк',
    note: 'Все обязательные события подтверждены в контуре проверки; можно отметить готовность без платежа платформой.'
  },
  {
    id: 'ESC-303',
    deal: 'DL-9111',
...
export default function BankEscrowPage() {
  ...
  const metrics = useMemo(() => {
    ...
    return [
      { title: 'Кейсы удержания', value: String(rows.length), note: 'Денежные кейсы по сделкам в контуре проверки' },
      { title: 'На удержании', value: formatMillions(held || 0), note: 'Сумма, которая ещё не может быть раскрыта по текущим условиям' },
      { title: 'Условия раскрытия', value: String(triggers), note: 'Приёмка, качество, документы, банковское подтверждение и спорный режим' },
      { title: 'Отмечено к раскрытию', value: formatMillions(released || 0), note: 'Отмечено к банковскому подтверждению; платёж платформой не выполнялся' }
    ];
  }, [rows]);
    ...
    setRows((current) => current.map((row) => {
      ...
      if (row.state === 'Готово к раскрытию') {
        ...
        return {
          ...
          owner: 'Банк / оператор',
          note: 'Первая часть отмечена в контуре проверки. Остаток ждёт финального подтверждения закрытия кейса.'
        };
      }
      if (row.state === 'Частично раскрыто') {
        setMessage(`Эскроу ${row.id}: полностью отмечено к раскрытию и закрыто по сделке ${row.deal}.`);
        return {
          ...row,
          released: row.amount,
          state: 'Раскрыто',
          owner: 'Банк',
          note: 'Все условия выполнены в контуре проверки; денежный кейс закрыт без платежа платформой.'
        };
      }
  ...
  const moveToReady = (id: string) => {
    setRows((current) => current.map((row) => {
      ...
      if (row.state === 'Удержание' || row.state === 'Ожидание решения') {
        setMessage(`Эскроу ${row.id}: причина удержания закрыта, кейс готов к раскрытию.`);
        return {
          ...row,
          state: 'Готово к раскрытию',
          owner: 'Банк',
          trigger: 'Финальная сверка + банковское подтверждение',
          note: 'Критическая причина удержания закрыта, можно переходить к проверке денежного основания.'
        };
      }
    ...
    setRows((current) => current.map((row) => {
      ...
      if (row.state === 'Частично раскрыто') {
        ...
        return {
          ...
          state: 'Удержание',
          owner: 'Оператор / банк',
          note: 'Часть денег уже отмечена в контуре проверки, остаток снова удержан до дополнительной проверки.'
        };
      }
    ...
    <div style={{ display: 'grid', gap: 18 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: '#6B778C', lineHeight: 1.7, marginTop: 8, maxWidth: 860 }}>
              Контур удержания и раскрытия денег по подтверждённым событиям: приёмка, лаборатория, документы, спор и банковское подтверждение. Это не эскроу-счёт платформы и не банковское списание.
            </div>
          </div>
          <div style={{ display: 'inline-flex', alignItems: 'center', padding: '8px 12px', borderRadius: 999, background: 'rgba(10,122,95,0.08)', border: '1px solid rgba(10,122,95,0.18)', color: '#0A7A5F', fontSize: 12, fontWeight: 800 }}>
            Контур проверки · ручное подтверждение
          </div>
        </div>
      </section>
      ...
      <section style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: '#0F1419' }}>Активные эскроу-кейсы</div>
          ...
          {filteredRows.map((item) => (
            <div key={item.id} style={{ border: '1px solid #E4E6EA', borderRadius: 14, padding: 14, display: 'grid', gap: 10 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
                ...
                <div style={{ border: '1px solid #E4E6EA', borderRadius: 12, padding: 12 }}>
                  <div style={{ fontSize: 11, color: '#6B778C', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 800 }}>Отмечено к раскрытию</div>
