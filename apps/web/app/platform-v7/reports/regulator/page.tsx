...
const METRICS = [
  ...
  { label: 'Средний цикл', value: '4.8 дня', note: 'От фиксации сделки до банковской проверки выплаты.' },
];
...
const BLOCKS = [
  ...
  },
  {
    ...
    body: 'Важен не просто объём, а качество исполнения: есть ли след логистики, подтверждены ли документы, где возникают споры, какова доля ручных вмешательств и где контур уже можно считать достаточно дисциплинированным для управляемый запуск.',
  },
  {
    title: 'Честные ограничения',
    body: 'Часть данных в текущем отчёте формируется по пилотному и предынтеграционный контуру. Это полезно для логики и проектной настройки, но не должно выдаваться за окончательную государственную отчётность без боевых подтверждений и интеграционного закрытия.',
  },
];
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 1080, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
        <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Отчёт для регулятора</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
        {METRICS.map((metric) => (
          <section key={metric.label} style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
            <div style={{ fontSize: 11, color: 'var(--pc-text-muted, #6B778C)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 800 }}>{metric.label}</div>
            <div style={{ marginTop: 8, fontSize: 28, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{metric.value}</div>
            <div style={{ marginTop: 8, fontSize: 12, color: 'var(--pc-text-muted, #6B778C)', lineHeight: 1.6 }}>{metric.note}</div>
          </section>
        ))}
      </div>
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Регионы и степень контроля</div>
        <div style={{ display: 'grid', gap: 8 }}>
          {REGIONS.map((region) => (
            <div key={region.name} style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 80px 160px 120px', gap: 12, border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 12, padding: 12, background: '#F8FAFB' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--pc-text-primary, #0F1419)' }}>{region.name}</div>
              <div style={{ fontSize: 13, fontWeight: 800, color: '#0A7A5F' }}>{region.deals}</div>
              <div style={{ fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>{region.status}</div>
              <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #475569)' }}>{region.control}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <Link href='/platform-v7/roadmap' style={{ textDecoration: 'none', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--pc-border, #E4E6EA)', background: '#fff', color: 'var(--pc-text-primary, #0F1419)', fontSize: 13, fontWeight: 700 }}>
          Roadmap
        </Link>
      </div>
    </div>
