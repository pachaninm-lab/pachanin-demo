...
const METRICS = [
  ...
  { label: 'Средний цикл', value: '4.8 дня', note: 'От фиксации сделки до банковского подтверждения основания.' },
];
...
const BLOCKS = [
  ...
  },
  {
    ...
    body: 'Важен не просто объём, а качество исполнения: есть ли след логистики, подтверждены ли документы, где возникают споры, какова доля ручных вмешательств и где контур уже можно считать достаточно дисциплинированным для проверочного запуска.',
  },
  {
    title: 'Честные ограничения',
    body: 'Часть данных в текущем отчёте формируется по проверочному контуру. Это полезно для логики и проектной настройки, но не должно выдаваться за окончательную государственную отчётность без внешних подтверждений и интеграционного закрытия.',
  },
];

const REGIONS = [
  { name: 'Тамбовская область', deals: '12', status: 'ядро проверки', control: 'высокий' },
