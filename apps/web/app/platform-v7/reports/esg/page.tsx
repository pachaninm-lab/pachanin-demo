...
const METRICS = [
  ...
  { label: 'Экспорт / внутренний рынок', value: '18 / 82', note: 'Соотношение по текущему локальному портфелю.' },
];
...
const BLOCKS = [
  ...
  },
  {
    ...
    body: 'Часть показателей в текущем контуре агрегируется на основе пилотных и локальных данных. Это полезно для логики и упаковки, но не должно выдаваться за финальную официальную отчётность без боевых подтверждений.',
  },
];
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 1080, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18 }}>
        <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>ESG-отчёт</div>
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>Регионы в контуре</div>
        <div style={{ display: 'grid', gap: 8 }}>
          {REGIONS.map((region) => (
            <div key={region.name} style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 90px 160px', gap: 12, border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 12, padding: 12, background: '#F8FAFB' }}>
