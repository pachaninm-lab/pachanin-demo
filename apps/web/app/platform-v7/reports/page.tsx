  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <section style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 20, padding: 22, background: '#fff', display: 'grid', gap: 14 }}>
        <div style={{ display: 'inline-flex', width: 'fit-content', padding: '6px 10px', borderRadius: 999, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)', fontSize: 12, fontWeight: 900 }}>Отчёты · проверочный сценарий</div>
        <h1 style={{ margin: 0, fontSize: 30, lineHeight: 1.12, color: 'var(--pc-text-primary, #0F1419)' }}>Центр отчётов</h1>
        <p style={{ margin: 0, maxWidth: 840, fontSize: 14, lineHeight: 1.7, color: '#5B6576' }}>Артефакты для банка, сценария, спора и операционного разбора. Материалы не подменяют внешние юридически значимые документы.</p>
      ...
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
        {rows.map((name) => (
          <article key={name} style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: 12, alignItems: 'center', padding: 12, borderRadius: 14, background: '#F8FAFB', border: '1px solid #EEF1F4' }}>
            <strong style={{ fontSize: 14, color: 'var(--pc-text-primary, #0F1419)' }}>{name}</strong>
