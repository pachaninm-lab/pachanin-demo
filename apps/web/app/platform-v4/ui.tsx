...
function path(icon:IconName){switch(icon){case'market':return'M4 6h16M4 12h16M4 18h10';case'buyer':return'M12 12a4 4 0 1 0-.01 0M5 20c1.6-3 4.1-4.5 7-4.5S17.4 17 19 20';case'bank':return'M3 9l9-4 9 4M5 10v8M9 10v8M15 10v8M4 19h16';case'deal':return'M4 8h6l2 3h8M4 16h6l2-3h8';case'docs':return'M7 3h7l4 4v14H7z M14 3v5h5';case'logistics':return'M3 8h11v7H3z M14 11h3l2 2v2h-5z M7 18a1.4 1.4 0 1 0 0 .01 M16 18a1.4 1.4 0 1 0 0 .01';case'lab':return'M9 3v5l-4 8a3 3 0 0 0 2.7 4h8.6A3 3 0 0 0 19 16l-4-8V3';case'observer':return'M2 12s4-6 10-6 10 6 10 6-4 6-10 6S2 18 2 12 M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6';case'shield':return'M12 3l7 3v5c0 5-3.5 8.5-7 10-3.5-1.5-7-5-7-10V6z';case'flow':return'M4 7h5v4H4z M15 13h5v4h-5z M9 9h6v2H9z M11 11v4';case'ai':return'M12 3l1.7 4.4L18 9l-4.3 1.6L12 15l-1.7-4.4L6 9l4.3-1.6z';case'onboarding':return'M12 4v16M4 12h16';case'regulatory':return'M4 5h16v14H4z M8 9h8 M8 13h5';case'pilot':return'M12 3l2.5 5L20 9l-4 4 .9 6L12 16l-4.9 3 .9-6-4-4 5.5-1z';case'integration':return'M7 7h4v4H7z M13 13h4v4h-4z M11 9h2v2h-2z M9 11h2v2H9z';default:return'M4 12h16';}}
...
export function DataTable({columns,rows}:{columns:string[];rows:Array<Array<ReactNode>>}){return <div className={styles.tableWrap}><div className={styles.tableDesktop}><table className={styles.denseTable}><thead><tr>{columns.map((column)=><th key={column}>{column}</th>)}</tr></thead><tbody>{rows.map((row,index)=><tr key={index}>{row.map((cell,cellIndex)=><td key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table></div><div className={styles.tableCards}>{rows.map((row,index)=><article key={index} className={styles.tableCard}>{row.map((cell,cellIndex)=><div key={cellIndex} className={styles.tableCardRow}><div className={styles.tableCardLabel}>{columns[cellIndex]}</div><div className={styles.tableCardValue}>{cell}</div></div>)}</article>)}</div></div>}
