...
export const metadata: Metadata = {
  ...
  title: { default: SITE_TITLE, template: '%s · Процент-Агро' },
  description: SITE_DESCRIPTION,
  applicationName: 'Процент-Агро',
  keywords: ['Процент-Агро', 'процент агро', 'процент-агро.рф', 'Прозрачная Цена', 'зерновая сделка', 'цифровой контур зерновой сделки', 'агротрейд', 'логистика зерна'],
  openGraph: { type: 'website', locale: 'ru_RU', siteName: 'Процент-Агро', title: SITE_TITLE, description: SITE_DESCRIPTION, url: SITE_URL },
  twitter: { card: 'summary_large_image', title: SITE_TITLE, description: SITE_DESCRIPTION },
  alternates: { canonical: SITE_URL },
};

export const viewport: Viewport = { width: 'device-width', initialScale: 1, maximumScale: 5, themeColor: '#0E6E60' };
...
export default async function RootLayout({ children }: { children: ReactNode }) {
  ...
  const messages: AbstractIntlMessages = PUBLIC_PLATFORM_PATHS.has(pathname)
      ...
      <body translate='no' className='notranslate'>
        ...
        {YM_ID ? (
          <>
            ...
            `}</Script>
            <noscript><div><img src={`https://mc.yandex.ru/watch/${YM_ID}`} style={{ position: 'absolute', left: -9999 }} alt='' /></div></noscript>
          </>
        ) : null}
      </body>
    </html>
