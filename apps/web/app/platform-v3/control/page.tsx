...
const items = [
  ['Подтверждение затягивается', 'Срок 00:14'],
  ['Повторный анализ не закрыт', 'Срок 00:42'],
  ['Окно приёмки под риском', 'Срок 00:26'],
  ['Финальный акт не загружен', 'Срок 01:10'],
] as const;
    ...
    <div style={{ padding: '22px 16px 48px' }}>
      <div style={{ maxWidth: 980, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <div style={{ color: '#22c55e', fontWeight: 800, fontSize: 14, textTransform: 'uppercase', letterSpacing: '.08em' }}>Контроль и спор</div>
            <h1 style={{ margin: '10px 0 0', fontSize: 'clamp(34px,6vw,54px)', lineHeight: 1.02, fontWeight: 900, letterSpacing: '-0.03em' }}>Очередь блокеров и следующий управленческий шаг</h1>
            <div style={{ marginTop: 14, color: '#94a3b8', fontSize: 18, lineHeight: 1.6 }}>Проблема не теряется: у неё есть срок, приоритет и понятный следующий шаг по сделке.</div>
          </div>
        ...
        <section style={{ marginTop: 18, background: 'linear-gradient(180deg, rgba(13,18,31,.98) 0%, rgba(10,15,27,.98) 100%)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 24, padding: 20, boxShadow: '0 12px 40px rgba(0,0,0,.22)' }}>
          <div style={{ fontSize: 30, fontWeight: 900 }}>Очередь контроля</div>
          <div style={{ display: 'grid', gap: 10, marginTop: 12 }}>
            {items.map(([a, b]) => (
              <div key={a} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 10, padding: '16px 0', borderTop: '1px solid rgba(255,255,255,.06)' }}>
                ...
                <div style={{ color: '#fbbf24', fontSize: 17, fontWeight: 800 }}>{b}</div>
              </div>
            ))}
          </div>
        </section>
