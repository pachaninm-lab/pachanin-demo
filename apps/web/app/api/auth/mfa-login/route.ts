...
function json(body: Record<string, unknown>, status = 200) {
  return NextResponse.json(body, { status, headers: { 'Cache-Control': 'no-store, no-cache, must-revalidate', Pragma: 'no-cache', 'X-Content-Type-Options': 'nosniff' } });
}
function requestIp(request: Request) {
  return request.headers.get('x-nf-client-connection-ip') || request.headers.get('cf-connecting-ip') || request.headers.get('x-real-ip') || request.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || '';
}
...
export async function POST(request: Request) {
  ...
  const ticket = openMfaLoginTicket(cookies().get(MFA_PENDING_COOKIE)?.value || '');
  ...
  try {
    ...
    const payload = await apiResponse.json().catch(() => ({} as AuthenticatedSessionPayload)) as AuthenticatedSessionPayload & { backupCodes?: string[] };
    if (!apiResponse.ok || !payload.accessToken || !payload.refreshToken || !payload.user?.email || !payload.user?.role) {
      ...
      const response = json({ ok: false, code: apiResponse.status === 429 ? 'RATE_LIMITED' : 'MFA_INVALID', message: UNIVERSAL_ERROR, correlationId }, apiResponse.status === 429 ? 429 : 401);
    ...
    const response = json({ ok: true, redirectTo: platformHome(role), backupCodes: Array.isArray(payload.backupCodes) ? payload.backupCodes : undefined, correlationId });
    const session = await applyAuthenticatedSession(response, payload);
    if (!session) return json({ ok: false, code: 'SESSION_CONFIGURATION_ERROR', message: UNIVERSAL_ERROR, correlationId }, 503);
    response.cookies.set(MFA_PENDING_COOKIE, '', clearMfaPendingCookieOptions());
    return response;
  } catch (error) {
    console.error('auth_mfa_transport_failure', JSON.stringify({ correlationId, reason: error instanceof Error ? error.name : 'unknown' }));
