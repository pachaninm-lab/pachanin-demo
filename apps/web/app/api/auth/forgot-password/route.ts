...
function response(body: Record<string, unknown>, status = 202) {
...
export async function POST(request: Request) {
  ...
  if (!/^\S+@\S+\.\S+$/.test(email) || email.length > 254) {
    return response({ accepted: false, code: 'INVALID_EMAIL', correlationId }, 400);
  }

  const deliveryKey = String(process.env.PASSWORD_RESET_DELIVERY_KEY || '').trim();
  if (!API_URL || deliveryKey.length < 32) {
    ...
    return response({ accepted: true, message: UNIVERSAL_MESSAGE, cooldownSeconds: 60, correlationId });
  }

  try {
    const ip = requestIp(request);
    const apiResponse = await fetch(`${API_URL}/auth/password-reset/request`, {
      method: 'POST',
      headers: {
