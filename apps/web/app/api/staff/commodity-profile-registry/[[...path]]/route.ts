...
const READ_PATHS = [
  ...
  /^[A-Za-z0-9][A-Za-z0-9:_.-]{2,239}\/versions$/,
] as const;
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  if (!origin) return secure({ code: 'COMMODITY_PROFILE_SERVICE_UNAVAILABLE', correlationId }, 503, correlationId);
  ...
  const target = `${origin}/platform-v7/commodity-profiles${path ? `/${path}` : ''}${query ? `?${query}` : ''}`;
  try {
