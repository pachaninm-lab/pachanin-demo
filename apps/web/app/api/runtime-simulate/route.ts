...
export async function POST(request: Request) {
  ...
  if (trusted.ok === false) {
    return NextResponse.json({ ok: false, error: trusted.reason, message: 'CSRF rejected' }, { status: 403 });
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: false, message: 'Runtime API unavailable, local simulation only' }, { status: 200 });
  }
}
