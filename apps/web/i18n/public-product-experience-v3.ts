...
const ru = {
  ...
  home: {
    ...
    preview: {
      ...
      afterValue: 'Документы и основания расчёта',
      lenses: {
  ...
  explorer: {
    ...
    stages: {
      ...
      logistics: { label: 'Логистика и поставка', happened: 'Назначены перевозчик, водитель, транспорт, маршрут и рейс.', owner: 'Логистика', action: 'Подтвердить события рейса и связанные основания.', evidence: 'Рейс, маршрут и транспортные события.', next: 'Прибытие открывает приёмку.' },
      acceptance: { label: 'Приёмка', happened: 'Партия прибыла на площадку; фиксируются вес и факты приёмки.', owner: 'Элеватор / хранение', action: 'Подтвердить фактическую приёмку.', evidence: 'Акт приёмки, вес, время и связанный рейс.', next: 'Открывается проверка качества.' },
      ...
      documents: { label: 'Документы', happened: 'Собираются версии и подтверждения по поставке, приёмке и качеству.', owner: 'Стороны и сотрудник платформы', action: 'Закрыть недостающие основания.', evidence: 'Подтверждённые версии документов и их связь с событиями.', next: 'Проверяются основания для расчёта.' },
    ...
    perspectives: {
      ...
      driver: { label: 'Водитель', value: 'Получает рейс, контрольные точки и ближайшее действие.', action: 'Подтвердить прибытие и передачу груза.', documents: 'Рейс, маршрут и доступные перевозочные документы.', responsibility: 'Передавать подтверждённые события рейса.', outcome: 'Доставка подтверждена без доступа к чужим данным.', risk: 'Контрольная точка без подтверждающего основания.', money: 'Факт выполнения рейса подтверждается отдельно от коммерческих условий товара.' },
      elevator: { label: 'Элеватор / хранение', value: 'Фиксирует прибытие, вес, приёмку, размещение и факты по партии.', action: 'Подтвердить фактическую приёмку.', documents: 'Рейс, весовые данные, акт приёмки и факты по партии.', responsibility: 'Создать достоверное основание приёмки.', outcome: 'Количество подтверждено и передано в контроль качества.', risk: 'Расхождение веса или неверная партия.', money: 'Фактический принятый объём влияет на расчёт.' },
      ...
      operator: { label: 'Сотрудник платформы', value: 'Контролирует блокеры, ответственных, сроки и хронологию.', action: 'Определить причину остановки и разрешённый следующий шаг.', documents: 'События, основания, версии документов и решения.', responsibility: 'Сохранить целостность процесса между участниками.', outcome: 'Сделка проходит без потери контекста.', risk: 'Несогласованные факты между участниками.', money: 'Видит денежное последствие, но не получает полномочия банка или стороны Сделки.' },
    ...
    scenarios: {
      standard: { label: 'Обычный', summary: 'Поставка подтверждена, качество соответствует, документы проверяются.', amount: 'Расчёт после подтверждения оснований', blocker: 'Блокирующее основание отсутствует', outcome: 'Обычное закрытие после проверки документов и финансового результата.' },
      partial: { label: 'Частичная приёмка', summary: 'Принята только часть согласованного объёма.', amount: 'Расчёт только по подтверждённому объёму', blocker: 'Для остатка объёма нет подтверждающего основания', outcome: 'Подтверждённая часть и остаток учитываются отдельно.' },
    ...
    documents: [
      { name: 'Акт приёмки', type: 'Приёмка', party: 'Партия примера', trip: 'Рейс примера', creator: 'Элеватор / хранение', signer: 'Уполномоченный участник', timestamp: 'После события приёмки', version: 'Текущая версия', status: 'Основание приёмки', checksum: 'Не публикуется в примере', basis: 'Подтверждает фактический принятый объём.' },
      { name: 'Протокол качества', type: 'Лаборатория', party: 'Партия примера', trip: 'Рейс примера', creator: 'Лаборатория', signer: 'Уполномоченный специалист', timestamp: 'После проверки качества', version: 'Текущая версия', status: 'Основание качества', checksum: 'Не публикуется в примере', basis: 'Позволяет сопоставить качество с условиями.' },
      { name: 'Расчётное основание', type: 'Расчёт', party: 'Партия примера', trip: 'Рейс примера', creator: 'Платформа', signer: 'Не применимо', timestamp: 'После комплектности оснований', version: 'Рабочая версия', status: 'Основание расчёта', checksum: 'Не публикуется в примере', basis: 'Показывает, какие подтверждения нужны до финансового действия.' },
    ],
    risks: {
      ...
      qualityDeviation: { label: 'Отклонение качества', event: 'Показатель не соответствует условиям.', blocked: 'Финансовое действие до подтверждения основания.', owner: 'Лаборатория', evidence: 'Проба, методика и подтверждённый протокол.', deadline: 'По условиям Сделки', outcome: 'Перерасчёт, повторная проверка или разногласие.' },
      missingDocument: { label: 'Отсутствующий документ', event: 'В комплекте нет обязательного основания.', blocked: 'Финансовое действие до комплекта оснований.', owner: 'Сторона документа и сотрудник платформы', evidence: 'Требуемый подтверждённый документ.', deadline: 'До расчётного действия', outcome: 'Дозапрос или перенос следующего шага.' },
...
const en: PublicProductExperienceCopy = {
  ...
  explorer: {
    ...
    deal: { idLabel: 'Example', id: 'Fictional Deal', commodityLabel: 'Product', commodity: 'Sunflower', classLabel: 'Category', classValue: 'Crop product', volumeLabel: 'Volume', volume: '1,200 tonnes', priceLabel: 'Price', price: 'From example terms', amountLabel: 'Settlement', amount: 'After grounds are confirmed', routeLabel: 'Route', route: 'Tambov Region — Voronezh Region', stageLabel: 'Operational stage', statusLabel: 'Stage context', status: 'Facts · grounds · next action', ownerLabel: 'Responsible', nextLabel: 'Next action', blockerLabel: 'What blocks progress', noBlocker: 'No blocking ground' },
    ...
    stages: {
      ...
      logistics: { label: 'Logistics and delivery', happened: 'Carrier, driver, vehicle, route and trip are assigned.', owner: 'Logistics', action: 'Confirm trip events and their supporting grounds.', evidence: 'Trip, route and transport events.', next: 'Arrival opens acceptance.' },
      acceptance: { label: 'Acceptance', happened: 'The lot arrives at the site; weight and acceptance facts are recorded.', owner: 'Elevator / storage', action: 'Confirm actual acceptance.', evidence: 'Acceptance act, weight, time and linked trip.', next: 'Quality verification opens.' },
      ...
      documents: { label: 'Documents', happened: 'Versions and confirmations for delivery, acceptance and quality are assembled.', owner: 'Parties and platform employee', action: 'Close missing grounds.', evidence: 'Confirmed document versions linked to events.', next: 'Settlement grounds are checked.' },
    ...
    perspectives: {
      ...
      driver: { label: 'Driver', value: 'Receives the trip, checkpoints and nearest action.', action: 'Confirm arrival and cargo handover.', documents: 'Trip, route and permitted transport documents.', responsibility: 'Provide confirmed trip events.', outcome: 'Delivery confirmed without unrelated data access.', risk: 'Checkpoint without supporting evidence.', money: 'Trip completion is confirmed separately from product commercial terms.' },
      elevator: { label: 'Elevator / storage', value: 'Records arrival, weight, acceptance, placement and lot facts.', action: 'Confirm actual acceptance.', documents: 'Trip, weight data, acceptance act and lot facts.', responsibility: 'Create reliable acceptance evidence.', outcome: 'Quantity confirmed and passed to quality control.', risk: 'Weight mismatch or wrong lot.', money: 'Accepted quantity affects settlement.' },
      ...
      operator: { label: 'Platform employee', value: 'Controls blockers, owners, deadlines and chronology.', action: 'Identify the cause of a stop and the next permitted step.', documents: 'Events, grounds, document versions and decisions.', responsibility: 'Preserve process integrity between participants.', outcome: 'The Deal progresses without context loss.', risk: 'Inconsistent facts between participants.', money: 'Sees monetary impact without inheriting bank or party authority.' },
    ...
    scenarios: { standard: { label: 'Ordinary', summary: 'Delivery evidence is present, quality meets the terms and documents are being checked.', amount: 'Settlement after grounds are confirmed', blocker: 'No blocking ground', outcome: 'Ordinary closure after document checks and financial result.' }, partial: { label: 'Partial acceptance', summary: 'Only part of the agreed volume was accepted.', amount: 'Settlement only for supported volume', blocker: 'The remaining volume lacks supporting grounds', outcome: 'Supported part and remaining volume are tracked separately.' }, dispute: { label: 'Deviation / dispute', summary: 'A quantity or quality discrepancy was found.', amount: 'Undisputed part separated from disputed part', blocker: 'Disputed part requires a decision', outcome: 'Evidence → party decision or procedure → final settlement.' } },
    documents: [
      { name: 'Acceptance act', type: 'Acceptance', party: 'Example lot', trip: 'Example trip', creator: 'Elevator / storage', signer: 'Authorised participant', timestamp: 'After the acceptance event', version: 'Current version', status: 'Acceptance ground', checksum: 'Not published in the example', basis: 'Confirms the actual accepted volume.' },
      { name: 'Quality protocol', type: 'Laboratory', party: 'Example lot', trip: 'Example trip', creator: 'Laboratory', signer: 'Authorised specialist', timestamp: 'After the quality check', version: 'Current version', status: 'Quality ground', checksum: 'Not published in the example', basis: 'Allows quality to be compared with terms.' },
      { name: 'Settlement basis', type: 'Settlement', party: 'Example lot', trip: 'Example trip', creator: 'Platform', signer: 'Not applicable', timestamp: 'After grounds are complete', version: 'Working version', status: 'Settlement ground', checksum: 'Not published in the example', basis: 'Shows which confirmations are required before a financial action.' },
    ],
    risks: {
      ...
      qualityDeviation: { label: 'Quality deviation', event: 'An indicator does not meet the terms.', blocked: 'Financial action before the relevant grounds are confirmed.', owner: 'Laboratory', evidence: 'Sample, method and confirmed protocol.', deadline: 'According to Deal terms', outcome: 'Recalculation, recheck or discrepancy.' },
      missingDocument: { label: 'Missing document', event: 'A mandatory ground is absent.', blocked: 'Financial action before the required grounds are complete.', owner: 'Document party and platform employee', evidence: 'Required confirmed document.', deadline: 'Before the settlement action', outcome: 'Request or delayed next step.' },
...
const zh: PublicProductExperienceCopy = {
  ...
  home: {
    ...
    preview: {
      eyebrow: '虚构交易示例', title: '葵花籽 · 履约示例', commodity: '葵花籽', volume: '1,200 吨', price: '按示例条件确定价格', route: '坦波夫州 — 沃罗涅日州', nowLabel: '当前步骤', nowValue: '验收与质量', requiredLabel: '需要操作', requiredValue: '确认实际履约', ownerLabel: '责任方', ownerValue: '买方 / 验收点', afterLabel: '确认后', afterValue: '文件与结算依据',
  ...
  explorer: {
    ...
    lenses: { execution: { label: '履约', summary: '事件、责任和允许的转换。' }, participants: { label: '参与方', summary: '同一笔交易中的不同任务和权限。' }, documents: { label: '文件', summary: '文件作为事件和结算的可核验依据。' }, money: { label: '结算', summary: '哪些确认允许进入金融操作。' }, risk: { label: '风险与偏差', summary: '阻塞、证据、双方立场和后果。' }, intelligence: { label: 'Gekta', summary: '解释事实和风险，但不拥有独立决策权限。' } },
    stages: {
      ...
      logistics: { label: '物流与交付', happened: '已指定承运方、司机、车辆、路线和运输任务。', owner: '物流', action: '确认运输事件及其相关依据。', evidence: '运输任务、路线和运输事件。', next: '到达后开放验收。' },
      acceptance: { label: '验收', happened: '批次到达验收点，记录重量和验收事实。', owner: '筒仓 / 仓储', action: '确认实际验收。', evidence: '验收记录、重量、时间和关联运输任务。', next: '进入质量核验。' },
      laboratory: { label: '质量', happened: '实验室结果与交易条件进行比对。', owner: '实验室', action: '确认样品、方法和结果。', evidence: '样品、方法和质量报告。', next: '检查文件完整性。' },
      documents: { label: '文件', happened: '汇总交付、验收和质量相关版本与确认。', owner: '双方与平台员工', action: '补齐缺失依据。', evidence: '已确认文件版本及其与事件的关联。', next: '核对结算依据。' },
    ...
    perspectives: {
      seller: { label: '卖方', value: '查看商品、交付、文件和结算依据。', action: '确认卖方义务履行。', documents: '条件、交付文件、验收和质量。', responsibility: '交付约定数量和质量。', outcome: '履约已确认，结算依据清晰。', risk: '交付证据不足。', money: '结算取决于验收数量和已确认依据。' },
      buyer: { label: '买方', value: '控制条件、验收、质量和付款依据。', action: '确认交付符合条件。', documents: '条件、验收、质量报告和结算版本。', responsibility: '接收合格交付并确认依据。', outcome: '获得质量可核验的商品。', risk: '履约依据不完整时进行金融操作。', money: '结算与实际履约保持关联。' },
      ...
      driver: { label: '司机', value: '接收运输任务、检查点和最近操作。', action: '确认到达和货物交接。', documents: '运输任务、路线和允许查看的运输文件。', responsibility: '提交已确认运输事件。', outcome: '交付已确认且不访问无关数据。', risk: '检查点缺少支持依据。', money: '运输完成事实与商品商业条件分开确认。' },
      elevator: { label: '筒仓 / 仓储', value: '记录到达、重量、验收、存放和批次事实。', action: '确认实际验收。', documents: '运输任务、称重数据、验收记录和批次事实。', responsibility: '形成可靠的验收依据。', outcome: '数量确认后进入质量控制。', risk: '重量不符或批次错误。', money: '实际验收数量影响结算。' },
      ...
      operator: { label: '平台员工', value: '控制阻塞项、责任方、期限和时间线。', action: '确定停止原因和允许的下一步。', documents: '事件、依据、文件版本和决定。', responsibility: '保持参与方之间流程完整。', outcome: '交易连续推进且不丢失上下文。', risk: '参与方事实不一致。', money: '看到资金影响，但不会自动获得银行或交易方权限。' },
      compliance: { label: '平台员工', value: '内部准入与权限检查职能。', action: '核验准入依据。', documents: '机构资料、权限和日志。', responsibility: '阻止不允许的转换。', outcome: '准入依赖可核验依据。', risk: '遗漏限制。', money: '不会产生独立金融权限。' },
    ...
    scenarios: { standard: { label: '普通', summary: '已有交付依据，质量符合条件，文件正在核验。', amount: '依据确认后进入结算', blocker: '不存在阻塞依据', outcome: '文件核验和金融结果确认后正常关闭。' }, partial: { label: '部分验收', summary: '仅验收约定数量的一部分。', amount: '仅对有支持依据的数量结算', blocker: '剩余数量缺少支持依据', outcome: '有支持依据的部分与剩余数量分别跟踪。' }, dispute: { label: '偏差 / 争议', summary: '发现数量或质量差异。', amount: '无争议部分与争议部分分离', blocker: '争议部分需要决定', outcome: '证据 → 双方决定或程序 → 最终结算。' } },
    documents: [
      { name: '验收记录', type: '验收', party: '示例批次', trip: '示例运输任务', creator: '筒仓 / 仓储', signer: '获授权参与方', timestamp: '验收事件后', version: '当前版本', status: '验收依据', checksum: '示例中不公开', basis: '确认实际验收数量。' },
      { name: '质量报告', type: '实验室', party: '示例批次', trip: '示例运输任务', creator: '实验室', signer: '获授权专业人员', timestamp: '质量核验后', version: '当前版本', status: '质量依据', checksum: '示例中不公开', basis: '用于把质量与条件进行比对。' },
      { name: '结算依据', type: '结算', party: '示例批次', trip: '示例运输任务', creator: '平台', signer: '不适用', timestamp: '依据完整后', version: '工作版本', status: '结算依据', checksum: '示例中不公开', basis: '显示金融操作前还需要哪些确认。' },
    ],
    risks: {
      ...
      qualityDeviation: { label: '质量偏差', event: '某项指标不符合条件。', blocked: '相关依据确认前不进入金融操作。', owner: '实验室', evidence: '样品、方法和已确认报告。', deadline: '按交易条件', outcome: '重算、复检或偏差处理。' },
      missingDocument: { label: '文件缺失', event: '缺少必需依据。', blocked: '所需依据完整前不进入金融操作。', owner: '文件责任方与平台员工', evidence: '所需已确认文件。', deadline: '金融操作前', outcome: '补充请求或延后下一步。' },
