...
const ru = {
  ...
  explorer: {
    ...
    controls: {
      ...
      previous: 'Назад',
      next: 'Далее',
      startGuide: 'Показать всю сделку автоматически',
      pause: 'Пауза',
      continue: 'Продолжить',
      stop: 'Пропустить',
      aiToggle: 'Показать помощь ИИ',
      allParticipants: 'Все участники',
      openDocument: 'Показать основание',
      closeDocument: 'Скрыть основание',
    },
    labels: {
      happened: 'Что произошло',
      responsible: 'Кто отвечает',
      action: 'Какое действие требуется',
      evidence: 'Какое доказательство возникнет',
      transition: 'Что станет доступно дальше',
      visibleDocuments: 'Видимые документы',
      responsibility: 'Ответственность',
      expectedOutcome: 'Ожидаемый результат',
      roleRisk: 'Риск роли',
      moneyContext: 'Денежный контекст',
      event: 'Событие',
      document: 'Документ',
      signature: 'Подпись',
      version: 'Версия',
      allowedAction: 'Разрешённое действие',
      confidence: 'Уверенность',
      affectedObject: 'Затронутый объект',
      recommendation: 'Рекомендуемое действие',
      whyImportant: 'Почему это важно',
      blockedAction: 'Заблокированное действие',
      deadline: 'Срок реакции',
      outcome: 'Возможный исход',
      disputedAmount: 'Спорная сумма',
    },
    lenses: {
      ...
      money: { label: 'Деньги', summary: 'Резервирование, основания, частичный или итоговый расчёт.' },
    ...
    stages: {
      terms: { label: 'Условия', happened: 'Стороны согласовали товар, объём, цену и требования к качеству.', owner: 'Покупатель и продавец', action: 'Подтвердить канонические условия.', evidence: 'Версия условий с авторством и временем.', next: 'Открывается проверка допуска.' },
      ...
      auction: { label: 'Аукцион', happened: 'Зафиксированы ставки и выбран победитель по правилам торгов.', owner: 'Оператор', action: 'Подтвердить результат торгов.', evidence: 'Неизменяемая история ставок и решение.', next: 'Создаётся основание сделки.' },
      deal: { label: 'Сделка', happened: 'Победившее предложение преобразовано в единый объект сделки.', owner: 'Покупатель и продавец', action: 'Подтвердить обязательства сторон.', evidence: 'Карточка сделки и версия договорённостей.', next: 'Открывается назначение перевозки.' },
      ...
      acceptance: { label: 'Приёмка', happened: 'Партия прибыла на элеватор; фиксируются фактический вес и состояние.', owner: 'Элеватор', action: 'Подтвердить вес партии.', evidence: 'Акт приёмки, время и связанный рейс.', next: 'Открывается лабораторная проверка.' },
      ...
      documents: { label: 'Документы', happened: 'Собирается комплект документов по партии, рейсу и приёмке.', owner: 'Оператор и стороны', action: 'Закрыть отсутствующие основания.', evidence: 'Подписанные версии и контрольные суммы.', next: 'Разрешается проверка денежного основания.' },
      settlement: { label: 'Расчёт', happened: 'Сумма сопоставляется с подтверждённым исполнением и комплектом оснований.', owner: 'Банк', action: 'Проверить основание расчёта.', evidence: 'Решение, денежное событие и сверка.', next: 'Полный или частичный расчёт ведёт к закрытию.' },
    ...
    perspectives: {
      ...
      logistics: { label: 'Логистика', value: 'Управляет перевозчиком, рейсом, маршрутом и отклонениями.', action: 'Обеспечить прибытие рейса.', documents: 'Заявка, маршрут, транспортные события, подтверждение прибытия.', responsibility: 'Доставить партию в согласованное окно.', outcome: 'Рейс завершён и связан с приёмкой.', risk: 'Опоздание или потеря связности событий.', money: 'Логистическое исполнение влияет на допустимые удержания.' },
      driver: { label: 'Водитель', value: 'Получает рейс, контрольные точки и понятный следующий шаг.', action: 'Подтвердить прибытие.', documents: 'Рейс, маршрут, транспортный документ.', responsibility: 'Передавать подтверждённые события рейса.', outcome: 'Прибытие открывает приёмку.', risk: 'Неподтверждённая контрольная точка.', money: 'Выплата перевозчику требует подтверждения рейса.' },
      elevator: { label: 'Элеватор', value: 'Фиксирует прибытие, вес, приёмку и связь с партией.', action: 'Подтвердить фактический вес.', documents: 'Рейс, талон, акт приёмки, весовые данные.', responsibility: 'Создать достоверное основание факта приёмки.', outcome: 'Принятый объём передан в контроль качества.', risk: 'Расхождение веса или неверная партия.', money: 'Фактический принятый объём влияет на расчёт.' },
      lab: { label: 'Лаборатория', value: 'Связывает пробу, методику и результат с конкретной партией.', action: 'Подписать протокол качества.', documents: 'Проба, методика, протокол, версия результата.', responsibility: 'Зафиксировать воспроизводимый результат.', outcome: 'Качество сопоставлено с условиями.', risk: 'Протокол связан не с той партией.', money: 'Отклонение качества меняет допустимую сумму.' },
      ...
      operator: { label: 'Оператор', value: 'Управляет блокерами, ответственными и сквозной хронологией.', action: 'Снять операционный блокер.', documents: 'Все события, статусы, документы и решения.', responsibility: 'Сохранить целостность процесса между организациями.', outcome: 'Сделка проходит без разрывов и ручной потери контекста.', risk: 'Несогласованное состояние участников.', money: 'Контролирует готовность денежного основания, но не принимает решение банка.' },
      ...
      executive: { label: 'Руководитель', value: 'Видит портфель, исключения, деньги и системные причины риска.', action: 'Устранить повторяющийся источник отклонений.', documents: 'Агрегированные события и проверяемые первичные основания.', responsibility: 'Управлять результатом без вмешательства в каждую операцию.', outcome: 'Снижение потерь, сроков и доли ручного сопровождения.', risk: 'Красивые показатели без подтверждаемой первички.', money: 'Контролирует GMV, выручку, спорные суммы и скорость расчёта.' },
    },
    scenarios: {
    ...
    documents: [
      { name: 'Акт приёмки', type: 'Приёмка', party: 'Партия B-2408', trip: 'Рейс R-318', creator: 'Элеватор', signer: 'Уполномоченный сотрудник', timestamp: '14.07.2026 · 11:42', version: 'v1.0', status: 'Ожидает подписи', checksum: 'SHA-256 · 71af…c904', basis: 'Подтверждает фактический принятый объём.' },
      { name: 'Протокол качества', type: 'Лаборатория', party: 'Партия B-2408', trip: 'Рейс R-318', creator: 'Лаборатория', signer: 'Лаборант', timestamp: '14.07.2026 · 12:18', version: 'v1.0', status: 'Подтверждён', checksum: 'SHA-256 · 9bc2…a811', basis: 'Разрешает сопоставление качества с условиями.' },
    ...
    aiSignals: [
      ...
      { title: 'Риск нарушения срока', why: 'До расчётного окна остаётся мало времени, а акт не подписан.', object: 'Этап «Документы»', recommendation: 'Назначить ответственного и запросить подпись.', confidence: 'Средняя · 78%' },
      { title: 'Недостаточное основание', why: 'Протокол качества есть, но не связан с текущей пробой.', object: 'Протокол качества', recommendation: 'Проверить связь пробы и партии.', confidence: 'Высокая · 89%' },
    ],
    boundaries: {
...
const en: typeof ru = {
  ...
  explorer: {
    ...
    lenses: { execution: { label: 'Execution', summary: 'Causal chain of events, actions and transitions.' }, participants: { label: 'Participants', summary: 'One deal with different responsibilities and outcomes.' }, documents: { label: 'Documents', summary: 'Documents as evidence for events and money actions.' }, money: { label: 'Money', summary: 'Reservation, evidence, partial or final settlement.' }, risk: { label: 'Risks and dispute', summary: 'Blocks, evidence, party positions and consequences.' }, intelligence: { label: 'AI and analytics', summary: 'Contextual signals without autonomous decisions.' } },
    stages: {
      terms: { label: 'Terms', happened: 'The parties agreed commodity, volume, price and quality requirements.', owner: 'Buyer and seller', action: 'Confirm canonical terms.', evidence: 'Versioned terms with authorship and time.', next: 'Admission checks become available.' }, admission: { label: 'Admission', happened: 'Organisations, authority and mandatory data were checked.', owner: 'Compliance', action: 'Resolve identified mismatches.', evidence: 'Admission decision and check log.', next: 'Auction participation is allowed.' }, auction: { label: 'Auction', happened: 'Bids were recorded and a winner selected under the rules.', owner: 'Operator', action: 'Confirm the auction result.', evidence: 'Immutable bid history and decision.', next: 'The deal basis is created.' }, deal: { label: 'Deal', happened: 'The winning offer became one canonical deal object.', owner: 'Buyer and seller', action: 'Confirm party obligations.', evidence: 'Deal card and terms version.', next: 'Transport assignment becomes available.' }, logistics: { label: 'Transport', happened: 'Carrier, driver, vehicle and route were assigned.', owner: 'Logistics', action: 'Confirm trip readiness.', evidence: 'Trip, route and checkpoints.', next: 'Arrival opens acceptance.' }, acceptance: { label: 'Acceptance', happened: 'The lot arrived at the elevator; actual weight and condition are recorded.', owner: 'Elevator', action: 'Confirm the lot weight.', evidence: 'Acceptance certificate, time and linked trip.', next: 'Laboratory inspection opens.' }, laboratory: { label: 'Laboratory', happened: 'Quality indicators were compared with deal terms.', owner: 'Laboratory', action: 'Sign the result protocol.', evidence: 'Protocol with sample, method and version.', next: 'Document completeness is assessed.' }, documents: { label: 'Documents', happened: 'The lot, trip and acceptance evidence set is assembled.', owner: 'Operator and parties', action: 'Close missing evidence.', evidence: 'Signed versions and checksums.', next: 'Money evidence can be checked.' }, settlement: { label: 'Settlement', happened: 'The amount is matched to confirmed execution and evidence.', owner: 'Bank', action: 'Check the settlement basis.', evidence: 'Decision, money event and reconciliation.', next: 'Full or partial settlement leads to closure.' }, closure: { label: 'Closure', happened: 'Obligations, documents and money events are reconciled.', owner: 'Operator', action: 'Record deal completion.', evidence: 'Final chronology and evidence pack.', next: 'Data remains available for analytics and audit.' },
    },
    perspectives: {
      seller: { label: 'Seller', value: 'Sees the lot, dispatch, documents and settlement.', action: 'Confirm delivery performance.', documents: 'Terms, lot, dispatch documents and acceptance certificate.', responsibility: 'Deliver the agreed volume and quality.', outcome: 'Confirmed performance and payment.', risk: 'Insufficient delivery evidence.', money: 'Payment depends on accepted volume and complete evidence.' }, buyer: { label: 'Buyer', value: 'Controls terms, acceptance, quality and payment basis.', action: 'Confirm delivery conformity.', documents: 'Terms, laboratory protocol, acceptance and settlement.', responsibility: 'Accept compliant delivery and confirm evidence.', outcome: 'Received commodity with verified quality.', risk: 'Paying for unconfirmed volume or quality.', money: 'Settlement is linked to actual execution.' }, logistics: { label: 'Logistics', value: 'Manages carrier, trip, route and deviations.', action: 'Ensure trip arrival.', documents: 'Order, route, transport events and arrival.', responsibility: 'Deliver within the agreed window.', outcome: 'Completed trip linked to acceptance.', risk: 'Delay or broken event continuity.', money: 'Transport performance affects allowed deductions.' }, driver: { label: 'Driver', value: 'Receives the trip, checkpoints and next action.', action: 'Confirm arrival.', documents: 'Trip, route and transport document.', responsibility: 'Transmit verified trip events.', outcome: 'Arrival opens acceptance.', risk: 'Unconfirmed checkpoint.', money: 'Carrier payment requires trip confirmation.' }, elevator: { label: 'Elevator', value: 'Records arrival, weight, acceptance and lot linkage.', action: 'Confirm actual weight.', documents: 'Trip, ticket, acceptance certificate and scale data.', responsibility: 'Create reliable acceptance evidence.', outcome: 'Accepted volume enters quality control.', risk: 'Weight mismatch or wrong lot.', money: 'Accepted volume affects settlement.' }, lab: { label: 'Laboratory', value: 'Links sample, method and result to a specific lot.', action: 'Sign the quality protocol.', documents: 'Sample, method, protocol and result version.', responsibility: 'Record a reproducible result.', outcome: 'Quality is compared with terms.', risk: 'Protocol linked to the wrong lot.', money: 'Quality deviation changes the eligible amount.' }, surveyor: { label: 'Surveyor', value: 'Creates independent evidence for a disputed event.', action: 'Record an independent inspection.', documents: 'Inspection act, photos, time and linked object.', responsibility: 'Preserve a neutral evidence chain.', outcome: 'The dispute gains independent evidence.', risk: 'Incomplete or non-reproducible record.', money: 'Evidence affects the disputed amount.' }, bank: { label: 'Bank', value: 'Sees amount, evidence, payment, reconciliation and disputed part.', action: 'Check the evidence set.', documents: 'Contract basis, acceptance, quality and dispute decision.', responsibility: 'Execute money actions only on an allowed event.', outcome: 'Auditable settlement and reconciliation.', risk: 'Incomplete or conflicting evidence.', money: 'Full, partial or delayed amount by scenario.' }, operator: { label: 'Operator', value: 'Manages blockers, owners and the end-to-end chronology.', action: 'Resolve the operational blocker.', documents: 'All events, statuses, documents and decisions.', responsibility: 'Preserve process integrity across organisations.', outcome: 'The deal progresses without context loss.', risk: 'Inconsistent participant states.', money: 'Controls readiness of evidence but does not make the bank decision.' }, compliance: { label: 'Compliance', value: 'Checks admission, authority and critical mismatches.', action: 'Complete the admission check.', documents: 'Organisation profile, authority, decision and log.', responsibility: 'Prevent a forbidden transition.', outcome: 'Participants are admitted on verified grounds.', risk: 'Missed restriction or data conflict.', money: 'A non-admitted party blocks the money flow.' }, arbitrator: { label: 'Arbitrator', value: 'Combines party positions, evidence and money consequences.', action: 'Assess evidence sufficiency.', documents: 'Positions, versions, protocols, acts and chronology.', responsibility: 'Make a reasoned procedural decision.', outcome: 'The dispute ends with a clear consequence.', risk: 'Decision without a complete evidence pack.', money: 'The decision defines disputed and undisputed amounts.' }, executive: { label: 'Executive', value: 'Sees portfolio, exceptions, money and systemic causes.', action: 'Remove recurring sources of deviation.', documents: 'Aggregated events with verifiable primary evidence.', responsibility: 'Manage outcomes without micromanaging operations.', outcome: 'Lower losses, cycle time and manual work.', risk: 'Good-looking metrics without primary evidence.', money: 'Controls GMV, revenue, disputed amounts and settlement speed.' },
    },
    ...
    documents: [
      { name: 'Acceptance certificate', type: 'Acceptance', party: 'Lot B-2408', trip: 'Trip R-318', creator: 'Elevator', signer: 'Authorised employee', timestamp: '14 Jul 2026 · 11:42', version: 'v1.0', status: 'Awaiting signature', checksum: 'SHA-256 · 71af…c904', basis: 'Confirms the actual accepted volume.' },
      { name: 'Quality protocol', type: 'Laboratory', party: 'Lot B-2408', trip: 'Trip R-318', creator: 'Laboratory', signer: 'Laboratory specialist', timestamp: '14 Jul 2026 · 12:18', version: 'v1.0', status: 'Confirmed', checksum: 'SHA-256 · 9bc2…a811', basis: 'Allows quality to be compared with terms.' },
    ...
    risks: {
      transportDelay: { label: 'Transport delay', event: 'The trip missed the agreed arrival window.', blocked: 'Timely acceptance.', owner: 'Logistics', evidence: 'Route, checkpoints and deviation reason.', deadline: '2 hours', outcome: 'New window, deduction or dispute.' }, weightMismatch: { label: 'Weight mismatch', event: 'Actual weight differs from declared weight.', blocked: 'Confirmation of the full volume.', owner: 'Elevator and surveyor', evidence: 'Scale data, act and independent record.', deadline: '4 hours', outcome: 'Volume correction or dispute.' }, qualityDeviation: { label: 'Quality deviation', event: 'An indicator does not meet the terms.', blocked: 'Full settlement.', owner: 'Laboratory', evidence: 'Sample, method and signed protocol.', deadline: '1 business day', outcome: 'Repricing, rejection or dispute.' }, missingDocument: { label: 'Missing document', event: 'A mandatory item is absent from the evidence set.', blocked: 'Transfer of the money basis.', owner: 'Operator and document owner', evidence: 'Required signed document.', deadline: 'Before the settlement window', outcome: 'Request or settlement delay.' }, documentVersion: { label: 'Changed document version', event: 'A new version appeared after approval.', blocked: 'Use of the previous basis.', owner: 'Document creator', evidence: 'Version history, signatures and checksums.', deadline: '4 hours', outcome: 'New version accepted or disputed.' }, paymentBasis: { label: 'Incomplete payment basis', event: 'Events and documents do not form a complete set.', blocked: 'Money action.', owner: 'Operator and bank', evidence: 'Missing performance confirmations.', deadline: 'Before the next settlement window', outcome: 'Wait, partial settlement or dispute.' },
    },
    aiSignals: [
...
const zh: typeof ru = {
  ...
  explorer: {
    ...
    lenses: { execution: { label: '履约', summary: '事件、操作和状态转换的因果链。' }, participants: { label: '参与方', summary: '同一交易中的不同责任与结果。' }, documents: { label: '文件', summary: '文件作为事件和资金操作的依据。' }, money: { label: '资金', summary: '预留、依据、部分或最终结算。' }, risk: { label: '风险与争议', summary: '阻塞、证据、双方立场及后果。' }, intelligence: { label: 'AI 与分析', summary: '提供上下文信号，不自动作出决定。' } },
    stages: {
      terms: { label: '条款', happened: '双方确认商品、数量、价格和质量要求。', owner: '买方与卖方', action: '确认标准条款。', evidence: '带作者和时间的条款版本。', next: '进入准入检查。' }, admission: { label: '准入', happened: '核验企业、授权与必填信息。', owner: '合规', action: '解决发现的不一致。', evidence: '准入决定与检查日志。', next: '允许参与竞价。' }, auction: { label: '竞价', happened: '记录报价并按规则选出中标方。', owner: '运营方', action: '确认竞价结果。', evidence: '不可变报价历史和决定。', next: '生成交易依据。' }, deal: { label: '交易', happened: '中标报价转化为统一交易对象。', owner: '买方与卖方', action: '确认双方义务。', evidence: '交易卡片与条款版本。', next: '开放运输安排。' }, logistics: { label: '运输', happened: '已指定承运人、司机、车辆和路线。', owner: '物流', action: '确认运输任务就绪。', evidence: '运输任务、路线和检查点。', next: '到达后开放验收。' }, acceptance: { label: '验收', happened: '批次到达粮库，记录实际重量和状态。', owner: '粮库', action: '确认批次重量。', evidence: '验收单、时间与关联运输任务。', next: '开放实验室检验。' }, laboratory: { label: '实验室', happened: '将质量指标与交易条款进行比对。', owner: '实验室', action: '签署检验结果。', evidence: '包含样品、方法和版本的报告。', next: '检查文件完整性。' }, documents: { label: '文件', happened: '汇总批次、运输与验收依据。', owner: '运营方与双方', action: '补齐缺失依据。', evidence: '已签署版本和校验和。', next: '允许核验资金依据。' }, settlement: { label: '结算', happened: '将金额与已确认履约及依据进行匹配。', owner: '银行', action: '核验结算依据。', evidence: '决定、资金事件与对账。', next: '全额或部分结算后进入关闭。' }, closure: { label: '关闭', happened: '汇总义务、文件和资金事件。', owner: '运营方', action: '记录交易完成。', evidence: '最终时间线与证据包。', next: '数据可用于分析和审计。' },
    },
    perspectives: {
      seller: { label: '卖方', value: '查看批次、发运、文件和结算。', action: '确认交付履约。', documents: '条款、批次、发运文件和验收单。', responsibility: '交付约定数量和质量。', outcome: '履约确认并获得付款。', risk: '交付证据不足。', money: '付款取决于验收数量和依据完整性。' }, buyer: { label: '买方', value: '控制条款、验收、质量和付款依据。', action: '确认交付符合要求。', documents: '条款、质检报告、验收单和结算。', responsibility: '接收合格交付并确认依据。', outcome: '获得质量可核验的商品。', risk: '为未确认数量或质量付款。', money: '结算金额与实际履约关联。' }, logistics: { label: '物流', value: '管理承运人、运输任务、路线和偏差。', action: '确保运输任务到达。', documents: '任务、路线、运输事件和到达确认。', responsibility: '在约定窗口内送达。', outcome: '运输完成并关联验收。', risk: '延误或事件链断裂。', money: '运输履约影响可扣减金额。' }, driver: { label: '司机', value: '接收运输任务、检查点和下一步操作。', action: '确认到达。', documents: '运输任务、路线和运输文件。', responsibility: '提交可验证的运输事件。', outcome: '到达后开放验收。', risk: '检查点未确认。', money: '承运付款需要任务确认。' }, elevator: { label: '粮库', value: '记录到达、重量、验收及批次关联。', action: '确认实际重量。', documents: '运输任务、票据、验收单和称重数据。', responsibility: '生成可靠的验收依据。', outcome: '验收数量进入质量控制。', risk: '重量不符或批次错误。', money: '验收数量影响结算。' }, lab: { label: '实验室', value: '将样品、方法和结果关联到具体批次。', action: '签署质量报告。', documents: '样品、方法、报告和结果版本。', responsibility: '记录可复核结果。', outcome: '质量与条款完成比对。', risk: '报告关联错误批次。', money: '质量偏差会改变可结算金额。' }, surveyor: { label: '检验人', value: '为争议事件生成独立证据。', action: '完成独立检查记录。', documents: '检查单、照片、时间和关联对象。', responsibility: '保存中立证据链。', outcome: '争议获得独立依据。', risk: '记录不完整或不可复核。', money: '证据影响争议金额。' }, bank: { label: '银行', value: '查看金额、依据、付款、对账和争议部分。', action: '核验依据集合。', documents: '合同依据、验收、质量和争议决定。', responsibility: '仅在允许事件下执行资金操作。', outcome: '可审计结算与对账。', risk: '依据不完整或相互矛盾。', money: '按场景全额、部分或延后结算。' }, operator: { label: '运营方', value: '管理阻塞项、责任人和全程时间线。', action: '解除运营阻塞。', documents: '全部事件、状态、文件和决定。', responsibility: '保持跨企业流程一致性。', outcome: '交易连续推进且不丢失上下文。', risk: '参与方状态不一致。', money: '控制资金依据就绪度，但不替银行决策。' }, compliance: { label: '合规', value: '检查准入、授权和关键不一致。', action: '完成准入检查。', documents: '企业资料、授权、决定和日志。', responsibility: '阻止不允许的状态转换。', outcome: '参与方基于可验证依据准入。', risk: '遗漏限制或数据冲突。', money: '未准入参与方会阻塞资金流程。' }, arbitrator: { label: '仲裁方', value: '汇总双方立场、证据和资金后果。', action: '判断证据充分性。', documents: '立场、版本、报告、记录和时间线。', responsibility: '在程序内作出有依据的决定。', outcome: '争议以明确后果结束。', risk: '在证据包不完整时作出决定。', money: '决定划分争议与无争议金额。' }, executive: { label: '管理者', value: '查看组合、异常、资金和系统性原因。', action: '消除重复出现的偏差来源。', documents: '聚合事件及可核验原始依据。', responsibility: '无需干预每笔操作即可管理结果。', outcome: '降低损失、周期和人工处理比例。', risk: '指标漂亮但缺少原始依据。', money: '控制 GMV、收入、争议金额和结算速度。' },
    },
    ...
    documents: [
      { name: '验收单', type: '验收', party: '批次 B-2408', trip: '运输任务 R-318', creator: '粮库', signer: '授权员工', timestamp: '2026-07-14 · 11:42', version: 'v1.0', status: '等待签署', checksum: 'SHA-256 · 71af…c904', basis: '确认实际验收数量。' },
      { name: '质量报告', type: '实验室', party: '批次 B-2408', trip: '运输任务 R-318', creator: '实验室', signer: '检验员', timestamp: '2026-07-14 · 12:18', version: 'v1.0', status: '已确认', checksum: 'SHA-256 · 9bc2…a811', basis: '允许将质量与条款进行比对。' },
    ...
    risks: {
      transportDelay: { label: '运输延误', event: '运输任务超过约定到达窗口。', blocked: '按时验收。', owner: '物流', evidence: '路线、检查点和偏差原因。', deadline: '2 小时', outcome: '新窗口、扣减或争议。' }, weightMismatch: { label: '重量不符', event: '实际重量与申报重量不同。', blocked: '确认全部数量。', owner: '粮库与检验人', evidence: '称重数据、记录和独立证据。', deadline: '4 小时', outcome: '修正数量或进入争议。' }, qualityDeviation: { label: '质量偏差', event: '某项指标不符合条款。', blocked: '全额结算。', owner: '实验室', evidence: '样品、方法和已签署报告。', deadline: '1 个工作日', outcome: '重新计价、拒收或争议。' }, missingDocument: { label: '文件缺失', event: '依据集合中缺少必需文件。', blocked: '传递资金依据。', owner: '运营方与文件责任方', evidence: '所需已签署文件。', deadline: '结算窗口前', outcome: '补充请求或延后结算。' }, documentVersion: { label: '文件版本变化', event: '批准后出现新版本。', blocked: '继续使用旧依据。', owner: '文件创建方', evidence: '版本历史、签名和校验和。', deadline: '4 小时', outcome: '接受新版本或进入争议。' }, paymentBasis: { label: '付款依据不完整', event: '事件和文件尚未形成完整集合。', blocked: '资金操作。', owner: '运营方与银行', evidence: '缺失的履约确认。', deadline: '下一结算窗口前', outcome: '等待、部分结算或争议。' },
    },
    aiSignals: [
...
export const PUBLIC_PRODUCT_EXPERIENCE_COPY = { ru, en, zh } as const;

export type PublicProductExperienceCopy = typeof ru;
