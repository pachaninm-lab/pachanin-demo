...
const ru: OrganizationConnectCopy = {
  ...
  lead: 'Укажите организацию и контакт. На втором шаге — роль и рабочий сценарий.',
  ...
  note: 'После отправки система выдаст номер заявки; доступ создаётся только после проверки.',
...
const en: OrganizationConnectCopy = {
  ...
  lead: 'Enter the organisation and contact. Step two confirms the role and operating scenario.',
  ...
  note: 'The system returns a request number; access is created only after verification.', required: 'Check the required fields in this step.',
...
const zh: OrganizationConnectCopy = {
  ...
  note: '提交后系统生成申请编号；审核通过后才创建访问权限。', required: '请检查当前步骤的必填字段。',
