...
export function GektaMobileDrawer({ open, closeLabel, onClose, children }: { open: boolean; closeLabel: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div className='fixed inset-0 z-50 md:hidden' role='dialog' aria-modal='true' aria-label='Gekta navigation'>
      ...
      <div ref={panelRef} className='absolute inset-y-0 left-0 w-[min(88vw,300px)] overflow-hidden bg-[#f6f5ef] shadow-2xl'>
        <button type='button' onClick={onClose} className='absolute right-3 top-3 z-10 flex h-11 w-11 items-center justify-center rounded-xl bg-white text-slate-700 shadow-sm' aria-label={closeLabel}><X className='h-5 w-5' aria-hidden='true' /></button>
        {children}
      </div>
    </div>
  );
