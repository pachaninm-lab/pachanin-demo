...
export function GektaHero({ locale }: { locale: GektaLocale }) {
  ...
  return (
    <header className='mx-auto w-full max-w-4xl px-3 pt-5 text-center sm:px-6 sm:pt-12' data-gekta-server-hero='true'>
      ...
      <p className='mt-4 hidden text-sm leading-5 text-slate-500 sm:block'>{copy.maker}</p>
    </header>
  );
}
