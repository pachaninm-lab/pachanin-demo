          ...
          <div key={conversation.id} className={`group relative rounded-xl ${activeId === conversation.id ? 'bg-emerald-50' : 'hover:bg-slate-50'}`}>
            {editing ? (
              ...
              <>
                ...
                {menuId === conversation.id ? <div className='absolute right-1 top-11 z-20 w-40 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl'><button type='button' onClick={() => { setDraft(conversation.title); setEditingId(conversation.id); setMenuId(null); }} className='flex min-h-11 w-full items-center gap-2 rounded-lg px-2.5 text-left text-sm hover:bg-slate-50'><Pencil className='h-4 w-4' aria-hidden='true' />{renameLabel}</button><button type='button' onClick={() => { setMenuId(null); onDelete(conversation.id); }} className='flex min-h-11 w-full items-center gap-2 rounded-lg px-2.5 text-left text-sm text-rose-700 hover:bg-rose-50'><Trash2 className='h-4 w-4' aria-hidden='true' />{deleteLabel}</button></div> : null}
              </>
            )}
          </div>
        );
