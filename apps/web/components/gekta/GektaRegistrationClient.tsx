...
export function GektaRegistrationClient({ initialLocale, initialEmailConfirmation }: {
  ...
  React.useEffect(() => {
    ...
    if (initialEmailConfirmation === 'email') {
      setStep('verify');
      return;
    }
    if (initialEmailConfirmation === 'invalid') {
      setMode('login');
      setStep('form');
  ...
  const confirmEmail = async () => {
    ...
    try {
      ...
      setError(reason instanceof Error && reason.message === 'unavailable' ? ui.unavailable : ui.verifyInvalid);
    } finally {
      setBusy(false);
    }
  };
        ...
        <div className='flex items-center justify-between gap-4'>
          ...
          <nav className='flex gap-1' aria-label='Language'>
            {(['ru', 'en', 'zh'] as const).map((locale) => (
              <Link key={locale} href={localeHref(locale, initialEmailConfirmation)} aria-current={locale === initialLocale ? 'page' : undefined} className={`grid min-h-11 min-w-11 place-items-center rounded-xl px-2 text-xs font-bold ${locale === initialLocale ? 'bg-emerald-800 text-white' : 'text-slate-600 hover:bg-white'}`}>{ui.languages[locale]}</Link>
            ))}
          </nav>
        </div>
