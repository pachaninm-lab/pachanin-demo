...
type Step = 'form' | 'email' | 'mfa' | 'backup';
...
function localeHref(locale: GektaLocale) {
  return `/gekta/register?lang=${locale}`;
}

export function GektaRegistrationClient({ initialLocale }: { initialLocale: GektaLocale }) {
  ...
  const showMfa = React.useCallback((payload: Record<string, unknown>) => {
    setMfa({
      ...
      setupSecret: typeof payload.setupSecret === 'string' ? payload.setupSecret : null,
      otpAuthUri: typeof payload.otpAuthUri === 'string' ? payload.otpAuthUri : null,
    });
    setStep('mfa');
    setError('');
  }, []);
  ...
  React.useEffect(() => {
    ...
    if (verificationToken) {
      // Remove the bearer token before the network request, render, analytics
      // or any later navigation can retain it in the address bar.
      params.delete('verify');
      const clean = `${window.location.pathname}${params.size ? `?${params.toString()}` : ''}`;
      window.history.replaceState(window.history.state, '', clean);
      setBusy(true);
      setStep('mfa');
      void post('/api/gekta/auth/email/verify', { token: verificationToken })
        .then(async (response) => {
        ...
        .catch(() => {
          setMode('login');
          setStep('form');
          setError(ui.verifyInvalid);
        })
        .finally(() => setBusy(false));
      return;
    }

    void fetch('/api/gekta/auth/mfa', { cache: 'no-store', credentials: 'same-origin' })
      .then(async (response) => {
        if (!response.ok) return;
        showMfa(await response.json() as Record<string, unknown>);
      })
      .catch(() => undefined);
  }, [showMfa, ui.verifyInvalid]);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const response = mode === 'register'
        ? await post('/api/gekta/auth/register', { fullName, phone, email, password, acceptedServiceTerms: terms, acceptedPersonalData: privacy, locale: initialLocale })
        : await post('/api/gekta/auth/login', { email, password });
      const payload = await response.json().catch(() => ({})) as Record<string, unknown>;
      if (!response.ok) {
        if (response.status >= 500) throw new Error('unavailable');
        throw new Error(mode === 'login' ? 'invalid_login' : 'invalid');
      }
      if (mode === 'register') {
        setStep('email');
        setPassword('');
      ...
      const name = reason instanceof Error ? reason.message : 'invalid';
      setError(name === 'unavailable' ? ui.unavailable : name === 'invalid_login' ? ui.invalidLogin : ui.genericError);
    } finally {
      setBusy(false);
    }
  };
  ...
  const resend = async () => {
    ...
    setNotice('');
    try {
      const response = await post('/api/gekta/auth/register/resend', { email, locale: initialLocale });
      if (!response.ok) throw new Error(response.status >= 500 ? 'unavailable' : 'invalid');
      setNotice(ui.resendDone);
    } catch (reason) {
      setError(reason instanceof Error && reason.message === 'unavailable' ? ui.unavailable : ui.genericError);
    } finally {
      setBusy(false);
    }
  };
  ...
  const verifyMfa = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(response.status >= 500 ? 'unavailable' : 'invalid');
      const codes = Array.isArray(payload.backupCodes)
        ? payload.backupCodes.filter((item): item is string => typeof item === 'string')
        : [];
      if (codes.length) {
        setBackupCodes(codes);
        setStep('backup');
      } else {
        window.location.assign(GEKTA_PATHS[initialLocale]);
      }
    } catch (reason) {
      setError(reason instanceof Error && reason.message === 'unavailable' ? ui.unavailable : ui.mfaInvalid);
    } finally {
      setBusy(false);
    }
  };
    ...
    <main className='min-h-screen bg-[#f6f5ef] px-4 py-6 text-slate-950 sm:py-10'>
      <div className='mx-auto max-w-xl'>
        <div className='flex items-center justify-between gap-4'>
          <Link href={GEKTA_PATHS[initialLocale]} className='inline-flex min-h-11 items-center gap-2 rounded-xl px-2 text-sm font-semibold text-emerald-900 hover:bg-white'>
            <span aria-hidden='true'>←</span>{ui.back}
          </Link>
          <nav className='flex gap-1' aria-label='Language'>
            {(['ru', 'en', 'zh'] as const).map((locale) => (
              <Link key={locale} href={localeHref(locale)} aria-current={locale === initialLocale ? 'page' : undefined} className={`grid min-h-11 min-w-11 place-items-center rounded-xl px-2 text-xs font-bold ${locale === initialLocale ? 'bg-emerald-800 text-white' : 'text-slate-600 hover:bg-white'}`}>{ui.languages[locale]}</Link>
            ))}
          </nav>
        </div>
        ...
        <section className='mt-5 overflow-hidden rounded-3xl border border-emerald-950/10 bg-white shadow-[0_20px_70px_rgba(15,23,42,0.08)]'>
          <header className='border-b border-slate-100 bg-gradient-to-br from-emerald-950 to-emerald-800 px-6 py-7 text-white sm:px-8'>
            <div className='flex items-center gap-3'><span className='grid h-11 w-11 place-items-center rounded-2xl bg-white text-xl font-black text-emerald-900'>G</span><div><p className='text-xs font-bold tracking-[0.18em] text-emerald-100'>GEKTA</p><h1 className='text-2xl font-bold'>{ui.title}</h1></div></div>
            <p className='mt-4 max-w-lg text-sm leading-6 text-emerald-50'>{ui.lead}</p>
          </header>

          <div className='p-6 sm:p-8'>
            {busy && step === 'mfa' && !mfa ? (
              <div className='py-10 text-center' role='status'><LoaderCircle className='mx-auto h-7 w-7 animate-spin text-emerald-700' /><p className='mt-3 text-sm text-slate-600'>{ui.verifying}</p></div>
            ) : null}

            {step === 'form' ? (
              <>
                <div className='grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1' role='tablist'>
                  {(['register', 'login'] as const).map((value) => <button key={value} type='button' role='tab' aria-selected={mode === value} onClick={() => switchMode(value)} className={`min-h-11 rounded-xl px-4 text-sm font-semibold ${mode === value ? 'bg-white text-emerald-900 shadow-sm' : 'text-slate-600'}`}>{value === 'register' ? ui.register : ui.login}</button>)}
                </div>
                <form className='mt-6 space-y-4' onSubmit={submit}>
                  {mode === 'register' ? <><label className='block text-sm font-semibold text-slate-800'>{ui.name}<input required minLength={2} maxLength={120} autoComplete='name' value={fullName} onChange={(event) => setFullName(event.target.value)} placeholder={ui.namePlaceholder} className='mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-4 font-normal outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100' /></label><label className='block text-sm font-semibold text-slate-800'>{ui.phone}<input required type='tel' minLength={8} maxLength={32} autoComplete='tel' value={phone} onChange={(event) => setPhone(event.target.value)} placeholder={ui.phonePlaceholder} className='mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-4 font-normal outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100' /><span className='mt-2 block text-xs font-normal leading-5 text-slate-500'>{ui.phoneHint}</span></label></> : null}
                  <label className='block text-sm font-semibold text-slate-800'>{ui.email}<input required type='email' maxLength={320} autoComplete='email' value={email} onChange={(event) => setEmail(event.target.value)} className='mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-4 font-normal outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100' /></label>
                  <label className='block text-sm font-semibold text-slate-800'>{ui.password}<input required type='password' minLength={mode === 'register' ? 12 : 1} maxLength={128} autoComplete={mode === 'register' ? 'new-password' : 'current-password'} value={password} onChange={(event) => setPassword(event.target.value)} className='mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-4 font-normal outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100' />{mode === 'register' ? <span className='mt-2 block text-xs font-normal leading-5 text-slate-500'>{ui.passwordHint}</span> : null}</label>
