...
export function FieldTaskTemplate({
  ...
  return (
    <div className={styles.root} data-testid={testId}>
      ...
      <WorkbenchTemplate
        density='field'
        title={<span className={styles.heading}><span className={styles.eyebrow}>{eyebrow}</span><span>{title}</span></span>}
...
export function IntakeWorkbenchTemplate({ intakeSummary, context, ...props }: IntakeWorkbenchTemplateProps) {
  return (
    <FieldTaskTemplate
      {...props}
      context={<div className={styles.contextStack}><section aria-label='Сводка приёмки'>{intakeSummary}</section>{context}</div>}
    />
  );
}
