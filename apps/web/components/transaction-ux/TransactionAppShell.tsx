...
export function TransactionAppShell({ children, role }: { children: React.ReactNode; role: PlatformRole }) {
  ...
  return (
    <div className={`${styles.root} pc-shell-root-v8`} data-drawer-open={drawerOpen ? 'true' : 'false'}>
      ...
      {drawerOpen ? (
        <button type='button' className={styles.backdrop} onClick={() => setDrawerOpen(false)} aria-label='Закрыть меню' />
      ) : null}

      <aside className={styles.navigation} aria-label='Основная навигация'>
      ...
      <nav className={styles.mobileNavigation} aria-label='Навигация кабинета'>
