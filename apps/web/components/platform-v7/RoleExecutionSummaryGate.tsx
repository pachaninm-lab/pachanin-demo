'use client';

import { usePathname } from 'next/navigation';
import { ExecutionHelpEntry } from './ExecutionHelpEntry';
import { RoleExecutionSummary, type PlatformV7ExecutionRole } from './RoleExecutionSummary';

export const PLATFORM_V7_SUMMARY_BY_ROUTE: Record<string, PlatformV7ExecutionRole> = {
  '/platform-v7/seller': 'seller',
  '/platform-v7/seller/offers': 'seller',
  '/platform-v7/seller/lots': 'seller',
  '/platform-v7/seller/batches': 'seller',
  '/platform-v7/buyer': 'buyer',
  '/platform-v7/buyer/rfq': 'buyer',
  '/platform-v7/logistics': 'logistics',
  '/platform-v7/logistics/inbox': 'logistics',
  '/platform-v7/driver': 'driver',
  '/platform-v7/driver/field': 'driver',
  '/platform-v7/elevator': 'elevator',
  '/platform-v7/lab': 'lab',
  '/platform-v7/surveyor': 'surveyor',
  '/platform-v7/bank': 'bank',
  '/platform-v7/bank/payment-basis': 'bank',
  '/platform-v7/bank/factoring': 'bank',
  '/platform-v7/bank/escrow': 'bank',
  '/platform-v7/bank/events': 'bank',
  '/platform-v7/arbitrator': 'arbitrator',
  '/platform-v7/compliance': 'compliance',
  '/platform-v7/control-tower': 'operator',
  '/platform-v7/operator': 'operator',
  '/platform-v7/operator/queues': 'operator',
  '/platform-v7/support/operator': 'operator',
  '/platform-v7/executive': 'executive',
  '/platform-v7/investor': 'investor',
};

export function RoleExecutionSummaryGate() {
  const pathname = usePathname();
  const role = pathname ? PLATFORM_V7_SUMMARY_BY_ROUTE[pathname] : undefined;

  return (
    <>
      <ExecutionHelpEntry />
      {role ? <RoleExecutionSummary role={role} /> : null}
    </>
  );
}
