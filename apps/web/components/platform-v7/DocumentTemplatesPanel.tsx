...
const ITEMS = [
  { title: 'Пакет документов сделки', status: 'Платформа', text: 'Шаблоны должны обслуживать сделку: договор, акт, УПД, ЭТрН, СДИЗ, качество и расчёт.', color: '#065F46', bg: '#D1FAE5' },
  { title: 'Версии и условия', status: 'Доработка', text: 'Нужно фиксировать версию шаблона, основание формирования и незакрытые условия сделки.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Юридическая проверка', status: 'Доработка', text: 'Шаблоны требуют проверки юристом и утверждённого регламента применения.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Подпись и ЭДО', status: 'Интеграция позже', text: 'КЭП, КриптоПро и ЭДО временно не подключены и не показываются как работающие.', color: '#92400E', bg: '#FEF3C7' },
];
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 8, background: '#EFF6FF', border: '1px solid #BFDBFE', fontSize: 10, color: '#1E40AF', fontWeight: 760, lineHeight: 1.55 }}>
        Шаблоны документов: настоящая платформа временно без интеграций. Создание, скачивание, подписание, КЭП и ЭДО не показываются как подтверждённые.
      </div>
      ...
      <div style={{ fontSize: 9, color: '#94A3B8', padding: '4px 8px', borderRadius: 6, background: '#F8FAFB', border: '1px solid #E4E6EA' }}>
        Шаблоны должны усиливать исполнение сделки; внешние интеграции временно не подключены.
      </div>
    </div>
  );
}
