  ...
  return (
    <main style={{ display: 'grid', gap: 14, padding: '4px 0 24px' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 10 }}>
        {items.map((item, index) => <Link key={`${item.title}-${index}`} href={item.href} style={itemCard}><b style={{ color: 'var(--pc-text-primary, #0F1419)' }}>{index + 1}. {item.title}</b><span style={{ color: toneColor(item.tone), fontWeight: 900 }}>{item.value}</span><span style={{ color: 'var(--pc-text-muted, #64748B)', fontSize: 13, lineHeight: 1.45 }}>{item.note ?? 'Открыть связанный экран.'}</span></Link>)}
      </section>
    </main>
  );
}
...
function toneColor(tone: GrainWorkflowItem['tone']) {
  return tone === 'bad' ? '#B91C1C' : tone === 'warn' ? '#B45309' : tone === 'good' ? '#0A7A5F' : 'var(--pc-text-primary, #0F1419)';
}
...
const leadStyle = { margin: 0, color: 'var(--pc-text-secondary, #475569)', fontSize: 15, lineHeight: 1.55 } as const;
...
const ghostBtn = { ...primaryBtn, background: '#fff', border: '1px solid #CBD5E1', color: 'var(--pc-text-primary, #0F1419)' } as const;
const itemCard = { textDecoration: 'none', minHeight: 150, display: 'grid', gap: 8, padding: 14, borderRadius: 18, background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', color: 'var(--pc-text-primary, #0F1419)' } as const;
