...
export function SecureDealChat({ text, actorId, counterpartyId }: { text: string; actorId: string; counterpartyId: string }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 14, display: 'grid', gap: 10 }}>
      <strong style={{ color: 'var(--pc-text-primary, #0F1419)', fontSize: 16 }}>Чат сделки</strong>
      <p style={{ margin: 0, color: 'var(--pc-text-secondary, #475569)', fontSize: 13, lineHeight: 1.5 }}>{scan.safeText}</p>
      <BypassRiskBadge profile={profile} />
      <span style={{ color: 'var(--pc-text-muted, #64748B)', fontSize: 12 }}>Действие проверки: {scan.action}</span>
    </div>
  );
}
