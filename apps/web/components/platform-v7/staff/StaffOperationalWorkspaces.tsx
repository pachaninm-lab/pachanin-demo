    ...
    <section className={styles.section} aria-labelledby="staff-operational-workspaces-title">
      ...
      {context.active && context.session && (
        <>
          ...
          {!loading && tab === 'critical' && <CriticalPanel rows={array(data)} copy={copy} locale={locale} busy={busy} decisionReasons={decisionReasons} setDecisionReasons={setDecisionReasons} decide={(id, decision, reason) => post(`critical-actions/${encodeURIComponent(id)}/decision`, { decision, reason }, `critical:${id}`)} />}
