...
export function OwnerAccessCenter({ locale, copy, identity, apiAvailable, accessCatalog }: Props) {
  ...
  const taskForRole = useCallback(
    (taskId: TaskId) => accessCatalog.find((task) => task.id === taskId && (task.permissionsByRole[activeRole] || []).length > 0) || null,
  ...
  const canBrowseOrganizations = Boolean(taskForRole('browse_organizations'));
  ...
  useEffect(() => { if (emergencyOpen) emergencyTitleRef.current?.focus(); }, [emergencyOpen]);

  const openTask = (taskId: TaskId, options?: { allowWhileActive?: boolean }) => {
    if (sessionContext.active && !options?.allowWhileActive) {
      setError(copy.request.activeSessionBlock);
      return;
    }
    const task = taskForRole(taskId);
    if (!task) {
      setError(copy.errors.forbidden);
      return;
    }
  ...
  const activateGrant = async (grantId: string, taskId?: TaskId) => {
    ...
    try {
      ...
      const activatedTask = taskId || selectedTaskId || undefined;
      if (activatedTask === 'browse_organizations') setView('organizations');
      else if (activatedTask === 'view_cabinet') setView('organizations');
      else setView('home');
    } catch (value) {
      setError(translateError(value));
    } finally {
  ...
  const submitRequest = async () => {
    ...
    try {
      ...
      if (result.grantId) {
        setBusy(null);
        await activateGrant(result.grantId, taskId);
      } else {
  ...
  const endSession = async (sessionId: string) => {
    ...
    setError(null);
    try {
      await closeSession(sessionId, 'Staff actor ended the protected session');
      setNotice(null);
    } catch (value) {
      setError(translateError(value));
    } finally {
  ...
  const selectOrganization = async (organization: Organization) => {
    const resumeTask = pendingTaskId || 'view_cabinet';
    setOrganizationId(organization.id);
    setTargetRole('FARMER');
    setPendingTaskId(null);
    setError(null);
    if (sessionContext.active && sessionContext.session?.accessMode === 'CONTROL_PLANE') {
      setBusy('switch-organization');
      try {
        await closeSession(sessionContext.session.accessSessionId, 'Organization selected; directory session closed');
      } catch (value) {
        setError(translateError(value));
        setBusy(null);
        return;
    ...
    openTask(resumeTask, { allowWhileActive: true });
  };
  ...
  const revokeSession = async (sessionId: string) => {
    ...
    try {
      await staffApi(`access/sessions/${encodeURIComponent(sessionId)}/revoke`, {
        method: 'POST', body: { reason: 'Revoked from Owner Access Center' },
      });
      await reload();
    } catch (value) {
      setError(translateError(value));
  ...
  return (
    <main className={styles.page} data-owner-access-center data-locale={locale}>
      ...
      <header className={styles.hero}>
        ...
        <button type="button" className={styles.refreshButton} onClick={() => void reload()} disabled={Boolean(busy)}>{busy ? copy.refreshing : copy.refresh}</button>
      </header>
      ...
      {view === 'home' && (
        <>
          <section className={styles.statusGrid} aria-label={copy.home.attention}>
            <article><span>{copy.home.role}</span><strong>{roleLabel(activeRole)}</strong><small>{identity?.email || identity?.fullName || identity?.id}</small></article>
            <article><span>{sessionContext.active ? copy.home.protectedMode : copy.home.ordinaryMode}</span><strong>{sessionContext.active ? modeLabel(sessionContext.session?.accessMode) : copy.home.protectedInactive}</strong><small>{sessionContext.active ? copy.home.protectedActive : copy.home.protectedInactive}</small></article>
          </section>
          <section className={styles.attentionPanel}>
            <div className={styles.sectionHeading}><h2>{copy.home.attention}</h2></div>
            {pendingOwn === 0 && sessions.length === 0 && reviewRequests.length === 0 ? <p className={styles.empty}>{copy.home.noAttention}</p> : (
          ...
          <section className={styles.tasksSection}>
            ...
            <div className={styles.taskGrid}>{availableTasks.map((task, index) => (
              <button key={task.id} type="button" className={styles.taskCard} onClick={() => openTask(task.id)}>
                <span className={styles.taskNumber}>{String(index + 1).padStart(2, '0')}</span><strong>{copy.tasks[task.id].title}</strong><p>{copy.tasks[task.id].description}</p><span className={styles.taskAction}>{copy.continue} →</span>
              </button>
            ))}</div>
          </section>
      ...
      {view === 'request' && selectedTask && (
        <section className={styles.requestPanel}>
          <div className={styles.sectionHeading}><div><h2>{copy.request.title}</h2><p>{copy.tasks[selectedTask.id].description}</p></div><button type="button" className={styles.textButton} onClick={() => setView('home')}>{copy.cancel}</button></div>
          <ol className={styles.steps} aria-label={copy.request.title}><li className={styles.stepDone}>{copy.request.stepTask}</li><li className={styles.stepActive}>{copy.request.stepScope}</li><li>{copy.request.stepConfirm}</li></ol>
          <div className={styles.selectedTask}><div><span>{copy.request.selectedTask}</span><strong>{copy.tasks[selectedTask.id].title}</strong></div><button type="button" onClick={() => setView('home')}>{copy.request.changeTask}</button></div>
          <div className={styles.formGrid}>
            <label><span>{copy.request.assignment}</span><select id="owner-access-assignment" value={assignmentId} onChange={(event) => setAssignmentId(event.target.value)} aria-invalid={Boolean(fieldErrors.assignment)}>{assignments.map((item) => <option key={item.id} value={item.id}>{roleLabel(item.role)} · {statusLabel(item.status)}</option>)}</select>{fieldErrors.assignment && <small className={styles.fieldError}>{fieldErrors.assignment}</small>}</label>
            {selectedTask.requiresOrganization && (
              <label>
                <span>{copy.request.organization}</span>
                {organizations.length > 0 ? (
                  <select id="owner-access-organization" value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} aria-invalid={Boolean(fieldErrors.organization)}><option value="">{copy.request.organizationPlaceholder}</option>{organizations.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.inn}</option>)}</select>
                ) : (
                ...
                {organizations.length === 0 && canBrowseOrganizations && <button type="button" className={styles.textButton} onClick={openOrganizationSearch}>{copy.request.openOrganizationSearch}</button>}
              </label>
            )}
            {selectedTask.requiresCabinetRole && <label><span>{copy.request.cabinetRole}</span><select id="owner-access-role" value={targetRole} onChange={(event) => setTargetRole(event.target.value)} aria-invalid={Boolean(fieldErrors.role)}>{CABINET_ROLES.map((item) => <option key={item} value={item}>{roleLabel(item)}</option>)}</select>{fieldErrors.role && <small className={styles.fieldError}>{fieldErrors.role}</small>}</label>}
            {selectedTask.allowsDeal && <label><span>{copy.request.deal}</span><input value={targetDealId} onChange={(event) => setTargetDealId(event.target.value)} placeholder={copy.request.dealPlaceholder} maxLength={128} /></label>}
            <label><span>{copy.request.ticket}</span><input id="owner-access-ticket" value={ticketId} onChange={(event) => setTicketId(event.target.value)} placeholder={copy.request.ticketPlaceholder} maxLength={128} aria-invalid={Boolean(fieldErrors.ticket)} />{fieldErrors.ticket && <small className={styles.fieldError}>{fieldErrors.ticket}</small>}</label>
            <label><span>{copy.request.duration}</span><select value={durationMinutes} onChange={(event) => setDurationMinutes(Number(event.target.value))}>{[5, 10, 15, 30, 60].map((item) => <option key={item} value={item}>{item} {copy.request.minutes}</option>)}</select></label>
            <label className={styles.fullWidth}><span>{copy.request.reason}</span><textarea id="owner-access-reason" value={reason} onChange={(event) => setReason(event.target.value)} placeholder={copy.request.reasonPlaceholder} maxLength={2000} rows={4} aria-invalid={Boolean(fieldErrors.reason)} />{fieldErrors.reason && <small className={styles.fieldError}>{fieldErrors.reason}</small>}</label>
          </div>
          <div className={styles.confirmGrid}><article><span>{copy.request.approval}</span><strong>{selectedTask.approvalLevel === 'automatic' ? copy.request.approvalAutomatic : selectedTask.approvalLevel === 'one_approver' ? copy.request.approvalOne : copy.request.approvalTwo}</strong></article><article><span>{copy.request.protection}</span><strong>{copy.request.protectionText}</strong></article></div>
          <details className={styles.technicalDetails}><summary>{copy.request.technicalDetails}</summary><dl><div><dt>Mode</dt><dd>{modeLabel(selectedTask.accessMode)}</dd></div><div><dt>{copy.request.permissions}</dt><dd>{selectedPermissions.join(', ')}</dd></div></dl></details>
          <button type="button" className={styles.primaryButton} onClick={() => void submitRequest()} disabled={busy === 'request' || sessionContext.active}>{busy === 'request' ? copy.request.submitting : selectedTask.approvalLevel === 'automatic' ? copy.request.submit : copy.request.send}</button>
        </section>
      )}
      ...
      {view === 'organizations' && (
        <section className={styles.contentPanel}>
          ...
          {!can('organization:list') ? <div className={styles.lockedCard}><h3>{copy.organizations.lockedTitle}</h3><p>{copy.organizations.lockedBody}</p>{canBrowseOrganizations && <button type="button" className={styles.primaryButton} onClick={openOrganizationSearch}>{copy.organizations.unlock}</button>}</div> : (
            <><input className={styles.search} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={copy.organizations.search} aria-label={copy.organizations.search} /><div className={styles.organizationList}>{filteredOrganizations.length === 0 ? <p className={styles.empty}>{copy.organizations.empty}</p> : filteredOrganizations.map((item) => <article key={item.id} className={styles.organizationCard}><div><strong>{item.name}</strong><small>{copy.organizations.inn}: {item.inn}</small></div><dl><div><dt>{copy.organizations.status}</dt><dd>{item.status}</dd></div><div><dt>{copy.organizations.kyc}</dt><dd>{item.kycStatus || item.kyc_status || '—'}</dd></div><div><dt>{copy.organizations.aml}</dt><dd>{item.amlStatus || item.aml_status || '—'}</dd></div></dl><button type="button" className={styles.secondaryButton} onClick={() => void selectOrganization(item)} disabled={busy === 'switch-organization'}>{copy.organizations.view}</button></article>)}</div></>
          )}
          {cabinet && <article className={styles.cabinetCard}><div className={styles.sectionHeading}><div><h3>{cabinet.effectiveOrganization.name}</h3><p>{roleLabel(cabinet.effectiveRole)} · {copy.activeSession.readOnly}</p></div></div><div className={styles.dealList}>{cabinet.deals.length === 0 ? <p className={styles.empty}>{copy.organizations.empty}</p> : cabinet.deals.map((deal) => <article key={deal.id}><strong>{deal.dealNumber || deal.deal_number || deal.id}</strong><span>{deal.status}</span><small>{deal.nextAction || deal.next_action || '—'} · {formatDate(deal.updatedAt || deal.updated_at, locale)}</small></article>)}</div></article>}
        </section>
      )}

      {view === 'requests' && <section className={styles.contentPanel}><div className={styles.sectionHeading}><h2>{copy.requests.title}</h2></div><RequestList requests={requests} copy={copy} locale={locale} modeLabel={modeLabel} statusLabel={statusLabel} busy={busy} onActivate={activateGrant} activeSession={sessionContext.active} /></section>}
      {view === 'sessions' && <section className={styles.contentPanel}><div className={styles.sectionHeading}><div><h2>{copy.sessions.title}</h2><p>{copy.sessions.description}</p></div></div>{sessions.length === 0 ? <p className={styles.empty}>{copy.sessions.empty}</p> : <div className={styles.sessionList}>{sessions.map((item) => <article key={item.id}><div><strong>{modeLabel(item.access_mode)}</strong><small>{copy.sessions.expires}: {formatDate(item.expires_at, locale)}</small></div><button type="button" className={styles.dangerTextButton} onClick={() => void endSession(item.id)} disabled={busy === `end:${item.id}`}>{copy.sessions.end}</button></article>)}</div>}</section>}

      {view === 'more' && (
        <section className={styles.moreGrid}>
          <article className={styles.contentPanel}><div className={styles.sectionHeading}><div><h2>{copy.more.approvals}</h2><p>{copy.more.approvalsDescription}</p></div></div>{!can('staff-request:read') ? <LockedAccess copy={copy} onOpen={() => openTask('manage_staff')} /> : reviewRequests.length === 0 ? <p className={styles.empty}>{copy.more.noApprovals}</p> : <div className={styles.reviewList}>{reviewRequests.map((item) => <article key={item.id}><header><div><strong>{modeLabel(item.access_mode)}</strong><small>{item.ticket_id || '—'} · {formatDate(item.requested_at, locale)}</small></div><span>{statusLabel(item.status)}</span></header><p>{item.reason}</p>{item.status === 'PENDING' && can('staff-request:approve') && <><textarea rows={2} value={decisionReasons[item.id] || ''} onChange={(event) => setDecisionReasons((current) => ({ ...current, [item.id]: event.target.value }))} placeholder={copy.more.decisionPlaceholder} /><div className={styles.inlineActions}><button type="button" className={styles.primaryButton} onClick={() => void decideRequest(item.id, 'APPROVE')} disabled={busy === `decision:${item.id}`}>{copy.more.approve}</button><button type="button" className={styles.dangerButton} onClick={() => void decideRequest(item.id, 'DENY')} disabled={busy === `decision:${item.id}`}>{copy.more.deny}</button></div></>}</article>)}</div>}</article>
          <article className={styles.contentPanel}><div className={styles.sectionHeading}><h2>{copy.more.activeStaffSessions}</h2></div>{!can('staff-session:read') ? <LockedAccess copy={copy} onOpen={() => openTask('manage_staff')} /> : reviewSessions.length === 0 ? <p className={styles.empty}>{copy.more.noStaffSessions}</p> : <div className={styles.sessionList}>{reviewSessions.map((item) => <article key={item.id}><div><strong>{roleLabel(item.staff_role)} · {modeLabel(item.access_mode)}</strong><small>{item.actor_user_id} · {formatDate(item.expires_at, locale)}</small></div>{can('staff-session:revoke') && item.id !== activeSessionId && <button type="button" className={styles.dangerTextButton} onClick={() => void revokeSession(item.id)} disabled={busy === `revoke:${item.id}`}>{copy.more.revoke}</button>}</article>)}</div>}</article>
          <article className={styles.contentPanel}><div className={styles.sectionHeading}><div><h2>{copy.more.audit}</h2><p>{copy.more.auditDescription}</p></div></div>{!can('audit:read') ? <LockedAccess copy={copy} onOpen={() => openTask('manage_staff')} /> : auditEvents.length === 0 ? <p className={styles.empty}>{copy.more.noAudit}</p> : <div className={styles.auditList}>{auditEvents.map((item) => <article key={item.id}><div><strong>{item.action}</strong><span>{statusLabel(item.outcome)}</span></div><dl><div><dt>{copy.more.actor}</dt><dd>{item.actor_user_id} · {roleLabel(item.staff_role)}</dd></div><div><dt>{copy.more.time}</dt><dd>{formatDate(item.created_at, locale)}</dd></div><div><dt>{copy.more.correlation}</dt><dd>{item.correlation_id}</dd></div></dl></article>)}</div>}</article>
      ...
      <nav className={styles.bottomNav} aria-label={copy.title}>{NAV_ITEMS.map((item) => <button key={item.id} type="button" className={view === item.id ? styles.navActive : ''} onClick={() => setView(item.id)} aria-current={view === item.id ? 'page' : undefined}><span aria-hidden="true" /><strong>{copy.nav[item.key]}</strong></button>)}</nav>

      {emergencyOpen && <div className={styles.dialogBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setEmergencyOpen(false); }}><section className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="emergency-title"><button type="button" className={styles.dialogClose} onClick={() => setEmergencyOpen(false)} aria-label={copy.close}>×</button><h2 id="emergency-title" ref={emergencyTitleRef} tabIndex={-1}>{copy.emergency.title}</h2><p>{copy.emergency.description}</p><label><span>{copy.emergency.incident}</span><input value={emergencyTicket} onChange={(event) => setEmergencyTicket(event.target.value)} placeholder={copy.emergency.incidentPlaceholder} aria-invalid={Boolean(emergencyErrors.ticket)} />{emergencyErrors.ticket && <small className={styles.fieldError}>{emergencyErrors.ticket}</small>}</label><label><span>{copy.emergency.reason}</span><textarea value={emergencyReason} onChange={(event) => setEmergencyReason(event.target.value)} placeholder={copy.emergency.reasonPlaceholder} rows={5} aria-invalid={Boolean(emergencyErrors.reason)} />{emergencyErrors.reason && <small className={styles.fieldError}>{emergencyErrors.reason}</small>}</label><label className={styles.confirm}><input type="checkbox" checked={emergencyConfirmed} onChange={(event) => setEmergencyConfirmed(event.target.checked)} /><span>{copy.emergency.confirm}</span></label>{emergencyErrors.confirmation && <small className={styles.fieldError}>{emergencyErrors.confirmation}</small>}<button type="button" className={styles.dangerButton} onClick={() => void activateEmergency()} disabled={busy === 'emergency'}>{busy === 'emergency' ? copy.emergency.activating : copy.emergency.activate}</button></section></div>}
    </main>
  );
}
...
function RequestList({ requests, copy, locale, modeLabel, statusLabel, busy, onActivate, activeSession }: {
  ...
  busy: string | null;
  onActivate: (grantId: string, taskId?: TaskId) => Promise<void>;
