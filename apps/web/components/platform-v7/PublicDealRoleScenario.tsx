...
const ui: Record<Locale, UiCopy> = {
  ru: {
    label: 'Сделка в работе', rolesLabel: 'Что видит каждый участник',
    note: 'Ролевое представление одного сценария. Переключение не открывает данные и не меняет права.',
    preview: 'Пример интерфейса · данные сценария', deal: 'Подсолнечник · 1 200 т', status: 'Отклонение качества',
    stageLabel: 'Этапы исполнения сценария', stages: ['Рейс', 'Прибытие', 'Вес', 'Лаборатория', 'Расчёт'],
  ...
  en: {
    label: 'Deal in execution', rolesLabel: 'What each participant sees',
    note: 'A role-based view of one scenario. Switching views grants no data access and changes no permissions.',
    preview: 'Interface example · scenario data', deal: 'Sunflower lot · 1,200 t', status: 'Quality deviation',
    stageLabel: 'Scenario execution stages', stages: ['Trip', 'Arrival', 'Weight', 'Laboratory', 'Settlement'],
  ...
  zh: {
    label: '执行中的交易', rolesLabel: '各参与方看到的内容',
    note: '同一场景的角色视图。切换视角不会授予数据访问权限，也不会更改权限。',
    preview: '界面示例 · 场景数据', deal: '葵花籽批次 · 1,200 吨', status: '质量偏差',
    stageLabel: '场景执行阶段', stages: ['运输', '到达', '称重', '实验室', '结算'],
