      ...
      {(title || subtitle || actions) && (
        <header style={{ display: 'flex', justifyContent: 'space-between', gap: PLATFORM_V7_TOKENS.spacing.md, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ display: 'grid', gap: PLATFORM_V7_TOKENS.spacing.xs }}>
            {title && <h2 style={{ margin: 0, color: PLATFORM_V7_TOKENS.color.text, fontSize: PLATFORM_V7_TOKENS.typography.h2.size, lineHeight: PLATFORM_V7_TOKENS.typography.h2.lineHeight, fontWeight: PLATFORM_V7_TOKENS.typography.h2.weight }}>{title}</h2>}
            {subtitle && <div style={{ color: PLATFORM_V7_TOKENS.color.textMuted, fontSize: PLATFORM_V7_TOKENS.typography.caption.size + 1, lineHeight: PLATFORM_V7_TOKENS.typography.body.lineHeight, maxWidth: 760 }}>{subtitle}</div>}
          </div>
