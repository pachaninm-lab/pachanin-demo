...
const ITEMS = [
  { title: 'Деньги как часть сделки', status: 'Платформа', text: 'Денежный lifecycle должен следовать событиям сделки: допуск, документы, приёмка, качество, спор и закрытие.', color: '#065F46', bg: '#D1FAE5' },
  { title: 'События и идемпотентность', status: 'Доработка', text: 'Каждое денежное событие должно иметь ключ операции, состояние, причину, источник и защиту от повторного выполнения.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Инварианты расчётов', status: 'Доработка', text: 'Суммы, удержания, комиссии и возвраты должны проверяться серверными правилами и журналом изменений.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Банк и сверка', status: 'Интеграция позже', text: 'Банковские callbacks, сверка и подтверждение операций временно не подключены и не показываются как работающие.', color: '#92400E', bg: '#FEF3C7' },
];
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 8, background: '#EFF6FF', border: '1px solid #BFDBFE', fontSize: 10, color: '#1E40AF', fontWeight: 760, lineHeight: 1.55 }}>
        Денежный lifecycle: настоящая платформа временно без интеграций. Выплаты, удержания, комиссии, возвраты, банковская сверка и runtime-события не показываются как действующие.
      </div>
      ...
      <div style={{ fontSize: 9, color: '#94A3B8', padding: '4px 8px', borderRadius: 6, background: '#F8FAFB', border: '1px solid #E4E6EA' }}>
        Денежный lifecycle должен усиливать контроль сделки и доказательность; внешние интеграции временно не подключены.
      </div>
    </div>
  );
}
