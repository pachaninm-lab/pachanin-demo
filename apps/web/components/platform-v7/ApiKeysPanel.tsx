...
const ITEMS = [
  { title: 'Доступ партнёров к сделке', status: 'Платформа', text: 'Интеграционный доступ должен быть связан со сделкой, организацией, ролью и разрешённым действием.', color: '#065F46', bg: '#D1FAE5' },
  { title: 'Выдача и отзыв доступа', status: 'Доработка', text: 'Создание, отзыв, срок действия и область доступа должны проходить серверную проверку и журналироваться.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Ограничение области', status: 'Доработка', text: 'Доступ должен быть минимальным: только нужные ресурсы сделки, без обхода RBAC и без прав через клиент.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Внешние подключения', status: 'Интеграция позже', text: 'ERP, CRM, BI, callback и партнёрские API временно не подключены и не показываются как работающие.', color: '#92400E', bg: '#FEF3C7' },
];
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 8, background: '#EFF6FF', border: '1px solid #BFDBFE', fontSize: 10, color: '#1E40AF', fontWeight: 760, lineHeight: 1.55 }}>
        Партнёрский доступ: настоящая платформа временно без интеграций. Секреты, эндпоинты, лимиты, подписи и внешние callbacks не показываются как подключённые.
      </div>
      ...
      <div style={{ fontSize: 9, color: '#94A3B8', padding: '4px 8px', borderRadius: 6, background: '#F8FAFB', border: '1px solid #E4E6EA' }}>
        Интеграционный доступ должен усиливать исполнение сделки; внешние интеграции временно не подключены.
      </div>
    </div>
  );
}
