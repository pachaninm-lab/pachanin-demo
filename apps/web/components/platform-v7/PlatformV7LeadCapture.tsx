  ...
  return (
    <section id='lead-request' className='p7-lead-capture' aria-labelledby='p7-lead-title'>
      ...
      <form className='p7-lead-card' action='/api/platform-v7/leads' method='post'>
        <input type='hidden' name='source' value='homepage_lead_form' />
        <input type='hidden' name='interest' value='demo' />
        <input type='hidden' name='pagePath' value='/platform-v7#lead-request' />
        ...
        <span>Форма не открывает рабочие кабинеты и не обещает боевые интеграции до проверки сценария.</span>

        <label>
          <em>Имя</em>
          <input name='name' type='text' autoComplete='name' placeholder='Как к вам обращаться' minLength={2} required />
        </label>
        <label>
          <em>Организация</em>
          <input name='organization' type='text' autoComplete='organization' placeholder='Компания / хозяйство / банк / регион' minLength={2} required />
        </label>
        ...
        <label>
          <em>Роль / интерес</em>
          <input name='role' type='text' placeholder='Покупатель, продавец, банк, регион...' />
        </label>
        <label>
          <em>Комментарий</em>
          <textarea name='message' rows={3} placeholder='Что нужно показать на демо или какой сценарий разобрать' />
        </label>
        <label className='p7-lead-consent'>
          <input name='consent' type='checkbox' value='yes' required />
          <span>Согласен на обработку контакта для связи по демо и пилоту.</span>
        </label>
        <button type='submit' className='p7-lead-button'>Оставить заявку<Send size={17} /><ArrowRight size={17} /></button>
      </form>
      <style>{css}</style>
    </section>
  );
