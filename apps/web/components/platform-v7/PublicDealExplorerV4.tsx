...
export function PublicDealExplorerV4({
  ...
  useEffect(() => {
    ...
    if (params.get('view') === 'detail' || source === 'role-first' || source === 'problem-first') {
      setJourneyMode('detailed');
    }
  }, []);
  ...
  const currentPerspective = adaptedCopy.explorer.perspectives[historyState.perspective];
  ...
  const visitorAction = historyState.perspective === 'executive'
    ? currentPerspective.action
    : activeForVisitor
      ? currentStage.action
      : journey.labels.noAction;
  ...
  const selectStage = (stage: TourStage, historyMode: 'push' | 'replace' = 'push') => {
    ...
    window.dispatchEvent(new CustomEvent('pc:public-product-analytics', {
      detail: { name: 'stage_selected', locale, perspective: next.perspective, stage, scenario: next.scenario, source: 'public_v5_quick_journey' },
    }));
  };
  ...
  const openTai = (selectedPrompt: string) => {
    ...
    window.dispatchEvent(new CustomEvent('pc:public-assistant-context', {
      detail: { context: `deal-${historyState.stage}`, prompts: ordered },
    }));
    window.dispatchEvent(new CustomEvent('pc:public-product-analytics', {
      detail: {
        name: 'tai_stage_prompt_opened',
        locale,
        perspective: historyState.perspective,
        stage: historyState.stage,
        scenario: historyState.scenario,
        source: 'public_v5_quick_journey',
      },
    }));
  };
  ...
  return (
    <div className='pc-ppe-v4-explorer pc-ppe-v5-journey'>
      {/* Compatibility selectors remain here because the detailed layer is still the governed V4 explorer. */}
      <style jsx global>{`
        ...
        @media (max-width: 720px) {
          .pc-ppe-v5-detailed .pc-ppe-explorer-grid {
            grid-template-areas:\n              'lenses'\n              'main'\n              'context';
          }
        ...
        <section className='pc-ppe-v5-intent pc-ppe-v5-surface' aria-labelledby='pc-ppe-v5-intent-title'>
          ...
          <details className='pc-ppe-v5-other-role'>
            ...
            <label>
              ...
              <select value={historyState.perspective} onChange={(event) => selectMobilePerspective(event.target.value as TourPerspective)}>
                {TOUR_PERSPECTIVES.map((key) => <option key={key} value={key}>{adaptedCopy.explorer.perspectives[key].label}</option>)}
              </select>
            </label>
          </details>
        </section>
        ...
        <>
          <section className='pc-ppe-v5-controls'>
            <div className='pc-ppe-v5-context-row'>
              <span className='pc-ppe-v5-context-chip'>
                <PublicExperienceIcon name={historyState.perspective} size={16} />
                {selectedIntent === 'other' ? currentPerspective.label : journey.intents[selectedIntent].label}
              </span>
            ...
            <div className='pc-ppe-v5-mode-notes' aria-hidden='true'>
              <p>{journey.labels.quickModeNote}</p><p>{journey.labels.detailedModeNote}</p>
            </div>
            ...
            <div className='pc-ppe-v5-scenario pc-ppe-v5-surface'>
              ...
              <div className='pc-ppe-v5-scenario-grid pc-ppe-v4-mobile-scenario-list' role='group' aria-label={journey.labels.scenarioQuestion}>
                {TOUR_SCENARIOS.map((key) => (
                  <button key={key} type='button' aria-pressed={historyState.scenario === key} data-active={historyState.scenario === key ? 'true' : 'false'} onClick={() => selectMobileScenario(key)}>
                    {journey.scenarios[key].label}
                  </button>
                ))}
              </div>
          ...
          <div className='pc-ppe-v5-deal-strip' data-testid='public-deal-journey-context'>
            <div className='pc-ppe-v5-deal-strip-top'>
              <strong>{journey.labels.yourDeal}</strong>
              <span className='pc-ppe-v5-demo-badge'>{journey.labels.demonstration}</span>
            </div>
            <div className='pc-ppe-v5-deal-summary'>
              <strong>{deal.commodity} · {deal.volume}</strong>
          ...
          {journeyMode === 'quick' ? (
            <>
              <section className='pc-ppe-v5-stage-card pc-ppe-v5-surface' data-testid='public-deal-quick-stage'>
                <div className='pc-ppe-v5-stage-heading'>
                  <div className='pc-ppe-v5-stage-counter'>
                    <span>{journey.labels.stage} {currentStageIndex + 1} {journey.labels.of} {TOUR_STAGES.length}</span>
                    <span>{currentPerspective.label}</span>
                  </div>
                ...
                <div className='pc-ppe-v5-next'>
                  <span>{journey.labels.next}</span>
                  <strong>{nextStageKey ? adaptedCopy.explorer.stages[nextStageKey].label : journey.labels.completeTitle}</strong>
                </div>
                ...
                <div className='pc-ppe-v5-tai'>
                  ...
                  <div className='pc-ppe-v5-tai-prompts'>
                    {journey.taiPrompts[historyState.stage].map((prompt) => (
                      <button key={prompt} type='button' className='pc-ppe-v5-tai-button' onClick={() => openTai(prompt)}>{prompt}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className='pc-ppe-v5-playback'>
                  {guideMode === 'idle' ? (
                    <button type='button' data-primary='true' onClick={startGuide}><PublicExperienceIcon name='play' size={16} /> {journey.labels.startQuick}</button>
                  ) : (
                    <>
                      <button type='button' data-primary='true' onClick={() => setGuideMode((mode) => mode === 'playing' ? 'paused' : 'playing')}>
                        <PublicExperienceIcon name={guideMode === 'playing' ? 'pause' : 'play'} size={16} /> {guideMode === 'playing' ? journey.labels.pause : journey.labels.continue}
                      </button>
                      <button type='button' onClick={() => setGuideMode('idle')}>{journey.labels.stop}</button>
                    </>
                  )}
              ...
              <div className='pc-ppe-v5-stage-rail-wrap'>
                ...
                <div className='pc-ppe-v5-stage-rail' role='group' aria-label={adaptedCopy.explorer.controls.stage}>
                  {TOUR_STAGES.map((stageKey, index) => (
                    <button key={stageKey} type='button' data-state={stageKey === historyState.stage ? 'active' : index < currentStageIndex ? 'complete' : 'pending'} aria-current={stageKey === historyState.stage ? 'step' : undefined} onClick={() => selectStage(stageKey)}>
                      {index + 1}. {adaptedCopy.explorer.stages[stageKey].label}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {historyState.stage === 'closure' ? (
                <section className='pc-ppe-v5-complete pc-ppe-v5-surface' aria-labelledby='pc-ppe-v5-complete-title'>
                  <h2 id='pc-ppe-v5-complete-title'>{journey.labels.completeTitle}</h2>
                  <p>{journey.labels.completeLead}</p>
                  <ul className='pc-ppe-v5-result-list'>
                    {journey.finalChecks.map((item) => <li key={item}><i>✓</i><span>{item}</span></li>)}
                  </ul>
            ...
            <section className='pc-ppe-v5-detailed' data-testid='public-deal-detailed-mode'>
              <div className='pc-ppe-v5-detailed-intro'>
                <strong>{journey.labels.detailedMode}</strong>
                <p>{journey.labels.detailedModeNote}</p>
                <button type='button' className='pc-ppe-text-button' onClick={() => setMode('quick')}>{journey.labels.detailedBack}</button>
              </div>
