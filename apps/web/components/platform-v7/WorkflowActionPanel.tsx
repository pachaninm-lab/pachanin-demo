  ...
  return (
    <section style={shell} data-testid={`workflow-action-panel-${context}`}>
      ...
      <div style={stateGrid}>
        ...
        <StateCell label='DealDraft' value={state.dealDraftStatus} />
      ...
      <div style={nextBox}>
        ...
        <strong style={{ color: '#0F1419', fontSize: 15, lineHeight: 1.35 }}>{state.nextAction}</strong>
        ...
        {model.actions.map((action) => (
          <button
            ...
            onClick={() => {
              ...
              setAuditEvents((events) => [result.auditEvent, ...events].slice(0, 6));
              setToast(result.toast);
            }}
          >
            <span style={{ color: '#0F1419', fontSize: 14, fontWeight: 950 }}>{action.label}</span>
            <span style={{ color: '#64748B', fontSize: 12, lineHeight: 1.4 }}>{action.description}</span>
          </button>
        ))}
      </div>
        ...
        {auditEvents.map((event) => (
          <div key={event.id} style={journalRow}>
            <div style={{ display: 'grid', gap: 3 }}>
              <strong style={{ color: '#0F1419', fontSize: 13 }}>{translateAction(event.action)}</strong>
              <span style={{ color: '#64748B', fontSize: 12 }}>{event.entityType} · {event.entityId}{event.dealId ? ` · ${event.dealId}` : ''}</span>
            </div>
...
function StateCell({ label, value, accent = false, warning = false }: { label: string; value: string; accent?: boolean; warning?: boolean }) {
  return (
    <div style={{ ...stateCell, borderColor: warning ? 'rgba(217,119,6,0.22)' : accent ? 'rgba(10,122,95,0.2)' : '#E4E6EA', background: warning ? 'rgba(217,119,6,0.055)' : accent ? 'rgba(10,122,95,0.055)' : '#fff' }}>
      ...
      <strong style={{ color: warning ? '#B45309' : accent ? '#0A7A5F' : '#0F1419', fontSize: 12.5, lineHeight: 1.35 }}>{value}</strong>
    </div>
  );
}
...
function translateAction(action: string): string {
  const map: Record<string, string> = {
    ...
    accept_offer_to_draft: 'Оффер принят, создан DealDraft',
...
const shell = { background: '#fff', border: '1px solid #E4E6EA', borderRadius: 24, padding: 18, display: 'grid', gap: 14, boxShadow: '0 14px 34px rgba(15,20,25,0.045)' } as const;
const eyebrow = { color: '#64748B', fontSize: 11, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.07em' } as const;
const h2 = { margin: 0, color: '#0F1419', fontSize: 'clamp(23px,5vw,32px)', lineHeight: 1.08, letterSpacing: '-0.035em', fontWeight: 950 } as const;
const lead = { margin: 0, color: '#475569', fontSize: 14, lineHeight: 1.55 } as const;
...
const stateGrid = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(155px,1fr))', gap: 8 } as const;
const stateCell = { minHeight: 82, display: 'grid', alignContent: 'start', gap: 6, border: '1px solid #E4E6EA', borderRadius: 14, padding: 10 } as const;
const nextBox = { background: '#F8FAFB', border: '1px solid #E4E6EA', borderRadius: 16, padding: 12, display: 'grid', gap: 5 } as const;
const actionGrid = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 8 } as const;
const actionButton = { appearance: 'none', textAlign: 'left', border: '1px solid #CBD5E1', background: '#fff', borderRadius: 16, padding: 13, minHeight: 112, display: 'grid', gap: 7, cursor: 'pointer' } as const;
const journal = { display: 'grid', gap: 8, background: '#F8FAFB', border: '1px solid #E4E6EA', borderRadius: 18, padding: 12 } as const;
const journalRow = { display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', background: '#fff', border: '1px solid #E4E6EA', borderRadius: 13, padding: 10 } as const;
const rolePill = { display: 'inline-flex', width: 'fit-content', alignItems: 'center', borderRadius: 999, padding: '6px 9px', background: 'rgba(15,23,42,0.06)', color: '#0F172A', fontSize: 11, fontWeight: 900 } as const;
