  ...
  return (
    <section style={shell} data-testid={`workflow-action-panel-${context}`}>
      ...
      <div style={nextBox}>
        ...
        <strong style={{ color: 'var(--pc-text-primary, #0F1419)', fontSize: 15, lineHeight: 1.35, overflowWrap: 'anywhere' }}>{state.nextAction}</strong>
        <span style={{ color: 'var(--pc-text-muted, #64748B)', fontSize: 12, fontWeight: 800 }}>{state.updatedAt}</span>
      </div>
      ...
      <div style={actionGrid}>
        {model.actions.map((action) => (
          ...
          >
            <span style={{ color: 'var(--pc-text-primary, #0F1419)', fontSize: 14, fontWeight: 950, overflowWrap: 'anywhere' }}>{action.label}</span>
            <span style={{ color: 'var(--pc-text-muted, #64748B)', fontSize: 12, lineHeight: 1.4, overflowWrap: 'anywhere' }}>{action.description}</span>
          </button>
        ))}
      </div>
        ...
        {auditEvents.map((event) => (
          <div key={event.id} style={journalRow}>
            <div style={{ display: 'grid', gap: 3, minWidth: 0 }}>
              <strong style={{ color: 'var(--pc-text-primary, #0F1419)', fontSize: 13, overflowWrap: 'anywhere' }}>{translateAction(event.action)}</strong>
              <span style={{ color: 'var(--pc-text-muted, #64748B)', fontSize: 12, overflowWrap: 'anywhere' }}>{entityLabel(event.entityType)} · {event.entityId}{event.dealId ? ` · ${event.dealId}` : ''}</span>
            </div>
...
function StateCell({ label, value, accent = false, warning = false }: { label: string; value: string; accent?: boolean; warning?: boolean }) {
  return (
    <div style={{ ...stateCell, borderColor: warning ? 'rgba(217,119,6,0.22)' : accent ? 'rgba(10,122,95,0.2)' : 'var(--pc-border, #E4E6EA)', background: warning ? 'rgba(217,119,6,0.055)' : accent ? 'rgba(10,122,95,0.055)' : '#fff' }}>
      <span style={eyebrow}>{label}</span>
      <strong style={{ color: warning ? '#B45309' : accent ? '#0A7A5F' : 'var(--pc-text-primary, #0F1419)', fontSize: 12.5, lineHeight: 1.35, overflowWrap: 'anywhere' }}>{value}</strong>
    </div>
  );
}
...
const shell = { background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 24, padding: 18, display: 'grid', gap: 14, boxShadow: '0 14px 34px rgba(15,20,25,0.045)', minWidth: 0, overflow: 'hidden' } as const;
const eyebrow = { color: 'var(--pc-text-muted, #64748B)', fontSize: 11, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.07em' } as const;
const h2 = { margin: 0, color: 'var(--pc-text-primary, #0F1419)', fontSize: 'clamp(23px,5vw,32px)', lineHeight: 1.08, letterSpacing: '-0.035em', fontWeight: 950, overflowWrap: 'anywhere' } as const;
const lead = { margin: 0, color: 'var(--pc-text-secondary, #475569)', fontSize: 14, lineHeight: 1.55, overflowWrap: 'anywhere' } as const;
...
const stateCell = { minHeight: 82, display: 'grid', alignContent: 'start', gap: 6, border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 14, padding: 10, minWidth: 0 } as const;
const nextBox = { background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 16, padding: 12, display: 'grid', gap: 5, minWidth: 0 } as const;
...
const journal = { display: 'grid', gap: 8, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 12, minWidth: 0 } as const;
const journalRow = { display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 13, padding: 10, flexWrap: 'wrap', minWidth: 0 } as const;
