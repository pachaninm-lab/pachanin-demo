...
const ITEMS = [
  { title: 'Оркестрация сделки', status: 'Платформа', text: 'Сценарий должен вести сделку через цену, допуск, аукцион, исполнение, документы, деньги, спор и закрытие.', color: '#065F46', bg: '#D1FAE5' },
  { title: 'Идемпотентность шагов', status: 'Доработка', text: 'Каждый шаг должен иметь ключ идемпотентности, безопасный повтор и понятное состояние восстановления.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Ошибки и компенсации', status: 'Доработка', text: 'Ошибки, откаты и ручные разборы должны попадать в журнал и не ломать доказательность сделки.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Внешние шаги', status: 'Интеграция позже', text: 'Банк, ЭДО, КЭП, ФГИС и другие внешние шаги временно не подключены и не показываются как выполненные.', color: '#92400E', bg: '#FEF3C7' },
];
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 8, background: '#EFF6FF', border: '1px solid #BFDBFE', fontSize: 10, color: '#1E40AF', fontWeight: 760, lineHeight: 1.55 }}>
        Оркестратор сделки: настоящая платформа временно без интеграций. Статичные saga-инстансы, очереди, повторы и внешние шаги не показываются как работающий runtime.
      </div>
      ...
      <div style={{ fontSize: 9, color: '#94A3B8', padding: '4px 8px', borderRadius: 6, background: '#F8FAFB', border: '1px solid #E4E6EA' }}>
        Оркестратор должен усиливать исполнение сделки; внешние интеграции временно не подключены.
      </div>
    </div>
  );
}
