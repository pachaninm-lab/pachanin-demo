...
export const PLATFORM_V7_AUDIT_SURFACES: Record<AuditSurface, AuditSurfaceConfig> = {
  bank: {
    ...
    now: 'DL-9106: резерв виден, банковское подтверждение закрыто до документов, приёмки, качества и спора',
    money: 'резерв, сумма к подтверждению, удержание и деньги под риском показаны как разные состояния одной сделки',
    blocker: 'банковское подтверждение не разрешено без СДИЗ, ЭТрН, УПД, акта, качества и закрытого спора',
    evidence: 'основания подтверждения должны ссылаться на документы, рейс, приёмку, протокол и журнал действий',
    next: 'банк или оператор подтверждает условие проверки только после закрытия всех оснований',
    hidden: 'скрыто: кнопка выплаты без условий, имитация live-платежа, ручной обход без журнала',
    cta: 'Проверить условия подтверждения',
  ...
  documents: {
    ...
    now: 'пакет сделки не является архивом: каждый документ должен объяснять, что именно блокирует банковское основание',
    money: 'СДИЗ, ЭТрН, УПД, акт, КЭП и протокол качества прямо влияют на подтверждение, удержание или спор',
  ...
  disputes: {
    ...
    money: 'сумма под риском связана с удержанием или запретом банковского подтверждения по конкретной сделке',
