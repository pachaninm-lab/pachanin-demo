...
export const PLATFORM_V7_AUDIT_SURFACES: Record<AuditSurface, AuditSurfaceConfig> = {
  bank: {
    ...
    now: 'DL-9106: резерв виден, банковская проверка выплаты закрыт до документов, приёмки, качества и спора',
    ...
    hidden: 'скрыто: кнопка выплаты без условий, имитация подтверждённый внешний контур-платежа, ручной обход без журнала',
    cta: 'Проверить условия выпуска',
    href: '/platform-v7/bank/выпуск-safety',
  },
  documents: {
    title: 'Документный контур',
    tone: 'document',
    status: 'источник → ответственный → статус → влияние на деньги',
    now: 'пакет сделки не является архивом: каждый документ должен объяснять, что именно блокирует банковскую проверку выплаты',
  ...
  disputes: {
    ...
    money: 'сумма под риском связана с удержанием или запретом банковской проверки выплаты по конкретной сделке',
