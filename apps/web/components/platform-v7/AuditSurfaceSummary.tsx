...
export const PLATFORM_V7_AUDIT_SURFACES: Record<AuditSurface, AuditSurfaceConfig> = {
  bank: {
    ...
    tone: 'bank',
    status: 'нет ложного банковского подтверждения',
    now: 'DL-9106: резерв виден, банковская проверка основания закрыта до документов, приёмки, качества и спора',
    money: 'резерв, банковский шаг, удержание и деньги под риском показаны как разные состояния одной сделки',
    blocker: 'банковский шаг не разрешён без СДИЗ, ЭТрН, УПД, акта, качества и закрытого спора',
    evidence: 'основания банковского шага должны ссылаться на документы, рейс, приёмку, протокол и журнал действий',
    next: 'банк или оператор подтверждает условие банковского шага только после закрытия всех оснований',
    hidden: 'скрыто: кнопка оплаты без условий, имитация подтверждённого внешнего контура платежа, ручной обход без журнала',
    cta: 'Проверить условия банка',
  ...
  documents: {
    ...
    now: 'пакет сделки не является архивом: каждый документ должен объяснять, что именно блокирует банковскую проверку основания',
    money: 'СДИЗ, ЭТрН, УПД, акт, КЭП и протокол качества прямо влияют на банковский шаг, удержание или спор',
  ...
  disputes: {
    ...
    now: 'спор показывает причину, сумму влияния, срок реакции, владельца, доказательства и следующий шаг',
    money: 'сумма под риском связана с удержанием или запретом банковской проверки основания по конкретной сделке',
