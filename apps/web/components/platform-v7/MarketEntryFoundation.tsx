...
export function MarketEntryFoundation() {
  ...
  return <main data-testid='platform-v7-market-entry-page' style={{ display: 'grid', gap: 16 }}>
    ...
    <section style={grid}><article style={box}><h2>Цена с источником</h2><select style={input} value={priceId} onChange={(e) => setPriceId(e.target.value)}>{MARKET_PRICE_RECORDS.map((item) => <option key={item.id} value={item.id}>{item.crop}</option>)}</select><select style={input} value={routeId} onChange={(e) => setRouteId(e.target.value)}>{MARKET_ROUTE_QUOTES.map((item) => <option key={item.id} value={item.id}>{item.from} → {item.to}</option>)}</select><p>{price.crop}: {fmt(price.pricePerTon)} ₽/т. Источник: {price.sourceName}, {price.observedAt}.</p><p>Логистика: {fmt(marketLogisticsCostPerTon(route))} ₽/т. Цена до точки: {fmt(delivered)} ₽/т.</p></article><article style={box}><h2>Намерение</h2><select style={input} value={side} onChange={(e) => setSide(e.target.value as MarketSide)}><option value='sell'>Продать</option><option value='buy'>Купить</option></select><input style={input} value={volume} onChange={(e) => setVolume(e.target.value)} /><button style={button} onClick={() => setIntent({ side, volume })}>Создать намерение</button>{intent ? <p>{intent.side === 'sell' ? 'Продажа' : 'Покупка'} · {price.crop} · {intent.volume} т · <Link href={marketIntentTargetHref(intent.side)}>следующий шаг</Link></p> : <p style={muted}>Пока не создано. После durable store это станет рабочей заявкой.</p>}</article><article style={box}><h2>Gate готовности: {marketReadinessScore(gate)}%</h2>{gate.map((item) => <p key={item.id}><b>{item.label}</b><br />{marketEntryStatusLabel(item.status)} · {item.note}</p>)}</article></section>
