...
function PreviewField({ label, value, testId }: { label: string; value: string; testId?: string }) {
  return (
    <div
      data-testid={testId}
      style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 12, padding: 10, display: 'grid', gap: 5 }}
    >
      <div style={micro}>{label}</div>
      <div style={{ color: '#0F1419', fontSize: 13, lineHeight: 1.45 }}>{value}</div>
    </div>
  );
}
    ...
    >
      <div style={{ display: 'grid', gap: 6 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <span style={{ color: '#0F1419', fontSize: 17, fontWeight: 950, lineHeight: 1.1 }}>
            {preview.actionLabel}
          </span>
          <span
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 8 }}>
        ...
        >
          ...
          <div style={{ color: '#0F1419', fontSize: 13, fontWeight: 900 }}>{preview.responsibleRole}</div>
        </div>
        <div style={{ background: '#fff', border: '1px solid #E4E6EA', borderRadius: 12, padding: 10, display: 'grid', gap: 5 }}>
          ...
          <div style={{ color: idempotencyColor, fontSize: 13, fontWeight: 900 }}>{preview.idempotencyLabel}</div>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ color: '#334155', fontSize: 13, lineHeight: 1.45 }}>{preview.externalConfirmationBoundary}</div>
      </div>
