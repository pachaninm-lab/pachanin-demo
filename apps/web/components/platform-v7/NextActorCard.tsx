...
export function NextActorCard({ actor, action }: Props) {
  return (
    <div style={{ background: 'rgba(180,83,9,0.06)', border: '1px solid rgba(180,83,9,0.18)', borderRadius: 14, padding: 14, display: 'grid', gap: 5 }}>
      ...
      <p style={{ margin: 0, color: 'var(--pc-text-primary, #0F1419)', fontSize: 14, fontWeight: 900 }}>{actor}</p>
      {action && <p style={{ margin: 0, color: 'var(--pc-text-secondary, #475569)', fontSize: 12, lineHeight: 1.4 }}>{action}</p>}
    </div>
  );
}
