...
const ITEMS = [
  { title: 'Рейс связан со сделкой', status: 'Платформа', text: 'Контур маршрута должен обслуживать исполнение сделки и фиксировать следующий шаг по рейсу.', color: '#065F46', bg: '#D1FAE5' },
  { title: 'Контрольные точки', status: 'Доработка', text: 'Точки маршрута должны формироваться из подтверждённых адресов и условий сделки.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'События рейса', status: 'Доработка', text: 'События должны попадать в доказательный контур только после проверки источника.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Внешние источники', status: 'Интеграция позже', text: 'Внешние источники движения временно не подключены и не показываются как работающие.', color: '#92400E', bg: '#FEF3C7' },
];
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 8, background: '#EFF6FF', border: '1px solid #BFDBFE', fontSize: 10, color: '#1E40AF', fontWeight: 760, lineHeight: 1.55 }}>
        Контур маршрута: настоящая платформа временно без интеграций. Статусы движения, события рейса и внешние источники не показываются как подключённые.
      </div>
      ...
      <div style={{ fontSize: 9, color: '#94A3B8', padding: '4px 8px', borderRadius: 6, background: '#F8FAFB', border: '1px solid #E4E6EA' }}>
        Контур маршрута должен усиливать исполнение сделки; внешние интеграции временно не подключены.
      </div>
    </div>
  );
}
