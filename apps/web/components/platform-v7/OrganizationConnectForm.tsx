...
export function OrganizationConnectForm({ locale }: { locale: string }) {
  ...
  return <section id='connect-organization' className={styles.section} aria-labelledby='connect-title'>
    ...
    <form className={styles.form} onSubmit={onSubmit} noValidate>
      <label><span>{copy.organization}</span><input name='organization' autoComplete='organization' required /></label>
      <label><span>{copy.inn}</span><input name='inn' inputMode='numeric' pattern='[0-9]{10}|[0-9]{12}' required /></label>
      <label><span>{copy.name}</span><input name='name' autoComplete='name' required /></label>
      <label><span>{copy.position}</span><input name='position' autoComplete='organization-title' required /></label>
      <label><span>{copy.phone}</span><input name='phone' type='tel' autoComplete='tel' required /></label>
      <label><span>{copy.email}</span><input name='email' type='email' autoComplete='email' required /></label>
      <label><span>{copy.role}</span><select name='role' required defaultValue=''><option value='' disabled>—</option>{copy.roles.map((role) => <option key={role}>{role}</option>)}</select></label>
      <label><span>{copy.scenario}</span><select name='scenario' required defaultValue=''><option value='' disabled>—</option>{copy.scenarios.map((scenario) => <option key={scenario}>{scenario}</option>)}</select></label>
      <label className={styles.consent}><input name='consent' type='checkbox' required /><span>{copy.consent}</span></label>
      {error ? <p className={styles.error} role='alert'>{error}</p> : null}
      <div className={styles.actions}>
        <button type='submit'>{copy.submit}<ArrowRight size={18}/></button>
