...
export const PLATFORM_V7_ROLE_EXECUTION_SUMMARIES: Record<PlatformV7ExecutionRole, RoleExecutionSummaryConfig> = {
  seller: {
    ...
    tone: 'success',
    mode: 'commercial',
    now: 'лот LOT-2403: победитель выбран, сделка DL-9106 создана, банковское основание ещё не закрыто',
  ...
  buyer: {
    ...
    execution: 'ставка → резерв → логистика → приёмка → основание / удержание',
  ...
  bank: {
    ...
    blocked: 'статус денег останавливают документы, спор или ручная проверка',
    money: 'резерв, основание к подтверждению и удержание показаны как части одного контура',
    documents: 'основания банковского подтверждения видны через пакет сделки',
    execution: 'рейс и приёмка учитываются только как основание',
    next: 'банк или оператор',
    cta: 'Открыть проверку основания',
  ...
  arbitrator: {
    ...
    execution: 'решение влияет на удержание или статус денег',
  ...
  compliance: {
    ...
    money: 'комплаенс-проверка может блокировать банковское подтверждение',
  ...
  operator: {
    ...
    money: 'к подтверждению, под удержанием и под риском сверяются по сделкам',
