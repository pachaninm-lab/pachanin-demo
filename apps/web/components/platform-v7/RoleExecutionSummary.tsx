...
export function RoleExecutionSummary({ role }: { role: PlatformV7ExecutionRole }) {
  ...
  const headline = role === 'driver' ? `${summary.title}: маршрут, документы рейса, событие и следующий шаг` : `${summary.title}: деньги, документы, груз, причина остановки и следующий шаг`;
  ...
  return (
    <section data-testid={`role-execution-summary-${role}`} aria-label={`Рабочая сводка роли: ${summary.title}`} style={{ background: PLATFORM_V7_TOKENS.color.surface, border: `1px solid ${PLATFORM_V7_TOKENS.color.border}`, borderRadius: PLATFORM_V7_TOKENS.radius.xl, padding: PLATFORM_V7_TOKENS.spacing.lg, display: 'grid', gap: PLATFORM_V7_TOKENS.spacing.md, boxShadow: PLATFORM_V7_TOKENS.shadow.soft }}>
      ...
      <div data-testid="platform-v7-role-workspace-hint" style={{ border: `1px solid ${tone.border}`, background: tone.bg, borderRadius: PLATFORM_V7_TOKENS.radius.lg, padding: PLATFORM_V7_TOKENS.spacing.sm, display: 'flex', gap: PLATFORM_V7_TOKENS.spacing.sm, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
        <div style={{ fontSize: 13, lineHeight: 1.45, color: PLATFORM_V7_TOKENS.color.textPrimary, fontWeight: 760 }}>Рабочий экран роли: {summary.title}. Основное действие — {summary.cta.toLowerCase()}. Никаких лишних данных за пределами роли.</div>
