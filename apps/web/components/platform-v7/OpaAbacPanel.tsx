...
const ITEMS = [
  { title: 'Политики вокруг сделки', status: 'Платформа', text: 'Контур доступа должен защищать сделку, документы, логистику, деньги, спор и доказательства.', color: '#065F46', bg: '#D1FAE5' },
  { title: 'Атрибуты доступа', status: 'Доработка', text: 'Решение должно учитывать организацию, роль, участие в сделке, объект и конкретное действие.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Серверное исполнение', status: 'Доработка', text: 'Политика доступа должна исполняться на сервере, а не подменяться клиентским состоянием.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Журнал решений', status: 'Доработка', text: 'Разрешённые и запрещённые действия должны попадать в проверяемый журнал платформы.', color: '#1E40AF', bg: '#DBEAFE' },
];
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 8, background: '#EFF6FF', border: '1px solid #BFDBFE', fontSize: 10, color: '#1E40AF', fontWeight: 760, lineHeight: 1.55 }}>
        Политики доступа: настоящая платформа временно без интеграций. Статичные политики, решения и журналы не показываются как действующий runtime.
      </div>
      ...
      <div style={{ fontSize: 9, color: '#94A3B8', padding: '4px 8px', borderRadius: 6, background: '#F8FAFB', border: '1px solid #E4E6EA' }}>
        Контур политик должен защищать исполнение сделки; доступ не должен определяться клиентом.
      </div>
    </div>
  );
}
