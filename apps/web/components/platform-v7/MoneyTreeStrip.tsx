...
export function MoneyTreeStrip() {
  ...
  return (
    <section data-testid="platform-v7-money-tree-strip" style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
          ...
          return (
            <div key={bucket.key} style={{ display: 'grid', gap: 5 }} title={bucket.formula}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
                <span style={{ fontSize: 13, fontWeight: 850, color: 'var(--pc-text-primary, #0F1419)' }}>{bucket.label}</span>
                <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--pc-text-muted, #64748B)' }}>{formatCompactMoney(bucket.amount)} · {bucket.dealIds.length} сделок</span>
              </div>
