...
const COPY: Record<Locale, Copy> = {
  ru: {
    ...
    role: {
      ...
      scenarios: {
        ...
        employee: { tab: 'Сотрудник подключённой организации', scope: 'Задачи и данные своей организации в пределах назначенных полномочий', blocker: 'Задача организации требует относящегося подтверждения.', impact: 'Влияние на расчёт определяется назначенной ролью и условиями Сделки.', action: 'Показать разрешённый следующий шаг в пределах полномочий организации.', evidence: ['Назначенная роль', 'Полномочия организации', 'Связанные документы', 'События Сделки'] },
      },
    },
    documents: {
  ...
  en: {
    ...
    role: { eyebrow: 'Role-aware explanation', title: 'One Deal, nine understandable public perspectives', lead: 'Choosing a role here only changes the explanation. Real role and access are determined server-side after registration and organisation verification.', scope: 'What matters to the role', blocker: 'What blocks progress', impact: 'What changes', action: 'Next step', evidence: 'Evidence used', scenarios: {
      ...
      employee: { tab: 'Employee of a connected organisation', scope: 'Own organisation tasks and data within assigned authority', blocker: 'An organisation task requires relevant confirmation.', impact: 'Settlement impact follows the assigned role and Deal terms.', action: 'Show the next permitted step within organisation authority.', evidence: ['Assigned role', 'Organisation authority', 'Linked documents', 'Deal events'] },
    } },
  ...
  zh: {
    ...
    role: { eyebrow: '角色化说明', title: '一笔交易，九个清晰的公开视角', lead: '这里选择角色只改变说明方式。真实角色和访问权限在注册并完成机构核验后由服务器确定。', scope: '角色关注点', blocker: '阻塞原因', impact: '影响', action: '下一步', evidence: '使用的依据', scenarios: {
      ...
      employee: { tab: '已接入机构员工', scope: '仅限已分配权限内的本机构任务和数据', blocker: '机构任务需要相关确认。', impact: '对结算的影响由已分配角色和交易条件决定。', action: '显示机构权限范围内允许的下一步。', evidence: ['已分配角色', '机构权限', '关联文件', '交易事件'] },
    } },
