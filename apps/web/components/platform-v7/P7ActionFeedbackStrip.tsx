...
export function P7ActionFeedbackStrip() {
  return (
    <section data-testid="platform-v7-action-feedback-strip" style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
      <div>
        <div style={{ fontSize: 11, color: 'var(--pc-text-muted, #64748B)', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Результат действия · тестовый режим</div>
        <div style={{ marginTop: 4, fontSize: 22, lineHeight: 1.12, fontWeight: 950, color: 'var(--pc-text-primary, #0F1419)' }}>Результат действия и следующий шаг</div>
        <div style={{ marginTop: 6, fontSize: 13, color: 'var(--pc-text-muted, #64748B)', lineHeight: 1.55 }}>Каждое действие должно показывать понятный результат, следующий шаг и запись для журнала. Ниже — единый контракт для старых и новых кнопок.</div>
      </div>
            ...
            <article key={item.feedback.actionId} style={{ border: '1px solid #EEF1F4', borderRadius: 14, padding: 12, background: '#F8FAFB', display: 'grid', gap: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div style={{ marginTop: 3, fontSize: 14, fontWeight: 900, color: 'var(--pc-text-primary, #0F1419)' }}>{item.feedback.actionId}</div>
                </div>
              ...
              <div style={{ fontSize: 12, lineHeight: 1.45, color: 'var(--pc-text-secondary, #334155)' }}>{item.feedback.message}</div>
              <div style={{ fontSize: 12, lineHeight: 1.45, color: 'var(--pc-text-secondary, #334155)', fontWeight: 750 }}>Следующий шаг: {item.nextStep}</div>
              <div style={{ fontSize: 11, lineHeight: 1.45, color: 'var(--pc-text-muted, #64748B)' }}>Журнал: {item.journal.actor} · {item.journal.severity}</div>
            </article>
          );
        })}
      </div>
