...
export function P7EvidenceProjectionPanel({ disputeId }: { disputeId: string }) {
  ...
  return (
    <section style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
      <div>
        <div style={{ fontSize: 16, fontWeight: 900, color: 'var(--pc-text-primary)' }}>Доказательная сводка</div>
        <div style={{ marginTop: 5, fontSize: 12, color: 'var(--pc-text-muted)', lineHeight: 1.5 }}>
          Readiness пакета связан с timeline сделки и журналом оператора. Срез работает только на управляемый запуск data layer.
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 10 }}>
        {cell('Оценка доказательств', projection.evidenceScore)}
        {cell('События линии времени', projection.summary.timelineEvents)}
        {cell('Persistence events', projection.summary.persistenceEvents)}
        {cell('Action log', projection.summary.actionLogEntries)}
      </div>

      <div style={{ display: 'grid', gap: 10 }}>
        <div style={{ fontSize: 13, fontWeight: 900, color: 'var(--pc-text-primary)' }}>Ожидаемая линия событий</div>
      ...
      <P7ActionLog title='Журнал доказательств actions' entries={projection.operatorActionLog} maxEntries={4} />
    </section>
  );
}
