...
const COPY = {
  ru: {
    ...
    sources: {
      ...
      argus: { name: 'ФГИС «Аргус-Фито»', checks: 'Фитосанитарные документы, акты контроля, заключения и карантинные зоны', capability: 'Публичная проверка подлинности документа и его статуса', impact: 'Недействительный или отсутствующий документ может ограничить маршрут и приёмку.', integrationMode: 'PUBLIC_REGISTRY / OFFICIAL_ACCESS_REQUIRED', status: 'PUBLIC_REGISTRY', statusLabel: 'Доступна публичная проверка', freshness: 'Проверка конкретного документа не выполнялась', limitation: 'Действия от имени организации требуют официального доступа; пароли ЕСИА не передаются Гекте.' },
      accreditation: { name: 'Росаккредитация', checks: 'Статус лаборатории, область аккредитации, сертификат или декларация', capability: 'Проверка действующего статуса и соответствия области исследования', impact: 'Приостановление или несоответствие области влияет на доказательную силу протокола.', integrationMode: 'PUBLIC_REGISTRY / OFFICIAL_EXTRACT', status: 'PUBLIC_REGISTRY', statusLabel: 'Доступна публичная проверка', freshness: 'Проверка конкретной лаборатории не выполнялась', limitation: 'Гекта не выполняет запись в реестры; сохраняется только проверяемая выписка или ссылка.' },
  ...
  zh: {
    ...
    sources: {
      ...
      argus: { name: 'FGIS Argus-Fito', checks: '植物检疫文件、检查记录、结论和隔离区', capability: '公开验证文件真实性和状态', impact: '无效或缺失文件可能限制路线和验收。', integrationMode: 'PUBLIC_REGISTRY / OFFICIAL_ACCESS_REQUIRED', status: 'PUBLIC_REGISTRY', statusLabel: '可进行公开验证', freshness: '尚未检查具体文件', limitation: '代表组织的操作需要正式访问；Gekta 不接收 ESIA 密码。' },
      accreditation: { name: '俄罗斯联邦认可署', checks: '实验室状态、认可范围、证书或声明', capability: '检查有效状态和研究范围', impact: '暂停或范围不符会影响证据效力。', integrationMode: 'PUBLIC_REGISTRY / OFFICIAL_EXTRACT', status: 'PUBLIC_REGISTRY', statusLabel: '可进行公开验证', freshness: '尚未检查具体实验室', limitation: 'Gekta 不写入登记系统，只保存可核验的摘录或链接。' },
