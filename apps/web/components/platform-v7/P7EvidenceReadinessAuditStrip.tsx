...
function toneToken(tone: 'success' | 'warning' | 'danger' | 'neutral') {
  ...
  return { label: 'Зафиксирован', background: '#F8FAFB', border: 'var(--pc-border, #E4E6EA)', color: 'var(--pc-text-secondary, #475569)' };
}
...
export function P7EvidenceReadinessAuditStrip() {
  ...
  return (
    <section data-testid="evidence-readiness-audit-strip" style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <div>
          <div style={{ fontSize: 20, lineHeight: 1.2, fontWeight: 900, color: 'var(--pc-text-primary, #0F1419)' }}>Готовность доказательств</div>
          ...
          return (
            <article key={row.disputeId} data-testid={`evidence-readiness-audit-row-${rowKey}`} style={{ display: 'grid', gap: 10, padding: 14, borderRadius: 14, background: token.background, border: `1px solid ${token.border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 13, fontWeight: 900, color: 'var(--pc-text-primary, #0F1419)' }}>{row.disputeId}</div>
              ...
              <div style={{ fontSize: 14, lineHeight: 1.45, fontWeight: 900, color: 'var(--pc-text-primary, #0F1419)' }}>{row.scoreLabel} · {row.requiredLabel}</div>
              <div style={{ fontSize: 12, lineHeight: 1.45, color: 'var(--pc-text-secondary, #475569)' }}>{row.totalLabel}</div>

              <div style={{ display: 'grid', gap: 6 }}>
                {(row.blockers.length ? row.blockers : ['Нет блокеров']).slice(0, 4).map((label) => (
                  <div key={label} style={{ fontSize: 12, lineHeight: 1.45, color: 'var(--pc-text-secondary, #475569)' }}>• {label.replace(/\bblockers\b/gi, 'блокеров').replace(/\bblocker\b/gi, 'блокер')}</div>
                ))}
              </div>
            </article>
          );
