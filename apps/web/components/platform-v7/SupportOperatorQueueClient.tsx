...
export function SupportOperatorQueueClient() {
  const params = useSearchParams();
  const { cases, updateCaseStatus } = useSupportCases();
  const priority = params.get('priority') as SupportPriority | null;
  const category = params.get('category') as SupportCategory | null;
  const entity = params.get('entity') as SupportRelatedEntityType | null;
  const moneyOnly = params.get('money') === 'risk';
  const todayOnly = params.get('sla') === 'today';

  const rows = supportSortCases(cases).filter((item) => {
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 1220, margin: '0 auto' }}>
      <section style={{ ...card, display: 'grid', gap: 8 }}>
        ...
        <p style={muted}>Оператор видит срок реакции, приоритет, категорию, роль, объект, сумму риска, блокер и следующий шаг. Действия оператора меняют состояние обращения и оставляют запись в журнале.</p>
      </section>
        ...
        {selected ? (
          <aside style={{ ...card, display: 'grid', gap: 12, alignContent: 'start' }}>
            ...
            <div style={{ borderTop: '1px solid var(--pc-border, #E4E6EA)', paddingTop: 12, display: 'grid', gap: 8 }}>
              ...
              <button onClick={() => updateCaseStatus(selected.id, 'accepted', 'Оператор поддержки', 'Обращение принято в работу.')} style={pill}>Принять в работу</button>
              <button onClick={() => updateCaseStatus(selected.id, 'waiting_user', 'Оператор поддержки', 'Запрошены дополнительные данные.')} style={pill}>Запросить данные</button>
              <button onClick={() => updateCaseStatus(selected.id, 'escalated', 'Оператор поддержки', 'Обращение эскалировано ответственному контуру.')} style={pill}>Эскалировать</button>
              <button onClick={() => updateCaseStatus(selected.id, 'resolved', 'Оператор поддержки', 'Решение подготовлено.')} style={pill}>Подготовить решение</button>
            </div>
            <div style={{ borderTop: '1px solid var(--pc-border, #E4E6EA)', paddingTop: 12, display: 'grid', gap: 8 }}>
              <b>Шаблоны ответа</b>
