...
const ITEMS = [
  { title: 'Безопасная сделка связана со сделкой', status: 'Платформа', text: 'Финансовый контур должен быть привязан к сделке, документам, приёмке, качеству, спору и закрытию.', color: '#065F46', bg: '#D1FAE5' },
  { title: 'Условия release', status: 'Доработка', text: 'Условия передачи основания банку должны быть формальными, проверяемыми и журналируемыми.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Удержание и спор', status: 'Доработка', text: 'Частичные удержания, блокировки и спор должны иметь источник, ответственного и доказательства.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Банковское подключение', status: 'Интеграция позже', text: 'Банк, номинальный счёт, резервирование и callbacks временно не подключены и не показываются как работающие.', color: '#92400E', bg: '#FEF3C7' },
];
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 8, background: '#EFF6FF', border: '1px solid #BFDBFE', fontSize: 10, color: '#1E40AF', fontWeight: 760, lineHeight: 1.55 }}>
        Безопасная сделка: настоящая платформа временно без интеграций. Счета, суммы, банковские ссылки, выпуск средств и внешние подтверждения не показываются как действующие.
      </div>
      ...
      <div style={{ fontSize: 9, color: '#94A3B8', padding: '4px 8px', borderRadius: 6, background: '#F8FAFB', border: '1px solid #E4E6EA' }}>
        Финансовый контур должен усиливать юридическую исполнимость сделки; внешние интеграции временно не подключены.
      </div>
    </div>
  );
}
