...
function applySupportNote() {
  document.querySelectorAll<HTMLElement>('.p7-support-chat-form small').forEach((node) => {
    ...
    node.dataset.p7NotePatched = 'yes';
  });
}

export function SupportNoteClientPatch() {
  React.useEffect(() => {
    applySupportNote();
    const observer = new MutationObserver(applySupportNote);
