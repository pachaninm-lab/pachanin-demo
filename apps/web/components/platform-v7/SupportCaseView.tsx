  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 1120, margin: '0 auto' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 12 }}>
        <div style={card}><div style={muted}>Объект</div><b>{supportObjectLabel(item)}</b></div><div style={card}><div style={muted}>SLA</div><b>{supportSlaLabel(item)} · {dt(item.slaDueAt)}</b></div>{showOwner ? <div style={card}><div style={muted}>Ответственный</div><b>{item.owner}</b></div> : null}{showMoney ? <div style={card}><div style={muted}>Деньги под риском</div><b>{supportFormatRub(item.moneyAtRiskRub)}</b></div> : null}
      </section>
