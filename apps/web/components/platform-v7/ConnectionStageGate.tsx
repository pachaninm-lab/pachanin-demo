...
export function ConnectionStageGate({ entry, context }: { entry: ContactVaultEntry; context: ContactRevealContext }) {
  ...
  return (
    <div style={{ border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 16, padding: 12, display: 'grid', gap: 6, background: '#fff' }}>
      <strong style={{ color: 'var(--pc-text-primary, #0F1419)', fontSize: 14 }}>Связь со стороной</strong>
      ...
      <span style={{ color: 'var(--pc-text-muted, #64748B)', fontSize: 12 }}>{decision.allowed ? `Тип данных: ${entry.contactType}` : `Причина: ${decision.reason ?? 'проверка не пройдена'}`}</span>
    </div>
  );
}
