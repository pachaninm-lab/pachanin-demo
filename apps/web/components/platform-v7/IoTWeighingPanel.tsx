...
const ITEMS = [
  { title: 'Вес как событие сделки', status: 'Платформа', text: 'Вес должен быть связан с рейсом, приёмкой, качеством, документами и основанием для расчёта.', color: '#065F46', bg: '#D1FAE5' },
  { title: 'Акт взвешивания', status: 'Доработка', text: 'Акт должен формироваться из подтверждённого события, без статичных номеров и ручной имитации.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Расхождения', status: 'Доработка', text: 'Отклонения по весу должны становиться блокером сделки и попадать в доказательный контур.', color: '#1E40AF', bg: '#DBEAFE' },
  { title: 'Весовое оборудование', status: 'Интеграция позже', text: 'Подключение весов, протоколов, фотофиксации и подписи требует отдельной технической приёмки.', color: '#92400E', bg: '#FEF3C7' },
];
  ...
  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ padding: '8px 12px', borderRadius: 8, background: '#EFF6FF', border: '1px solid #BFDBFE', fontSize: 10, color: '#1E40AF', fontWeight: 760, lineHeight: 1.55 }}>
        Весовой контур: настоящая платформа временно без интеграций. Акты, фактический вес, фото, операторы, устройства и подписи не показываются как подключённые без технической приёмки.
      </div>
      ...
      <div style={{ fontSize: 9, color: '#94A3B8', padding: '4px 8px', borderRadius: 6, background: '#F8FAFB', border: '1px solid #E4E6EA' }}>
        Весовой контур должен усиливать исполнение сделки; внешние интеграции временно не подключены.
      </div>
    </div>
  );
}
