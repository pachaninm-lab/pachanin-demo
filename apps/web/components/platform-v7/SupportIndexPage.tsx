...
function intro(role: PlatformRole) {
  if (fieldRoles.has(role)) return { title: role === 'driver' ? 'Помощь по моему рейсу' : 'Помощь по моей проверке', text: 'Только обращения по рейсу, документу, фото, пломбе, весу, пробе или задержке. Деньги, банк и операторские очереди скрыты.', money: false, operator: false };
  ...
  return { title: 'Центр поддержки исполнения сделки', text: 'Обращения привязаны к сделке, документу, рейсу, деньгам, спору или блокеру. Детали открываются отдельно.', money: true, operator: operatorRoles.has(role) };
}
...
export function SupportIndexPage() {
  ...
  return (
    <div data-testid='platform-v7-support-page' style={{ display: 'grid', gap: 16, maxWidth: 1180, margin: '0 auto' }}>
      <style>{`@media(max-width:767px){[data-testid='platform-v7-support-page']{gap:12px!important}.p7-support-hero{padding:16px!important;border-radius:24px!important}.p7-support-hero h1{font-size:clamp(28px,8vw,38px)!important;line-height:1.04!important}.p7-support-create{width:100%!important;justify-content:center!important;min-height:52px!important}.p7-support-metrics{grid-template-columns:1fr 1fr!important;gap:8px!important}.p7-support-metrics>div:nth-child(n+3){display:none!important}.p7-support-case{padding:13px!important;border-radius:16px!important}.p7-support-case-grid{grid-template-columns:1fr!important}.p7-support-case-grid>div:nth-child(3){display:none!important}}`}</style>
      ...
      <section style={{ ...card, display: 'grid', gap: 12 }}>
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {sortedCases.map((item) => (
            <article className='p7-support-case' key={item.id} style={{ border: '1px solid var(--pc-border,#E4E6EA)', borderRadius: 16, padding: 14, background: 'var(--pc-bg-elevated,rgba(15,20,25,.02))' }}>
              <Link href={`/platform-v7/support/detail?id=${encodeURIComponent(item.id)}&role=${role}`} style={{ color: 'inherit', textDecoration: 'none' }}>
                <div className='p7-support-case-grid' style={{ display: 'grid', gridTemplateColumns: 'minmax(220px,1.2fr) minmax(180px,.8fr) minmax(180px,.8fr)', gap: 12 }}>
                  <div style={{ display: 'grid', gap: 7 }}><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}><span style={pill}>{item.id}</span><span style={pill}>{SUPPORT_PRIORITY_LABELS[item.priority]}</span><span style={pill}>{SUPPORT_CATEGORY_LABELS[item.category]}</span></div><b>{item.title}</b></div>
