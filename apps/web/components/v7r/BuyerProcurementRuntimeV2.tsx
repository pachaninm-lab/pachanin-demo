...
function Badge({ text, tone='neutral' }: { text: string; tone?: 'neutral'|'success'|'warning' }) {
  const styles = tone === 'success' ? { background:'rgba(10,122,95,0.08)', border:'1px solid rgba(10,122,95,0.16)', color:'#0A7A5F' } : tone === 'warning' ? { background:'rgba(217,119,6,0.08)', border:'1px solid rgba(217,119,6,0.16)', color:'#B45309' } : { background:'#F8FAFB', border:'1px solid var(--pc-border, #E4E6EA)', color:'var(--pc-text-secondary, #475569)' };
    ...
    <div style={{ display:'grid', gap:18, padding:'8px 0' }}>
      <section style={{ background:'#fff', border:'1px solid var(--pc-border, #E4E6EA)', borderRadius:18, padding:18 }}>
        <div style={{ display:'flex', justifyContent:'space-between', gap:14, flexWrap:'wrap', alignItems:'flex-start' }}>
          <div>
            <div style={{ fontSize:28, lineHeight:1.15, fontWeight:800, color:'var(--pc-text-primary, #0F1419)' }}>Закупка и отбор вариантов</div>
            <div style={{ fontSize:13, color:'var(--pc-text-muted, #6B778C)', lineHeight:1.7, marginTop:8, maxWidth:920 }}>Запросы закупки, отобранные варианты и черновики сделок сохраняются внутри устойчивого runtime-контура. После обновления страницы сценарий не теряется.</div>
          </div>
      ...
      <section style={{ background:'#fff', border:'1px solid var(--pc-border, #E4E6EA)', borderRadius:18, padding:18, display:'grid', gap:12 }}>
        <div style={{ fontSize:18, fontWeight:800, color:'var(--pc-text-primary, #0F1419)' }}>Текущее действие</div>
        <div style={{ padding:14, borderRadius:14, background:'rgba(10,122,95,0.08)', border:'1px solid rgba(10,122,95,0.16)', fontSize:14, color:'var(--pc-text-primary, #0F1419)', lineHeight:1.7 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'minmax(0, 1fr) minmax(0, 1fr)', gap:16 }}>
        <section style={{ background:'#fff', border:'1px solid var(--pc-border, #E4E6EA)', borderRadius:18, padding:18, display:'grid', gap:14 }}>
          <div><div style={{ fontSize:18, fontWeight:800, color:'var(--pc-text-primary, #0F1419)' }}>Создать запрос закупки</div><div style={{ fontSize:13, color:'var(--pc-text-muted, #6B778C)', marginTop:4 }}>Покупатель формирует параметры партии и создаёт свой запрос.</div></div>
          <div style={{ display:'grid', gap:12 }}>
            <input value={grain} onChange={(e)=>setGrain(e.target.value)} placeholder='Культура' style={{ padding:'12px 14px', borderRadius:12, border:'1px solid var(--pc-border, #E4E6EA)', fontSize:14 }} />
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              <input value={volume} onChange={(e)=>setVolume(e.target.value)} inputMode='decimal' placeholder='Объём, т' style={{ padding:'12px 14px', borderRadius:12, border:'1px solid var(--pc-border, #E4E6EA)', fontSize:14 }} />
              <input value={targetPrice} onChange={(e)=>setTargetPrice(e.target.value)} inputMode='decimal' placeholder='Целевая цена, ₽/т' style={{ padding:'12px 14px', borderRadius:12, border:'1px solid var(--pc-border, #E4E6EA)', fontSize:14 }} />
            </div>
            <input value={region} onChange={(e)=>setRegion(e.target.value)} placeholder='Регион' style={{ padding:'12px 14px', borderRadius:12, border:'1px solid var(--pc-border, #E4E6EA)', fontSize:14 }} />
            <input value={quality} onChange={(e)=>setQuality(e.target.value)} placeholder='Качество' style={{ padding:'12px 14px', borderRadius:12, border:'1px solid var(--pc-border, #E4E6EA)', fontSize:14 }} />
            <input value={payment} onChange={(e)=>setPayment(e.target.value)} placeholder='Оплата' style={{ padding:'12px 14px', borderRadius:12, border:'1px solid var(--pc-border, #E4E6EA)', fontSize:14 }} />
          </div>
        ...
        <section style={{ background:'#fff', border:'1px solid var(--pc-border, #E4E6EA)', borderRadius:18, padding:18, display:'grid', gap:14 }}>
          <div><div style={{ fontSize:18, fontWeight:800, color:'var(--pc-text-primary, #0F1419)' }}>Последние черновики сделок</div><div style={{ fontSize:13, color:'var(--pc-text-muted, #6B778C)', marginTop:4 }}>Черновики появляются сразу после выбора варианта.</div></div>
          <div style={{ display:'grid', gap:10 }}>
            {draftDeals.length === 0 ? <div style={{ padding:14, borderRadius:14, background:'#F8FAFB', border:'1px solid var(--pc-border, #E4E6EA)', fontSize:13, color:'var(--pc-text-muted, #6B778C)' }}>Пока нет черновиков сделок.</div> : draftDeals.slice(0,3).map((item) => (
      ...
      <section style={{ background:'#fff', border:'1px solid var(--pc-border, #E4E6EA)', borderRadius:18, padding:18, display:'grid', gap:14 }}>
        <div style={{ display:'flex', justifyContent:'space-between', gap:14, flexWrap:'wrap', alignItems:'center' }}>
          <div><div style={{ fontSize:18, fontWeight:800, color:'var(--pc-text-primary, #0F1419)' }}>Источники предложений</div><div style={{ fontSize:13, color:'var(--pc-text-muted, #6B778C)', marginTop:4 }}>Рынок и внутренние запросы объединены в один контур отбора.</div></div>
        ...
        <div style={{ display:'grid', gap:10 }}>
          {visibleMarket.map((item) => {
            ...
            return <div key={item.id} style={{ background:'#fff', border:'1px solid var(--pc-border, #E4E6EA)', borderRadius:16, padding:16, display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:12, alignItems:'center' }}><div><div style={{ fontFamily:'JetBrains Mono, monospace', fontWeight:800, color:'#0A7A5F', fontSize:13 }}>{item.id}</div><div style={{ fontSize:15, fontWeight:800, color:'var(--pc-text-primary, #0F1419)', marginTop:4 }}>{item.grain}</div><div style={{ fontSize:12, color:'var(--pc-text-muted, #6B778C)', marginTop:4 }}>{item.volume} т · {item.region}</div></div><div style={{ fontSize:14, fontWeight:700 }}>{formatMoney(item.price)} / т</div><div style={{ fontSize:12, color:'var(--pc-text-muted, #6B778C)' }}>{item.quality}</div><div style={{ fontSize:12, color:'var(--pc-text-muted, #6B778C)' }}>{item.payment}</div><div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>{shortlistButton(item.id)}<button onClick={() => { const created = createDraftDealFromMarket(item); toast(`Создан черновик сделки ${created.id}.`, 'success'); router.push(`/platform-v7/deal-drafts/${created.id}`); }} style={{ borderRadius:12, padding:'10px 14px', background:'rgba(10,122,95,0.08)', border:'1px solid rgba(10,122,95,0.16)', color:'#0A7A5F', fontSize:13, fontWeight:700, cursor:'pointer' }}>Создать черновик</button>{shortlisted ? <Badge text='ОТОБРАНО' tone='success' /> : null}</div></div>;
          })}

          {visibleLocal.map((item) => {
            const shortlisted = shortlistedSourceIds.includes(item.id);
            return <div key={item.id} style={{ background:'#fff', border:'1px solid var(--pc-border, #E4E6EA)', borderRadius:16, padding:16, display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:12, alignItems:'center' }}><div><div style={{ fontFamily:'JetBrains Mono, monospace', fontWeight:800, color:'#0A7A5F', fontSize:13 }}>{item.id}</div><div style={{ fontSize:15, fontWeight:800, color:'var(--pc-text-primary, #0F1419)', marginTop:4 }}>{item.grain}</div><div style={{ fontSize:12, color:'var(--pc-text-muted, #6B778C)', marginTop:4 }}>{item.volume} т · {item.region}</div></div><div style={{ fontSize:14, fontWeight:700 }}>{formatMoney(item.targetPrice)} / т</div><div style={{ fontSize:12, color:'var(--pc-text-muted, #6B778C)' }}>{item.quality}</div><div style={{ fontSize:12, color:'var(--pc-text-muted, #6B778C)' }}>{item.payment}</div><div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>{shortlistButton(item.id)}<button onClick={() => { const created = createDraftDealFromLocalRfq(item); toast(`Создан черновик сделки ${created.id}.`, 'success'); router.push(`/platform-v7/deal-drafts/${created.id}`); }} style={{ borderRadius:12, padding:'10px 14px', background:'rgba(10,122,95,0.08)', border:'1px solid rgba(10,122,95,0.16)', color:'#0A7A5F', fontSize:13, fontWeight:700, cursor:'pointer' }}>Создать черновик</button>{shortlisted ? <Badge text='ОТОБРАНО' tone='success' /> : null}<Badge text='ВНУТРЕННИЙ ЗАПРОС' /></div></div>;
          })}
        </div>
      </section>
    </div>
