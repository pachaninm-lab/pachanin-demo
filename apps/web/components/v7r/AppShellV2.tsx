        ...
        <div style={{ padding: 18, borderBottom: '1px solid var(--pc-border, #E4E6EA)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--pc-text-muted, #6B778C)', marginTop: 3 }}>Единый контур сделки</div>
            </div>
          ...
          <div style={{ marginTop: 14, padding: 12, borderRadius: 14, background: '#F8FAFB', border: '1px solid var(--pc-border, #E4E6EA)' }}>
      ...
      <div style={{ paddingBottom: 76 }}>
        <header style={{ position: 'sticky', top: 0, zIndex: 60, padding: '12px 12px', background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--pc-border, #E4E6EA)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 10, alignItems: 'center', maxWidth: 1360, margin: '0 auto' }}>
            <button onClick={() => setSidebarOpen(true)} style={{ background: '#FFFFFF', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 10, padding: 10, cursor: 'pointer' }} aria-label="Открыть меню">≡</button>
              ...
              <nav aria-label="Хлебные крошки" style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                {crumbs.map((crumb, index) => (
                  <React.Fragment key={crumb.href}>
                    ...
                    {crumb.isLast ? <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)', whiteSpace: 'nowrap' }}>{crumb.label}</span> : <Link href={crumb.href} style={{ textDecoration: 'none', color: 'var(--pc-text-muted, #6B778C)', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>{crumb.label}</Link>}
                  </React.Fragment>
                ))}
              </nav>
