...
const labelStyle: React.CSSProperties = {
  fontSize: 11, color: '#6B778C', fontWeight: 700,
...
const inputStyle: React.CSSProperties = {
  ...
  border: '1px solid #E4E6EA', fontSize: 14,
  background: 'var(--pc-bg-card)', color: 'var(--pc-text-primary)',
};
...
export function SellerLotCreateRuntimeV2() {
  ...
  const nextStep = docsReady
    ? 'Можно публиковать лот и переходить к переговорам.'
    : 'Нужно дозагрузить обязательные документы продавца.';

  function resetForm() {
    setGrain('Пшеница 4 кл.'); setVolumeTons('240'); setRegion('Тамбовская');
    setPriceRaw(''); setBasis('EXW'); setShipDate(''); setDocsReady(false);
      ...
      <section style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 18, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--pc-text-muted)', lineHeight: 1.7, marginTop: 8 }}>Заполните параметры. Справа — live-preview readiness gate.</div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.3fr) minmax(0,0.7fr)', gap: 16 }}>
        <section style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
          ...
          <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: 14, borderRadius: 14, background: 'var(--pc-bg-subtle)', border: '1px solid var(--pc-border)', cursor: 'pointer' }}>
            <input type='checkbox' checked={docsReady} onChange={e => setDocsReady(e.target.checked)} style={{ marginTop: 2, width: 16, height: 16 }} />
            <span>
          ...
          <div>
            <button
              type='button'
              onClick={() => setGostOpen(v => !v)}
              style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 700, color: 'var(--pc-accent)', padding: 0 }}
            >
              ...
              <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                {[
                  ...
                  <div key={key}>
                    <label style={labelStyle}>{label}</label>
                    <input
                      value={gost[key as keyof typeof gost]}
                      onChange={e => setGost(g => ({ ...g, [key]: e.target.value }))}
                      inputMode='decimal'
                      placeholder='—'
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
            <button
              ...
              disabled={saving}
              style={{ borderRadius: 12, padding: '12px 20px', background: '#0A7A5F', border: '1px solid #0A7A5F', color: '#fff', fontSize: 14, fontWeight: 800, cursor: saving ? 'wait' : 'pointer' }}
            >
        ...
        <section style={{ display: 'grid', gap: 16, alignContent: 'start' }}>
          <div style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 18, padding: 18, display: 'grid', gap: 14 }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--pc-text-primary)' }}>Предпросмотр</div>
            <div style={{ padding: 16, borderRadius: 16, background: 'var(--pc-bg-subtle)', border: '1px solid var(--pc-border)' }}>
              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 800, color: 'var(--pc-accent)', fontSize: 12 }}>NEW MANUAL LOT</div>
              <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--pc-text-primary)', marginTop: 8, lineHeight: 1.3 }}>{grain || '—'}</div>
              <div style={{ fontSize: 12, color: 'var(--pc-text-muted)', marginTop: 6 }}>
                {volumeTons || '0'} т · {region || '—'}{priceRaw ? ` · ${formatPrice(priceRaw)} ₽/т` : ''}
              </div>
              {basis || shipDate ? (
                <div style={{ fontSize: 12, color: 'var(--pc-text-muted)', marginTop: 4 }}>
            ...
            <div style={{ padding: 14, borderRadius: 14, background: docsReady ? 'rgba(10,122,95,0.06)' : 'rgba(217,119,6,0.08)', border: docsReady ? '1px solid rgba(10,122,95,0.14)' : '1px solid rgba(217,119,6,0.16)' }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--pc-text-primary)', marginTop: 8 }}>{nextStep}</div>
            </div>

            {!docsReady ? (
              <div style={{ padding: 12, borderRadius: 12, background: 'rgba(217,119,6,0.08)', border: '1px solid rgba(217,119,6,0.16)', fontSize: 12, color: '#92400E' }}>
                <strong>Блокер:</strong> Документы не загружены. Лот получит статус REVIEW.
              </div>
            ) : (
              <div style={{ padding: 12, borderRadius: 12, background: 'rgba(10,122,95,0.06)', border: '1px solid rgba(10,122,95,0.14)', fontSize: 12, fontWeight: 700, color: '#0A7A5F' }}>
