...
const COUNTERPARTIES = [
  { inn: '6829123456', name: 'Агрохолдинг СК', status: 'verified', risk: 'low',   deals: 12, lastCheck: '2026-04-01' },
  { inn: '3664098765', name: 'МаслоПресс ООО', status: 'verified', risk: 'low',   deals: 8,  lastCheck: '2026-03-28' },
  { inn: '4632198234', name: 'ГрейнТрейд ЗАО', status: 'review',   risk: 'medium', deals: 3,  lastCheck: '2026-04-10' },
  { inn: '7701234567', name: 'ПромАгро АО',     status: 'verified', risk: 'low',   deals: 21, lastCheck: '2026-03-15' },
  { inn: '2301456789', name: 'КубаньЗерно ООО', status: 'blocked',  risk: 'high',  deals: 0,  lastCheck: '2026-04-12' },
  { inn: '6820987654', name: 'ТамбовАгро ИП',  status: 'verified', risk: 'low',   deals: 5,  lastCheck: '2026-04-05' },
];
...
function downloadCsv(data: typeof COUNTERPARTIES) {
  const header = ['ИНН', 'Наименование', 'Статус', 'Риск', 'Сделок', 'Последняя проверка'];
  const rows = data.map(r => [r.inn, r.name, statusPalette(r.status).label, riskPalette(r.risk).label, String(r.deals), r.lastCheck]);
...
export function ComplianceRuntime() {
  ...
  function handleCsv() {
    downloadCsv(COUNTERPARTIES);
    trackEvent('compliance_csv_exported', { count: COUNTERPARTIES.length });
    ...
    <div style={{ display: 'grid', gap: 18, padding: '8px 0' }}>
      <section style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 18, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          ...
          <button
            onClick={handleCsv}
            style={{ borderRadius: 12, padding: '10px 16px', background: 'var(--pc-accent)', border: '1px solid var(--pc-accent)', color: '#fff', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}
          >
            Выгрузить CSV
          </button>
        </div>
              ...
              <div key={c.inn} style={{ background: 'var(--pc-bg-subtle)', border: '1px solid var(--pc-border)', borderRadius: 14, padding: '14px 16px', display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 12, alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--pc-text-muted)' }}>{c.deals} сделок · проверен {c.lastCheck}</span>
                  </div>
                </div>
      ...
      <section style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
        <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--pc-text-primary)' }}>Интеграции комплаенса</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10 }}>
          {[
            { name: 'СПАРК Интерфакс', status: 'Подключён', color: '#0A7A5F' },
            { name: 'ФНС ЕГРЮЛ',       status: 'Подключён', color: '#0A7A5F' },
            { name: 'Росфинмониторинг', status: 'Нет связи', color: '#B91C1C' },
            { name: 'Санкционные списки', status: 'Подключён', color: '#0A7A5F' },
          ].map(({ name, status, color }) => (
            <div key={name} style={{ padding: 14, borderRadius: 12, background: 'var(--pc-bg-subtle)', border: '1px solid var(--pc-border)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--pc-text-primary)' }}>{name}</div>
              <div style={{ fontSize: 12, color, fontWeight: 700, marginTop: 4 }}>{status}</div>
            </div>
          ))}
        </div>
      </section>
