...
function metricPalette(tone: StaticMetric['tone']) {
  ...
  return { bg: 'rgba(15,20,25,0.04)', border: 'rgba(15,20,25,0.08)', color: 'var(--pc-text-primary, #0F1419)' };
}
    ...
    <div style={{ display: 'grid', gap: 16, maxWidth: 1080, margin: '0 auto' }}>
      <section style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 12 }}>
        <div style={{ display: 'inline-flex', width: 'fit-content', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 999, background: 'rgba(10,122,95,0.08)', border: '1px solid rgba(10,122,95,0.18)' }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 900, color: 'var(--pc-text-primary, #0F1419)' }}>{eyebrow}</span>
        </div>
        <div>
          <div style={{ fontSize: 28, lineHeight: 1.15, fontWeight: 900, color: 'var(--pc-text-primary, #0F1419)' }}>{title}</div>
        ...
        {metrics.map((metric) => {
          ...
          return (
            <section key={metric.label} style={{ background: palette.bg, border: `1px solid ${palette.border}`, borderRadius: 16, padding: 16, display: 'grid', gap: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--pc-text-muted, #6B778C)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{metric.label}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12 }}>
        {sections.map((section) => (
          <section key={section.title} style={{ background: '#fff', border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
            <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--pc-text-primary, #0F1419)' }}>{section.title}</div>
            <div style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--pc-text-secondary, #475569)' }}>{section.body}</div>
          </section>
        ))}
      </div>
        ...
        {actions.map((action) => (
          <Link
            ...
            style={{
              ...
              border: `1px solid ${action.primary ? '#0A7A5F' : 'var(--pc-border, #E4E6EA)'}`,
              color: action.primary ? '#fff' : 'var(--pc-text-primary, #0F1419)',
              fontSize: 13,
              fontWeight: 800,
            }}
          >
