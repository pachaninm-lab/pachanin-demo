...
export function DemoDealAutoplay({ dealId, amount }: { dealId: string; amount: number }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      ...
      {callback && (
        <div style={{ padding: 12, border: '1px solid var(--pc-border, #E4E6EA)', borderRadius: 12, background: '#fff', display: 'grid', gap: 6 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #334155)' }}>Принят банком: {new Date(callback.acceptedAt).toLocaleTimeString('ru-RU')}</div>
          <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #334155)' }}>Сумма к выплате: {callback.amountRub} ₽</div>
          <div style={{ fontSize: 12, color: 'var(--pc-text-muted, #6B778C)' }}>Задержка шлюза: {callback.latencyMs} мс</div>
          {callback.confirmedAt ? <div style={{ fontSize: 12, color: 'var(--pc-text-secondary, #334155)' }}>Подтверждён: {new Date(callback.confirmedAt).toLocaleTimeString('ru-RU')}</div> : null}
        </div>
      )}
    </div>
  );
