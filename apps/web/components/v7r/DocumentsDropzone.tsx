    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'baseline', flexWrap: 'wrap' }}>
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 12, color: '#6B778C' }}>Файлы и чек-лист готовности. В текущем контуре сохраняется в памяти браузера по каждой сделке.</div>
        </div>
        ...
        {REQUIRED_DOCS.map((doc) => (
          <label key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 12, border: '1px solid #E4E6EA', background: checklist[doc.id] ? 'rgba(10,122,95,0.06)' : '#F8FAFB', cursor: 'pointer' }}>
            ...
            <div style={{ display: 'grid', gap: 2, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: '#6B778C' }}>{doc.blocker ? 'Блокирует банковское основание' : 'Желательно для полного досье сделки'}</div>
            </div>
