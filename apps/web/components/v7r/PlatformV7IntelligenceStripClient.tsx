  ...
  return (
    <section id='intelligence' className='entry-section entry-intelligence-section' aria-labelledby='intelligence-title' data-p7-no-translate='true' data-lang={lang}>
      ...
      <div className='entry-intelligence-panel'>
        ...
        <div className='entry-intelligence-flow'>{t.flow.map((item) => <span key={item}>{item}</span>)}</div>
        <div className='entry-intelligence-grid'>
          {t.items.map(([title, text], index) => {
            const Icon = icons[index];
            return <article className='entry-intelligence-tile' key={title}><span><Icon size={20} /></span><strong>{title}</strong><small>{text}</small></article>;
          })}
        </div>
      </div>
    </section>
