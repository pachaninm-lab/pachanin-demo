...
export function AppShellV3({ children, initialRole = 'operator' }: { children: React.ReactNode; initialRole?: PlatformRole }) {
  ...
  return (
    <div className='pc-shell-root' style={{ minHeight: '100dvh', background: 'var(--pc-bg)', overflowX: 'hidden' }}>
      ...
      <aside style={{ position: 'fixed', top: 0, bottom: 0, left: 0, width: 286, maxWidth: '84vw', zIndex: 110, transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform 0.2s ease', background: 'var(--pc-bg-card)', borderRight: '1px solid var(--pc-border)', boxShadow: sidebarOpen ? 'var(--pc-shadow)' : 'none', display: 'flex', flexDirection: 'column' }}>
