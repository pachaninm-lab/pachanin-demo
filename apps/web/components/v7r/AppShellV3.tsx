...
export function AppShellV3({ children, initialRole = 'operator' }: { children: React.ReactNode; initialRole?: PlatformRole }) {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--pc-bg)', overflowX: 'hidden' }}>
      <style>{`html,body{overflow-x:hidden;max-width:100%}*,*::before,*::after{box-sizing:border-box}.pc-shell-header{max-width:1360px;margin:0 auto;padding:10px 16px;display:flex;flex-wrap:wrap;gap:12px;align-items:center}.pc-shell-header > *{min-width:0}.pc-alert-panel{position:absolute;right:0;top:42px;width:340px;max-width:calc(100vw - 32px);background:var(--pc-bg-card);border:1px solid var(--pc-border);border-radius:14px;box-shadow:var(--pc-shadow);padding:10px;z-index:71;max-height:70vh;overflow-y:auto;overflow-x:hidden}.pc-role-banner{display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:12px;background:var(--pc-bg-card);border:1px solid var(--pc-border);margin-bottom:12px;font-size:12px;color:var(--pc-text-secondary);flex-wrap:wrap}.pc-giga{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;padding:12px 14px;border-radius:16px;background:var(--pc-accent-bg);border:1px solid var(--pc-accent-border);margin-top:12px}.pc-giga-title{font-size:13px;font-weight:900;color:var(--pc-text-primary)}.pc-giga-text{font-size:12px;color:var(--pc-text-secondary);line-height:1.5;margin-top:4px}.pc-giga-chiprow{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.pc-giga-chip{display:inline-flex;align-items:center;padding:4px 8px;border-radius:999px;background:var(--pc-bg-card);border:1px solid var(--pc-accent-border);color:var(--pc-accent);font-size:11px;font-weight:800;cursor:pointer}.pc-giga-chip:hover{background:var(--pc-accent-bg)}.pc-mobile-role{display:none}@media (max-width: 768px){.pc-shell-header{padding:10px 12px;gap:8px;display:grid;grid-template-columns:1fr}.pc-alert-panel{position:fixed;left:12px;right:12px;top:72px;width:auto;max-width:none;max-height:min(70vh,calc(100dvh - 96px))}.pc-header-search{max-width:none !important;min-width:0 !important;width:100%}.pc-header-controls{display:flex;align-items:center;gap:8px;justify-content:space-between;width:100%}.pc-header-controls .v9-desktop-only{display:none !important}.pc-header-controls select{display:none !important}.pc-mobile-role{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:8px 12px;border-radius:10px;border:1px solid var(--pc-border);background:var(--pc-bg-card);font-size:13px;font-weight:700;color:var(--pc-text-primary);cursor:pointer}}@media (max-width: 560px){.pc-role-banner{align-items:flex-start}.pc-role-banner a{margin-left:0 !important}.pc-giga{grid-template-columns:1fr}}`}</style>
      ...
      <div><header style={{ position: 'sticky', top: 0, zIndex: 60, background: 'var(--pc-bg-header)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--pc-border)' }}><div className='pc-shell-header'><div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, flex: '1 1 auto' }}><button onClick={() => setSidebarOpen(true)} aria-label='Открыть меню' style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 10, padding: 8, cursor: 'pointer', lineHeight: 0, display: 'inline-flex', alignItems: 'center' }}><Menu size={18} aria-hidden /></button><div style={{ minWidth: 0 }}><div style={{ fontSize: 16, fontWeight: 700, color: 'var(--pc-text-primary)' }}>Прозрачная Цена</div><nav aria-label='Хлебные крошки' style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', minWidth: 0 }}>{crumbs.map((crumb, index) => <React.Fragment key={crumb.href}>{index > 0 ? <span style={{ color: 'var(--pc-text-muted)', fontSize: 12 }}>/</span> : null}{crumb.isLast ? <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--pc-text-primary)' }}>{crumb.label}</span> : <Link href={crumb.href} style={{ textDecoration: 'none', color: 'var(--pc-text-muted)', fontSize: 12, fontWeight: 500 }}>{crumb.label}</Link>}</React.Fragment>)}</nav></div></div><button className='pc-header-search' onClick={() => setPaletteOpen(true)} aria-label='Открыть быстрый поиск (Cmd+K)' style={{ flex: '1 1 280px', minWidth: 240, maxWidth: 520, display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 10, border: '1px solid var(--pc-border)', background: 'var(--pc-bg-card)', cursor: 'pointer' }}><Search size={14} aria-hidden style={{ color: 'var(--pc-text-muted)', flexShrink: 0 }} /><span style={{ color: 'var(--pc-text-muted)', fontSize: 13, textAlign: 'left', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Поиск по сделкам, лотам, спорам…</span><span className='v9-desktop-only' style={{ color: 'var(--pc-text-muted)', fontSize: 11, fontFamily: 'JetBrains Mono, monospace' }}>⌘ K</span></button><div className='pc-header-controls' style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end', flex: '0 1 auto' }}>{statuses.map((item) => (<span key={item.label} className='v9-desktop-only' style={{ display: 'inline-flex', alignItems: 'center', padding: '6px 10px', borderRadius: 999, background: item.bg, border: `1px solid ${item.border}`, color: item.color, fontSize: 11, fontWeight: 800 }}>{item.label}</span>))}<div style={{ position: 'relative' }}><button onClick={() => setAlertsOpen((v) => !v)} aria-label={`Уведомления: ${NOTIFICATIONS.length}`} aria-expanded={alertsOpen} style={{ position: 'relative', background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 10, padding: 8, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', lineHeight: 0 }}><Bell size={18} aria-hidden /><span style={{ position: 'absolute', top: -6, right: -4, minWidth: 18, height: 18, borderRadius: 999, background: '#DC2626', color: '#fff', fontSize: 10, fontWeight: 800, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px' }}>{NOTIFICATIONS.length}</span></button>{alertsOpen ? (<div role='dialog' aria-label='Уведомления' className='pc-alert-panel'><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 6px 10px', gap: 10 }}><span style={{ fontSize: 13, fontWeight: 800, color: 'var(--pc-text-primary)' }}>Уведомления</span><span style={{ fontSize: 11, color: 'var(--pc-text-muted)', whiteSpace: 'nowrap' }}>{NOTIFICATIONS.length} активных</span></div>{Object.entries(groupedNotifications).map(([group, items]) => (<div key={group} style={{ display: 'grid', gap: 4, marginBottom: 8 }}><div style={{ padding: '4px 6px', fontSize: 10, fontWeight: 800, color: 'var(--pc-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{NOTIFICATION_GROUPS[group as NotificationGroup]} · {items.length}</div>{items.map((item) => (<Link key={item.id} href={item.href} onClick={() => setAlertsOpen(false)} style={{ textDecoration: 'none', padding: '10px 12px', borderRadius: 10, background: 'var(--pc-bg-subtle)', border: '1px solid var(--pc-border)', color: 'var(--pc-text-primary)', fontSize: 12, lineHeight: 1.5, wordBreak: 'break-word' }}>{item.text}</Link>))}</div>))}</div>) : null}</div><button onClick={toggleTheme} aria-label={theme === 'dark' ? 'Переключить на светлую тему' : 'Переключить на тёмную тему'} title={theme === 'dark' ? 'Светлая тема' : 'Тёмная тема'} style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 10, padding: 8, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', lineHeight: 0, color: 'var(--pc-text-secondary)' }}>{theme === 'dark' ? <Sun size={18} aria-hidden /> : <Moon size={18} aria-hidden />}</button><button suppressHydrationWarning className='pc-mobile-role' type='button' onClick={() => setSidebarOpen(true)} aria-label={`Текущая роль: ${ROLE_LABELS[displayRole]}. Открыть меню роли`}>{ROLE_LABELS[displayRole]}</button><select suppressHydrationWarning value={displayRole} onChange={(event) => { const nextRole = event.target.value as PlatformRole; setRole(nextRole); trackRoleSwitch(nextRole); router.push(ROLE_ROUTES[nextRole]); }} style={{ minWidth: 150, border: '1px solid var(--pc-border)', borderRadius: 10, padding: '8px 12px', fontSize: 13, background: 'var(--pc-bg-card)', color: 'var(--pc-text-primary)', fontWeight: 600 }}>{Object.entries(ROLE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div></div></header><main style={{ padding: 16, maxWidth: 1360, margin: '0 auto', overflowX: 'hidden' }}>{showBanner ? (<div role='status' aria-live='polite' className='pc-role-banner'><span suppressHydrationWarning style={{ display: 'inline-flex', alignItems: 'center', padding: '3px 8px', borderRadius: 999, background: stageTone.bg, border: `1px solid ${stageTone.border}`, color: stageTone.color, fontSize: 10, fontWeight: 800 }}>{stage.label}</span><span suppressHydrationWarning style={{ minWidth: 0, wordBreak: 'break-word' }}>Вы на роли <strong style={{ color: '#0F1419' }}>{ROLE_LABELS[displayRole]}</strong>. Переключитесь в меню, если нужен другой рабочий контекст.</span><Link suppressHydrationWarning href={ROLE_ROUTES[displayRole]} style={{ marginLeft: 'auto', textDecoration: 'none', color: '#0A7A5F', fontWeight: 700, fontSize: 12 }}>Главная роли →</Link></div>) : null}{children}{showBanner ? (<section className='pc-giga'><div><div className='pc-giga-title'>GigaChat · быстрые вопросы по сделке</div><div className='pc-giga-text'>Решает: где спор, кто следующий владелец, почему деньги стоят, каких документов не хватает и куда идти дальше.</div><div className='pc-giga-chiprow'>{chips.map((chip) => (<span key={chip} className='pc-giga-chip' onClick={() => { trackGigaChatAsked(pathname); setPaletteOpen(true); }}>{chip}</span>))}</div></div><button onClick={() => { trackGigaChatAsked(pathname); setPaletteOpen(true); }} style={{ borderRadius: 12, padding: '10px 14px', background: '#0A7A5F', border: '1px solid #0A7A5F', color: '#fff', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}>Спросить</button></section>) : null}</main></div>
