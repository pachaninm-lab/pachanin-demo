...
function gigaChatChips(pathname: string): string[] {
  ...
  if (dealMatch) {
    ...
    return [
      ...
      `Кто следующий владелец по ${id}?`,
      `Каких документов не хватает в ${id}?`,
      `Куда идти дальше по ${id}?`,
    ];
  }
  ...
  if (lotMatch && lotMatch[1] !== 'create') {
    ...
    return [
      `Что за лот ${id}?`,
      `Статус документов по ${id}`,
      `Как перевести ${id} в PASS?`,
      `Куда идти дальше по ${id}?`,
    ];
  }
  if (pathname.startsWith('/platform-v7/disputes')) {
    return ['Как открыть спор?', 'Какие документы нужны для арбитража?', 'Сколько длится рассмотрение спора?', 'Кто может закрыть спор?'];
  }
  if (pathname.startsWith('/platform-v7/bank')) {
    return ['Почему деньги стоят?', 'Когда разморозят счёт?', 'Какие условия аккредитива?', 'Что нужно для выплаты?'];
  }
  if (pathname.startsWith('/platform-v7/lots')) {
    return ['Как создать лот быстро?', 'Какие документы обязательны?', 'Почему лот в статусе REVIEW?', 'Как получить статус PASS?'];
  }
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--pc-bg)', overflowX: 'hidden' }}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box}
        ...
        .pc-alert-panel{position:absolute;right:0;top:42px;width:340px;max-width:calc(100vw - 32px);background:var(--pc-bg-card);border:1px solid var(--pc-border);border-radius:14px;box-shadow:var(--pc-shadow);padding:10px;z-index:71;max-height:70vh;overflow-y:auto;overflow-x:hidden}
        .pc-role-banner{display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:12px;background:var(--pc-bg-card);border:1px solid var(--pc-border);margin-bottom:12px;font-size:12px;color:var(--pc-text-secondary);flex-wrap:wrap}
        .pc-giga{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;padding:12px 14px;border-radius:16px;background:var(--pc-accent-bg);border:1px solid var(--pc-accent-border);margin-top:12px}
        ...
        .pc-giga-chip{display:inline-flex;align-items:center;padding:4px 8px;border-radius:999px;background:var(--pc-bg-card);border:1px solid var(--pc-accent-border);color:var(--pc-accent);font-size:11px;font-weight:800;cursor:pointer}
        ...
        @media (max-width: 768px){
          ...
          .pc-mobile-role{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:8px 12px;border-radius:10px;border:1px solid var(--pc-border);background:var(--pc-bg-card);font-size:13px;font-weight:700;color:var(--pc-text-primary);cursor:pointer}
        }
      ...
      <aside style={{ position: 'fixed', top: 0, bottom: 0, left: 0, width: 286, maxWidth: '84vw', zIndex: 80, transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform 0.2s ease', background: 'var(--pc-bg-card)', borderRight: '1px solid var(--pc-border)', boxShadow: sidebarOpen ? 'var(--pc-shadow)' : 'none', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: 18, borderBottom: '1px solid var(--pc-border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
            <div>
              <div style={{ fontSize: 19, fontWeight: 800, color: 'var(--pc-text-primary)' }}>Прозрачная Цена</div>
              <div style={{ fontSize: 11, color: 'var(--pc-text-muted)', marginTop: 4 }}>Цифровой контур исполнения сделки</div>
            </div>
            <button onClick={() => setSidebarOpen(false)} aria-label='Закрыть меню' style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--pc-text-muted)', padding: 12, minWidth: 44, minHeight: 44, borderRadius: 10 }}><X size={18} aria-hidden /></button>
          </div>
          <div style={{ marginTop: 14, padding: 12, borderRadius: 14, background: 'var(--pc-bg-subtle)', border: '1px solid var(--pc-border)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginTop: 6, alignItems: 'center' }}>
              <div suppressHydrationWarning style={{ fontSize: 15, fontWeight: 800, color: 'var(--pc-text-primary)' }}>{ROLE_LABELS[displayRole]}</div>
        ...
        <nav style={{ padding: 12, display: 'grid', gap: 4, overflowY: 'auto' }}>
          {items.map((item) => {
            ...
            return <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)} style={{ textDecoration: 'none', padding: '14px 12px', borderRadius: 10, fontSize: 13, fontWeight: active ? 700 : 600, background: active ? 'var(--pc-accent-bg)' : 'transparent', color: active ? 'var(--pc-accent)' : 'var(--pc-text-primary)', border: active ? '1px solid var(--pc-accent-border)' : '1px solid transparent' }}>{item.label}</Link>;
          })}
        </nav>
        <div style={{ marginTop: 'auto', padding: 12, borderTop: '1px solid var(--pc-border)', display: 'grid', gap: 8 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <Link href='/platform-v7/investor' style={{ textDecoration: 'none', padding: '10px 12px', borderRadius: 10, border: '1px solid var(--pc-border)', background: 'var(--pc-bg-card)', color: 'var(--pc-text-primary)', textAlign: 'center', fontSize: 13, fontWeight: 700 }}>Инвестор</Link>
            <Link href='/platform-v7/demo' style={{ textDecoration: 'none', padding: '10px 12px', borderRadius: 10, border: '1px solid var(--pc-border)', background: 'var(--pc-bg-card)', color: 'var(--pc-text-primary)', textAlign: 'center', fontSize: 13, fontWeight: 700 }}>Демо</Link>
          </div>
          <Link href='/platform-v7/roles' onClick={() => { clearRoleSelection(); setSidebarOpen(false); }} style={{ textDecoration: 'none', padding: '10px 12px', borderRadius: 10, background: 'var(--pc-bg-subtle)', border: '1px solid var(--pc-border)', color: 'var(--pc-text-primary)', fontSize: 13, fontWeight: 700 }}>Все роли</Link>
        </div>
      </aside>
        ...
        <header style={{ position: 'sticky', top: 0, zIndex: 60, background: 'var(--pc-bg-header)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--pc-border)' }}>
          <div className='pc-shell-header'>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, flex: '1 1 auto' }}>
              <button onClick={() => setSidebarOpen(true)} aria-label='Открыть меню' style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 10, padding: 8, cursor: 'pointer', lineHeight: 0, display: 'inline-flex', alignItems: 'center' }}><Menu size={18} aria-hidden /></button>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--pc-text-primary)' }}>Прозрачная Цена</div>
                <nav aria-label='Хлебные крошки' style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', minWidth: 0 }}>
                  {crumbs.map((crumb, index) => <React.Fragment key={crumb.href}>{index > 0 ? <span style={{ color: 'var(--pc-text-muted)', fontSize: 12 }}>/</span> : null}{crumb.isLast ? <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--pc-text-primary)' }}>{crumb.label}</span> : <Link href={crumb.href} style={{ textDecoration: 'none', color: 'var(--pc-text-muted)', fontSize: 12, fontWeight: 500 }}>{crumb.label}</Link>}</React.Fragment>)}
                </nav>
              </div>
            </div>

            <button className='pc-header-search' onClick={() => setPaletteOpen(true)} aria-label='Открыть быстрый поиск (Cmd+K)' style={{ flex: '1 1 280px', minWidth: 240, maxWidth: 520, display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 10, border: '1px solid var(--pc-border)', background: 'var(--pc-bg-card)', cursor: 'pointer' }}>
              <Search size={14} aria-hidden style={{ color: 'var(--pc-text-muted)', flexShrink: 0 }} />
              <span style={{ color: 'var(--pc-text-muted)', fontSize: 13, textAlign: 'left', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Поиск по сделкам, лотам, спорам…</span>
            ...
            <div className='pc-header-controls' style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end', flex: '0 1 auto' }}>
              ...
              <div style={{ position: 'relative' }}>
                <button onClick={() => setAlertsOpen((v) => !v)} aria-label={`Уведомления: ${NOTIFICATIONS.length}`} aria-expanded={alertsOpen} style={{ position: 'relative', background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 10, padding: 8, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', lineHeight: 0 }}>
                ...
                {alertsOpen ? (
                  <div role='dialog' aria-label='Уведомления' className='pc-alert-panel'>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 6px 10px', gap: 10 }}>
                      <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--pc-text-primary)' }}>Уведомления</span>
                      <span style={{ fontSize: 11, color: 'var(--pc-text-muted)', whiteSpace: 'nowrap' }}>{NOTIFICATIONS.length} активных</span>
                    </div>
                    {Object.entries(groupedNotifications).map(([group, items]) => (
                      <div key={group} style={{ display: 'grid', gap: 4, marginBottom: 8 }}>
                        <div style={{ padding: '4px 6px', fontSize: 10, fontWeight: 800, color: 'var(--pc-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{NOTIFICATION_GROUPS[group as NotificationGroup]} · {items.length}</div>
                        {items.map((item) => (
                          <Link key={item.id} href={item.href} onClick={() => setAlertsOpen(false)} style={{ textDecoration: 'none', padding: '10px 12px', borderRadius: 10, background: 'var(--pc-bg-subtle)', border: '1px solid var(--pc-border)', color: 'var(--pc-text-primary)', fontSize: 12, lineHeight: 1.5, wordBreak: 'break-word' }}>{item.text}</Link>
                        ))}
                      </div>
                    ))}
                  </div>
              ...
              <button onClick={toggleTheme} aria-label={theme === 'dark' ? 'Переключить на светлую тему' : 'Переключить на тёмную тему'} title={theme === 'dark' ? 'Светлая тема' : 'Тёмная тема'} style={{ background: 'var(--pc-bg-card)', border: '1px solid var(--pc-border)', borderRadius: 10, padding: 8, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', lineHeight: 0, color: 'var(--pc-text-secondary)' }}>
              ...
              <select suppressHydrationWarning value={displayRole} onChange={(event) => { const nextRole = event.target.value as PlatformRole; setRole(nextRole); trackRoleSwitch(nextRole); router.push(ROLE_ROUTES[nextRole]); }} style={{ minWidth: 150, border: '1px solid var(--pc-border)', borderRadius: 10, padding: '8px 12px', fontSize: 13, background: 'var(--pc-bg-card)', color: 'var(--pc-text-primary)', fontWeight: 600 }}>
            ...
            <section className='pc-giga'>
              <div>
                ...
                <div className='pc-giga-chiprow'>
                  {chips.map((chip) => (
                    <span key={chip} className='pc-giga-chip' onClick={() => { trackGigaChatAsked(pathname); setPaletteOpen(true); }}>{chip}</span>
                  ))}
                </div>
              </div>
              <button onClick={() => { trackGigaChatAsked(pathname); setPaletteOpen(true); }} style={{ borderRadius: 12, padding: '10px 14px', background: '#0A7A5F', border: '1px solid #0A7A5F', color: '#fff', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}>Спросить</button>
            </section>
          ) : null}
        </main>
      </div>
