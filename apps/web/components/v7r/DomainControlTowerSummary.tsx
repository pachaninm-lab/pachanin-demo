...
export function DomainControlTowerSummary() {
  ...
  const bankStepFormula = `К банковскому шагу = сделки без открытых причин остановки: ${canonicalKpis.readyToRelease}`;
  ...
  return (
    <section style={{ display: 'grid', gap: PLATFORM_V7_TOKENS.spacing.md }} aria-label='Сводка центра управления'>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: PLATFORM_V7_TOKENS.spacing.md }}>
        <P7MetricLinkCard testId='kpi-readyToRelease' title='К банковскому шагу' value={formatCompactMoney(canonicalKpis.readyToRelease)} note='Сумма основания, которая ближе всего к подтверждению банком.' formula={bankStepFormula} href='/platform-v7/bank' tone='success' />
        <P7MetricLinkCard testId='kpi-heldAmount' title='Под удержанием' value={formatCompactMoney(canonicalKpis.totalHold)} note='Сумма, которую нельзя передавать к банковскому шагу до закрытия причины остановки.' formula={holdFormula} href='/platform-v7/disputes' tone='danger' />
