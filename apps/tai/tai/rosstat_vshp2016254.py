...
_SDMX_NAMESPACE_PREFIXES: Final = (
    "http://www.sdmx.org/resources/sdmxml/schemas/v2_0/",
    "http://www.SDMX.org/resources/SDMXML/schemas/v2_0/",
)
...
class RosstatVshp2016254Materializer:
    ...
    def _validate_structure(self, payload: bytes) -> None:
        ...
        if local != "Structure" or namespace not in {
            f"{prefix}message" for prefix in _SDMX_NAMESPACE_PREFIXES
        }:
            raise ValueError("ROSSTAT_SDMX_STRUCTURE_ROOT_MISMATCH")
        rendered = text.casefold()
        for suffix in ("message", "structure", "common"):
            if f"{_SDMX_NAMESPACE_PREFIXES[0]}{suffix}" not in rendered:
    ...
    def _records(self, payload: bytes) -> tuple[RosstatSdmxRecord, ...]:
        ...
        if not namespace.startswith(_SDMX_NAMESPACE_PREFIXES):
