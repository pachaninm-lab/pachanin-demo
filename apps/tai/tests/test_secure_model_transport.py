...
def _transport(response: _Response) -> tuple[BearerHTTPClientJSONTransport, _Connection]:
...
def test_bearer_transport_sends_secret_only_in_authorization_header() -> None:
    ...
    transport = BearerHTTPClientJSONTransport(token, connection_factory=factory)
...
def test_bearer_transport_revalidates_endpoint_shape(endpoint: str, message: str) -> None:
...
def test_bearer_transport_fails_closed_and_closes_connection() -> None:
    transport, connection = _transport(
        _Response(b"{}", status=401, reason="Unauthorized")
    )
...
@pytest.mark.parametrize("payload", [b"not-json", b"[]", b"x" * 1_025])
def test_bearer_transport_rejects_invalid_or_oversized_response(payload: bytes) -> None:
    transport, connection = _transport(_Response(payload))
...
@pytest.mark.parametrize(
    ...
    [(0.0, 1_024, "timeout_seconds"), (1.0, 0, "maximum_response_bytes")],
)
