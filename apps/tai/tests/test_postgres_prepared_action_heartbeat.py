...
def test_heartbeat_policy_requires_safe_interval() -> None:
    with pytest.raises(ValueError, match="at least 50 milliseconds"):
        _repository(heartbeat_seconds=0.01)
    with pytest.raises(ValueError, match="shorter than execution_lease"):
        _repository(lease_seconds=1.0, heartbeat_seconds=1.0)
