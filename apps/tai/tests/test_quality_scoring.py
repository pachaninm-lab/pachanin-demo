...
def test_quality_threshold_failure_is_reported_without_admission(
    ...
    for annotation in fixture["manifest"]["annotations"]:
        if annotation["profile_id"] == PROFILES[0] and annotation["case_id"] in failed_cases:
...
def test_low_level_contract_rejections(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualityScoringError, match="unsupported quality scoring lifecycle"):
        load_scoring_manifest(path)
...
def test_manifest_root_schema_and_pending_contamination_reject(tmp_path: Path) -> None:
    ...
    path = tmp_path / "contamin    write_json(path, pending)
    ...
    with pytest.raises(QualityScoringError, match="unsupported quality scoring manifest schema"):
        load_scoring_manifest(path)
