...
def test_repository_manifest_preserves_historical_paths_with_unique_authority_order() -> None:
    ...
    assert [item.version for item in inventory.migrations] == list(range(1, 23))
    ...
    assert paths[-6].endswith("0016_official_source_coverage.sql")
    assert paths[-5].endswith("0017_official_source_run_evidence.sql")
    assert paths[-4].endswith("0018_confirmation_denial_audit.sql")
    assert paths[-3].endswith("0019_public_official_corpus.sql")
    assert paths[-2].endswith("0020_public_official_corpus_audit_authority.sql")
    assert paths[-1].endswith("0021_public_official_acquisition_authority.sql")
