...
ORCHESTRATOR_PATH = (
    TAI_ROOT / "scripts" / "model-bundle-upload-restore-orchestrator.v1.sh"
)
...
def test_scope_is_narrow_and_includes_every_executable_surface() -> None:
    ...
    assert {
        ...
        "apps/tai/scripts/model-bundle-upload-restore-orchestrator.v1.sh",
...
def test_workflow_is_owner_only_exact_main_and_fail_closed() -> None:
    ...
    assert "Enforce final verified status" in workflow
    assert "VERIFIED_BUNDLES_RESTORED" in workflow
    assert "production_operational_status' \"$report\")\" = 'NOT_ATTESTED'" in workflow
    for secret in (
        "TAI_MODEL_HOST",
        "TAI_MODEL_SSH_USER",
        "TAI_MODEL_SSH_PORT",
...
def test_driver_streams_twice_but_completes_only_exact_second_digest() -> None:
    ...
    assert '["sha256"]' in driver
    assert 'versionId=' in driver
    ...
    assert "rm -rf \"$original_root\" \"$restore_root\" \"$transport_root\"" in driver
    assert "DeleteObject" not in driver
    assert "aws s3 cp" not in driver
    assert "bundle.tar >" not in driver
    assert "production_operational_status': 'NOT_ATTESTED'" in driver
...
def test_orchestrator_verifies_upstream_evidence_and_excludes_production_fallback() -> None:
    orchestrator = ORCHESTRATOR_PATH.read_text(encoding="utf-8")
    assert "TAI Release Acceptance" in orchestrator
    assert "READY_FOR_BUNDLE_UPLOAD" in orchestrator
    assert "actions/artifacts/$artifact_id" in orchestrator
    assert "control-manifest.sha256" in orchestrator
    assert "BatchMode=yes" in orchestrator
    assert "IdentitiesOnly=yes" in orchestrator
    assert "StrictHostKeyChecking=yes" in orchestrator
    assert "bounded-evidence.tar.gz" in orchestrator
    assert "S3 secret leaked into bounded evidence" in orchestrator
    for forbidden in (
        "PC_PROD_HOST",
        "PC_PROD_SSH_KEY",
        "VPS_SSH_KEY",
...
def test_runbook_preserves_human_and_operational_boundaries() -> None:
