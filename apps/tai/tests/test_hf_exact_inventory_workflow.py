...
def test_authority_contains_only_the_two_exact_candidates() -> None:
    authority = _load(AUTHORITY_PATH)
    identities = {
        (item["model_id"], item["revision"], item["role"])
        for item in authority["models"]
    }

    assert authority["schema_version"] == "tai.model-bundle-authority.v2"
    assert identities == {
        ("Qwen/Qwen3-8B", QWEN_REVISION, "PRIMARY"),
        (
...
def test_workflow_derives_exact_revisions_from_the_accepted_authority() -> None:
    ...
    assert QWEN_REVISION not in workflow
    assert MISTRAL_REVISION not in workflow
