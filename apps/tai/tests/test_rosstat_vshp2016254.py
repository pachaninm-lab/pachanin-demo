...
LEGACY_SDMX_PREFIX = "http://www.SDMX.org/resources/SDMXML/schemas/v2_0/"
...
def test_accepts_rosstat_legacy_cased_sdmx_namespaces(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    passport, structure, data = _payloads()
    for suffix in ("message", "structure", "common"):
        standard = f"http://www.sdmx.org/resources/sdmxml/schemas/v2_0/{suffix}"
        legacy = f"{LEGACY_SDMX_PREFIX}{suffix}"
        structure = structure.replace(standard.encode(), legacy.encode())
    for suffix in ("message", "generic", "common"):
        standard = f"http://www.sdmx.org/resources/sdmxml/schemas/v2_0/{suffix}"
        legacy = f"{LEGACY_SDMX_PREFIX}{suffix}"
        data = data.replace(standard.encode(), legacy.encode())
    monkeypatch.setattr(rosstat, "PASSPORT_SHA256", _sha(passport))
    monkeypatch.setattr(rosstat, "STRUCTURE_SHA256", _sha(structure))
    monkeypatch.setattr(rosstat, "DATA_SHA256", _sha(data))
    ...
    assert all(record.xpath.startswith("/GenericData[1]") for record in result.records)


def test_accepts_official_windows_1251_passport(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _, structure, data = _payloads()
    passport = (
        f"Описание набора;{DATASET_CODE};structure-20181211T0212.xsd;"
        "data-20181211T0212-structure-20181211T0212.xml"
    ).encode("cp1251")
    monkeypatch.setattr(rosstat, "PASSPORT_SHA256", _sha(passport))
    monkeypatch.setattr(rosstat, "STRUCTURE_SHA256", _sha(structure))
