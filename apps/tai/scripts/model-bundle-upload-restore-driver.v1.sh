...
for model_key in qwen3-8b mistral-7b-instruct-v0.3; do
  ...
  readarray -t retention_values < <(python3 - "$retain_until" <<'PY'
import sys
from datetime import datetime, timedelta, timezone
expires = datetime.fromisoformat(sys.argv[1].replace('Z', '+00:00'))
if expires.tzinfo is None:
    raise SystemExit('retention expiry is timezone-naive')
uploaded = expires - timedelta(days=90)
if expires < datetime.now(timezone.utc) + timedelta(days=89):
    raise SystemExit('retention horizon is below 90-day authority')
