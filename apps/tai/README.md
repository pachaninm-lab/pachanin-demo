...
Confirmed-write handlers намеренно отсутствуют. `acknowledgeRisk`, `createSupportCase`, деньги, документы, подписи, лаборатория, спор и изменение состояния Сделки остаются fail-closed. Model tool planning по умолчанию также отключён.

## Оставшийся AP-10

1. Governed planner и отдельный контур подтверждённых write-tools без передачи authority модели.
