const route = [
  ['цена', 'условия зафиксированы'],
  ['рейс', 'маршрут и водитель видны'],
  ['вес', 'расхождение не теряется'],
  ['качество', 'протокол влияет на расчёт'],
  ['СДИЗ', 'госслед не выпал'],
  ['ЭДО', 'понятно, чья подпись ждёт'],
  ['деньги', 'выпуск только по основанию'],
];

const money = [
  ['сделка', '3,87 млн ₽'],
  ['под риском', 'рейс · качество · документы'],
  ['первый результат', 'карта потерь'],
];

const proof = [
  ['47', 'интервью с рынком'],
  ['87%', 'допускали оплату за результат'],
  ['24 недели', 'горизонт controlled pilot'],
];
...
export default function LandingHero() {
  return (
    <section id="top" className="lux-line relative min-h-screen pt-28 grid-bg">
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-12 px-6 pb-24 pt-14 lg:grid-cols-[0.95fr_1.05fr]">
        <div>
          <div className="reveal mb-7 inline-flex items-center gap-2 rounded-full border border-[rgba(126,242,196,0.2)] bg-[rgba(126,242,196,0.05)] px-4 py-1.5">
            ...
            <span className="text-xs font-semibold uppercase tracking-wide text-mint">не marketplace · цифровой контур исполнения сделки</span>
          </div>
          ...
          <p className="reveal reveal-delay-2 mb-10 max-w-2xl text-sm leading-relaxed text-[#6F8C82]">
            Первый вход — карта потерь по одной реальной сделке. Без обещаний боевых интеграций без договоров, доступов и проверки на controlled pilot.
          </p>
          ...
          <div className="reveal reveal-delay-3 mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">
            {proof.map(([value, label]) => (
              <div key={label} className="rounded-2xl border border-[rgba(126,242,196,0.1)] bg-[rgba(126,242,196,0.035)] p-4">
                ...
                <div className="mt-1 text-xs leading-snug text-[#6F8C82]">{label}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="rounded-[28px] border border-[rgba(126,242,196,0.08)] bg-[#07110E]/95 p-5">
            <div className="mb-5 flex items-start justify-between gap-4 border-b border-[rgba(126,242,196,0.08)] pb-5">
              <div>
                ...
                <div className="mt-2 text-sm text-[#6F8C82]">Партия 240 т · покупатель · элеватор · логистика · банк</div>
              </div>
            ...
            <div className="space-y-2.5">
              {route.map(([item, text], index) => (
                <div key={item} className="flow-dot flex items-center gap-3 rounded-xl border border-[rgba(126,242,196,0.07)] bg-[#111C19] px-4 py-3 transition hover:translate-x-1 hover:border-[rgba(126,242,196,0.22)]">
                  ...
                  <span className="min-w-[74px] text-sm font-semibold text-[#EAF1EE]">{item}</span>
                  <span className="text-xs text-[#6F8C82]">{text}</span>
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 rounded-2xl border border-[rgba(255,139,144,0.18)] bg-[rgba(255,139,144,0.055)] p-4 text-sm leading-relaxed text-[#F3B2B5]">
              Если нет единого следа, спор начинается поздно: стороны вспоминают, кто что сказал. Если след собран заранее — видно, что выпускать, что удержать и кому действовать.
            </div>
          </div>
        </div>
      </div>
