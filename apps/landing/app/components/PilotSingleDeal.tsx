const items = ['1 сделка', '1 партия', '1 маршрут', '1 точка приёмки', '1 пакет документов', '1 сценарий оплаты', '1 спорный риск'];
const results = ['где теряются деньги', 'кто тормозит сделку', 'какие данные нужны банку', 'что можно автоматизировать дальше'];
    ...
    <section id="pilot" className="lux-line relative z-10 py-24 grid-bg">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div>
          ...
          <p className="text-lg leading-relaxed text-[#8BA89E]">Проверяем не красивую презентацию, а реальную цепочку: где риск, кто отвечает, какие документы тормозят расчёт и почему деньги нельзя выпускать вслепую.</p>
        </div>
        <div className="premium-panel rounded-3xl p-6">
          ...
          <div className="mt-5 rounded-2xl border border-[rgba(126,242,196,0.1)] bg-[rgba(126,242,196,0.045)] p-5">
            <div className="mb-3 text-xs font-semibold uppercase tracking-[0.14em] text-[#6F8C82]">результат пилота</div>
            <div className="grid gap-2 md:grid-cols-2">
              {results.map((item) => <div key={item} className="text-sm text-[#C9D8D2]">Покажем, {item}.</div>)}
            </div>
          </div>
