  ...
  return (
    <main className="premium-surface relative overflow-hidden bg-[#030D0A] text-[#EAF1EE]">
      <header className="premium-nav fixed inset-x-0 top-0 z-50 border-b border-[rgba(126,242,196,0.08)] backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
          <a href="#top" className="flex items-center gap-3" aria-label="Прозрачная Цена">
            <span className="glow-sm flex h-8 w-8 items-center justify-center rounded-lg bg-brand text-[12px] font-black text-mint">ПЦ</span>
          ...
          <nav className="hidden items-center gap-7 md:flex">
            ...
            <a href="#contact" className="lux-button rounded-lg bg-brand px-5 py-2 text-sm font-semibold text-white transition-colors hover:bg-brand-hover">Обсудить сделку</a>
          </nav>
      ...
      <section id="top" className="lux-line relative min-h-screen pt-28 grid-bg">
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-12 px-6 pb-24 pt-14 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            <div className="reveal mb-7 inline-flex items-center gap-2 rounded-full border border-[rgba(126,242,196,0.2)] bg-[rgba(126,242,196,0.05)] px-4 py-1.5 shadow-[0_0_40px_rgba(126,242,196,0.08)]">
            ...
            <div className="reveal reveal-delay-3 flex flex-col gap-4 sm:flex-row">
              <a href="#contact" className="lux-button rounded-xl bg-brand px-8 py-4 text-center font-semibold text-white transition-all glow hover:scale-[1.02] hover:bg-brand-hover">Разобрать пилотную сделку</a>
              <a href="#contour" className="rounded-xl border border-[rgba(126,242,196,0.2)] bg-[rgba(255,255,255,0.02)] px-8 py-4 text-center font-medium text-[#EAF1EE] transition-all hover:border-[rgba(126,242,196,0.4)] hover:bg-[rgba(126,242,196,0.04)]">Как устроен контур</a>
            </div>
          </div>

          <div className="reveal reveal-delay-2 premium-panel rounded-[32px] p-5">
            <div className="rounded-[24px] bg-[#0B1513]/90 p-5">
              ...
              <div className="space-y-2.5">
                {dealPath.map((item, index) => (
                  <div key={item} className="flow-dot group flex items-center gap-3 rounded-xl border border-[rgba(126,242,196,0.07)] bg-[#111C19] px-4 py-3 transition-all hover:translate-x-1 hover:border-[rgba(126,242,196,0.22)] hover:bg-[#14231f]">
                    ...
                    <span className="ml-auto mr-8 h-px w-8 bg-[rgba(126,242,196,0.16)]" />
                  </div>
                ))}
              </div>
      ...
      <section className="lux-line relative z-10 border-y border-[rgba(126,242,196,0.08)] py-9">
        <div className="mx-auto grid max-w-7xl gap-3 px-6 md:grid-cols-4">
          {['Без фейковых KPI', 'Без «production-ready»', 'Без обещаний интеграций', 'Без пересечения с платформой'].map((item) => (
            <div key={item} className="premium-card rounded-2xl p-4 text-center text-sm font-semibold text-[#C9D8D2]">
              {item}
            </div>
          ))}
        </div>
      ...
      <section id="why" className="relative z-10 py-24">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {pains.map((point) => (
              <div key={point.title} className="premium-card rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1">
      ...
      <section id="contour" className="lux-line relative z-10 py-24 grid-bg">
        ...
        <div className="relative mx-auto max-w-7xl px-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {principles.map((item) => (
              <div key={item.title} className="premium-card rounded-2xl p-6 transition-all hover:-translate-y-1">
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="grid gap-3 lg:grid-cols-7">
            {stages.map(([num, title, text]) => (
              <div key={num} className="premium-card rounded-2xl p-5 transition-all hover:-translate-y-1">
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {whoNeedsIt.map(([role, text]) => (
              <div key={role} className="premium-card rounded-2xl p-6 transition-all hover:-translate-y-1">
      ...
      <section id="pilot" className="relative z-10 py-24">
          ...
          <div className="premium-panel rounded-3xl p-6">
            ...
            <div className="space-y-3">
              {pilot.map((item, index) => (
                <div key={item} className="flow-dot flex gap-3 rounded-xl border border-[rgba(126,242,196,0.07)] bg-[#111C19] p-4 pr-10 transition-all hover:translate-x-1 hover:border-[rgba(126,242,196,0.22)]">
      ...
      <section id="contact" className="lux-line relative z-10 overflow-hidden py-24 grid-bg">
        ...
        <div className="relative mx-auto max-w-4xl px-6 text-center">
          ...
          <div className="premium-panel mx-auto max-w-2xl rounded-3xl p-6 text-left">
            ...
            <div className="lux-button mt-6 rounded-xl bg-brand px-5 py-4 text-center font-semibold text-white">Форма заявки подключается отдельно: почта, CRM или защищённый приём заявок</div>
          </div>
        </div>
      </section>
