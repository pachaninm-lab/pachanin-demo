...
CREATE SCHEMA IF NOT EXISTS organization_intake;
REVOKE ALL ON SCHEMA organization_intake FROM PUBLIC;

DO $move_public_intake_table$
BEGIN
  IF to_regclass('organization_intake.requests') IS NULL
    AND to_regclass('public.public_organization_connection_requests') IS NOT NULL THEN
    ALTER TABLE public.public_organization_connection_requests SET SCHEMA organization_intake;
    ALTER TABLE organization_intake.public_organization_connection_requests RENAME TO requests;
  ELSIF to_regclass('organization_intake.requests') IS NULL THEN
    RAISE EXCEPTION 'PUBLIC_INTAKE_SOURCE_TABLE_MISSING';
  END IF;
END
$move_public_intake_table$;

REVOKE ALL ON TABLE organization_intake.requests FROM PUBLIC;
ALTER TABLE organization_intake.requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE organization_intake.requests FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS public_org_intake_owner_select ON organization_intake.requests;
DROP POLICY IF EXISTS public_org_intake_owner_insert ON organization_intake.requests;
CREATE POLICY public_org_intake_owner_select
ON organization_intake.requests FOR SELECT
USING (
  current_user = pg_get_userbyid(
    (SELECT relowner FROM pg_class WHERE oid = 'organization_intake.requests'::regclass)
  )
);
CREATE POLICY public_org_intake_owner_insert
ON organization_intake.requests FOR INSERT
WITH CHECK (
  current_user = pg_get_userbyid(
    (SELECT relowner FROM pg_class WHERE oid = 'organization_intake.requests'::regclass)
  )
...
CREATE OR REPLACE FUNCTION organization_intake.lookup_request(
...
SECURITY DEFINER
STABLE
SET search_path = pg_catalog, public, organization_intake
AS $function$
DECLARE
  stored organization_intake.requests%ROWTYPE;
BEGIN
  ...
  SELECT * INTO stored
  FROM organization_intake.requests request
...
CREATE OR REPLACE FUNCTION organization_intake.create_request(
...
BEGIN
  ...
  INSERT INTO organization_intake.requests (
...
-- Compatibility wrappers keep the previous application image rollback-safe.
...
SET search_path = pg_catalog, public, organization_intake
AS $function$
  SELECT * FROM organization_intake.lookup_request(p_idempotency_key, p_payload_hash)
$function$;
...
SET search_path = pg_catalog, public, organization_intake
AS $function$
  SELECT * FROM organization_intake.create_request(
    p_organization_name,
    p_inn,
    p_contact_name,
    p_position,
...
REVOKE ALL ON FUNCTION organization_intake.lookup_request(text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION organization_intake.create_request(text, text, text, text, text, text, text, text, text, text, text, text, text) FROM PUBLIC;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_deal') THEN
    GRANT USAGE ON SCHEMA organization_intake TO app_deal;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_service') THEN
    GRANT USAGE ON SCHEMA organization_intake TO app_service;
    GRANT EXECUTE ON FUNCTION organization_intake.lookup_request(text, text) TO app_service;
    GRANT EXECUTE ON FUNCTION organization_intake.create_request(text, text, text, text, text, text, text, text, text, text, text, text, text) TO app_service;
...
COMMENT ON SCHEMA organization_intake IS
  'Pre-tenant bounded context for public organization connection requests.';
COMMENT ON TABLE organization_intake.requests IS
  'Pre-tenant requests only. Does not create Organization, User, membership, role or tenant authority.';
COMMENT ON FUNCTION organization_intake.create_request(text, text, text, text, text, text, text, text, text, text, text, text, text) IS
