-- A valid OPEN appeal is created only after a resolved outcome. Finalization
-- therefore relies exclusively on the atomic appeal compare-and-set. The case
-- status is a projection and cannot veto the command before the CAS executes.
