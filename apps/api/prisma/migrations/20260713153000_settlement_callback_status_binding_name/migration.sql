-- IR-10.4 callback status binding name correction.
--
-- Keep the full resting-state and exact callback guard unchanged, but use a
-- local variable name that cannot be confused with bank_callbacks.callback_status
-- by PL/pgSQL name resolution.
...
COMMENT ON FUNCTION settlement.validate_payment_transition() IS
  'Exact callback and CAS guard with unambiguous callback status binding and full resting-state precedence.';
