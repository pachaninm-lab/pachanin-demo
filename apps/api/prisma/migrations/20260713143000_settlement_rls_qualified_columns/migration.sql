-- Qualify every Settlement RLS column reference.
--
-- Unqualified `deal_id` inside a policy can be resolved against a PL/pgSQL
-- variable of a calling trigger function, producing SQLSTATE 42702. Policies
-- must bind explicitly to their protected relation.

DO $settlement_qualified_policies$
DECLARE
  table_name TEXT;
BEGIN
