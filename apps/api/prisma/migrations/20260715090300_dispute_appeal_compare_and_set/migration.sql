-- Finalize an appeal through one atomic compare-and-set mutation. A preliminary
-- SELECT of the FORCE-RLS table is not an authority and can diverge from the
-- mutation path. UPDATE ... WHERE status='OPEN' plus ROW_COUNT=1 is the lock,
-- ownership proof and exactly-once terminal transition.
...
BEGIN
  ...
  END IF;
  IF c.status <> 'APPEALED' OR c.outcome IS NULL THEN
