-- IR-10.4 full Settlement resting-state alignment.
--
-- PostgreSQL and the application must derive payment status from the same full
-- authoritative counter set. In particular, MANUAL_REVIEW and HOLD_ACTIVE take
-- precedence over aggregate release/refund disposition, followed by pending
-- operations and then the final consumed-funds state.
...
DECLARE
  ...
  callback_status TEXT := NULLIF(current_setting('app.current_callback_status', true), '');
...
BEGIN
  ...
  IF actor_role = 'BANK_CALLBACK' THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE callback.operation_id = operation_record.id
        ...
        AND callback.callback_status = callback_status
    ) THEN
  ...
  IF NEW.status <> expected_status THEN
    RAISE EXCEPTION 'payment status % does not match derived resting status %', NEW.status, expected_status
      USING ERRCODE = '23514';
  END IF;
...
COMMENT ON FUNCTION settlement.validate_payment_transition() IS
  'Exact callback and CAS guard with the same full MANUAL_REVIEW, hold, pending and aggregate disposition precedence as SettlementPostgresqlRepository.restingStatus.';
