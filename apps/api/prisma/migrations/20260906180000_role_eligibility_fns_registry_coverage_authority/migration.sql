...
ALTER TABLE eligibility.registry_generations
  ADD COLUMN registry_domain TEXT;
...
ALTER TABLE eligibility.registry_generations
  ADD CONSTRAINT registry_generations_source_domain_generation_key
    UNIQUE (source, registry_domain, generation),
  ADD CONSTRAINT registry_generations_id_source_domain_key
    UNIQUE (id, source, registry_domain);
...
CREATE UNIQUE INDEX registry_generations_one_active_per_source_domain_idx
  ON eligibility.registry_generations(source, registry_domain)
  WHERE status = 'ACTIVE';
...
ALTER TABLE eligibility.source_health
  ...
  ADD CONSTRAINT source_health_source_domain_check CHECK (
    (source = 'FNS' AND registry_domain IN ('EGRUL','EGRIP','UNKNOWN'))
    OR (source = 'CBR' AND registry_domain = 'CBR')
    OR (source = 'FGIS_GRAIN' AND registry_domain = 'FGIS_GRAIN')
    OR (source = 'ROSACCREDITATION' AND registry_domain = 'ROSACCREDITATION')
  );

ALTER TABLE eligibility.source_health
  DROP CONSTRAINT IF EXISTS source_health_pkey;
ALTER TABLE eligibility.source_health
  ADD PRIMARY KEY (source, registry_domain);
...
CREATE TABLE eligibility.registry_generation_authority (
  ...
  CONSTRAINT registry_generation_authority_generation_fk
    ...
    REFERENCES eligibility.registry_generations(id, source, registry_domain)
    ON DELETE RESTRICT,
  ...
  CONSTRAINT registry_generation_authority_sha_check CHECK (
    authority_token ~ '^[0-9a-f]{64}$'
    AND (update_package_sha256 IS NULL OR update_package_sha256 ~ '^[0-9a-f]{64}$')
    AND (continuity_policy_hash IS NULL OR continuity_policy_hash ~ '^[0-9a-f]{64}$')
    AND (finality_policy_hash IS NULL OR finality_policy_hash ~ '^[0-9a-f]{64}$')
  ),
  CONSTRAINT registry_generation_authority_finality_check CHECK (
    NOT source_finality OR (
      acquisition_complete
      AND local_import_integrity
      AND baseline_coverage
      AND update_continuity
      AND coverage_kind IN ('COMPLETE_NATIONAL_CORPUS','COMPLETE_EFFECTIVE_CORPUS')
      AND effective_cutoff IS NOT NULL
      AND continuity_policy_version IS NOT NULL
      AND continuity_policy_hash IS NOT NULL
      AND finality_policy_version IS NOT NULL
      AND finality_policy_hash IS NOT NULL
    )
  )
);
...
-- Legacy generations get immutable conservative authority facts only.
...
SELECT
  id, source, registry_domain, 'UNKNOWN', 'UNKNOWN',
...
BEGIN
  ...
  WHERE source = p_source
    AND registry_domain = p_registry_domain
    AND generation = p_generation
  FOR UPDATE;

  IF target_id IS NULL THEN
    RAISE EXCEPTION 'generation not found for source/domain';
  END IF;
...
-- Compatibility wrapper: resolve the domain from the exact target row; fail on ambiguity.
...
BEGIN
  SELECT min(registry_domain), count(*)::INTEGER
    INTO target_domain, target_count
  FROM eligibility.registry_generations
  WHERE source = p_source AND generation = p_generation;

  IF target_count <> 1 THEN
    RAISE EXCEPTION 'generation domain is missing or ambiguous';
  END IF;
