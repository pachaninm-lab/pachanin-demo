...
-- the declared table type exactly. The persisted correlationId is varchar(128),
-- while the public function contract intentionally exposes correlation_id as text.
-- Keep the stable external contract and cast the stored value explicitly.
...
BEGIN
  ...
  RETURN QUERY
  SELECT stored."requestNumber", stored.status, TRUE, stored."correlationId"::text;
END
$function$;
