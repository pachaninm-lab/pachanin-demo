-- IR-10.3: bind laboratory evidence purpose while the object is still pending.
--
-- The object identity, storage key, uploader and deal scope stay immutable. Only
-- purpose metadata may be enriched before storage verification, under the same
-- trusted RLS scope and optimistic version boundary. Verified evidence remains
-- fully immutable.

CREATE OR REPLACE FUNCTION public.app_document_bind_pending_evidence_metadata()
RETURNS trigger
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'evidence purpose can only be bound before storage verification'
      USING ERRCODE = '23514';
  END IF;
...
DROP TRIGGER IF EXISTS deal_documents_bind_pending_evidence_metadata ON public."deal_documents";
...
CREATE TRIGGER deal_documents_bind_pending_evidence_metadata
...
EXECUTE FUNCTION public.app_document_bind_pending_evidence_metadata();
