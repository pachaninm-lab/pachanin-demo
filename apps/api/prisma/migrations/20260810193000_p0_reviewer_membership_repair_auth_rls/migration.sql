...
--
-- The previous migration deliberately kept pc_staff_runtime function-only and
-- created a NOLOGIN/NOBYPASSRLS SECURITY DEFINER owner. It granted that owner
-- the exact table privileges required by auth.repair_single_reviewer_membership(),
-- but did not add matching FORCE-RLS policies for auth.staff_assignments and
-- auth.staff_access_events. The production ceremony therefore failed closed
-- inside the database and the transaction was rolled back.
--
...
-- no role membership and no BYPASSRLS authority.
