...
function restrictedUrl(adminUrl: string): string {
  const parsed = new URL(adminUrl);
  parsed.username = 'app_deal';
  parsed.password = 'app_deal';
  return parsed.toString();
}

const describeDisputeAuthority = ADMIN_URL ? describe : describe.skip;
...
describeDisputeAuthority('IR-10.5 PostgreSQL dispute authority', () => {
  ...
  beforeAll(async () => {
    if (!ADMIN_URL) throw new Error('TEST_ADMIN_DATABASE_URL is required');
    admin = new PrismaClient({ datasources: { db: { url: ADMIN_URL } } });
    await admin.$connect();
    await admin.$executeRawUnsafe(`ALTER ROLE app_deal WITH LOGIN PASSWORD 'app_deal'`);
    process.env.DATABASE_URL = restrictedUrl(ADMIN_URL);
  ...
  afterAll(async () => {
    ...
    await Promise.all([appA?.$disconnect(), appB?.$disconnect(), admin?.$disconnect()]);
  ...
  it('allows exactly one conflicting terminal decision, survives restart/replay and compensates a granted appeal', async () => {
    ...
    const fulfilled = race.filter(
      (item): item is PromiseFulfilledResult<Record<string, unknown>> => item.status === 'fulfilled',
    );
    ...
    const winnerReason = winnerOutcome === 'BUYER_WINS'
      ? 'Buyer evidence prevails'
      : 'Seller evidence prevails';
    ...
    expect(counts.ledgerLinks).toBe(2);
    ...
    expect(finalCounts.ledgerLinks).toBe(4);
  ...
  it('rolls back case, hold, audit, outbox and receipt when escrow balance is insufficient', async () => {
    ...
    await expect(commandsA.open({
      ...
      response: { code: 'DISPUTE_INSUFFICIENT_LEDGER_BALANCE' },
    });

    expect(await dealAuthorityCounts(admin, dealId)).toEqual({
      cases: 0,
      holds: 0,
      disputeLedgerLinks: 0,
      disputeAudits: 0,
      disputeOutbox: 0,
      receipts: 0,
      ledgerEntries: 1,
  ...
  it('enforces FORCE-RLS visibility and denies direct application writes', async () => {
    ...
    const hidden = await queriesA.list(foreignBuyer);
    ...
    await expect(rls.withTrustedContext(buyer, (tx) => tx.$executeRawUnsafe(
      `INSERT INTO dispute.cases (
        id, tenant_id, deal_id, case_type, description, initiator_org_id, respondent_org_id
      ) VALUES (
        'forged-dispute', '${TENANT}', 'deal-dispute-race', 'FORGED', 'forged',
  ...
  for (const user of [buyer, seller, operator, arbitrator, foreignBuyer]) {
    await admin.user.create({
      data: {
        id: user.id,
        email: user.email,
        passwordHash: 'industrial-test-only',
        fullName: user.id,
        status: 'ACTIVE',
      },
    });
  ...
  for (const user of [buyer, seller, operator, arbitrator]) {
    await admin.dealParticipant.create({
      data: {
        ...
        userId: user.id,
        role: user.role,
        accessLevel: user.role === Role.ARBITRATOR || user.role === Role.SUPPORT_MANAGER
          ? 'APPROVE'
          : 'WORK',
        status: 'ACTIVE',
      },
...
async function authorityCounts(admin: PrismaClient, caseId: string): Promise<{
  resolveReceipts: number;
  resolutionAudits: number;
  settlementEvents: number;
  holds: number;
  ledgerLinks: number;
  finalSettlementEvents: number;
  openAppeals: number;
}> {
  const rows = await admin.$queryRawUnsafe<Array<Record<string, bigint>>>(`
    SELECT
      (SELECT count(*) FROM dispute.command_receipts
        WHERE command_type='RESOLVE_CASE' AND result->>'id'=$1)::bigint AS "resolveReceipts",
      (SELECT count(*) FROM public.audit_events
        WHERE action='dispute:resolved' AND "objectType"='dispute_case' AND "objectId"=$1)::bigint AS "resolutionAudits",
      (SELECT count(*) FROM public.outbox_entries
        WHERE type='DISPUTE_SETTLEMENT_BASIS_READY' AND payload->>'disputeId'=$1)::bigint AS "settlementEvents",
      ...
      (SELECT count(*) FROM public.outbox_entries
        WHERE type='DISPUTE_FINAL_SETTLEMENT_BASIS_READY' AND payload->>'disputeId'=$1)::bigint AS "finalSettlementEvents",
      (SELECT count(*) FROM dispute.appeals WHERE case_id=$1 AND status='OPEN')::bigint AS "openAppeals"
  `, caseId);
  return numbers(rows[0]) as Awaited<ReturnType<typeof authorityCounts>>;
}

async function dealAuthorityCounts(admin: PrismaClient, dealId: string): Promise<{
  cases: number;
  holds: number;
  disputeLedgerLinks: number;
  disputeAudits: number;
  disputeOutbox: number;
  receipts: number;
  ...
  const rows = await admin.$queryRawUnsafe<Array<Record<string, bigint>>>(`
    SELECT
      ...
      (SELECT count(*) FROM dispute.holds h JOIN dispute.cases c ON c.id=h.case_id
        WHERE c.deal_id=$1)::bigint AS holds,
      (SELECT count(*) FROM dispute.ledger_links l JOIN dispute.cases c ON c.id=l.case_id
        WHERE c.deal_id=$1)::bigint AS "disputeLedgerLinks",
      (SELECT count(*) FROM public.audit_events
        WHERE "dealId"=$1 AND action LIKE 'dispute:%')::bigint AS "disputeAudits",
      (SELECT count(*) FROM public.outbox_entries
        WHERE "dealId"=$1 AND type LIKE 'DISPUTE_%')::bigint AS "disputeOutbox",
      (SELECT count(*) FROM dispute.command_receipts
        WHERE result->>'dealId'=$1)::bigint AS receipts,
      (SELECT count(*) FROM public.ledger_entries WHERE "dealId"=$1)::bigint AS "ledgerEntries"
  `, dealId);
  return numbers(rows[0]) as Awaited<ReturnType<typeof dealAuthorityCounts>>;
}

function numbers(row: Record<string, bigint>): Record<string, number> {
...
async function installOutboxFailure(admin: PrismaClient, dealId: string): Promise<void> {
  await admin.$executeRawUnsafe(`
    ...
    $$
  `);
  await admin.$executeRawUnsafe(`DROP TRIGGER IF EXISTS fail_dispute_outbox_test ON public.outbox_entries`);
  await admin.$executeRawUnsafe(`
    CREATE TRIGGER fail_dispute_outbox_test
...
async function removeOutboxFailure(admin: PrismaClient): Promise<void> {
  await admin.$executeRawUnsafe(`DROP TRIGGER IF EXISTS fail_dispute_outbox_test ON public.outbox_entries`);
  await admin.$executeRawUnsafe(`DROP FUNCTION IF EXISTS public.fail_dispute_outbox_test()`);
}
