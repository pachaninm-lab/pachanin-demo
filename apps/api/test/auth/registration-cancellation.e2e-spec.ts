...
import { RegistrationCancellationService } from '../../src/modules/auth/registration-cancellation.service';
...
function serviceSource(): string {
  const relative = 'src/modules/auth/registration-cancellation.service.ts';
  const candidates = [path.resolve(process.cwd(), relative), path.resolve(process.cwd(), 'apps/api', relative)];
