...
import {
  ...
  type FnsEgrulReorganizationFact,
...
const XML_DECLARATION_ATTRIBUTES = new Set(['version', 'encoding', 'standalone']);
...
function parseAttributes(raw: string): Record<string, string> {
  ...
  while (cursor < raw.length) {
    ...
    if (cursor === whitespaceStart) throw sourceError('FNS_EGRUL_XML_ATTRIBUTE_SYNTAX_INVALID');
...
function parseDeclaration(body: string): string | null {
  ...
  if (attrs.version !== '1.0') throw sourceError('FNS_EGRUL_XML_DECLARATION_INVALID');
  if (Object.keys(attrs).some((name) => !XML_DECLARATION_ATTRIBUTES.has(name))) {
    throw sourceError('FNS_EGRUL_XML_DECLARATION_INVALID');
  }
  const standalone = String(attrs.standalone || '').trim();
...
function tokenizeXml(xml: string): ParsedXml {
  ...
  while (cursor < xml.length) {
    ...
    if (xml.startsWith('<!--', open)) {
      ...
      if (body.includes('--') || body.endsWith('-')) throw sourceError('FNS_EGRUL_XML_STRUCTURE_INVALID');
      cursor = end + 3;
      continue;
    }
...
function strictOptionalGrn(value: string | undefined, errorCode: string): string | null {
  const normalized = String(value || '').trim();
  if (!normalized) return null;
  if (!/^\d{13}$/u.test(normalized)) throw sourceError(errorCode);
  return normalized;
}

function validateAccessRestriction(restriction: XmlNode, errorCode: string): void {
  if (String(restriction.attrs['ОгрДосСв'] || '').trim() !== '1') throw sourceError(errorCode);
  const grnDates = directChildren(restriction, 'ГРНДата');
  if (grnDates.length !== 1) throw sourceError(errorCode);
  strictOptionalGrn(grnDates[0].attrs['ГРН'], errorCode);
  strictRequiredDate(grnDates[0].attrs['ДатаЗаписи'], errorCode);
}

function accessRestrictionPresent(node: XmlNode, errorCode: string): boolean {
  const restriction = directChild(node, 'ОгрДосСв');
  if (!restriction) return false;
  validateAccessRestriction(restriction, errorCode);
  return true;
}

function provenanceFromGrnDate(node: XmlNode, errorCode: string): { grn: string | null; recordedAt: string } {
  return {
    grn: strictOptionalGrn(node.attrs['ГРН'], errorCode),
    recordedAt: strictRequiredDate(node.attrs['ДатаЗаписи'], errorCode),
  };
}

function singleGrnDate(node: XmlNode, errorCode: string): { grn: string | null; recordedAt: string } {
  const grnDates = directChildren(node, 'ГРНДата');
  if (grnDates.length !== 1) throw sourceError(errorCode);
  return provenanceFromGrnDate(grnDates[0], errorCode);
}

function statusFacts(subject: XmlNode, format: FnsEgrulFormat): {
  ...
  for (const container of directChildren(subject, 'СвСтатус')) {
    const statusNodes = directChildren(container, 'СвСтатус');
    if (statusNodes.length !== 1) throw sourceError('FNS_EGRUL_STATUS_CONTAINER_INVALID');
    if (container.attrs['КодСтатусЮЛ'] || container.attrs['НаимСтатусЮЛ']) {
      throw sourceError('FNS_EGRUL_STATUS_CONTAINER_AMBIGUOUS');
    }

    const provenance = singleGrnDate(container, 'FNS_EGRUL_STATUS_PROVENANCE_INVALID');
    const restriction = accessRestrictionPresent(container, 'FNS_EGRUL_STATUS_RESTRICTION_INVALID');
    ...
    const liquidationDeadlineRaw = statusNode.attrs['СрокЛиквООО'];
    if (format === '4.07' && liquidationDeadlineRaw !== undefined && code !== '101') {
      throw sourceError('FNS_EGRUL_STATUS_LIQUIDATION_DEADLINE_NOT_ALLOWED');
    }
    statuses.push({
      code,
      name,
      liquidationDeadline: strictOptionalDate(liquidationDeadlineRaw, 'FNS_EGRUL_STATUS_DATE_INVALID'),
      grn: provenance.grn,
      recordedAt: provenance.recordedAt,
      accessRestricted: restriction,
    });

    const decisionNode = directChild(container, 'СвРешИсклЮЛ');
    if (decisionNode) {
      const numericCode = Number(code);
      if (numericCode < 105 || numericCode > 110) {
        throw sourceError('FNS_EGRUL_EXCLUSION_DECISION_STATUS_MISMATCH');
      }
      decisions.push({
...
function reorganizationFacts(subject: XmlNode): FnsEgrulReorganizationFact[] {
  const output: FnsEgrulReorganizationFact[] = [];

  for (const reorganization of directChildren(subject, 'СвРеорг')) {
    const statusNodes = directChildren(reorganization, 'СвСтатус');
    if (statusNodes.length !== 1) throw sourceError('FNS_EGRUL_REORGANIZATION_STATUS_INVALID');
    const provenance = singleGrnDate(reorganization, 'FNS_EGRUL_REORGANIZATION_PROVENANCE_INVALID');

    const statusNode = statusNodes[0];
    const code = String(statusNode.attrs['КодСтатусЮЛ'] || '').trim();
    const name = String(statusNode.attrs['НаимСтатусЮЛ'] || '').trim();
    if (!code || !name) throw sourceError('FNS_EGRUL_REORGANIZATION_STATUS_INVALID');
    ...
    output.push({
      ...
      grn: provenance.grn,
      recordedAt: provenance.recordedAt,
      accessRestricted: accessRestrictionPresent(reorganization, 'FNS_EGRUL_REORGANIZATION_RESTRICTION_INVALID'),
    });
  }
...
function terminationFact(subject: XmlNode): FnsEgrulTerminationFact | null {
  ...
  const terminatedAt = strictRequiredDate(termination.attrs['ДатаПрекрЮЛ'], 'FNS_EGRUL_TERMINATION_DATE_INVALID');
  const method = directChild(termination, 'СпПрекрЮЛ');
  if (!method) throw sourceError('FNS_EGRUL_TERMINATION_METHOD_INVALID');
  const methodCode = String(method.attrs['КодСпПрекрЮЛ'] || '').trim();
  const methodName = String(method.attrs['НаимСпПрекрЮЛ'] || '').trim();
  if (!methodCode || !methodName) throw sourceError('FNS_EGRUL_TERMINATION_METHOD_INVALID');

  const registrar = directChild(termination, 'СвРегОрг');
  if (!registrar) throw sourceError('FNS_EGRUL_TERMINATION_REGISTRAR_INVALID');
  const registrarCode = String(registrar.attrs['КодНО'] || '').trim();
  const registrarName = String(registrar.attrs['НаимНО'] || '').trim();
  if (!/^\d{4}$/u.test(registrarCode) || registrarName.length < 10 || registrarName.length > 250) {
    throw sourceError('FNS_EGRUL_TERMINATION_REGISTRAR_INVALID');
  }

  const provenance = singleGrnDate(termination, 'FNS_EGRUL_TERMINATION_PROVENANCE_INVALID');
  return {
    terminatedAt,
    methodCode,
    methodName,
...
function reliabilityFactFromGrnDate(
  node: XmlNode,
  area: FnsEgrulReliabilityFact['area'],
  basisAttribute: string,
  sourceTag: FnsEgrulReliabilityFact['sourceTag'],
): FnsEgrulReliabilityFact {
  const basisCode = String(node.attrs[basisAttribute] || '').trim();
  if (!basisCode) throw sourceError('FNS_EGRUL_RELIABILITY_BASIS_INVALID');
  const provenance = singleGrnDate(node, 'FNS_EGRUL_RELIABILITY_PROVENANCE_INVALID');
  return { area, basisCode, sourceTag, grn: provenance.grn, recordedAt: provenance.recordedAt };
}

function reliabilityFacts(subject: XmlNode): { facts: FnsEgrulReliabilityFact[]; accessRestricted: boolean } {
  const facts: FnsEgrulReliabilityFact[] = [];
  let accessRestricted = false;

  const address = directChild(subject, 'СвАдресЮЛ');
  if (address) {
    for (const node of directChildren(address, 'СвНедАдресЮЛ')) {
      facts.push(reliabilityFactFromGrnDate(node, 'ADDRESS', 'ПризнНедАдресЮЛ', 'СвНедАдресЮЛ'));
    }
  }

  for (const section of directChildren(subject, 'СвУпрОрг')) {
    accessRestricted ||= accessRestrictionPresent(section, 'FNS_EGRUL_MANAGEMENT_RESTRICTION_INVALID');
    for (const node of descendantsWithin(section, 'СвНедДанУпрОрг')) {
      facts.push(reliabilityFactFromGrnDate(node, 'MANAGEMENT', 'ПризнНедДанУпрОрг', 'СвНедДанУпрОрг'));
    }
  }

  for (const section of directChildren(subject, 'СведДолжнФЛ')) {
    accessRestricted ||= accessRestrictionPresent(section, 'FNS_EGRUL_MANAGEMENT_RESTRICTION_INVALID');
    for (const node of descendantsWithin(section, 'СвНедДанДолжнФЛ')) {
      facts.push(reliabilityFactFromGrnDate(node, 'MANAGEMENT', 'ПризнНедДанДолжнФЛ', 'СвНедДанДолжнФЛ'));
    }
  }

  for (const section of directChildren(subject, 'СвУчредит')) {
    for (const restriction of descendantsWithin(section, 'ОгрДосСв')) {
      validateAccessRestriction(restriction, 'FNS_EGRUL_PARTICIPANT_RESTRICTION_INVALID');
      accessRestricted = true;
    }
    for (const node of descendantsWithin(section, 'СвНедДанУчр')) {
      facts.push(reliabilityFactFromGrnDate(node, 'PARTICIPANT', 'ПризнНедДанУчр', 'СвНедДанУчр'));
    }
  }

  return { facts, accessRestricted };
}
...
export function parseFnsEgrulXml(xml: string, format: FnsEgrulFormat): {
  ...
  for (const subject of subjects) {
    ...
    const status = statusFacts(subject, format);
    const reorganizations = reorganizationFacts(subject);
    const termination = terminationFact(subject);
    const reliability = reliabilityFacts(subject);
    const statusEnvelope = interpretFnsEgrulStatus({
      ...
      reorganizations,
      accessRestricted: status.accessRestricted || reliability.accessRestricted,
    });
