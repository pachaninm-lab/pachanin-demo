  ...
  if (address) {
    for (const node of directChildren(address, 'СвНедАдресЮЛ')) {
      ...
      facts.push({
        ...
        grn: strictOptionalGrn(node.attrs['ГРН'], 'FNS_EGRUL_RELIABILITY_GRN_INVALID'),
        recordedAt: strictRequiredDate(node.attrs['ДатаЗап'], 'FNS_EGRUL_RELIABILITY_PROVENANCE_INVALID'),
      });
    }
  }
