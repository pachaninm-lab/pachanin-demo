...
export class RoleEligibilitySourceHealthService {
  ...
  async get(
    ...
    return (await this.list()).find((row) => row.source === source && row.registryDomain === registryDomain) ?? null;
  }

  async assertFetchAllowed(
    source: EligibilitySource,
    registryDomain: RegistryDomain = defaultRegistryDomainForSource(source),
  ): Promise<void> {
    const row = await this.get(source, registryDomain);
    ...
    throw new Error(`${source}_${registryDomain}_CIRCUIT_OPEN`);
  }
  ...
  async failure(
    source: EligibilitySource,
    status: Extract<SourceHealthStatus, 'DEGRADED' | 'UNAVAILABLE' | 'SCHEMA_CHANGED'>,
    errorCode: string,
    registryDomain: RegistryDomain = defaultRegistryDomainForSource(source),
  ): Promise<void> {
    const current = await this.get(source, registryDomain);
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.$executeRaw(Prisma.sql`
        INSERT INTO eligibility.source_health (
          ...
          ${source},${registryDomain},${status},${circuit},${generation},${parserVersion},${schemaVersion},
          ${success ? now : null},${success ? null : now},clock_timestamp(),${freshUntil},${failures},${errorCode},clock_timestamp()
