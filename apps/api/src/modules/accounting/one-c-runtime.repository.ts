...
function credentialIdFromBearer(bearer: string): string | null {
  // Keep repository lookup admission exactly aligned with the machine policy:
  // UUID (36) + dot + 32 random bytes encoded as 43 base64url characters.
  // Reject before database lookup/hash work so oversized or malformed input
  // cannot become an unbounded authentication workload.
  if (typeof bearer !== 'string' || bearer.length !== 80 || bearer[36] !== '.') return null;
