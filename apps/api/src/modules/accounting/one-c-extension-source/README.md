...
1. `TransparentPriceConnectorHttp.bsl`
   ...
   - redirect запрещён;
   - correlation/bearer header values проходят exact shape/alphabet/length gates;
   - one-time job lease передаётся только как `X-One-C-Job-Lease: Bearer …`;
   - ACK/result/fail используют exact durable-runtime envelope: `idempotencyKey`,
     `payloadHash`, `revision`, `attempt` и только разрешённые terminal fields;
...
2. `TransparentPriceConnectorCommands.bsl`
   ...
   - детерминированные per-lease idempotency keys для ACK/result/fail;
   - безопасный повтор неоднозначной квитанции с тем же lease/key;
...
`REPORTED_SUCCESS` допустим только когда profile возвращает собственный
идентификатор факта/объекта 1С как `externalEvidenceId` и точное состояние
`CREATED_IN_1C` либо `POSTED` как `resultState`.

## Где хранится machine bearer

Эти исходники **не имеют plaintext fallback**.

Конкретная сборка расширения должна подключить provider защищённого хранения.
Для конфигураций на БСП использовать безопасное хранилище БСП
(`ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище` / чтение / удаление);
для конфигурации без такого механизма отдельный compatibility/security review
обязан выбрать защищённый вариант. Обычная константа, регистр сведений, файл,
local user setting или журнал регистрации не допускаются.
...
## Gate оформления перед сертификационной сборкой

Эти reference-тексты ещё не являются модулем, готовым к подаче на
сертификацию. При формировании EDT/XML-проекта для exact target configuration
необходимо отдельно зафиксировать зарегистрированный префикс расширения,
применить его ко всем собственным объектам/методам/переменным, привести
синтаксические отступы к tab-based стандарту 1С, добавить полную документацию
экспортного API и пройти синтаксический контроль и проверку конфигурации без
ошибок. До evidence этих проверок исходники нельзя называть submission-ready.
