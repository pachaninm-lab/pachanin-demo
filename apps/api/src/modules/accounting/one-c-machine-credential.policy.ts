...
const CREDENTIAL_ID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const CREDENTIAL_SALT_PATTERN = /^[a-f0-9]{32}$/i;
const CREDENTIAL_SECRET_PATTERN = /^[A-Za-z0-9_-]{43}$/;
const CREDENTIAL_BEARER_LENGTH = 36 + 1 + 43;
...
function isPersistedRecordShapeSafe(record: OneCMachineCredentialRecord): boolean {
  try {
    if (!CREDENTIAL_ID_PATTERN.test(record.credentialId)) return false;
    if (!CREDENTIAL_SALT_PATTERN.test(record.salt)) return false;
    validateScope(record.scope);
    validateDate(record.issuedAt, 'issuedAt');
    validateDate(record.expiresAt, 'expiresAt');
    if (record.revokedAt !== null) {
      validateDate(record.revokedAt, 'revokedAt');
      if (record.revokedAt.getTime() < record.issuedAt.getTime()) return false;
    }
...
function parseBearer(
  ...
  if (typeof bearer !== 'string' || bearer.length !== CREDENTIAL_BEARER_LENGTH) return null;
  if (bearer[36] !== '.') return null;

  const credentialId = bearer.slice(0, 36);
  const secret = bearer.slice(37);
  if (!CREDENTIAL_ID_PATTERN.test(credentialId)) return null;
  if (!CREDENTIAL_SECRET_PATTERN.test(secret)) return null;
