 ...
 *
 * Тело, объявленное инлайн-литералом типа, стирается до `Object`, и глобальный
 * ValidationPipe на него не действует — измерено в
 * `common/validation/request-body-validation-mechanism.spec.ts`.
 *
 * Дефект намеренно описан словами, а не образцом кода: рэтчет
 * `verify-request-validation-coverage.mjs` на этой базе ищет образец по тексту
 * и засчитал бы объяснение за носителя дефекта. Разбор комментариев в гейте
 * исправляется отдельно и сейчас на ревью; когда он придёт в main, оговорка
