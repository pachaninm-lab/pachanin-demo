...
export class RegulatoryIntegrationControlTowerRepository {
  ...
  async detail(
    ...
    const recentEvents = await this.transactions.withTrustedContext(
      user,
      async (tx, context) => {
...
function safeCount(value: bigint): number {
  ...
  if (!Number.isSafeInteger(converted) || converted < 0) {
    throw new IntegrationControlTowerRepositoryError(
      'QUERY_INVALID',
      'Aggregate count is outside the safe range.',
    );
  }
  return converted;
}
...
function decodeCursor(
...
): { timestamp: number; adapterCode: string } | null {
  ...
  try {
    ...
    throw new IntegrationControlTowerRepositoryError(
      'CURSOR_INVALID',
      'Integration registry cursor is invalid.',
    );
  }
}
