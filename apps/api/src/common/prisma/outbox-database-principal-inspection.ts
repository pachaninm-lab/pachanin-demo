  ...
  const rows = await client.$queryRaw<OutboxInspectionRow[]>(Prisma.sql`
    SELECT
      ...
      (
        ...
        AND has_column_privilege(current_user, 'public.outbox_entries', 'lastErrorCode', 'UPDATE')
        AND has_column_privilege(current_user, 'public.outbox_entries', 'lastErrorCategory', 'UPDATE')
        AND has_column_privilege(current_user, 'public.outbox_entries', 'lastAttemptAt', 'UPDATE')
        AND has_column_privilege(current_user, 'public.outbox_entries', 'manualReviewAt', 'UPDATE')
