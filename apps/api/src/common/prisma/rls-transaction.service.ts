...
export type OrganizationMemberContextErrorCode = 'organization_membership_required';

export class OrganizationMemberContextError extends Error {
  constructor(readonly code: OrganizationMemberContextErrorCode) {
    super('A durable active organization membership is required.');
    this.name = 'OrganizationMemberContextError';
  }
}
          ...
          async (tx) => {
            ...
            if (requireOrganizationMembership) {
              ...
              if (!memberships[0]?.membership) {
                throw new OrganizationMemberContextError('organization_membership_required');
              }
            }
